...
APedestalCalibration::APedestalCalibration(TEvent &eventIn, TSetup &setup, algorithm_parameter & descr):AAlgorithm("Generate Pedestal calibration"), event(eventIn), minEntriesPerHisto(descr.getParam<int>(0).getData())
{
...
APedestalCalibration::APedestalCalibration(TEvent &eventIn, TSetup &setup, vector<int> &detectors, int minEntriesPerHistoIn):AAlgorithm("Generate Pedestal calibration"), event(eventIn), minEntriesPerHisto(minEntriesPerHistoIn)
{
...
void* APedestalCalibration::process(void *ptr)
{
  for(int i=0;i<numberOfDetectors;i++)
	...
	qdcHistograms[i][event.getHit(detectorIDs[i],j).getElement()]->Fill(event.getHit(detectorIDs[i],j).getRawADC());
    }
  return ptr;
}
