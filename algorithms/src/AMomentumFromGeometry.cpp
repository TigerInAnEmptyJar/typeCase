...
#include <stdlib.h>
AMomentumFromGeometry::AMomentumFromGeometry(TSetup& setup, TTrack** tracksIn,
                                             int& numberOfTracksIn)
    : AAlgorithm("Calculate momentum from Geometry"), numberOfTracks(numberOfTracksIn)
{
  tracks = tracksIn;
...
void* AMomentumFromGeometry::process(void* ptr)
{
  ...
  case 2: {
    ...
    float p1, p2;
    float /*sith1=sin(th1),*/ csth1 = cos(th1), tnth1 = tan(th1), sith2 = sin(th2),
                              /*csth2=cos(th2),*/ tnth2 = tan(th2);
    float siph1 = sin(ph1), csph1 = cos(ph1), siph2 = sin(ph2), csph2 = cos(ph2);
    float s1 = siph1 + csph1, s2 = siph2 + csph2;
    p1 =
        (CM.Z() / csth1 - (CM.Y() + CM.X()) / (csth1 * tnth2 * s2)) / (1 - tnth1 * s1 / tnth2 / s2);
    // 	p1=(CM.Z()/cos(th1)+CM.Y()/(sin(ph2)*cos(th1)*tan(th2)))/(1-(sin(ph1)*tan(th1))/(sin(ph2)*tan(th2)));
    p2 = ((CM.X() + CM.Y()) / s2 / sith2 - CM.Z() * tnth1 * s1 / sith2 / s2) /
         (1 - tnth1 / tnth2 * s1 / s2);
