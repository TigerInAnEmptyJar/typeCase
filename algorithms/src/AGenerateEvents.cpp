...
#include "logger.h"
AGenerateEvents::AGenerateEvents(TTrack **tracksIn, int &numberOfTracksIn, int &eventNumberIn, TSetup &setup, vector<int> IDs, string functionIn, vector<int> parameter,bool useSAIDIN, bool &validInIn):AAlgorithm("Generate Events"),numberOfTracks(numberOfTracksIn),eventNumber(eventNumberIn),validIn(validInIn)
{
  useSAID=useSAIDIN;
  tracks=tracksIn;
...
//     m2=setup.getTarget().getParticle();
  TLorentzVector p1(m1.X(),m1.Y(),m1.Z(),m1.Energy()),p2(m2.X(),m2.Y(),m2.Z(),m2.Energy()),p=p1+p2;
  ...
  //  function=new TF1("saidFunction","0.518962*pow([0]-TMath::Pi()/2,6)+0.02661",0,TMath::Pi());//elastic p_beam=3.059Gev/c
  nParameters=parameter.size();
  parameters=new int[nParameters];
  for(int i=0;i<nParameters;i++)parameters[i]=parameter[i];
//  double par;
  int pN,parN;
  if(useSAID)
    {
...
//	  par=0;
	  ...
	  switch(parN)
	    {
	    case 0: anaLog<<"Particle: "<<pN<<" Theta"<<endli;break;
	    case 1: anaLog<<"Particle: "<<pN<<" Phi"<<endli;break;
	    case 2: anaLog<<"Particle: "<<pN<<" Beta"<<endli;break;
	    case 3: anaLog<<"Particle: "<<pN<<" Momentum"<<endli;break;
	    case 4: anaLog<<"Particle: "<<pN<<" Pt"<<endli;break;
	    case 5: anaLog<<"Particle: "<<pN<<" Pl"<<endli;break;
	    case 6: anaLog<<"Particle: "<<pN<<" Theta_CM"<<endli;break;
	    case 7: anaLog<<"Particle: "<<pN<<" Phi_CM"<<endli;break;
	    case 8: anaLog<<"Particle: "<<pN<<" Beta_CM"<<endli;break;
	    case 9: anaLog<<"Particle: "<<pN<<" Momenutm_CMP"<<endli;break;
	    }
	}
    }
//  float x[25];//,y[25];
...
AGenerateEvents::~AGenerateEvents()
{
  delete []particleIDs;
}
#include <stdlib.h>
...
void* AGenerateEvents::process(void* ptr)
{
  ...
  while(weight<number)//||mm.Momentum().Theta()*180/M_PI>150||mm.Momentum().Theta()*180/M_PI<30||v1.Theta()>M_PI/3.0||v1.Theta()<0.17||v2.Theta()>M_PI/3.0||v2.Theta()<0.17)
    {
  ...
  for(int i=0;i<numberOfParticles;i++)used[i]=false;
  int pos;
  for(int i=0;i< numberOfParticles;i++)
    {
      pos=0;
