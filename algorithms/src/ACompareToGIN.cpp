...
ACompareToGIN::ACompareToGIN(TTrack** tracksIn, int& numberOfTracksIn, int& eventNumberIn,
                             int maxTracksIn, const algorithm_parameter& descr)
    : AAlgorithm("Compare tracks to GIN"), eventNumber(eventNumberIn), maxTracks(maxTracksIn),
      ...
      histoNotFoundD("Decay: Tracks not found", "Decay: Tracks not found", 21, 0, 20)
{
  ...
  //   histos[0]=new TH2F("Number Of tracks found","Number Of tracks
  //   found",100,0,99,100,0,99);
  //   histos[1]=new TH1F("\\Delta\\Phi","\\Delta\\Phi",100,-M_PI,M_PI);
  //   histos[2]=new
  //   TH1F("\\Delta\\theta","\\Delta\\theta",100,-M_PI/2.,M_PI/2.);
  //   histos[3]=new TH1F("\\Delta\\beta","\\Delta\\beta",100,-1,1);
  //   for(int i=0;i<numSepDet;i++)
  //     {
  //       cout<<4+i*3<<" "<<4+i*3+1<<" "<<4+i*3+2<<endl;
  //       histos[4+i*3+0]=new TH1F((string("\\Delta\\Phi tracks det
  //       ")+string_number(sepDets[i])).data(),(string("\\Delta\\Phi tracks det
  //       ")+string_number(sepDets[i])).data(),100,-M_PI,M_PI);
  //       histos[4+i*3+1]=new TH1F((string("\\Delta\\theta tracks det
  //       ")+string_number(sepDets[i])).data(),(string("\\Delta\\theta tracks
  //       det
  //       ")+string_number(sepDets[i])).data(),100,-M_PI,M_PI);
  //       histos[4+i*3+2]=new TH1F((string("\\Delta\\beta tracks det
  //       ")+string_number(sepDets[i])).data(),(string("\\Delta\\beta tracks
  //       det
  //       ")+string_number(sepDets[i])).data(),100,-M_PI,M_PI);
  //     }
  //   histos[4+numSepDet*3]=new TH1F("Tracks not found","Tracks not
  //   found",100,0,99);
  //   cout<<4+numSepDet*3<<" "<<nHistos<<" "<<histos[4+numSepDet*3]<<"
  //   "<<histos[4+numSepDet*3]->GetName()<<endl;
}
#include <TCanvas.h>
#include <TF1.h>
#include <TStyle.h>
...
void ACompareToGIN::process()
{
  ...
  for (int i = 0; i < numberOfTracks; i++) {
    if (RecTracks[i]->isSecondary())
      continue;
    if (RecTracks[i]->getNumberOfSecondaryTracks() <= 0)
      nPrompt++;
    else {
      if (RecTracks[i]->getNumberOfSecondaryTracks() > 1)
        nVees++;
      else if ((RecTracks[i]->getNumberOfSecondaryTracks() % 2) == 1)
        nKinks++;
    }
  }
  ...
  for (int i = 0; i < numberOfTracks; i++) {
    if (RecTracks[i]->isSecondary())
      continue;
    if (RecTracks[i]->getNumberOfSecondaryTracks() <= 0)
      Prompt[nPrompt++] = RecTracks[i];
    else {
      if (RecTracks[i]->getNumberOfSecondaryTracks() > 1)
        Vees[nVees++] = RecTracks[i];
      else if ((RecTracks[i]->getNumberOfSecondaryTracks() % 2) == 1)
        Kinks[nKinks++] = RecTracks[i];
    }
  }
  /////////////////////////
  (*input) >> nt;
  ...
  for (int i = 0; i < numberOfInTracks; i++) {
    ...
    if (InTracks[i]->getNumberOfSecondaryTracks() <= 0)
      ...
      cout << IDs[i] << " " << Eparticles::getLifeTimeG(IDs[i]) << " "
           << InTracks[i]->getSecondary(0)->getPath().Foot()
           << InTracks[i]->getNumberOfSecondaryTracks() << endl;
      if (InTracks[i]->getSecondary(0)->getPath().Foot().Z() > 400 ||
          Eparticles::getLifeTimeG(IDs[i]) > 1 || InTracks[i]->getCharge())
        nInPrompt++;
      else if (InTracks[i]->getCharge() == 0 && InTracks[i]->getNumberOfSecondaryTracks() > 1)
        nInVees++;
      else if (InTracks[i]->getCharge() != 0 &&
               (InTracks[i]->getNumberOfSecondaryTracks() % 2) == 1)
        nInKinks++;
    }
  }
  ...
  for (int i = 0; i < numberOfInTracks; i++) {
    ...
    if (InTracks[i]->getNumberOfSecondaryTracks() <= 0)
      ...
      if (InTracks[i]->getSecondary(0)->getPath().Foot().Z() > 400 ||
        ...
        InVees[nInVees++] = InTracks[i];
      else if (InTracks[i]->getCharge() != 0 &&
               (InTracks[i]->getNumberOfSecondaryTracks() % 2) == 1)
        InKinks[nInKinks++] = InTracks[i];
    }
  }
  /////////////////////////////////////
  ...
  for (int i = 0; i < nInKinks; i++) {
    InFound[i + nInPrompt + nInVees] =
        getCorrespondingNumber(InKinks[i], nKinks, Kinks, maxAngularDist, 0);
  }
  ///////misstaken///////////
  int misstaken[(nInPrompt + nInVees + nInKinks) * 3];
  for (int i = 0; i < (nInPrompt + nInVees + nInKinks) * 3; i++)
    misstaken[i] = -1;
  for (int i = 0; i < nInPrompt; i++) {
    if (InFound[i] >= 0)
      continue;
    ...
    if (misstaken[i] >= 0) {
      misstaken[i + (nInPrompt + nInVees + nInKinks) * 2] = -1;
      misstaken[i + (nInPrompt + nInVees + nInKinks)] = -1;
      continue;
    }
    misstaken[i + (nInPrompt + nInVees + nInKinks)] =
        getCorrespondingNumber(InPrompt[i], nKinks, Kinks, maxAngularDist, 0);
    if (misstaken[i + (nInPrompt + nInVees + nInKinks)] >= 0) {
      misstaken[i + (nInPrompt + nInVees + nInKinks) * 2] = -1;
      continue;
    }
    misstaken[i + (nInPrompt + nInVees + nInKinks) * 2] =
        getCorrespondingNumber(InPrompt[i], nVees, Vees, maxAngularDist, 2);
    if (misstaken[i + (nInPrompt + nInVees + nInKinks) * 2] == -1)
      misstaken[i + (nInPrompt + nInVees + nInKinks) * 2] =
          getCorrespondingNumber(InPrompt[i], nKinks, Kinks, maxAngularDist, 1);
  }
  for (int i = 0; i < nInVees; i++) {
    if (InFound[i + nInPrompt] >= 0)
      continue;
    misstaken[i + nInPrompt] =
        getCorrespondingNumber(InVees[i]->getSecondary(0), nPrompt, Prompt, maxAngularDist, 0);
    if (misstaken[i + nInPrompt] < 0)
      misstaken[i + nInPrompt] =
          getCorrespondingNumber(InVees[i]->getSecondary(1), nPrompt, Prompt, maxAngularDist, 0);
    if (misstaken[i + nInPrompt] >= 0) {
      misstaken[i + nInPrompt + (nInPrompt + nInVees + nInKinks) * 2] = -1;
      misstaken[i + nInPrompt + (nInPrompt + nInVees + nInKinks)] = -1;
      continue;
    }
  }
  for (int i = 0; i < nInKinks; i++) {
    if (InFound[i + nInPrompt + nInVees] >= 0)
      continue;
    misstaken[i + nInPrompt + nInVees] =
        getCorrespondingNumber(InKinks[i], nPrompt, Prompt, maxAngularDist, 0);
    if (misstaken[i + nInPrompt + nInVees] >= 0) {
      misstaken[i + (nInPrompt + nInVees + nInKinks) * 2 + nInPrompt + nInVees] = -1;
      misstaken[i + (nInPrompt + nInVees + nInKinks) + nInPrompt + nInVees] = -1;
      continue;
    }
    misstaken[i + nInPrompt + nInVees + (nInPrompt + nInVees + nInKinks)] =
        getCorrespondingNumber(InKinks[i]->getSecondary(0), nPrompt, Prompt, maxAngularDist, 0);
    if (misstaken[i + nInPrompt + nInVees + (nInPrompt + nInVees + nInKinks)] >= 0) {
      misstaken[i + nInPrompt + nInVees + (nInPrompt + nInVees + nInKinks) * 2] = -1;
      continue;
    }
  }
  //////////////////
  ...
  for (int i = 0; i < nPrompt; i++) {
    ...
    for (int j = 0; j < nInPrompt; j++)
      if (InFound[j] == i)
        num = j;
    Found[i] = num; // getCorrespondingNumber(Prompt[i], nInPrompt, InPrompt,
                    // maxAngularDist, 0);
  }
  for (int i = 0; i < nVees; i++) {
    ...
    for (int j = 0; j < nInVees; j++)
      if (InFound[j + nPrompt] == i)
        num = j;
    Found[i + nPrompt] = num; // getCorrespondingNumber(Vees[i], nInVees,
                              // InVees, maxAngularDist, -1);
  }
  for (int i = 0; i < nKinks; i++) {
    ...
    for (int j = 0; j < nInKinks; j++)
      if (InFound[j + nPrompt + nVees] == i)
        num = j;
    Found[i + nPrompt + nVees] = num; // getCorrespondingNumber(Kinks[i],
                                      // nInKinks, InKinks, maxAngularDist, 0);
  }
  ///////misstaken///////////
  int Rec_misstaken[(nPrompt + nVees + nKinks) * 3];
  for (int i = 0; i < (nPrompt + nVees + nKinks) * 3; i++)
    Rec_misstaken[i] = -1;
  for (int i = 0; i < nPrompt; i++) {
    if (Found[i] >= 0)
      continue;
    ...
    if (Rec_misstaken[i] >= 0) {
      Rec_misstaken[i + (nPrompt + nVees + nKinks) * 2] = -1;
      Rec_misstaken[i + (nPrompt + nVees + nKinks)] = -1;
      continue;
    }
    Rec_misstaken[i + (nPrompt + nVees + nKinks)] =
        getCorrespondingNumber(Prompt[i], nInKinks, InKinks, maxAngularDist, 0);
    if (Rec_misstaken[i + (nPrompt + nVees + nKinks)] >= 0) {
      Rec_misstaken[i + (nPrompt + nVees + nKinks) * 2] = -1;
      continue;
    }
    Rec_misstaken[i + (nPrompt + nVees + nKinks) * 2] =
        getCorrespondingNumber(Prompt[i], nInVees, InVees, maxAngularDist * 1.2, 2);
    if (Rec_misstaken[i + (nPrompt + nVees + nKinks) * 2] == -1)
      Rec_misstaken[i + (nPrompt + nVees + nKinks) * 2] =
          getCorrespondingNumber(Prompt[i], nInKinks, InKinks, maxAngularDist, 1);
  }
  for (int i = 0; i < nVees; i++) {
    if (Found[i + nPrompt] >= 0)
      continue;
    Rec_misstaken[i + nPrompt] =
        getCorrespondingNumber(Vees[i]->getSecondary(0), nInPrompt, InPrompt, maxAngularDist, 0);
    if (Rec_misstaken[i + nPrompt] < 0)
      Rec_misstaken[i + nPrompt] =
          getCorrespondingNumber(Vees[i]->getSecondary(1), nInPrompt, InPrompt, maxAngularDist, 0);
    if (Rec_misstaken[i + nPrompt] >= 0) {
      Rec_misstaken[i + nPrompt + (nPrompt + nVees + nKinks) * 2] = -1;
      Rec_misstaken[i + nPrompt + (nPrompt + nVees + nKinks)] = -1;
      continue;
    }
  }
  for (int i = 0; i < nKinks; i++) {
    if (Found[i + nPrompt + nVees] >= 0)
      continue;
    Rec_misstaken[i + nPrompt + nVees] =
        getCorrespondingNumber(Kinks[i], nInPrompt, InPrompt, maxAngularDist, 0);
    if (Rec_misstaken[i + nPrompt + nVees] >= 0) {
      Rec_misstaken[i + (nPrompt + nVees + nKinks) * 2 + nPrompt + nVees] = -1;
      Rec_misstaken[i + (nPrompt + nVees + nKinks) + nPrompt + nVees] = -1;
      continue;
    }
    Rec_misstaken[i + nPrompt + nVees + (nPrompt + nVees + nKinks)] =
        getCorrespondingNumber(Kinks[i]->getSecondary(0), nInPrompt, InPrompt, maxAngularDist, 0);
    if (Rec_misstaken[i + nPrompt + nVees + (nPrompt + nVees + nKinks)] >= 0) {
      Rec_misstaken[i + nPrompt + nVees + (nPrompt + nVees + nKinks) * 2] = -1;
      continue;
    }
  }
  //////////////////
  ...
  */
  if (toCout) {
    ...
    for (int i = 0; i < nInPrompt; i++) {
      num = 0;
      for (int j = 0; j < numberOfInTracks; j++)
        if (InPrompt[i] == InTracks[j])
          num = j;
      if (InPrompt[i]->getNumberOfSecondaryTracks() > 0)
        cout << InFound[i] << " " << IDs[num] << " "
             << InPrompt[i]->getPath().Direction().toString(_spheric_).data()
             << InPrompt[i]->getSecondary(0)->getPath().Foot() << endl;
      else
        cout << InFound[i] << " " << IDs[num] << " "
             << InPrompt[i]->getPath().Direction().toString(_spheric_).data() << endl;
    }
    for (int i = 0; i < nInVees; i++) {
      for (int j = 0; j < numberOfInTracks; j++)
        if (InVees[i] == InTracks[j])
          num = j;
      cout << InFound[i + nInPrompt] << " " << IDs[num] << " "
           << InVees[i]->getPath().Direction().toString(_spheric_).data() << flush;
      if (InVees[i]->getNumberOfSecondaryTracks() > 0) {
        if (InVees[i]->getSecondary(0)->getPath().Foot().Z() > 3200 || (IDs[num] != 18))
          ...
          for (int j = 0; j < InVees[i]->getNumberOfSecondaryTracks(); j++)
            cout << "   "
                 << " d="
                 << InVees[i]->getSecondary(j)->getPath().Direction().toString(_spheric_).data()
                 << endl;
        }
      } else
        cout << endl;
    ...
    for (int i = 0; i < nInKinks; i++) {
      for (int j = 0; j < numberOfInTracks; j++)
        if (InKinks[i] == InTracks[j])
          num = j;
      cout << InFound[i + nInPrompt + nInVees] << " " << IDs[num] << " "
           << InKinks[i]->getPath().Direction().toString(_spheric_).data() << flush;
      if (InKinks[i]->getNumberOfSecondaryTracks() > 0) {
        if (InKinks[i]->getSecondary(0)->getPath().Foot().Z() > 3200 || (IDs[num] != 18))
          ...
          for (int j = 0; j < InKinks[i]->getNumberOfSecondaryTracks(); j++)
            cout << "   "
                 << " d="
                 << InKinks[i]->getSecondary(j)->getPath().Direction().toString(_spheric_).data()
                 << endl;
        }
      } else
        cout << endl;
    ...
    for (int i = 0; i < nPrompt; i++) {
      ...
      if (Found[i] >= 0)
        ...
        if (Rec_misstaken[i] > 0)
          ...
          cout << "misstaken as secondary: " << Rec_misstaken[i + (nPrompt + nVees + nKinks) * 2]
               << endl;
        else
          cout << "not found" << endl;
      }
    ...
    for (int i = 0; i < nVees; i++) {
      cout << Found[i + nPrompt] << " " << Vees[i]->getPath().Direction().toString(_spheric_).data()
           << flush;
      if (Vees[i]->getNumberOfSecondaryTracks() > 0) {
        ...
        for (int j = 0; j < Vees[i]->getNumberOfSecondaryTracks(); j++)
          cout << "   "
               << " d="
               << Vees[i]->getSecondary(j)->getPath().Direction().toString(_spheric_).data()
               << endl;
      } else
        cout << endl;
    }
    for (int i = 0; i < nKinks; i++) {
      cout << Found[i + nPrompt + nVees] << " "
           << Kinks[i]->getPath().Direction().toString(_spheric_).data() << flush;
      if (Kinks[i]->getNumberOfSecondaryTracks() > 0) {
        ...
        for (int j = 0; j < Kinks[i]->getNumberOfSecondaryTracks(); j++)
          cout << "   "
               << " d="
               << Kinks[i]->getSecondary(j)->getPath().Direction().toString(_spheric_).data()
               << endl;
      } else
        cout << endl;
    }
  ...
  for (int i = 0; i < nPrompt; i++) {
    ...
    histoDbeta./*histos[3]->*/ Fill(Prompt[i]->getSpeed() - InPrompt[Found[i]]->getSpeed());
    histoDphiPr./*histos[1]->*/ Fill(Prompt[i]->getPath().Direction().Phi() -
                                     InPrompt[Found[i]]->getPath().Direction().Phi());
    histoDthetaPr./*histos[2]->*/ Fill(Prompt[i]->getPath().Direction().Theta() -
                                       InPrompt[Found[i]]->getPath().Direction().Theta());
    histoDbetaPr./*histos[3]->*/ Fill(Prompt[i]->getSpeed() - InPrompt[Found[i]]->getSpeed());
  }
  for (int i = 0; i < nVees; i++) {
    if (Found[i + nPrompt] < 0) {
      continue;
    ...
    histoDbeta./*histos[3]->*/ Fill(Vees[i]->getSpeed() - InVees[Found[i + nPrompt]]->getSpeed());
    histoDphiN./*histos[1]->*/ Fill(Vees[i]->getPath().Direction().Phi() -
                                    InVees[Found[i + nPrompt]]->getPath().Direction().Phi());
    histoDthetaN./*histos[2]->*/ Fill(Vees[i]->getPath().Direction().Theta() -
                                      InVees[Found[i + nPrompt]]->getPath().Direction().Theta());
    histoDbetaN./*histos[3]->*/ Fill(Vees[i]->getSpeed() - InVees[Found[i + nPrompt]]->getSpeed());
    for (int j = 0; j < Vees[i]->getNumberOfSecondaryTracks(); j++) {
      num = 0;
      float a = Vees[i]->getSecondary(j)->getPath().Direction() *
                InVees[Found[i + nPrompt]]->getSecondary(0)->getPath().Direction(),
            b;
      for (int k = 1; k < InVees[Found[i + nPrompt]]->getNumberOfSecondaryTracks(); k++) {
        b = Vees[i]->getSecondary(j)->getPath().Direction() *
            InVees[Found[i + nPrompt]]->getSecondary(k)->getPath().Direction();
        if (a > b)
          num = k;
      }
      histoDphi./*histos[1]->*/ Fill(
          Vees[i]->getSecondary(j)->getPath().Direction().Phi() -
          InVees[Found[i + nPrompt]]->getSecondary(num)->getPath().Direction().Phi());
      histoDtheta./*histos[2]->*/ Fill(
          Vees[i]->getSecondary(j)->getPath().Direction().Theta() -
      ...
      histoDphiD./*histos[1]->*/ Fill(
          Vees[i]->getSecondary(j)->getPath().Direction().Phi() -
          InVees[Found[i + nPrompt]]->getSecondary(num)->getPath().Direction().Phi());
      histoDthetaD./*histos[2]->*/ Fill(
          Vees[i]->getSecondary(j)->getPath().Direction().Theta() -
          InVees[Found[i + nPrompt]]->getSecondary(num)->getPath().Direction().Theta());
  ...
  for (int i = 0; i < nKinks; i++) {
    ...
    histoDphi./*histos[1]->*/ Fill(
        Kinks[i]->getPath().Direction().Phi() -
        InKinks[Found[i + nPrompt + nVees]]->getPath().Direction().Phi());
    histoDtheta./*histos[2]->*/ Fill(
        Kinks[i]->getPath().Direction().Theta() -
    ...
    for (int j = 0; j < Kinks[i]->getNumberOfSecondaryTracks(); j++) {
      num = 0;
      float a = Kinks[i]->getSecondary(j)->getPath().Direction() *
                InKinks[Found[i + nPrompt + nVees]]->getSecondary(0)->getPath().Direction(),
            b;
      for (int k = 1; k < InKinks[Found[i + nPrompt + nVees]]->getNumberOfSecondaryTracks(); k++) {
        b = Kinks[i]->getSecondary(j)->getPath().Direction() *
            InKinks[Found[i + nPrompt + nVees]]->getSecondary(k)->getPath().Direction();
        if (a > b)
          num = k;
      }
      histoDphi./*histos[1]->*/ Fill(
          Kinks[i]->getSecondary(j)->getPath().Direction().Phi() -
          InKinks[Found[i + nPrompt + nVees]]->getSecondary(num)->getPath().Direction().Phi());
      histoDtheta./*histos[2]->*/ Fill(
          Kinks[i]->getSecondary(j)->getPath().Direction().Theta() -
          InKinks[Found[i + nPrompt + nVees]]->getSecondary(num)->getPath().Direction().Theta());
      histoDbeta./*histos[3]->*/ Fill(
          Kinks[i]->getSecondary(j)->getSpeed() -
...
int getCorrespondingNumber(TTrack* compare, int nTo, TTrack** array, float maxDist, int mode)
{
  int num = 0;
  int ret = -1;
  // cout<<"getCorrespondingNumber(compare="<<compare<<", nTo="<<nTo<<",
  // array="<<array<<",
  // mode="<<mode<<") "<<endl;
  if (mode == 0) {
    if (nTo == 0)
      return -1;
    for (int j = 1; j < nTo; j++)
      if (acos(compare->getPath().Direction() * array[j]->getPath().Direction()) <
          acos(compare->getPath().Direction() * array[num]->getPath().Direction())) {
        num = j;
      }
    if (num == 0) {
    ...
    if (compare->getNumberOfSecondaryTracks() < 2)
      return -1;
    if (nTo == 0)
      return -1;
    float a1, a2, a3, a4;
    float b1, b2, b3, b4;
    b1 = acos(compare->getSecondary(0)->getPath().Direction() *
              array[0]->getSecondary(0)->getPath().Direction());
    b2 = acos(compare->getSecondary(1)->getPath().Direction() *
              array[0]->getSecondary(1)->getPath().Direction());
    b3 = acos(compare->getSecondary(1)->getPath().Direction() *
              array[0]->getSecondary(0)->getPath().Direction());
    b4 = acos(compare->getSecondary(0)->getPath().Direction() *
              array[0]->getSecondary(1)->getPath().Direction());
    for (int j = 1; j < nTo; j++) {
      if (array[j]->getNumberOfSecondaryTracks() < 2)
        continue;
      a1 = acos(compare->getSecondary(0)->getPath().Direction() *
                array[j]->getSecondary(0)->getPath().Direction());
      a2 = acos(compare->getSecondary(1)->getPath().Direction() *
                array[j]->getSecondary(1)->getPath().Direction());
      a3 = acos(compare->getSecondary(1)->getPath().Direction() *
                array[j]->getSecondary(0)->getPath().Direction());
      a4 = acos(compare->getSecondary(0)->getPath().Direction() *
                array[j]->getSecondary(1)->getPath().Direction());
    ...
    for (int j = 1; j < nTo; j++) {
      for (int k = 0; k < mode; k++) {
        if (k >= array[j]->getNumberOfSecondaryTracks())
          continue;
        if (cos(compare->getPath().Direction() * array[j]->getSecondary(k)->getPath().Direction()) <
            acos(compare->getPath().Direction() *
                 array[num / mode]->getSecondary(num % mode)->getPath().Direction())) {
          num = j * mode + k;
        }
      }
    }
    if (num == 0) {
      if (acos(compare->getPath().Direction() *
               array[num / mode]->getSecondary(num % mode)->getPath().Direction()) > maxDist)
        ret = -1;
      else
        ret = num;
    } else
