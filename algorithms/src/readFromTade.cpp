...
void setFee(feederstruct_t& a,feederstruct_t *f){a.feederName=f->feederName;a.inputFile=f->inputFile;a.fifoFile=f->fifoFile;};
...
AReadFromTade::AReadFromTade(/*ifstream **inputIn,*/ TRawHit*** hitIn,int &eventNumberIn, int &triggerIn, int **numHits, int maxdet, int maxhit, int &readID,void *input_mutexIn,bool &validInputIn):
  AAlgorithm("read from tade"),
  EventNumber(eventNumberIn),
  trigger(triggerIn),
  maxHit(maxhit),
  maxDet(maxdet),
  whichID(readID),
  validInput(validInputIn),filetype(0)
{
...
//   if(whichID==9)
      ...
      if(!init)
	{
	  ...
	  while (!(s.find("vent")<s.npos||s.find("Mcarlo")<s.npos||s.find("onte")<s.npos||input->eof()))
	    {
	      //  cout<<s.length()<<endl;
      ...
      while(condition)
	{
	  ...
	  while (!((s.find("vent")<s.npos)||(s.find("Mcarlo")<s.npos)||s.find("onte")<s.npos))
	    {
	      if(count>=0)
		{
		  if (((data[2]<maxDet)&&(data[3]<1000000)&&(*numberOfHits[data[2]]<maxHit)&&(data[2]>=0)&&(data[3]>=0)))
		    {
		      ...
		      //		      cout<<(*(numberOfHits[data[2]]))+1<<" "<<*hut<<endl;
		    }
		}
	      count++;
	      for(int i=0;i<4;i++)
		{
	      ...
	      //	      cout<<nextEventNumber<<", det:"<<data[2]<<", el:"<<data[3];
	      s=reader.getLine();//string(li);
	      //   cout<<"\""<<s.data()<<"\""<<endl;
	      if(data[2]<0)
		{
	  ...
	  if(count==nextEventCount) 
	    ...
	    {
	      condition=true;
	      anaLog<<"corrupt event ("<<nextEventNumber<<"): wrong number of hits in header ("<<count<<" vs. "<<nextEventCount<<"). Read new event "<<data[1]<<endli;
	    }
...
// #endif
  ...
  if(file_name.find("/")<file_name.npos)
    {
      ...
      file_name=file_name.substr(file_name.rfind("/")+1,file_name.length()-1-file_name.rfind("/"));
    }
  if(file_name.find(".")<file_name.npos)
    {
      file_ext=file_name.substr(file_name.find(".")+1,file_name.length()-1-file_name.find("."));
      file_name=file_name.substr(0,file_name.find("."));
    }
  //  cout<<"\""<<filename.data()<<"\" \""<<file_name.data()<<"\" \""<<file_ext.data()<<"\" \""<<directory_name.data()<<"\""<<endl;
  ...
  string exts[8]={"tade","evt","tade.zip","tade.gz","evt.zip","evt.gz","tade.bz2","evt.bz2"};
  int ft=-1;
  for(int i=7;i>=0;i--)
    {
      if(file_ext==exts[i]||existing(directory_name+"/"+file_name+"."+exts[i]))ft=i;
    }
//   cout<<filename.data()<<" "<<file_ext.data()<<" "<<ft<<endl;;
  if(ft==-1)
	...
	filetype=0;
      else if(file_ext=="tade.zip"||file_ext=="tade.gz"||file_ext=="evt.zip"||file_ext=="evt.gz")
	filetype=1;
  ...
  else filetype=(ft>1?1:0);
  if(needsFifo||ft==2||ft==3||ft==4||ft==5||ft==6||ft==7)needsFifo=true;
  if(ft==2||ft==3||ft==4||ft==5||ft==6||ft==7)needsdecompression=true;
  if(ft==6||ft==7)needsdecompression2=true;
  validInput=(ft>=0);
//   cout<<needsdecompression<<" "<<needsdecompression2<<endl;
  ...
  if(needsFifo)
	...
	((QProcess*)feeder)->start((scriptFile+" "+actualFileName+" "+fifoFile).data());
#endif
      */
	...
	{
	  anaLog<<" fifo via ";
	  if(!needsdecompression)anaLog<<"cat ";
	  else if(!needsdecompression2)anaLog<<"zcat ";
	  else anaLog<<"bzcat ";
	...
	{
	  cout<<" fifo via";
	  if(!needsdecompression)cout<<" cat ";
	  else if(!needsdecompression2)cout<<" zcat ";
	  else cout<<" bzcat ";
