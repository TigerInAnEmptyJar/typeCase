...
algorithm_parameter AVLineTrackSearch::getDescription()
{
  ...
  ret.addParam<vector<float> >(single_parameter<vector<float> >("pixel: max chi squared for track",tmp2));
  ret.addParam<vector<float> >(single_parameter<vector<float> >("cluster: max chi squared for track",tmp2));
  ret.addParam<vector<float> >(single_parameter<vector<float> >("max distance",tmp2));
  ret.addParam<vector<float> >(single_parameter<vector<float> >("vertex z-position from target",tmp2));
  return ret;
}
void coutHitsInTrack(TTrack *tr, const char*separator)
{
  for(int i=0;i<tr->getNumberOfCalibHits(-2);i++)
    for(int j=0;j<tr->getNumberOfCalibHits(i);j++)
      cout<<tr->getCalibSrcr(i,j)->getDetector()<<"-"<<tr->getCalibSrcr(i,j)->getElement()<<separator;
}
...
void trackVertexTrack::setTracks(TTrack *tr1,TTrack *tr2)
{
  ...
  for(int j=0;j<maxDet;j++)
    for(int i=0;i<tr1->getNumberOfCalibHits(j);i++)
      {
	if(!(tr1->getCalibSrcr(j,i)->isValid()&&tr1->getCalibSrcr(j,i)->getHitShape()!=NULL))continue;
  ...
  for(int j=0;j<maxDet;j++)
    for(int i=0;i<tr2->getNumberOfCalibHits(j);i++)
      {
	if(!(tr2->getCalibSrcr(j,i)->isValid()&&tr2->getCalibSrcr(j,i)->getHitShape()!=NULL))continue;
  ...
  if(nInCommon>2)chi=1000;
}
int nEventsWithVees;
AVLineTrackSearch::AVLineTrackSearch(TSetup &setupIn, TTrack **tracksIn, TCluster ***clustersIn, TPixel ***pixelsIn, TCalibHit ***hitsIn, int maxTracksIn, int &numberOfTracksIn,int **numberOfClustersIn, int **numberOfPixelsIn, int **numberOfHitsIn, const algorithm_parameter &descr):AAlgorithm("V-Line Track search"),setup(setupIn), maxTracks(maxTracksIn),numberOfTracks(numberOfTracksIn)
{
  nEventsWithVees=0;
  hits=hitsIn;
  pixels=pixelsIn;
  ...
  for(int i=0;i<maxPlanes;i++)vee[i]=new trackVertexTrack(100);
  ...
  for(int i=0;i<nDet;i++)chels[i]=new TCalibHit*[maxEl];
  ...
  cout<<"pixels   :"<<flush;
  for(int i=0;i<numStart;i++)cout<<startIDs[i]<<"\t"<<flush;
  cout<<"\nclusters :"<<flush;
  for(int i=0;i<numStartCluster;i++)cout<<startClusterIDs[i]<<"\t"<<flush;
  cout<<"\n\nDetectors : "<<flush;
  for(int i=0;i<numDetectors;i++)cout<<detectorIDs[i]<<"\t"<<flush;
  cout<<endl<<"          : "<<flush;
  for(int i=0;i<numDetectors;i++)cout<<conventional[i]<<"\t"<<flush;
  ...
  for(int i=0;i<numNoGo;i++)cout<<noGoDetectors[i]<<"\t"<<flush;
  cout<<endl<<"  suspectDist:\n";
  for(int i=0;i<numStop;i++)
    {
      cout<<"  pix "<<stopIDs[i]<<"       : "<<flush;
...
AVLineTrackSearch::~AVLineTrackSearch()
{
  ...
  for(int i=0;i<numStop;i++)delete []essentials[i];
  delete []essentials;
  delete []minElements;
  delete []maxChi;
  delete[]stopClusterIDs;
  delete[]numClusterEssentials;
  for(int i=0;i<numStopCluster;i++)delete []clusterEssentials[i];
  ...
  delete []nEl;
  for(int i=0;i<nDet;i++)delete[] chels[i];
  delete []chels;
...
{
  if(tracknum>=maxTracks)return false;
  if(pix==NULL)return false;
  if(spix==NULL)return false;
  if(!(pix->name()=="TPixel"||pix->name()=="TCluster"))return false;
  if(!(spix->name()=="TPixel"||spix->name()=="TCluster"))return false;
  point3D vertex(0,0,0),p1,p2;
  int typ1,typ2;
  if(pix->name()=="TPixel")typ1=0;else typ1=1;
  if(spix->name()=="TPixel")typ2=0;else typ2=1;
  if(typ1==0)
  ...
  for(int i=0;i<maxHitPerTrack;i++)parameter[i].reDimension(13);
  ...
  for(int i=0;i<numNoGo;i++)
      ...
      if(suspect>=0&&suspect<det->getNumberOfElements())
	{
	  for(int j=0;j</*(*(numberOfHits[noGoDetectors[i]]))*/nEl[noGoDetectors[i]];j++)
	    {
	      if(!/*hits*/chels[noGoDetectors[i]][j]->isValid())continue;
	      if(/*hits*/chels[noGoDetectors[i]][j]->getHitShape()==NULL)continue;
		...
		return false;//has no go detector
	      ...
	      else if(abs(/*hits*/chels[noGoDetectors[i]][j]->getElement()-suspect)<=away[constant+i])
	      {
#ifdef DEBUGTRACKSEARCH
		cout<<"no go detector found "<<noGoDetectors[i]<<endl;
#endif
  ...
  for(int i=0;i<tr->getNumberOfCalibHits(-2);i++)
	...
	if(vector3D(bestHit[10],bestHit[11],bestHit[12]).R()>maxDistance[typ1*numStop+stopI])
	  {
	    tr->removeCalibSrc(tr->getCalibSrcr(i,j));
	    continue;
	  }
	parameter[num]=bestHit;
	if(num<maxHitPerTrack-1)num++;
      }
  ...
  for(int i=0;i<numDetectors;i++)
      ...
      if(!found)
	{
	  if(conventional[i]==1)
		...
		{
		  if(!/*hits*/chels[detectorIDs[i]][j]->isValid())continue;
		  ...
		  if(dist.R()<__prec){bestEl=j;bestHit=hitParam;break;}
		  if(dist.R()>maxDistance[typ1*numStop+stopI])
		    continue;
		  if(bestD>dist.R()){bestEl=j;bestD=dist.R();bestHit=hitParam;}
		}
	      if(bestEl<0)continue;
	      n1=tr->getNumberOfCalibHits(detectorIDs[i]);
	      tr->addCalibSrc(/*hits*/chels[detectorIDs[i]][bestEl]);
	      if(n1==tr->getNumberOfCalibHits(detectorIDs[i]))continue;
	      parameter[num]=bestHit;
	      if(num<maxHitPerTrack-1)num++;
	    }
	  else
	      ...
	      if(suspect>=0&&suspect<det->getNumberOfElements())
		{
		  for(int j=0;j<nEl[detectorIDs[i]]/*(*(numberOfHits[detectorIDs[i]]))*/;j++)
		    {
		      if(!/*hits*/chels[detectorIDs[i]][j]->isValid())continue;
		      if(/*hits*/chels[detectorIDs[i]][j]->getHitShape()==NULL)continue;
			...
			tr->addCalibSrc(/*hits*/chels[detectorIDs[i]][j]);
		      else if(abs(/*hits*/chels[detectorIDs[i]][j]->getElement()-suspect)<=away[constant+i])
			tr->addCalibSrc(/*hits*/chels[detectorIDs[i]][j]);
		    }
		}
	    }
...
#ifdef DEBUGTRACKSEARCH
  cout<<"# hits:"<<tr->getNumberOfCalibHits(-1)<<" min: "<<(typ1==0?minElements[stopI]:minClusterElements[stopI])<<endl;
  for(int i=0;i<tr->getNumberOfCalibHits(-2);i++)
    {
      for(int j=0;j<tr->getNumberOfCalibHits(i);j++)
...
#endif
  bool condition=tr->getNumberOfCalibHits(-1)>=
    (typ1==0?minElements[stopI]:minClusterElements[stopI]);
  if(!condition)
    {
#ifdef DEBUGTRACKING
      cout<<"searchATrack() condition not met "<<tr->getNumberOfCalibHits(-1)<<"("<<(typ1==0?minElements[stopI]:minClusterElements[stopI])<<")"<<endl;
#endif
      return false;
    }
  ...
  point3D enter,htt;
  vector3D tmp,tmp2;
  tmp.setRep(_spheric_);tmp2.setRep(_spheric_);
  v1.setRep(_spheric_);
  for(int i=0;i<numDetectors;i++)
    {
      if(conventional[i])continue;
  ...
  condition=tr->getNumberOfCalibHits(-1)>=
    (typ1==0?minElements[stopI]:minClusterElements[stopI]);
  condition=(condition&&(tr->getNumberOfCalibHits(6)+tr->getNumberOfCalibHits(0)+tr->getNumberOfCalibHits(3)>0));
#ifdef DEBUGTRACKSEARCH
  cout<<"# hits:"<<tr->getNumberOfCalibHits(-1)<<" min: "<<(typ1==0?minElements[stopI]:minClusterElements[stopI])<<endl;
#endif
  if(!condition)
...
#endif
	  line1.addPoint(vector3D(parameter[i][7],
				  parameter[i][8],
				  parameter[i][9]),
			 vector3D(parameter[i][4],
				  parameter[i][5],
				  parameter[i][6]),
			 parameter[i][0]);
	}
...
bool AVLineTrackSearch::trackAPlane(int planenum, int id[2])
{
...
#ifdef DEBUGTRACKING
  cout<<"trackAPlane(plane number "<<planenum<<", ids={"<<id[0]<<","<<id[1]<<"})"<<endl;
  cout<<"points"<<numPlanePoints[planenum]<<endl;
#endif
  if(numPlanePoints[planenum]<4)
    {
#ifdef DEBUGINFO
      cout<<"trackAPlane("<<planenum<<") not enough points "<<numPlanePoints[planenum]<<endl;
#endif
      return false;
    }
  if(planePoints[planenum][0]==NULL||planePoints[planenum][1]==NULL)return false;
  for(int i=0;i<numPlanePoints[planenum];i++)
    if(!(planePoints[planenum][i]->name()=="TPixel"||
	 planePoints[planenum][i]->name()=="TCluster"))return false;
  ...
  for(int l=0;l<2;l++)
	...
	cout<<sLine3D(points[i],points[l]-points[i])/*.toString(_spheric_*4).data()*/<<endl;
      }
#endif
...
//	ID=((TPixel*)planePoints[planenum][l])->getID();
      ...
      for(int i=2;i<numPlanePoints[planenum];i++)
	{
	  if(numPlaneTracks[l]>=maxPlanes)continue;
...
#ifdef DEBUGTRACKING
	  cout<<"search track for: "<<pointTracks[l][numPlaneTracks[l]]->getPath().toString(_spheric_*4).data()<<endl;
#endif
	  if(searchATrack(numPlaneTracks[l],l, id[l], planePoints[planenum][l], planePoints[planenum][i]))
	    {
#ifdef DEBUGTRACKING
	      cout<<"taken: "<<pointTracks[l][numPlaneTracks[l]]->getPath().toString(_spheric_*4).data()<<endl;
#endif
	      numPlaneTracks[l]++;
	    }
	  for(int j=i+1;j<numPlanePoints[planenum];j++)
	      ...
	      for(int jj=0;jj<nPixels[l];jj++)
		{
	      ...
	      vector3D dist;
	      if(typ[l]==0)
		dist=((TPixel*)planePoints[planenum][l])->getElementr(0).getHitShape()->distance(pointTracks[l][numPlaneTracks[l]]->getPath());
	      else
		dist=pointTracks[l][numPlaneTracks[l]]->getPath()-((TCluster*)planePoints[planenum][l])->getCenter();
...
#ifdef DEBUGINFO
	cout<<"trackAPlane("<<planenum<<") not enough tracks found for point "<<l<<"("<<numPlanePoints[planenum]-2<<")"<<endl;
#endif
	delete []pixs[0];delete []pixs[1];return false;}
    }
  ...
  for(int i=0;i<numPlaneTracks[0];i++)
	...
	cout<<"# hits "<<pointTracks[0][i]->getNumberOfCalibHits()<<" "
	    <<pointTracks[1][j]->getNumberOfCalibHits()<<" minimum "
	    <<minElementsPerVee<<endl;
#endif
#ifdef DEBUGINFO
	chivees[j+i*numPlaneTracks[1]]=-1;
#endif
	if(numVees>=maxPlanes-1)continue;
	if(pointTracks[0][i]->getNumberOfCalibHits()+
	   pointTracks[1][j]->getNumberOfCalibHits()<minElementsPerVee)continue;
	int nInCommon=0;
	...
	for(int kkk=0;kkk<pointTracks[1][j]->getNumberOfCalibHits(iii);kkk++)
	  {
	    if(pointTracks[0][i]->getCalibSrcr(iii,jjj)->getElement()==pointTracks[1][j]->getCalibSrcr(iii,kkk)->getElement())
	      nInCommon++;
	  }
    if(nInCommon>2)continue;
...
#endif
	if(vee[numVees]->getChiSquared()>maxVertexChi)continue;
	inDists=false;
	for(int i=0;i<numZDists;i++)
	...
	d3.normalize();d1.normalize();d2.normalize();
#ifdef DEBUGTRACKING
	cout<<"distance vertex-plane "<<dist.R()<<" max "<<maxDistanceVertexPlane<<endl;
	cout<<"angle "<<abs(acos(d1*d3)+acos(d2*d3)-acos(d1*d2))<<" max "<<maxAngle<<endl;
#endif
	if(dist.R()>maxDistanceVertexPlane)continue;
	if((abs(acos(d1*d3)+acos(d2*d3)-acos(d1*d2))>maxAngle))continue;
	numVees++;
      }
#ifdef DEBUGTRACKING
...
bool AVLineTrackSearch::assignPointsToPlane(int planenum)
{
  ...
  for(int i=0;i<numStartCluster;i++)//search all start cluster types
    {
      n=0;
#ifdef DEBUGPLANING
      cout<<"clusterID "<<startClusterIDs[i]<<" "<<*(numberOfClusters[startClusterIDs[i]])<<endl;
#endif
      for(int j=0;j<*(numberOfClusters[startClusterIDs[i]]);j++)//search all clusters of that type
	{
	  if(checkWithPrompt(clusters[startClusterIDs[i]][j])>maxPixelPrompt)continue;
      ...
      if(n<minStarts[i+numStart])return false;
    }
#ifdef DEBUGPLANING
  ...
  cout<<"assignPointsToPlane() "<<numPlanePoints[planenum]<<" points found ("<<npix<<","<<nclu<<")"<<endl;
#endif
  return false;
}
bool AVLineTrackSearch::checkPlane(int planenum,int l,int k)
{
  if(numPlanePoints[planenum]<4)return false;
      ...
      for(int i=0;i<numberOfTracks;i++)
	{
	  ...
	  for(int j=0;j<pix->getNumberOfActualElements();j++)
		...
		if(tracks[i]->getCalibSrcr(det,ii)==&pix->getElementr(j))
		  {
#ifdef DEBUGINFO
		    cout<<"double element found: Track "<<i<<" det:"<<det<<" el: "<<pix->getElementr(j).getElement()<<" - "<<tracks[i]->getCalibSrcr(det,ii)->getElement()<<" "<<&pix->getElementr(j)<<" "<<tracks[i]->getCalibSrcr(det,ii)<<endl;
#endif
		    counter2++;
		  }
	      }
      ...
      for(int i=0;i<numberOfTracks;i++)
	{
	  ...
	  for(int j=0;j<pix->getNumberOfPixels();j++)
	    for(int jj=0;jj<pix->getPixelr(j).getNumberOfActualElements();jj++)
	      for(int ii=0;ii<tracks[i]->getNumberOfCalibHits(pix->getPixelr(j).getElementr(jj).getDetector());ii++)
		{
		  if(tracks[i]->getCalibSrcr(pix->getPixelr(j).getElementr(jj).getDetector(),ii)==&pix->getPixelr(j).getElementr(jj))
		    counter2++;
		}
	}
    }
  if(k==0)
      ...
      for(int i=0;i<numberOfTracks;i++)
	{
	  ...
	  for(int j=0;j<pix->getNumberOfActualElements();j++)
	    {
	      det=pix->getElementr(j).getDetector();
	      for(int ii=0;ii<tracks[i]->getNumberOfCalibHits(pix->getElementr(j).getDetector());ii++)
		if(tracks[i]->getCalibSrcr(pix->getElementr(j).getDetector(),ii)==&pix->getElementr(j))
		  {
#ifdef DEBUGINFO
		    cout<<"double element found: Track "<<i<<" det:"<<det<<" el: "<<pix->getElementr(j).getElement()<<" - "<<tracks[i]->getCalibSrcr(det,ii)->getElement()<<" "<<&pix->getElementr(j)<<" "<<tracks[i]->getCalibSrcr(det,ii)<<endl;
#endif
...
#endif
  if(counter1<=maxEqualElements&&counter2<=maxEqualElements)return true;
#ifdef DEBUGINFO
  cout<<"checkPlane() counter1: "<<counter1<<"("<<maxEqualElements<<"), counter2: "<<counter2<<"("<<maxEqualElements<<")"<<endl;
#endif
  return false;
}
bool AVLineTrackSearch::checkWithPrompt(TBase *p1, TBase *p2, int i, int j)
{
  ...
  cout<<"t1    :"<<i<< " t2    :"<<j<<endl;
  cout<<"t1_c  :"<<(p1->name()=="TPixel"?0:(p1->name()=="TCluster"?1:2))<<" t2_c  :"<<(p2->name()=="TPixel"?0:(p2->name()=="TCluster"?1:2))<<endl;
#endif
...
int AVLineTrackSearch::checkWithPrompt(TPixel *pix)
{
  ...
  for(int i=0;i<pix->getNumberOfActualElements();i++)
	...
	for(int k=0;k<tracks[j]->getNumberOfCalibHits(pix->getElementr(i).getDetector());k++)
	  if(tracks[j]->getCalibSrcr(pix->getElementr(i).getDetector(),k)->getElement()==pix->getElementr(i).getElement())found=true;
      if(found)nEls++;
    }
  return nEls;
}
int AVLineTrackSearch::checkWithPrompt(TCluster *clu)
{
  ...
  for(int l=0;l<clu->getNumberOfPixels();l++)
	...
	found=false;
	for(int j=0;j<numberOfTracks;j++)
	  for(int k=0;k<tracks[j]->getNumberOfCalibHits(clu->getPixelr(l).getElementr(i).getDetector());k++)
	    if(tracks[j]->getCalibSrcr(clu->getPixelr(l).getElementr(i).getDetector(),k)->getElement()==clu->getPixelr(l).getElementr(i).getElement())found=true;
	if(found)nEls++;
      }
  return nEls;
}
...
//   TBase *pto[MAX];
  ...
  for(int i=0;i<numStop;i++)
	...
	{
	  if(pixels[stopIDs[i]][j]->getShape()==NULL)continue;
	  if(checkWithPrompt(pixels[stopIDs[i]][j])>maxPixelPrompt)continue;
	  pto[nPoints]=pixels[stopIDs[i]][j];
	  pID[nPoints]=stopIDs[i];
	  id[nPoints]=0;
	  pts[nPoints]=pixels[stopIDs[i]][j]->getShape()->getCenter();
	  if(nPoints>=(int)MAX-1)continue;
	  nPoints++;
	}
    }
  for(int i=0;i<numStopCluster;i++)
	...
	{
	  if(checkWithPrompt(clusters[stopClusterIDs[i]][j])>maxPixelPrompt)continue;
  ...
  for(int i=0;i<nPoints-1;i++)
      ...
      for(int j=i+1;j<nPoints;j++)
	{
...
#endif
	  if(tmpNum>=maxPlanes-1)continue;
	  if(!checkWithPrompt(pto[i],pto[j],id[i],id[j]))
	    continue;
	  if(outcode<1)outcode=1;
...
#endif
	  if((v1*v2)/v1.R()/v2.R()>.9987)continue;
	  ...
	  if(outcode<3)outcode=3;
	  IDs[0]=pID[i];
	  IDs[1]=pID[j];
	  if(!trackAPlane(tmpNum,IDs))
	    continue;
	  if(outcode<4)outcode=4;
	  tmpNum++;
  ...
  if(tmpNum<=0)cout<<"done"<<endl;
  if(outcode>2)cout<<outcode<<endl;
#endif
  if(tmpNum<=0)return ptr;
  int position[tmpNum];
  int pos;
  int tmp;
  ...
  for(int i=0;i<tmpNum;i++)
	...
	if(planeTrack[0][position[pos]]->getChiSquared()>planeTrack[0][position[j]]->getChiSquared())
	  pos=j;
      tmp=position[i];
  ...
  for(int i=0;i<tmpNum;i++)
    for(int j=i+1;j<tmpNum;j++)
      {
#ifdef DEBUGINFO
	// 	cout<<"vertex distance "<<(planeTrack[0][position[i]]->getPath().Foot()-planeTrack[0][position[j]]->getPath().Foot()).toString(_spheric_).data()<<endl;
	// 	cout<<"Normal : "<<plane[position[i]].Normal()*plane[position[j]].Normal()<<" "<<plane[position[i]].Normal()<<plane[position[j]].Normal()<<endl;
// 	cout<<"0-0  :"<<planeTrack[0][position[i]]->getPath().Direction()*planeTrack[0][position[j]]->getPath().Direction()<<endl;
...
// 	  planeTrack[0][position[j]]->getPath().Direction()<<endl;
	...
	if(abs(plane[position[i]].Normal()*plane[position[j]].Normal())>0.9987&&
	   (((planeTrack[0][position[i]]->getPath().Direction()*
	      planeTrack[0][position[j]]->getPath().Direction()>0.9987)&&
	     (planeTrack[1][position[i]]->getPath().Direction()*
	      planeTrack[1][position[j]]->getPath().Direction()>0.9987))||
	    ((planeTrack[0][position[i]]->getPath().Direction()*
	      planeTrack[1][position[j]]->getPath().Direction()>0.9987)&&
	     (planeTrack[1][position[i]]->getPath().Direction()*
	      planeTrack[0][position[j]]->getPath().Direction()>0.9987))))
...
// 	       planeTrack[0][position[j]]->getPath().Foot()).R()<5))
  ...
  cout<<numberOfTracks<<" "<<maxTracks<<" "<<maxTracks-numberOfTracks<<" "<<(maxTracks-numberOfTracks)/3<<endl;
#endif
  ...
  for(int i=0;i<nums;i++)
    {
      ...
      direction.normalize();
#ifdef DEBUGINFO
      cout<<"plane "<<i<<":"<<position[i]<<" "<<numberOfTracks+nums+i*2+1<<" "<<maxTracks<<endl;
      cout<<(tracks[numberOfTracks+nums+i*2+0])<<" "
      ...
      cout<<" first  :"<<planeTrack[0][position[i]]->getPath().Direction().toString(_spheric_).data()<<endl;
      cout<<" second :"<<planeTrack[1][position[i]]->getPath().Direction().toString(_spheric_).data()<<endl;
#endif
    }
#ifdef DEBUGINFO
...
do Vee-fit
  ...
  for(int i=0;i<numStart;i++)
    for(int j=0;j<*(numberOfPixels[startIDs[i]]);j++)
      {
	if(pixels[startIDs[i]][j]->getShape()==NULL)continue;
	if(checkWithPrompt(pixels[startIDs[i]][j])>maxPixelPrompt)continue;
	...
	s_id[nStartP]=i;
	zBuffer[nStartP]=pixels[startIDs[i]][j]->getShape()->getCenter().R();///startPoints[nStartP].R();
	nStartP++;
      }
  for(int i=0;i<numStartCluster;i++)
	...
	zBuffer[nStartP]=clusters[startClusterIDs[i]][j]->getCenter().R();///startPoints[nStartP].R();
	nStartP++;
      }
  int pop[100];
  int npop=0;
  ...
  float size1,size2;  
  vector2D dist;
  int nvee=0;int best=0,ncommon=0;
  point3D pp;
  bool hasPrimary;
  for(int i=0;i<nStopP;i++)
    {
      if(planeNum>=maxPlanes)continue;
      if(pto[i]->name()=="TPixel"){
	id1=((TPixel*)pto[i])->getID();
	size1=((TPixel*)pto[i])->getShape()->circumscribedRadius()/((TPixel*)pto[i])->getShape()->getCenter().Z();
      }
      else {
	...
	if(acos(tracks[ii]->getPath().Direction()*(vector3D(pts[i])/pts[i].R()))<0.01)hasPrimary=true;
      ...
      for(int j=i+1;j<nStopP;j++)
	{
	  ...
	  hasPrimary=false;
	  for(int ii=0;ii<numberOfTracks;ii++)
	    if(acos(tracks[ii]->getPath().Direction()*(vector3D(pts[j])/pts[j].R()))<0.01)hasPrimary=true;
	  if(hasPrimary)continue;
	  if(pto[j]->name()=="TPixel"){
	    id2=((TPixel*)pto[j])->getID();
	    size2=((TPixel*)pto[j])->getShape()->circumscribedRadius()/((TPixel*)pto[j])->getShape()->getCenter().Z();
	  }
	  else {
	  ...
	  for(int k=0;k<numStart+numStartCluster;k++)nOn[k]=0;
	  ...
	  if(npop<(numStart+numStartCluster)*2)continue;
	  bool enough=true;
	  for(int k=0;k<numStart+numStartCluster;k++)
	    if(nOn[k]<2)enough=false;
	  if(!enough)continue;
	  int smallest,smallestz=0;
	  int tmp;
	  for(int k=1;k<npop;k++)
	    if(zBuffer[pop[k]]<zBuffer[smallestz])smallestz=pop[k];
	  for(int k=0;k<npop;k++)
	    {
	      smallest=k;
	      for(int l=k+1;l<npop;l++)
		{
		  if((stopPoints[i]-startPoints[pop[smallest]]).R()>(stopPoints[i]-startPoints[pop[l]]).R())smallest=l;
		}
	      tmp=pop[k];
	      pop[k]=pop[smallest];
	      pop[smallest]=tmp;
	  ...
	  tmp=1;
	  while(zBuffer[pop[tmp-1]]>=zBuffer[pop[tmp]]&&tmp<npop-1)tmp++;
	  smallest=tmp;
	  while(zBuffer[pop[tmp-1]]<=zBuffer[pop[tmp]]&&tmp<npop-1)tmp++;
	  if(tmp<npop-2)continue;
	  enough=true;
	  int ndt[2]={0,0};
	  for(int k=0;k<2;k++)
	    {
	      for(int l=k*(smallest-1);l<(k==0?smallest+1:npop);l++)
		{
		  ...
		  pointTracks[k][ndt[k]]->setPath(sLine3D(pts[(k==0?i:j)],s_pts[k]-pts[(k==0?i:j)]));
		  if(searchATrack(ndt[k],k,(k==0?id1:id2),pto[(k==0?i:j)],
				  s_pto[pop[l]]))//smallest-1*k+1]]));
		    ndt[k]++;
		}
	    }
	  if(ndt[0]==0||ndt[1]==0)continue;
	  nvee=0;best=0;
	  for(int k=0;k<ndt[0];k++)
	    {
	      for(int l=0;l<ndt[1];l++)
		{
		  if(nvee>=maxPlanes)continue;
		  if(pointTracks[0][k]->getNumberOfCalibHits()+pointTracks[1][l]->getNumberOfCalibHits()<minElementsPerVee)continue;
		  ncommon=0;
		  for(int ii=0;ii<pointTracks[0][k]->getNumberOfCalibHits(-2);ii++)
			...
			if(pointTracks[0][k]->getCalibSrcr(ii,iii)==pointTracks[1][l]->getCalibSrcr(ii,jj))
			  ncommon++;
		  if(ncommon>=2)continue;
		  vee[nvee]->setTracks(pointTracks[0][k],pointTracks[1][l]);
		  if(!(vee[nvee]->getChiSquared()<=1000))continue;
		  if(!(vee[nvee]->getVertex().Z()>=5&&vee[nvee]->getVertex().Z()<=120))continue;
		  //if(vee[nvee]->getVertex().Z()>zBuffer[smallest]*cos(vee[nvee]->getVertex().theta())*0.9)continue;
		  if(vee[nvee]->getChiSquared()<vee[best]->getChiSquared())best=nvee;
		  nvee++;
		}
	    }
	  if(nvee<=0)continue;
	  ...
	  if(vee[best]->getChiSquared()>maxVertexChi)continue;
