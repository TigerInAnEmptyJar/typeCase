...
ATeufelCorrection::ATeufelCorrection(TCalibHit ** hitsIn, const int &numberOfHitsIn, const int detID, int nElIn, int& evtNrIn, int&runNrIn, vector<string> calibrationFiles, string referencefile):AAlgorithm((string("TeufelCorrection for det ")+string_number(detID))),numberOfHits(numberOfHitsIn),nEl(nElIn),evtNr(evtNrIn),runNr(runNrIn)
{
  hits=hitsIn;
  sranges=NULL;
  shift=NULL;
  ...
  for(int i=0;i<nEl;i++)
    {
      ...
      for(int j=0;j<9;j++){refe.get(c);}//cout<<c<<flush;}
      refe>>f1>>f2;
      params[i][0]=f1;
      params[i][1]=f2;
//       refe>>params[i][0];
...
//   cout<<"check range"<<endl;
  if(!(evtNr<sranges[actualrange][1]&&evtNr>sranges[actualrange][0]&&
	...
	actualrange++;
      if(actualrange>=numSranges)return ptr;
    }
  int adc,el;
  double adcC;
...
//       cout<<shift[actualrange]<<" "<<params[el][0]<<" "<<params[el][1]<<endl;
