  ...
  for(int i=0;i<maxDet;i++)
    {
      ...
      tree->Branch((string("clusters")+string_number(i)).data(),"TObjArray",&clusters[i],32000,0);
    }
  tree->Branch("Tracks","TObjArray",&tracks,32000,0);

}

AWriteDataToRootFile::AWriteDataToRootFile(TFile **rootFileIn, const string &directory, const string &treeName, TEvent &eventIn,TSetup &setupIn,int maxTracks,bool serv):AAlgorithm("Write data to root file"),setup(setupIn),maxDet(eventIn.getMaxNumber<TDetector>()),maxTrack(maxTracks),maxCluster(eventIn.getMaxNumber<TCluster>()),maxPixel(eventIn.getMaxNumber<TPixel>()),maxHit(eventIn.getMaxNumber<TCalibHit>()),server(serv)
{
  //this is used
...
TFile *AWriteDataToRootFile::getFile()
{
  return *rootFile;
}
