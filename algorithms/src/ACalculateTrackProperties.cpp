...
void setTwotwoTracks(TTrack *p1,TTrack *p2,TTrack *s1, TTrack *s2, point3D *pf,point3D *pb, float *tf, float *tb);
...
ACalculateTrackProperties::ACalculateTrackProperties(TTrack **t, int& numberOfTracksIn, int max, const TDetector &st, const algorithm_parameter & descr):AAlgorithm("Calculate track properties"),maxTracks(max),numberOfTracks(numberOfTracksIn),numStart(descr.getParam<vector<int> >(0).getData().size()),numStop(descr.getParam<vector<int> >(1).getData().size())
{
  startIDs=new int[numStart];
  for(int i=0;i<numStart;i++) startIDs[i]=descr.getParam<vector<int> >(0).getData().at(i);
  stopIDs=new int[numStop];
  for(int i=0;i<numStop;i++)  stopIDs[i]=descr.getParam<vector<int> >(1).getData().at(i);
#ifdef SHOWPARAMS
  ...
  else chargedPrompt=true;
  if(descr.getNumberOfParam<bool>()>1)
    neutralPrompt=descr.getParam<bool>(1).getData();
  else neutralPrompt=false;
  for(int i=0;i<4;i++)
    if(descr.getNumberOfParam<bool>()>2+i)
      secondary[i]=descr.getParam<bool>(2+i).getData();
    else secondary[i]=false;
#ifdef TWOTWO_out
  defineTwotwoTree();
#endif
}

ACalculateTrackProperties::ACalculateTrackProperties(TTrack **t, int& numberOfTracksIn, int max, const TDetector &st, vector<int> stopDetectors, vector<int> startDetectors):AAlgorithm("Calculate track properties"),maxTracks(max),numberOfTracks(numberOfTracksIn),numStart(startDetectors.size()),numStop(stopDetectors.size())
{
  startIDs=new int[numStart];
  for(int i=0;i<numStart;i++) startIDs[i]=startDetectors[i];
  stopIDs=new int[numStop];
  for(int i=0;i<numStop;i++)  stopIDs[i]=stopDetectors[i];
  tracks=t;
  timing1=new float[max];
  timing2=new float[max];
  hit1=new point3D[max];
  ...
  chargedPrompt=true;
  neutralPrompt=false;
  for(int i=0;i<4;i++) secondary[i]=false;
#ifdef TWOTWO_out
  defineTwotwoTree();
#endif
}
...
//   cout<<"new event"<<endl;
  ...
  for(int i=0;i<numberOfTracks;i++)
    {
      if(tracks[i]==NULL)continue;
//       cout<<"track "<<i<<" "<<tracks[i]->getNumberOfCalibHits()<<" hits"<<endl;
      if(tracks[i]->getNumberOfCalibHits()==0)continue;
      n1=0;n2=0;
...
//       cout<<"track "<<i<<" "<<"d="<<distance<<" tof="<<flightTime<<" beta="<<tracks[i]->getSpeed()<<endl;
    }
#ifdef TWOTWO_out
  if(numberOfTracks==5&&tracks[2]->getNumberOfSecondaryTracks()==2)
	...
	setTwotwoTracks(tracks[0], tracks[1], tracks[2]->getSecondary(0), tracks[2]->getSecondary(1),pf,pb,tf,tb);
      else
	{
	  float tmpf=tf[3];tf[3]=tf[2];tf[2]=tmpf;
	  tmpf=tb[3];tb[3]=tb[2];tb[2]=tmpf;
	  point3D tmpp=pb[3];pb[3]=pb[2];pb[2]=tmpp;
	  setTwotwoTracks(tracks[0], tracks[1], tracks[2]->getSecondary(1), tracks[2]->getSecondary(0),pf,pb,tf,tb);
	}
    }
#endif
  return ptr;
...
void defineTwotwoTree()
{
  ...
  twotwoTree->Branch("p1",&twotwoTrack[0],"vertex[3]/D:fitted[3]/D:el[30]/I:det[30]/I:phi/D:theta:beta:energy:momentum:front:back:path:lp:ID/I:nEl");
  twotwoTree->Branch("p2",&twotwoTrack[1],"vertex[3]/D:fitted[3]/D:el[30]/I:det[30]/I:phi/D:theta:beta:energy:momentum:front:back:path:lp:ID/I:nEl");
  twotwoTree->Branch("s1",&twotwoTrack[2],"vertex[3]/D:fitted[3]/D:el[30]/I:det[30]/I:phi/D:theta:beta:energy:momentum:front:back:path:lp:ID/I:nEl");
  twotwoTree->Branch("s2",&twotwoTrack[3],"vertex[3]/D:fitted[3]/D:el[30]/I:det[30]/I:phi/D:theta:beta:energy:momentum:front:back:path:lp:ID/I:nEl");
...
void doKinfitTwoTwo(kinFit *kfit, vector3D v1, vector3D v2, vector3D v3, vector3D v4, float m1, float m2, float m3, float m4, float b1, float b2, float b3, float b4);
extern float getLightWay(volumeShape *sh, point3D onPoint, bool from);
extern vector3D solve(const vector3D& v1,const vector3D& v2, const vector3D &v3, const vector3D &init);
void doVfit_twotwo(TTrack *p1,TTrack* p2,vector3D &v0,vector3D& v1,point3D &vert);
void setTwotwoTracks(TTrack *p1,TTrack *p2,TTrack *s1, TTrack *s2, point3D *pf,point3D *pb, float *tf, float *tb)
{
  if(twotwoTree==NULL)return;
  int n;  
  ...
  for(int i=0;i<4;i++)
    {
      currentTrack=NULL;
      switch(i){
      case 0: currentTrack=p1;break;
      case 1: currentTrack=p2;break;
      case 2: currentTrack=s1;break;
      case 3: currentTrack=s2;break;
      }	
      if(currentTrack==NULL)continue;
      v[i][0]=currentTrack->getPath().Direction();
      if(i>=2)v[i][1]=currentTrack->getPath().Direction();
//       if(i>=2)
	{
...
// 	}
      ...
      if(i>=2)
	twotwoTrack[i].path=(pf[i]-currentTrack->getPath().Foot()).R()+(currentTrack->getPath().Foot()-pb[i]).R();
      else
	...
	{
	case 0:twotwoTrack[i].lpath=getLightWay(currentTrack->getCalibSrcr(0,0)->getHitShape(),pb[i], false);
	  break;
	case 3:twotwoTrack[i].lpath=getLightWay(currentTrack->getCalibSrcr(3,0)->getHitShape(),pb[i], false);
	  break;
	case 6:twotwoTrack[i].lpath=getLightWay(currentTrack->getCalibSrcr(6,0)->getHitShape(),pb[i], false);
	  break;
	}
    }
  //
  ...
  ld[2]=(initA+initB-tmp1-tmp2).Momentum();ld[2]=ld[2]/ld[2].R();
  ld[3]=(initA+initB-tmp11-tmp22).Momentum();ld[3]=ld[3]/ld[3].R();
  minv[0]=sqrt((tmp1+tmp2)*(tmp1+tmp2));
  minv[1]=sqrt((tmp11+tmp22)*(tmp11+tmp22));
  minv[2]=RtwoParticleDecay::getPrimary(Eparticles::getMassG(twotwoTrack[2].ID), Eparticles::getMassG(twotwoTrack[3].ID), ld[1], v[2][0], v[3][0], s1->getSpeed()).Mass();
  ...
  if(abs(mmiss[0]-1.11568*1.11568)<abs(mmiss[1]-1.11568*1.11568))
    {d=(initA+initB-tmp1-tmp2).Momentum();d.normalize();}
  else
    {d=(initA+initB-tmp11-tmp22).Momentum();d.normalize();}
  ...
  ld[4]=(initA+initB-tmp1-tmp2).Momentum();ld[4]=ld[4]/ld[4].R();
  ld[5]=(initA+initB-tmp11-tmp22).Momentum();ld[5]=ld[5]/ld[5].R();
  ...
  tmp2.setVM((twotwoTrack[2].beta<1?twotwoTrack[2].beta:0.95)*v[3][0],Eparticles::getMassG(twotwoTrack[3].ID));
...
//     cout<<acos(d*v)<<endl;
  ...
  kinFit *myKinFit[4];for(int i=0;i<4;i++)myKinFit[i]=new kinFit(50,7,1e-5);
  doKinfitTwoTwo(myKinFit[0], v[0][0], v[1][0], v[2][0], v[3][0],mp, mk, mp, mpi, 
		 p1->getSpeed(), p2->getSpeed(), s1->getSpeed(), s2->getSpeed());
  doKinfitTwoTwo(myKinFit[1], v[0][0], v[1][0], v[2][0], v[3][0],mp, mk, mpi, mp, 
		 p1->getSpeed(), p2->getSpeed(), s1->getSpeed(), s2->getSpeed());
  doKinfitTwoTwo(myKinFit[2], v[0][0], v[1][0], v[2][0], v[3][0],mk, mp, mp, mpi, 
		 p1->getSpeed(), p2->getSpeed(), s1->getSpeed(), s2->getSpeed());
  doKinfitTwoTwo(myKinFit[3], v[0][0], v[1][0], v[2][0], v[3][0],mk, mp, mpi, mp, 
		 p1->getSpeed(), p2->getSpeed(), s1->getSpeed(), s2->getSpeed());
  int best=0;
  if(myKinFit[1]->Chi2()>=0&&((!(myKinFit[best]->Chi2()<1e20))||myKinFit[1]->Chi2()<myKinFit[best]->Chi2()))best=1;
  if(myKinFit[2]->Chi2()>=0&&((!(myKinFit[best]->Chi2()<1e20))||myKinFit[2]->Chi2()<myKinFit[best]->Chi2()))best=2;
  if(myKinFit[3]->Chi2()>=0&&((!(myKinFit[best]->Chi2()<1e20))||myKinFit[3]->Chi2()<myKinFit[best]->Chi2()))best=3;
...
void doVfit_twotwo(TTrack *p1,TTrack* p2,vector3D &v0,vector3D& v1,point3D &vert)
{
  ...
  for(int i=0;i<30;i++)
	...
	{
	  if(p1->getCalibSrcr(i,j)->getHitShape()==NULL)continue;
	...
	{
	  if(p2->getCalibSrcr(i,j)->getHitShape()==NULL)continue;
...
{
  int mode=0;
  kfit->add(vector3D(0,0,0),vector3D(0,0,0),vector3D(2,2,2),Eparticles::getMassG(14),1,-1);
  kfit->add(vector3D(0,0,3.059),vector3D(0,0,0),vector3D(2,2,2),Eparticles::getMassG(14),1,-1);
  momentum4D tmp;
  vector3D mea(1,1,1);
  float e[4];
  float p[4];
  ...
  for(int i=0;i<4;i++)
    {
      if(b[i]>1)b[i]=0.99;
      tmp.setVM(v[i]*b[i],m[i]);e[i]=tmp.Energy()-m[i];p[i]=tmp.Momentum().R();
  ...
  for(int i=0;i<4;i++)
	...
	{
	case 0:
	  kfit->add(vector3D(e[i],v[i].Theta(),v[i].Phi()),             err[i][mode],mea,m[i],2);break;
	case 1:
	  kfit->add(         p[i]*v[i],                                 err[i][mode],mea,m[i],1);break;
	case 2:
	  kfit->add(vector3D(e[i],v[i].X()/v[i].Z(),v[i].Y()/v[i].Z()), err[i][mode],mea,m[i],5);break;
	case 3:
	  kfit->add(vector3D(b[i],v[i].Theta(),v[i].Phi()),             err[i][mode],mea,m[i],3);break;
	case 4:
	  kfit->add(vector3D(b[i],v[i].X()/v[i].Z(),v[i].Y()/v[i].Z()), err[i][mode],mea,m[i],7);break;
	case 5:
	  kfit->add(vector3D(p[i],v[i].X()/v[i].Z(),v[i].Y()/v[i].Z()), err[i][mode],mea,m[i],6);break;
	}
    }
  kfit->fit();
}
