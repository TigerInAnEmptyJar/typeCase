  ...
  for(int i=0;i<nParameters;i++)
    {
      ...
      switch(parN)
	{
	case 0: par=parts[pN].Momentum().Theta();break;//theta
	case 1: par=parts[pN].Momentum().Phi();break;//phi
	case 2: par=parts[pN].Beta();break;//beta
	case 3: par=parts[pN].Momentum().R();break;//momentum
	case 4: par=parts[pN].Momentum().Rho();break;//pt
	case 5: par=parts[pN].Momentum().Z();break;//pl
	case 6: par=parts[pN+numberOfParticles].Momentum().Theta();break;//thetaCM
	case 7: par=parts[pN+numberOfParticles].Momentum().Phi();break;//phiCM
	case 8: par=parts[pN+numberOfParticles].Beta();break;//betaCM
	case 9: par=parts[pN+numberOfParticles].Momentum().R();break;//momentumCM
	}
...
bool AGenerateEventsWithDecay::accept(sLine3D *trajectories)
{
  ...
  for(int i=0;i<setup.getNumberOfDetectors();i++)
    {
      if(requiredHits[i]==0)continue;
      if(setup.getDetectorr(i).getNumberOfElements()<=0)continue;
      ...
      for(int j=0;j<numberOfParticles;j++)
	{
	  ...
	  if(requiredHits[i]>0&&charge[j]==0)continue;
	  if(susp[i]>=0&&susp[i]<setup.getDetectorr(i).getNumberOfElements())
	    nHit++;
	}
      if(nHit<abs(requiredHits[i]))return false;
    }
  for(int i=0;i<numSums;i++)
	...
	if(susp[SumDetID[i][j]]>=0&&(requiredHits[i]<0||charge[j]!=0))
	  nHit++;
      if(nHit<SumDetHits[i])return false;
    }
  return true;
}
...
AGenerateEventsWithDecay::AGenerateEventsWithDecay(TTrack **tracksIn, int &numberOfTracksIn, int &eventNumberIf, TSetup &setupIn, bool &validInputIn, const algorithm_parameter &descr):AAlgorithm(""),setup(setupIn),numberOfTracks(numberOfTracksIn),eventNumber(eventNumberIf),validInput(validInputIn)
{
  tracks=tracksIn;
  valid=true;
  ...
  for(int i=0;i<numberOfParticles;i++)generated[i]=false;
  numberOfPrimary=0;
  for(int i=0;i<numberOfParticles;i++)
    if(pri[i])numberOfPrimary++;
  ...
  for(int i=0;i<numberOfDecays+1;i++)generator[i]=NULL;
...
// 	  cout<<tmpP->Px()<<" "<<tmpP->Py()<<" "<<tmpP->Pz()<<" "<<tmpP->E()<<endl;
// 	}
//     }
  if(!PhaseSpace)
    {
  ...
  for(int j=0;j<numberOfDecays;j++)
    {
      ...
      for(int i=0;i<numberOfParticles;i++)
	{
	  if(!decays[i])continue;
	  taken=false;
	  for(int k=0;k<j;k++)
	    if(decayParticle[k]==i)
	      taken=true;
	  if(!taken){num=i;break;}
	}
      if(num<0)
	{
	  numberOfDecays=j;
	...
	if(decayedOff[i]==num)
      ...
      if(!PhaseSpace)
	function[j+1]=new TF1((string("F")+string_number(j)).data(),descr.getParam<vector<string> >(0).getData().at(j+1).data());
      else
	function[j+1]=NULL;
...
//     }
  ...
  while(condition)
	...
	{
	  if(generator[i]==NULL){validInput=false;return ptr;}
	  weight=0;number=1;
	  ...
	  if(i==0)
	  ...
	  else
	    {
	      ...
	      time=-log(decayLength)*halfLife[decayParticle[i-1]]/log(2.)/parts[decayParticle[i-1]].Gamma();
	      path=beta.R()*time*2.99792e11;
	      vertex=lines[decayParticle[i-1]].Foot()+lines[decayParticle[i-1]].Direction()*path;
	    }
	  while(weight<number)
	    {
	      ...
	      for(int j=0;j<num;j++)
		{
		  ...
		  parts[pps[j]+(i==0?0:numberOfParticles)].setPE(vector3D(v.Px(),v.Py(),v.Pz()),v.E());
		  parts[pps[j]+(i==0?numberOfParticles:0)]=parts[pps[j]+(i==0?0:numberOfParticles)].boost(beta);
		  lines[pps[j]]=sLine3D(vertex,parts[pps[j]].Direction());
		  //cout<<generator[i]->GetDecay(j)->Px()<<" "<<generator[i]->GetDecay(j)->Py()<<" "<<generator[i]->GetDecay(j)->Pz()<<" "<<generator[i]->GetDecay(j)->E()<<endl;
...
// 	cout<<j<<" "<<particleID[j]<<" "<<parts[j]<<" "<<lines[j]<<endl;
  ...
  for(int i=0;i<numberOfPrimary;i++)p=p+parts[primaries[i]];
  for(int i=0;i<numberOfParticles;i++)
    {
