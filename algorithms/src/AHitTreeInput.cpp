...
AHitTreeInput::AHitTreeInput(TSetup& setupIn, TCalibHit ***hitsIn, int **nH, int &evtNr, int &rnNr, int &trggr, int maxH,int MaxD, bool &readInValid,const algorithm_parameter &param):AAlgorithm("Read hit tree"),EventNumber(evtNr),RunNumber(rnNr),Trigger(trggr),maxHit(maxH),maxDet(MaxD),valid(readInValid),setup(setupIn)
{
  hits=hitsIn;
  numberOfHits=nH;
  ...
  valid=false;
  useChain=false;
  if(param.getNumberOfParam<bool>()>3)useChain=param.getParam<bool>(3).getData();
  if(useChain)hitChain=new TChain("hits","hits");
}
  ...
  if(hitTree!=NULL)
    {
      ...
      if(useChain&&hitChain!=NULL)max=(int)hitChain->GetEntries();
...
void AHitTreeInput::readEvent(int EvtNr)
{
  ...
  if(useChain)
      ...
      else
	{
	  if(eh_en->GetValue()<EvtNr)
	      ...
	      if(hitTree!=hitChain->GetTree())
		{
	      ...
	      if(eh_en->GetValue()>EvtNr){cout<<"bad event "<<EvtNr<<endl;currentEntry--;return;}
	    }
	  else
	    {
	      hitChain->GetEntry(--currentEntry);
	      if(hitTree!=hitChain->GetTree())
		{
	      ...
	      if(eh_en->GetValue()<EvtNr){cout<<"bad event "<<EvtNr<<endl;return;}
	    }
	  if(eh_en->GetValue()==EvtNr)
	    {
	      readEntry(currentEntry);
    ...
    {
      if(eh_en==NULL||eh_rn==NULL||eh_tr==NULL||eh_nh==NULL)return;
...
void AHitTreeInput::readEntry(int entry)
{
  ...
  if(useChain)
      ...
      if(hitTree!=hitChain->GetTree())
	{
      ...
      if(tdcRLeaf==NULL||qdcRLeaf==NULL||tdcCLeaf==NULL||qdcCLeaf==NULL||elLeaf==NULL||
	 detLeaf==NULL||eh_en==NULL||eh_rn==NULL||eh_tr==NULL||eh_nh==NULL)return;
    }
  else
    {
      if(tdcRLeaf==NULL||qdcRLeaf==NULL||tdcCLeaf==NULL||qdcCLeaf==NULL||elLeaf==NULL||
  ...
  for(int i=0;i<maxDet;i++)(*(numberOfHits[i]))=0;
  ...
  for(int i=0;i<numHits;i++)
    {
      ...
      if(d>=setup.getNumberOfDetectors())continue;
      if((*(numberOfHits[d]))>=maxHit)continue;
      if(e>=setup.getDetectorr(d).getNumberOfElements()||e<0)continue;
...
void AHitTreeInput::newRun(run_parameter &r)
{
  ...
  if(useChain)
	  ...
	  for(unsigned int i=0;i<filenames.size();i++)
	      ...
	      if(s!=filenames[i])
		{
		  ...
		  if(!existing(copyFile.back()))
		    {
		      cout<<"error during copy file \""<<copyFile.back().data()<<"\" doesn\'t exist"<<endl;
		    }
		}
	    }
	}
	...
	hitChain->Add(filenames[i].data());
      cout<<"TChain defined for "<<filenames.size()<<" files "<<hitChain->GetNtrees()<<" trees found with "<<hitChain->GetEntries()<<" events"<<endl;
      readWriteLog<<"TChain defined for "<<(int)filenames.size()<<" files "<<(int)hitChain->GetNtrees()<<" trees found with "<<(int)hitChain->GetEntries()<<" events"<<endli;
      hitChain->GetEntry(0);
      hitTree=hitChain->GetTree();
      if(hitTree==NULL)    
	{
  ...
  getHeader();
  getMain();
  readWriteLog<<"Open file \""<<filenames.back().data()<<"\" for hit tree input with"<<(int)hitTree->GetEntries()<<" events"<<endli;
  if(tdcRLeaf==NULL||qdcRLeaf==NULL||tdcCLeaf==NULL||qdcCLeaf==NULL||elLeaf==NULL||
     detLeaf==NULL||eh_en==NULL||eh_rn==NULL||eh_tr==NULL||eh_nh==NULL)return;
  currentEntry=0;
  valid=true;
