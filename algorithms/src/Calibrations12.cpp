...
AqdcRadialPolCalibration::AqdcRadialPolCalibration(TEvent &eventIn, TSetup &setup, algorithm_parameter & descr):AAlgorithm("Generate qdcRadialPol calibration"), event(eventIn), minEntriesPerHisto(descr.getParam<int>(0).getData())
{
  ...
  for(int i=0;i<numberOfDetectors;i++)
	...
	{
	  nn=(string("qdc-difference: detID-")+string_number(detectorIDs[i])+" el-"+string_number(j)).data();
	  qdcHistograms[i][j]=new TH2F(nn.data(),nn.data(),100,xmin,xmax,100,ymin,ymax);
	}
    }
}
AqdcRadialPolCalibration::AqdcRadialPolCalibration(TEvent &eventIn, TSetup &setup, vector<int> &detectors, vector<int> &Stdetectors,vector<int> readSide, vector<string> reactionNames, int minEntriesPerHistoIn):AAlgorithm("Generate qdcRadialPix calibration"), event(eventIn), minEntriesPerHisto(minEntriesPerHistoIn)
{
