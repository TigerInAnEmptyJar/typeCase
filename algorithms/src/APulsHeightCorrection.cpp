...
void getTheParameter(int &numberOfRanges, int **ranges, int DetID, int numberOfElements, TF1 ***correctionFunction,vector<string> &calibrationFiles)
{
    numberOfRanges=0;
    ranges=new int*[4];
    ...
    {
	if(calibrationFiles[i]=="")continue;
	ifstream input;
	...
	{
	    correctionFunction[i][j]=new TF1("[0]+[1]*x+[2]*pow(x,2)+[3]*pow(x,3)+[4]*pow(x,4)+[5]*pow(x,5)",(string("f")+string_number(i)+"/"+string_number(j)).data());
	    for(int k=0;k<6;k++)
...
APulsHeightCorrectionWedge::APulsHeightCorrectionWedge(TTrack **tracksIn,int &numberOfTracksIn, TDetector &det, int& runNumber, int &eventNumber, vector<string> &calibrationFiles):AAlgorithm("Puls height correction for wedges"),numberOfTracks(numberOfTracksIn),runNr(runNumber),eventNr(eventNumber)
{
    DetID=det.getID();
    numberOfElements=det.getNumberOfElements();
    tracks=tracksIn;
    getTheParameter(numberOfRanges, ranges, DetID, numberOfElements, correctionFunction, calibrationFiles);
    actualRange=0;
}
...
void *APulsHeightCorrectionWedge::process(void*ptr)
{
  ...
  if(!((runNr>=ranges[0][actualRange])&&
       (runNr<=ranges[1][actualRange])&&
       (eventNr<=ranges[3][actualRange])&&
       (eventNr>=ranges[2][actualRange])))
  {
      rangeFound=false;
      for(int j=0;j<numberOfRanges;j++)
      {
...
APulsHeightCorrectionSpiral::APulsHeightCorrectionSpiral(TTrack **tracksIn,int &numberOfTracksIn, TDetector &det, int& runNumber, int &eventNumber, vector<string> &calibrationFiles):AAlgorithm("Puls height correction for spirals"),numberOfTracks(numberOfTracksIn),runNr(runNumber),eventNr(eventNumber)
{
    DetID=det.getID();
    numberOfElements=det.getNumberOfElements();
    tracks=tracksIn;
...
APulsHeightCorrectionTube::APulsHeightCorrectionTube(TTrack **tracksIn,int &numberOfTracksIn, TDetector &det, int& runNumber, int &eventNumber, vector<string> &calibrationFiles):AAlgorithm("Puls height correction for proportional chamber"),numberOfTracks(numberOfTracksIn),runNr(runNumber),eventNr(eventNumber)
{
    DetID=det.getID();
    numberOfElements=det.getNumberOfElements();
    tracks=tracksIn;
