...
#include "wedge.h"
#include "ring.h"
APromptHistoTracker::APromptHistoTracker(TSetup &setup, TTrack **tracksIn, int &numTracks, int maxTracksIn, TCalibHit ***hitsIn, int **numHitsIn, bool flat, int minElementsIn, bool outOfSetup, float percentage, bool function, TFormula*f, AFitAlgorithm *fitIn, int binsP, int binsT, float minPIn, float minTIn, float maxPIn, float maxTIn, bool readFromFile, bool writeToFile, string filename, float maxChiIn):AAlgorithm("Prompt Tracking using histogramms"), numDets(setup.getNumberOfDetectors()), numberOfTracks(numTracks), maxTracks(maxTracksIn), maxChi(maxChiIn),vertex(0,0,0)
{
  minT=minTIn;
  maxT=maxTIn;
  minP=minPIn;
  ...
  for(int i=0;i<binsT;i++)dirs[i]=new vector3D[binsP];
  for(int k=0;k<binsT;k++)
    for(int l=0;l<binsP;l++)
      	dirs[k][l].setValues(sin(minT+k*(maxT-minT)/(binsT-1))*cos(minP+l*(maxP-minP)/(binsP-1)),
  ...
  for(int i=0;i<numDets;i++)
      ...
      for(int j=0;j<nel[i];j++)
	{
	  if(readFromFile)
	    {
	      Elements[i][j]=(TH2C*)fil->Get((string("Det")+string_number(i)+"el"+string_number(j)).data());
	      Elements[i][j]->SetDirectory(0);
	    }
	  else
	    {
	      Elements[i][j]=new TH2C((string("Det")+string_number(i)+"el"+string_number(j)).data(),
				     (string("Det")+string_number(i)+"el"+string_number(j)).data(),
				     binsT,minT,maxT,binsP,minP,maxP);
	      shape=setup.getDetectorr(i).getShape(j);
// 	      if(shape->getName()!="wedge")continue;
// 	      cout<<shape->getName().data()<<" "<<((wedge*)shape)->getLowerPhiEdge()<<" "<<((wedge*)shape)->getPhiRange()<<endl;
	      ...
	      else
		{
		  if(shape->getName()=="wedge")
		    {
		      point3D p1=((wedge*)shape)->getInnerRadius()*((wedge*)shape)->getLowerPhiEdge()+shape->getCenter();
		      point3D p2=((wedge*)shape)->getOuterRadius()*((wedge*)shape)->getLowerPhiEdge()+shape->getCenter();
		      point3D p3=((wedge*)shape)->getInnerRadius()*(matrix3D(((wedge*)shape)->getNormal(),((wedge*)shape)->getPhiRange())*((wedge*)shape)->getLowerPhiEdge())+shape->getCenter();
		      point3D p4=((wedge*)shape)->getOuterRadius()*(matrix3D(((wedge*)shape)->getNormal(),((wedge*)shape)->getPhiRange())*((wedge*)shape)->getLowerPhiEdge())+shape->getCenter();
...
// // 			  maxPhi=tmp;
// // 			}
//		      cout<<minPhi<<" "<<maxPhi<<" "<<minTh<<" "<<maxTh<<endl;
		      for(int k=0;k<binsT;k++)
			for(int l=0;l<binsP;l++)
			  if(minTh<=dirs[k][l].Theta()&&maxTh>=dirs[k][l].Theta())
				...
				maxPhi=maxPhi-2*M_PI;
			      if((minPhi<=dirs[k][l].Phi()&&maxPhi>=dirs[k][l].Phi())||(maxPhi<minPhi&&(dirs[k][l].Phi()>=minPhi||dirs[k][l].Phi()<=maxPhi)))
				Elements[i][j]->SetBinContent(k+1,l+1,1);
			      else
				Elements[i][j]->SetBinContent(k+1,l+1,0);
			    }
		    ...
		    {
		      float minTh=(((ring*)shape)->getInnerRadius()*vector3D(1,0,0)+((ring*)shape)->getCenter()).Theta();
		      float maxTh=(((ring*)shape)->getOuterRadius()*vector3D(1,0,0)+((ring*)shape)->getCenter()).Theta();
		      if(maxTh-minTh<deltaTheta)
			{
			  for(int k=0;k<binsT;k++)
			    if(abs(dirs[k][0].Theta()-minTh)<deltaTheta&&abs(dirs[k][0].Theta()-maxTh)<deltaTheta)
			      for(int l=0;l<binsP;l++)
				Elements[i][j]->SetBinContent(k+1,l+1,1);
			    else
			      for(int l=0;l<binsP;l++)
	...
	for(int j=0;j<binsP;j++)
	  {
	    ...
	    if(grL[0]<=dirs[i][j].Theta()&&grL[3]<dirs[i][j].Theta()&&grH[0]>dirs[i][j].Theta())
	      minElements->SetBinContent(i+1,j+1,7);
	    if(grL[1]<=dirs[i][j].Theta()&&grH[1]>dirs[i][j].Theta())
	      minElements->SetBinContent(i+1,j+1,7);
	    if(grL[2]<=dirs[i][j].Theta()&&grH[2]>dirs[i][j].Theta()&&grH[3]>dirs[i][j].Theta())
  ...
  for(int i=0;i<nbinsT;i++)workHisto[i]=new float[nbinsP];
  for(int i=0;i<numDets;i++)
    {
      for(int j=0;j<nel[i];j++)
	delete Elements[i][j];
...
APromptHistoTracker::~APromptHistoTracker()
{
  ...
  delete []nel;
  delete minElements;
  for(int i=0;i<nbinsT;i++)delete []workHisto[i];
  delete []workHisto;
}
  ...
  while(max>=1&&numberOfTracks<maxTracks&&loop<3*maxTracks)
    {
      ...
      direction.setValues(sin(axisT[maxX])*cos(axisP[maxY]),
			  sin(axisT[maxX])*sin(axisP[maxY]),
			  cos(axisT[maxX]));
	  ...
	  if(found)
	    {
	      ...
	      Vector tmp(5);	  
	      tmp.setValues((best[4]*best[7]+best[5]*best[8])/best[0],
			    best[4]/best[0], best[5]/best[0],
			    best[4]*best[9]/best[0], best[5]*best[9]/best[0]);
	      fit->add(tmp);
	    }
	}
      float chi=fit->fit();
...
// {
// }
// TH1C*getHistoFromCylinder(cylinder *c, point3D center, vector3D x, vector3D z)
// {
// }
// TH1C*getHistoFromHexprism(hexPrism *h, point3D center, vector3D x, vector3D z)
// {
// }
