...
#include <stdlib.h>
#include <float.h>
AAssignHitsToTracks::AAssignHitsToTracks(TSetup &setupIn,TTrack **tracksIn, TCalibHit ***hitIn, int **numberOfHitsIn, int &numberOfTracksIn, const algorithm_parameter &descr): AAlgorithm("Assign hits in remaining detectors to defined tracks"), setup(setupIn), numberOfTracks(numberOfTracksIn)
{
    hits=hitIn;
    tracks=tracksIn;
...
void *AAssignHitsToTracks::process(void*ptr)
{
    ...
    for(int i=0;i<numberOfDetectors;i++)params[i].reDimension(13);
    int nEl;
	...
	for(int j=0;j<numberOfDetectors;j++)
	{
	    ...
	    if((*(numberOfHits[detectorIDs[j]]))<=0)continue;
	    suspectmode=true;
	    if(isPrompt)
		...
		{
		    if(!hits[detectorIDs[j]][k]->isValid())continue;
		    ...
		    {
			if(abs(abs(hits[detectorIDs[j]][k]->getElement()-elem)-det->getNumberOfElements()))
			{
			    eldis=(int)abs(hits[detectorIDs[j]][k]->getElement()-elem);
			    closest=k;
			}
		...
		if(closest>=0&&closest<(*(numberOfHits[detectorIDs[j]]))&&eldis<maxElementDistance[j])
		{
	...
	if(doFit)
	{
	    ...
	    for(int j=0;j<nEl;j++)
	      {
		myFit.addPoint(point3D(params[i][7],params[i][8],params[i][9]),vector3D(params[i][4],params[i][5],params[i][6]),params[i][0]);
	      }
	    float chi=myFit.fit();
	    if(chi>=0&&chi<maxChi)
	      {
