...
#endif
ABarrelPixel::ABarrelPixel(int maxPixelsIn, int IDIn, int &numPixels, TPixel **pixelsIn, int &numHits1, int &numHits2, TCalibHit **h1, TCalibHit **h2, TDetector *front, float pixelSizeIn, int &eventNr, int &runNr, const vector<string> & fileNames):AAlgorithm("barrel pixel"),maxPixels(maxPixelsIn),ID(IDIn),numberOfPixels(numPixels),numberOfHits1(numHits1),numberOfHits2(numHits2), eventNumber(eventNr), runNumber(runNr)
{
  pixels=pixelsIn;
  hits1=h1;
  hits2=h2;
  ...
  for(int i=0;i<numRanges;i++)zParams[i]=NULL;
  range=new int*[numParams];
  for(int i=0;i<numParams;i++) range[i]=new int[numRanges];
...
{
  numberOfPixels = 0;
  if(!((runNumber>=range[0][actualRange])&&
       (runNumber<=range[1][actualRange])&&
       (eventNumber>=range[2][actualRange])&&
       (eventNumber<=range[3][actualRange])))
    {
  ...
  for (int i = 0; i < numberOfHits1; i++)
    {
      ...
      if(hits1[i]->getRawTDC()==-1)continue;
      for (int j = 0; j < numberOfHits2; j++)
	{
	  if(!(hits2[j]->isValid())) continue;
	  if(hits2[j]->getRawTDC()==-1) continue;
	  if (numberOfPixels < maxPixels)
	      ...
	      if (hits1[i]->getElement() == hits2[j]->getElement())
		{
		  ...
		  if(!(hits1[i]->getHitShape()))continue;
		  if((hits1[i]->getRawTDC()==-1)||(hits2[j]->getRawTDC()==-1))continue;
#ifdef USEHPEFORDEF
...
planeShape* ABarrelPixel::getShape(int e,float tdcF,float tdcB)
{
  float zBarrel=((zParams[actualRange]->getParameter(e,0)*(tdcB-tdcF)/2)
    +zParams[actualRange]->getParameter(e,1))*10;//zBarrel*10; from cm to mm!!!
...
#else
  planeShape *ret=new sphericRectangle(center+(lpe*(zBarrel-ps)), center+(lpe*(zBarrel+ps)),
...
void ABarrelPixel::onNewRun(run_parameter &rp)
{
  ...
  int numParams=4;
  zParams=new CommonCalibrationParser*[numRanges];
  for(int i=0;i<numRanges;i++)zParams[i]=Parser[i];
  range=new int*[numParams];
  for(int i=0;i<numParams;i++) range[i]=new int[numRanges];
  for(int i=0;i<numParams;i++)
    for(int j=0;j<numRanges;j++)
