...
void AMultipleTreeInput::readATrack(TTrack *out, TLeaf** in)
{
  ...
  for(int i=0;i<n;i++)
    {
      if(in[6]->GetValue(i)<0||in[7]->GetValue(i)<0)continue;
	  ...
	  if(!found)
	    {
	      ...
	      for(int j=0;j<(*numberOfHits[d]);j++)cout<<hits[d][j]->getElement()<<",";
	      cout<<endl;
	    }
	  //	  else cout<<endl;
	}
#endif
      //      if(out->getNumberOfCalibHits()!=n)cout<<"hits not found: "<<n<<" "<<out->getNumberOfCalibHits()<<endl;
    }
  if(out->getNumberOfCalibHits()>0)setTrackErrors(out);
}
void AMultipleTreeInput::cleanUp()
{
  ...
  if(entryNumber!=NULL)delete []entryNumber;
  if(trees!=NULL)delete []trees;
  if(leaves!=NULL)delete []leaves;
  entryNumber=NULL;
  trees=NULL;
  leaves=NULL;
  if(inputFile!=NULL)
...
void AMultipleTreeInput::OnNewRun(run_parameter &rp)
{
  ...
  anaLog<<nTrees[0]<<" prompt-, "<<nTrees[1]<<" kink-, "<<nTrees[2]<<" vee-trees found"<<endli;
}
...
void AMultipleTreeInput::setTree(const string& filename)
{
  ...
  for(int i=0;i<3;i++)
    {
      ...
      switch(i)
	{
	case 0:treename+="p";break;
	case 1:treename+="k";break;
	case 2:treename+="v";break;
	}
      nTrees[i]=0;
  ...
  while(trees[0][0]->FindLeaf((string("Track")+string_number(nTracksPerTree)+".theta").data()))
    nTracksPerTree++;
  readWriteLog<<" found "<<nTrees[0]<<" prompt track trees, "<<nTrees[1]<<" kink trees, "<<nTrees[2]<<" vee trees"<<endli;
	  ...
	  for(int l=0;l<nTracksPerTree;l++)
	    {
	      for(int m=0;m<=j;m++)
		{
		  leaves[j][m*nTrees[j]*nTracksPerTree+(l+nTracksPerTree*k)][0]=trees[j][k]->FindLeaf((string("Track")+pre[m*j]+string_number(l)+".ID").data());
		  leaves[j][m*nTrees[j]*nTracksPerTree+(l+nTracksPerTree*k)][1]=trees[j][k]->FindLeaf((string("Track")+pre[m*j]+string_number(l)+".beta").data());
		  leaves[j][m*nTrees[j]*nTracksPerTree+(l+nTracksPerTree*k)][2]=trees[j][k]->FindLeaf((string("Track")+pre[m*j]+string_number(l)+".theta").data());
		  leaves[j][m*nTrees[j]*nTracksPerTree+(l+nTracksPerTree*k)][3]=trees[j][k]->FindLeaf((string("Track")+pre[m*j]+string_number(l)+".phi").data());
		  leaves[j][m*nTrees[j]*nTracksPerTree+(l+nTracksPerTree*k)][4]=trees[j][k]->FindLeaf((string("Track")+pre[m*j]+string_number(l)+".vertex").data());
		  leaves[j][m*nTrees[j]*nTracksPerTree+(l+nTracksPerTree*k)][5]=trees[j][k]->FindLeaf((string("Track")+pre[m*j]+string_number(l)+".nEl").data());
		  leaves[j][m*nTrees[j]*nTracksPerTree+(l+nTracksPerTree*k)][6]=trees[j][k]->FindLeaf((string("Track")+pre[m*j]+string_number(l)+".det").data());
		  leaves[j][m*nTrees[j]*nTracksPerTree+(l+nTracksPerTree*k)][7]=trees[j][k]->FindLeaf((string("Track")+pre[m*j]+string_number(l)+".el").data());
		  leaves[j][m*nTrees[j]*nTracksPerTree+(l+nTracksPerTree*k)][8]=trees[j][k]->FindLeaf((string("Track")+pre[m*j]+string_number(l)+".energy").data());
		  for(int nn=0;nn<9;nn++)if(leaves[j][m*nTrees[j]*nTracksPerTree+(l+nTracksPerTree*k)][nn]==NULL)allLeavesFound=false;
		}
	    }
	}
    }
   ...
   if(read_given_event)
      ...
      if(eventNr!=evtNr)
	{
	  while(eventNr>evtNr&&currentEntry<countTree->GetEntries()-1)
	    countTree->GetEntry(++currentEntry);
	  if(currentEntry>=countTree->GetEntries())readInValid=false;
	  if(eventNr!=evtNr)
	    {
	      //numberOfTracks=0;
	      return ptr;
...
AMultipleTreeInput::AMultipleTreeInput(int &eventNumber, int &runNumber, int &trigger, int**numberOfHitsIn, TTrack **tracksIn, int &numberOfTracksIn, TCalibHit ***hitsIn, bool &readInValidIn,const algorithm_parameter &param):AAlgorithm("read from multiple trees"),numberOfTracks(numberOfTracksIn),eventNr(eventNumber),runNr(runNumber),trgg(trigger),readInValid(readInValidIn)
{
  tracks=tracksIn;
  ...
  for(int i=0;i<4;i++)
    treeinput_out->Branch(string("track"+string_number(i)).data(),&tra_info_out[i],"tdcB1/F:tdcB2:tdcB3:tdcF1:tdcF2:theta:beta:phi:det1/I:det2:det3:elB1:elB2:elB3:elF1:elF2");
#endif
}
