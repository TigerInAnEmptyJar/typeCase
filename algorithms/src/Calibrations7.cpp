...
TPixel tmpPixel_7;
AtdcRadialPixCalibration::AtdcRadialPixCalibration(TEvent &eventIn, TSetup &setup, algorithm_parameter & descr):AAlgorithm("Generate tdcRadialPix calibration"), event(eventIn), minEntriesPerHisto(descr.getParam<int>(0).getData())
{
  eventStart=0;
  ...
  for(int i=0;i<numberOfDetectors;i++)
      ...
      for(int j=0;j<numberOfElements[i];j++)
	{
	  ...
	  for(int k=0;k<pixelpoints[i];k++)
	    {
	      string nn=nn1+string_number(detectorIDs[i])+" el-"+string_number(j)+" pix-"+string_number(k);
...
// 	      qdcHistograms[i][j][k]=new TH1F(nn.data(),nn.data(),100,xmin,xmax);
  ...
  //  cout<<qdcHistograms[0][0][0]<<" "<<qdcHistograms[0][0][0]->GetName()<<endl;
}
AtdcRadialPixCalibration::AtdcRadialPixCalibration(TEvent &eventIn, TSetup &setup, vector<int> &detectors, vector<int> &Stdetectors,vector<int> pixIDs,vector<int> readSide,vector<int> pointsPerPixel, vector<string> reactionNames, int minEntriesPerHistoIn):AAlgorithm("Generate tdcRadialPix calibration"), event(eventIn), minEntriesPerHisto(minEntriesPerHistoIn)
{
  postScriptHistos=false;
  ...
  for(int i=0;i<numberOfDetectors;i++)
      ...
      for(int j=0;j<numberOfElements[i];j++)
	{
	  ...
	  for(int k=0;k<pixelpoints[i];k++)
	    {
	      nn=(string("\\Delta tdc: detID-")+string_number(detectorIDs[i])+" el-"+string_number(j)+" pix-"+string_number(k));
	      qdcHistograms[i][j][k]=new TH1F(nn.data(),nn.data(),100,xmin,xmax);
	    }
	}
    }
...
{
  //  cout<<qdcHistograms[0][0][0]<<" "<<flush;cout<<qdcHistograms[0][0][0]->GetName()<<endl;
      ...
      for(int j=0;j<numberOfPixels;j++)
	{
	  ...
	  for(int k=0;k<track->getNumberOfPixels();k++)
	      ...
	      for(int m=0;m<pix->getNumberOfActualElements();m++)
		{
		  ...
		  num=tmp->getElement();
// 		  cout<<pos<<" "<<num<<"/"<<numberOfElements[pos]<<" "<<pixpos<<"/"<<pixelpoints[pos]<<" "<<tdcCalculated-tmp->getTDC()<<endl;
// 		  cout<<qdcHistograms[0][0][0]<<" "<<qdcHistograms[pos][num][pixpos]<<endl;
...
{
  tmpPixel_7.reset();
  if(t->getNumberOfCalibHits(0)>0&&t->getNumberOfCalibHits(1)>0&&t->getNumberOfCalibHits(2)>0)
    {
  ...
  else if(t->getNumberOfCalibHits(3)>0&&t->getNumberOfCalibHits(4)>0&&t->getNumberOfCalibHits(5)>0)
    {
