...
ARingpixel::ARingpixel(bool ParallelprojectionIn, 
bool SpecialprojectionplaneIn, 
bool OriginforprojectionpointIn, 
bool TargetforprojectionpointIn, 
bool UsetimedifferenceIn, 
bool CorrectlightflighttimeIn, 
bool TriangleapproximationIn, 
point3D projectionpointIn, 
point3D pointofprojectionplaneIn, 
vector3D normaltoprojectionplaneIn, 
TDetector &detS, TDetector &detL, TDetector &detR, 
int minimumdeltaphiIn, int maximumdeltaphiIn, 
int minimumnumberofelementsIn, int numberOfSpiralSpiralCrossingsIn, 
int pixelIDIn, float maxChiIn, float maxDeltaTIn, int *NumberOfPixelsIn, 
TCalibHit**straightE, TCalibHit**leftE, TCalibHit**rightE, 
int& nStraight, int &nLeft, int &nRight, 
TPixel** PixelsIn, int maxOfPixelsIn):
  AAlgorithm("Ring pixel"),
  maxChi(maxChiIn),
  NumberOfRightElements(nRight),
  NumberOfLeftElements(nLeft),
  NumberOfStraightElements(nStraight),
  maxDeltaT(maxDeltaTIn),
  maxOfPixels(maxOfPixelsIn)
{
	...
	if((Lshape->getBending()!=-Rshape->getBending())||
	   ...
	   (!((Sshape->getNormal()*Lshape->getNormal()/Sshape->getNormal().R()/Lshape->getNormal().R())>0.99))||
	   (!((Rshape->getNormal()*Lshape->getNormal()/Rshape->getNormal().R()/Lshape->getNormal().R())>0.99)))
	  {
	    anaLog<<"Ring-Pixel: normal vectors: "<<Sshape->getNormal().toString(_spheric_)<<Lshape->getNormal().toString(_spheric_)<<Rshape->getNormal().toString(_spheric_)<<Sshape->getNormal()*Lshape->getNormal()/Sshape->getNormal().R()/Lshape->getNormal().R()<<" "<<Rshape->getNormal()*Lshape->getNormal()/Rshape->getNormal().R()/Lshape->getNormal().R()<<endli;
	    anaLog<<"Ring-Pixel: inner radii: "<<Sshape->getInnerRadius()<<" "<<Lshape->getInnerRadius()<<" "<<Rshape->getInnerRadius()<<endli;
	    anaLog<<"Ring-Pixel: outer radii: "<<Sshape->getOuterRadius()<<" "<<Lshape->getOuterRadius()<<" "<<Rshape->getOuterRadius()<<endli;
...
void *ARingpixel::process(void*ptr)
{
  ...
  for (lL = 0; lL < NumberOfLeftElements; lL++)
    {
      if((leftElements[lL]->getHitShape()==0)||(!(leftElements[lL]->isValid()))) continue;
      for (rL = 0; rL < NumberOfRightElements; rL++)
	{
	  if((rightElements[rL]->getHitShape()==0)||(!(rightElements[rL]->isValid()))) continue;
	  le=leftElements[lL]->getElement();
	  re=rightElements[rL]->getElement();
	  if((le<0)||(le>=numberOfSpirals)||(re<0)||(re>=numberOfSpirals))continue;
	  radpix = le - re;// l-r is a measure for theta angle (or radius) of 
	  if (radpix<0)radpix+=numberOfSpirals;
	  ...
	  if (radpix < numberOfSpiralSpiralCrossings)
		...
		{
		  if((straightElements[gL]->getHitShape()==0)||(!(straightElements[gL]->isValid())))continue;
		  ge=straightElements[gL]->getElement();
		  if((ge<0)||(ge>=numberOfWedges)) continue;
		  delphi = phi - ge;
		  if((abs(delphi)==numberOfSpirals)||(delphi==0)) delphi=0;
		  else if((abs(delphi+1)==numberOfSpirals)||(delphi==-1)) delphi=-1;
		  else if((abs(delphi-1)==numberOfSpirals)||(delphi==1)) delphi=1; 
		  /*********************************/
		  //int ppp=le-re+(le<re?numberOfSpirals:0);
		  if(!((Usetimedifference)&&
		       ((abs(straightElements[gL]->getTDC()
			     -leftElements[lL]->getTDC())>maxDeltaT)||
			(abs(rightElements[rL]->getTDC()
			     -leftElements[lL]->getTDC())>maxDeltaT)||
			(abs(straightElements[gL]->getTDC()
			     -rightElements[rL]->getTDC())>maxDeltaT))))
		    {
			  ...
			  if(sh!=NULL) 
			    {
			      ...
			      //cout<<Pixels[*NumberOfPixels]->getShape()<<" "<<sh<<" "<<maxOfPixels<<" "<<*NumberOfPixels<<endl;
...
planeShape* ARingpixel::calculateCenter(int re, int le, int ge, int dPhi, int rp)
{
  ...
  if(Triangleapproximation) return triangleApprox(r,l,g,dPhi,rp);
  if((straightElements[ge]->getHitShape()==0)||
     (leftElements[le]->getHitShape()==0)||
...
//     }
  ...
  if(abs(phi11-phi1)>M_PI/2) return NULL;
  ...
  if (phi12<phi2)
	...
	(
	 (dRq                  -dLq)                  *x2*x2*x2/3.0+
	 (dRq*phiOff2r         -dLq*phiOff1l)         *x2*x2+
	 (dRq*phiOff2r*phiOff2r-dLq*phiOff1l*phiOff1l)*x2-
	 (dRq                  -dLq)                  *x1*x1*x1/3.0-
	 (dRq*phiOff2r         -dLq*phiOff1l)         *x1*x1-
	...
	(
	 ...
	 (dRq*phiOff2r*phiOff2r-dLq*phiOff1l*phiOff1l)*x1*x1       /2.0
      ...
      float rCP = - dRdPhi*dRdPhi*dRdPhi/3.0/area*
	(
  ...
  //triangle to the right
  else if (phi21>phi1)
    {
      ...
      float rCP = dRdPhi*dRdPhi*dRdPhi*
	(
	 ...
	 (dLk*phiOff2l*phiOff2l*phiOff2l+dRk*phiOff1r*phiOff1r*phiOff1r)*(x2-x1)
	 )/(3.0*area);
      centerpoint.setValues(rCP*cos(phiCp),rCP*sin(phiCp),pointofprojectionplane.Z());
  ...
  //sixangle
      ...
      if (phi11<phi22)
	{
	  ...
	  pH=dLq*((phiOff2l-phiOff1l)*(phi22*phi22*phi22-phi11*phi11*phi11)/1.5+
		  (phiOff2l*phiOff2l-phiOff1l*phiOff1l)*(phi22*phi22-phi11*phi11)*0.5);
	  rH=dLk*((phiOff2l-phiOff1l)*               (phi22*phi22*phi22-phi11*phi11*phi11)+
	  ...
	  pH=dRq*((phiOff2r-phiOff1r)*(-phi22*phi22*phi22+phi11*phi11*phi11)/1.5+
		  (phiOff2r*phiOff2r-phiOff1r*phiOff1r)*(-phi22*phi22+phi11*phi11)*0.5);
	  rH=dLk*((phiOff2r-phiOff1r)*               (phi22*phi22*phi22-phi11*phi11*phi11)+
	...
	(
	 ...
	 rH-
	 (
	  (dRk                           +dLk)             *(x4*x4*x4*x4-x3*x3*x3*x3)/4.0+
	  (dRk*phiOff2r                  +dLk*phiOff1l)            *(x4*x4*x4-x3*x3*x3)+
	  (dRk*phiOff2r*phiOff2r         +dLk*phiOff1l*phiOff1l)      *(x4*x4-x3*x3)*1.5+
	  (dRk*phiOff2r*phiOff2r*phiOff2r+dLk*phiOff1l*phiOff1l*phiOff1l)*(x4-x3))
	 );
      centerpoint.setValues(rCP*cos(phiCP),rCP*sin(phiCP),pointofprojectionplane.Z());
...
//   double phi2=(g+1)*phiR;
  ...
  if (!dPhi)
    {
      p1.setValues(cos (g * phiR - phiR/2. + centerPhi)*rad1,sin (g * phiR - phiR/2. + centerPhi)*rad1,z);
      p2.setValues(cos (g * phiR + phiR/2. + centerPhi)*rad2,sin (g * phiR + phiR/2. + centerPhi)*rad2,z);
      p3.setValues(cos (g * phiR + phiR/2. + centerPhi)*rad3,sin (g * phiR + phiR/2. + centerPhi)*rad3,z);
    }
  else
    {
      p1.setValues(cos (g * phiR + phiR/2. + centerPhi)*rad1,sin (g * phiR + phiR/2. + centerPhi)*rad1,z);
  ...
  for(int i=0;i<NumberOfLeftElements;i++)
    if(leftElements[i]->getElement()==l)
      nm=i;
  vector3D cc(straightElements[nn]->getHitShape()->getCenter().X(),straightElements[nn]->getHitShape()->getCenter().Y(),0);
  if(!(nm==-1||nn==-1))
    {
      vector3D vs=((wedge*)straightElements[nn]->getHitShape())->getLowerPhiEdge(),
	vl=((spiral*)leftElements[nm]->getHitShape())->getLowerPhiEdge(),
	r=p1,
