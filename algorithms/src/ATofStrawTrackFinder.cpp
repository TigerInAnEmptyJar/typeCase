...
{
  if(setup.getNumberOfDetectors()==0)return;
  tracks=tracksIn;
...
{
  if(resetTrackNumber)numberOfTracks=0;
...
/** - A straw type hit consists of the z and x coordinate of the 
    ...
    For the straw type hit the x and z coordinates are the wire coordinates
    taken in the projection along the straw wire*/
	tofHit newHit((center-planeCenter[i]).R(), 0., 
		      vector3D(planeCenter[i])*beamDirection, 
  ...
  for(int i=0;i<trackFinder->GetNumberOfTracks();i++)
    {
      tofTrack
	newTrack = trackFinder->GetTrack(i);
      float /*x,y,z,*/p,t;
      p=newTrack.GetPhi();
      t=newTrack.GetTheta();
      tracks[numberOfTracks+i]->setPath(sLine3D(point3D(newTrack.GetFootX(),newTrack.GetFootY(),0.),
  ...
  for (int i = 0; i < numDets;i++)
	...
	for(int k=0;k<trackFinder->GetNumberOfTracks();k++)
	  {
	    tofTrack
	      newTrack = trackFinder->GetTrack(k);
	    for(int l=0;l<newTrack.GetNumberOfHits();l++)
	      {
		tmpHit=(tofHit*)tofDataManager::Instance()->At(newTrack.GetHitIndex(l));
		if(tmpHit->GetX()==(center-planeCenter[i]).R()&&tmpHit->GetY()==0&&tmpHit->GetZ()==vector3D(planeCenter[i])*beamDirection)
...
algorithm_parameter ATofStrawTrackFinder::getDescription()
{
  ...
  des.push_back("Assumptions are: the straws come in planar double-layers. The layers can be ");
  ...
  des.push_back("http://www.pit.physik.uni-tuebingen.de/~ehrhardt/KTOF/classes/ATofStrawTrackFinder.html.");
