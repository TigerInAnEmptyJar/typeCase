  ...
  for(int i2=0;i2<2;i2++)//switch prompt pixel/cluster
      ...
      for(int i=0;i<pNum;i++)//loop over all prompt point IDs
	{
	  ...
	  for(int ii=0;ii<(*nP)[pPID];ii++)//loop over all prompt points of that ID
	    {
	      if(i2==0)
		{if(!pixels[pPID][ii]->getShape())continue;}
	      else
		{if(clusters[pPID][ii]->getNumberOfPixels()<=0)continue;}
		...
		for(int l=0;l<clusters[pPID][ii]->getNumberOfPixels();l++)
	      ...
	      if(!searchPrompt(tmpPrompt, (i2==0?(void*)pixels[pPID][ii]:(void*)clusters[pPID][ii]), i2,PromptParams,numPromptParams))continue;
	      if(tmpPrompt->getNumberOfCalibHits()<minElementsP[i])continue;
	      //stop loop 
	      for(int j2=0;j2<2;j2++)//switch stop pixel/cluster
		{
		  ...
		  for(int j=0;j<sNum;j++)//loop over all stop IDs
			...
			{
			  if(j2==0)
			    {if(!pixels[PID][jj]->getShape())continue;}
			  else
			    {if(clusters[PID][jj]->getNumberOfPixels()<=0)continue;}
			  //decay loop
			  for(int k2=0;k2<2;k2++)//switch decay pixel/cluster
			      ...
			      for(int k=0;k<dNum;k++)//loop over all decay IDs
				{
				  ...
				  for(int kk=0;kk<(*nD)[dPID];kk++)//loop over all decay points of that ID
				    {
				      if(k2==0)
					{if(!pixels[dPID][kk]->getShape())continue;}
				      else
					{if(clusters[dPID][kk]->getNumberOfPixels()<=0)continue;}
				      decayLine.newLine((k2==0?pixels[dPID][kk]->getShape()->getCenter():clusters[dPID][kk]->getCenter()),
							(j2==0?pixels[PID][jj]->getShape()->getCenter():clusters[PID][jj]->getCenter())-
							(k2==0?pixels[dPID][kk]->getShape()->getCenter():clusters[dPID][kk]->getCenter()));
					...
					vFit.addPoint(vector3D(PromptParams[iii][7],
							       PromptParams[iii][8],
							       PromptParams[iii][9]),
							   vector3D(PromptParams[iii][4],
								    PromptParams[iii][5],
								    PromptParams[iii][6]),
...
// 			  switch(i2+2*j2)
// 			    {
// 			    case 0: decayLine=sLine3D(pixels[dPID][jj]->getShape()->getCenter(),pixels[PID][ii]->getShape()->getCenter()-pixels[dPID][jj]->getShape()->getCenter());
// 			      break;
// 			    case 1: decayLine=sLine3D(pixels[dPID][jj]->getShape()->getCenter(),clusters[PID][ii]->getCenter()-pixels[dPID][jj]->getShape()->getCenter());
// 			      break;
// 			    case 2: decayLine=sLine3D(clusters[dPID][jj]->getCenter(),pixels[PID][ii]->getShape()->getCenter()-clusters[dPID][jj]->getCenter());
// 			      break;
// 			    case 3: decayLine=sLine3D(clusters[dPID][jj]->getCenter(),clusters[PID][ii]->getCenter()-clusters[dPID][jj]->getCenter());
// 			      break;
// 			    }
...
// 				      else
// 					for(int l=0;l<clusters[pPID][kk]->getNumberOfPixels();l++)
...
// 	}
//     }
  if(tmpNum+numberOfTracks>maxTrack)tmpNum=maxTrack-numberOfTracks;
...
bool AKinkSearch::searchTrack(int trNum, int *dets, int numDets, void *p1, void *p2, int d1,int d2)
{
  ...
  if(d1==0)
      ...
      for(int i=0;i<numTmpP;i++)
	{
	  ...
	  if((project^projc)*tmpPix[i]->getShape()->getNormal()<0)phis[i]=-phis[i];
	  if(thetas[i]<minT)minT=thetas[i];
	  if(thetas[i]>maxT)maxT=thetas[i];
	  if(phis[i]<minP)minP=phis[i];
	  if(phis[i]>maxP)maxP=phis[i];
	}
      pR=maxP+minP+tmpPix[0]->getShape()->angularRange_Phi(path.Foot());
      tR=maxT-minT+tmpPix[0]->getShape()->angularRange_Theta(path.Foot());
    }
  else tmpPix=new TPixel*[1];
  TPixel **tmpP2;
  int numTP2=0;
  if(d2==0)
    {
      ...
      for(int i=0;i<numTP2;i++)
	{
	  ...
	  if((project^projc)*tmpP2[i]->getShape()->getNormal()<0)ph=-ph;
	  if(th<minT)minT=th;
	  if(th>maxT)maxT=th;
	  if(ph<minP)minP=ph;
	  if(ph>maxP)maxP=ph;
      ...
      for(int j=0;j<numTmpP;j++)
	{
	  for(int kk=0;kk<tmpPix[j]->getNumberOfActualElements();kk++)
	      ...
	      if(tmpPix[j]->getElementr(kk).getDetector()==k)
		{
		  ...
		  Vector tmp(7);
		  tmp.setValue(0,(bestHit.getValue(4)*bestHit.getValue(7)+bestHit.getValue(5)*bestHit.getValue(8))/bestHit.getValue(0));
		  ...
		  vFit.addPoint(vector3D(bestHit[7],bestHit[8], bestHit[9]),
				vector3D(bestHit[4],bestHit[5], bestHit[6]),
				bestHit[0],1);
		}
	    }
	}
...
// 	      else
      {
	elnum=bestElement(tmpTracks[trNum],k,/*maxDistPix[ii]*/pR,/*maxDistPix[ii]*/tR,/*completePixel[ii]*/false,bestHit);
...
#include "fiber.h"
int AKinkSearch::bestElement(TTrack* tracki, int det,float maxDistP, float maxDistT,bool cp,Vector&bestHit)
{
  /******************************************
 ...
 ********************************************/
  if(*(numberOfHits[det])<=0)return-1;
  int elnum=-1;
  ...
  for(int l=0;l<*(numberOfHits[det]);l++)
      ...
      if((shape)&&(hits[det][l]->isValid()))
	{
	  ...
	  if(shape->getName()=="fiber")
	    {
	      middleLineFiber=sLine3D(((fiber*)shape)->getCorner()+((fiber*)shape)->getDirection(1)*0.5+((fiber*)shape)->getDirection(2)*0.5,((fiber*)shape)->getDirection(0));
	  ...
	  v=point3D(hitParam.getValue(7),hitParam.getValue(8),hitParam.getValue(9))-path.Foot();//vertex;
	  n=v-z*(v*z);
	  v.normalize();
	  n.normalize();
	  ...
	  if(phiD>=2*M_PI)phiD=phiD-2*M_PI;
	  ...
	  if((phiD<maxDistP)&&
	     (thetaD<maxDistT)&&
	     (distance.length()<bestDist.length())&&
...
bool AKinkSearch::checkLine(const sLine3D &promptLine)//check if detectors just allowed for decay tracks lie on the line
{
  float pr,tr;
  vector3D c;
  for(int i=0;i<numStopPixels;i++)
    for(int j=0;j<(*numberOfPixels[stopPixelIDs[i]]);j++)
      {
	if(!pixels[stopPixelIDs[i]][j]->getShape())continue;
  ...
  for(int i=0;i<numStopCluster;i++)
	...
	for(int k=0;k<clusters[stopClusterIDs[i]][j]->getNumberOfPixels();k++)
	  {
	    if(!clusters[stopClusterIDs[i]][j]->getPixelr(k).getShape())continue;
	    ...
	    c=clusters[stopClusterIDs[i]][j]->getPixelr(k).getShape()->getCenter()-promptLine.Foot();
	    if(promptLine.Direction()*c<pr&&promptLine.Direction()*c<tr)
	      return true;
	  }
      }
...
bool AKinkSearch::searchPrompt(TTrack* tmpPrompt, void* pix, int d1, Vector *PromptParams,int &numPromptP)
{
  numPromptP=0;
      ...
      for(int j=0;j<numTmpP;j++)
	{
	  for(int kk=0;kk<tmpPix[j]->getNumberOfActualElements();kk++)
	      ...
	      if(tmpPix[j]->getElementr(kk).getDetector()==k)
		{
		  ...
		  Vector tmp(7);
		  tmp.setValues((bestHit.getValue(4)*bestHit.getValue(7)+bestHit.getValue(5)*bestHit.getValue(8))/bestHit.getValue(0),
				bestHit.getValue(4)/bestHit.getValue(0),bestHit.getValue(5)/bestHit.getValue(0),
				bestHit.getValue(4)*bestHit.getValue(3)/bestHit.getValue(0),
				bestHit.getValue(5)*bestHit.getValue(3)/bestHit.getValue(0),bestHit.getValue(3),0);
...
// 	      else
      {
	elnum=bestElement(tmpPrompt,k,/*maxDistPix[ii]*/pR,/*maxDistPix[ii]*/tR,/*completePixel[ii]*/false,bestHit);
	if((elnum>=0)&&(elnum<*(numberOfHits[k])))
	  {
...
AKinkSearch::AKinkSearch(TTrack **tracksI,int maxTrackI,int&numberOfTracksI,TPixel***pixelsI,int**numberOfPixelsI,TCluster***clustersI,int**numberOfClustersI,TCalibHit***hitsI,int**numberOfHitsI,vector<int>stopPixelIDsI,vector<int>stopClusterIDsI,vector<int>definePromptPixIDsI,vector<int>definePromptClIDsI,vector<int>defineDecPixIDsI,vector<int>defineDecClIDsI,vector<int>promptDetIDsI,vector<int>decDetIDsI,vector<int>interDetIDsI,int minPromPixI,int minDecPixI,vector<int>minElementsPI,vector<int>minElementsDI,float maxChiKinkI):AAlgorithm("Tracks with Kink search "),maxTrack(maxTrackI),numberOfTracks(numberOfTracksI),vFit(2,100)
{
  tracks=tracksI;
  pixels=pixelsI;
  clusters=clustersI;
  ...
  for(int i=0;i<numPromptPix+numPromptCl;i++)
    if((int)minElementsPI.size()>i)minElementsP[i]=minElementsPI[i];
    else minElementsP[i]=0;
  minElementsD=new int[numStopPixels+numStopCluster];
  for(int i=0;i<numStopPixels+numStopCluster;i++)
    if((int)minElementsDI.size()>i)minElementsD[i]=minElementsDI[i];
    else minElementsD[i]=0;
