...
{
  if(parameter.getDimension()<4)return;
  if(parameter.getDimension()<=5)
    {
      ...
      //      cout<<N_a<<" "<<Z[0]<<" "<<A[0]<<" "<<m_electron<<" "<<c2[0]<<" "<<c3[0]<<endl;
      weight[0]=1;
      return;
    }
  density=parameter[0];
...
double ABetheBloch::stepSize()const
{
  return stepwidth;
}
void ABetheBloch::setStepsize(double value)
{
  stepwidth=abs(value);
}
void *ABetheBloch::process(void*ptr)
{return ptr;}
...
float ABetheBloch::energyLossM(int geantId, float momentum, float path)
{
  ...
  if(geantId<7)return energyLoss_photon(geantId, energy, path);
...
float ABetheBloch::energyLoss(float path, float mass, float beta, float energy, float charge2)
{
  ...
  //cout<<c1<<" "<<T_max<<" "<<charge2<<" "<<beta2<<" "<<gamma2<<" "<<E_kin<<endl;
  for(int i=0;i<nComponents;i++)
    {
      dEdx+=weight[i]*c1*c2[i]*charge2/beta2*(0.5*log(c3[i]*beta2*gamma2*T_max)-beta2);
      //cout<<c2[i]<<" "<<c3[i]<<endl;
      // cout<<c1*c2[i]*charge2/beta2*(0.5*log(2.*c3[i]*beta2*gamma2*T_max)-beta2)<<" "<<weight[i]<<" "<<c1*c2[i]<<" "<<charge2/beta2<<" "<<0.5*log(2.*c3[i]*beta2*gamma2*T_max)<<" "<<c3[i]/T_max<<" "<<-beta2<<endl;
    }
...
float ABetheBloch::energyLoss_photon(int geantId, float energy, float path)
{
  ...
  else return energyLossE(geantId,energy,path);
}
float ABetheBloch::penetrationDepth(momentum4D particle, double charge)
{
  ...
  double T_max=2.*m_electron/*c_light*c_light*/*beta2*gamma2/(1.+2.*gamma*m_electron/mass+m_electron*m_electron/mass/mass);
  //  dEdx=(charge*charge*/(beta*beta))*k*Z/A*(0.5*log(2*m_electron*c*c*beta*beta*gamma*gamma*T_max/excitationE/excitationE)-beta*beta-densityModifications/2);
  //dEdx=(charge2*beta2)*constant1*(0.5*log(constant2*beta2*gamma2*T_max)-beta2-densityModifications/2.);
  double dEdx=0;
  for(int i=0;i<nComponents;i++)
    dEdx+=weight[i]*c1*c2[i]*charge2/beta2*(0.5*log(2.*c3[i]*beta2*gamma2*T_max)-beta2);
  ...
  while(E_kin>dE+dEdx*stepwidth)
    {
      ...
      beta=sqrt(-(1./gamma2-1.));
      beta2=beta*beta;
      T_max=2.*m_electron/*c_light*c_light*/*beta2*gamma2/(1.+2.*gamma*m_electron/mass+m_electron*m_electron/mass/mass);
      dEdx=0;
      for(int i=0;i<nComponents;i++)
	dEdx+=weight[i]*c1*c2[i]*charge2/beta2*(0.5*log(2.*c3[i]*beta2*gamma2*T_max)-beta2);
      dEdx=dEdx/sumOfWeights;
...
float ABetheBloch::betaAfter(float path, momentum4D before, float charge)
{
  ...
  if(de>=before.Energy()-before.Mass())return 0.;
