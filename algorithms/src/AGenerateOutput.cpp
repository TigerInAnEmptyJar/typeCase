...
AGenerateOutput::AGenerateOutput(TEvent &evIn, const string& mcFileName, const string &outFileNameIn, const string &localD, bool useLocalD, int maxt,bool umc,bool writeT,bool writeC,bool writeP, bool writeH,bool writeR,vector<string>reacts, int serverIn, bool addFileIn, bool exchangeFileIn, void *outmutexIn):AAlgorithm("Generate output for track comparison"),event(evIn),maxOfHits(evIn.getMaxNumber<TCalibHit>()*2),maxOfPixel(evIn.getMaxNumber<TPixel>()*3),maxOfCluster(evIn.getMaxNumber<TCluster>()*3),server(serverIn==0),addFile(addFileIn),exchangeFile(exchangeFileIn)
{
...
//   f->cd();
  ...
  if(useLocal)
    {
      string fn=outFileNameIn.substr(outFileNameIn.rfind("/")+1,outFileNameIn.length()-outFileNameIn.rfind("/")-1);
  ...
  if(writeTracks||writeReactions)
    {
      ...
      for(int i=0;i<maxtr;i++)trackTree[i]=NULL;
  ...
  //cout<<9<<endl;
      ...
      if(writeReactions)
	{
	  ...
	  if(reactionFile!=NULL)reactionFile->cd();
  ...
  if(writeHits)
    {
      ...
      for(int i=0;i<maxtr;i++)hitTree[i]=NULL;
      Hits=new hit*[maxtr];
      ...
      if(writeTracks||writeReactions)
	{
	  ...
	  if(writeTracks)
	      ...
	      for(int i=0;i<numT;i++)
		{
	      ...
	      outLogged<<"last Event number for tracks: "<<trackTree[0]->FindLeaf("EventNumber")->GetValue();
...
#else
		  QProcess copy;
		  copy.start(QString((string("cp ")+f->GetName()+" "+filepattern.data()+"T.root").data()));
		  while(copy.waitForFinished()){}
#endif
		  cout<<"copied to:"<<filepattern.data()<<"T.root "<<(existing(filepattern+"T.root")?"successful":"failed")<<endl;
		}
	    }
	  if(writeReactions)
	    {
	      ...
	      for(int i=0;i<numReact;i++)fil[i]=NULL;
	      for(int i=0;i<numReact;i++)
		{
		  ...
		  reactTree[i]->Write();
		  for(int j=0;j<i;j++)
		    if(fil[i]==fil[j])fil[i]=NULL;
		  cout<<reactTree[i]->GetName()<<" written to file "<<reactTree[i]->GetEntries()<<" Entries"<<endl;
		}
	      for(int i=0;i<numReact;i++)
		if(fil[i]!=NULL)
		  {
	      ...
	      delete fil;
	      if(reactionFile->IsOpen())reactionFile->Close();
	      if(useLocal)
		{
#if QT_VERSION < 0x040000
...
#else
		  QProcess copy;
		  copy.start(QString((string("cp ")+f->GetName()+" "+filepattern.data()+"R.root").data()));
		  while(copy.waitForFinished()){}
#endif
...
//     }
  ...
  if(outmutex!=NULL&&server)delete (pthread_mutex_t*)outmutex;
  if(useMC)
    {
      mcFile.close();
      delete mcTree;
  ...
  if(writeTracks||writeReactions)
      ...
      if(writeTracks)
	{
	  ...
	  for(int i=0; i<numT;i++)
	    {
	      if(fromTrack[i]!=NULL)delete []fromTrack[i];
	      if(i==2)n=10;
	      if(i==3)n=20;
	      for(int j=0;j<n;j++)
		if(hitsOfTrack[i][j]!=NULL)delete []hitsOfTrack[i][j];
	      if(hitsOfTrack[i]!=NULL)delete []hitsOfTrack[i];
	    }
	}
      for(int i=0; i<numT;i++)
	delete []Track[i];
...
void AGenerateOutput::write_Tracks()
{
  ...
  if(numTrees>numT)expandTree(3,numTr);
  if(num>maxtr)num=maxtr;
  TTrack *tr;
  int j=-1,b=0;
	      ...
	      if(tr->getNumberOfCalibHits(jj)>0&&(tr->getCalibSrcr(jj,0)))
		{
		  if(tr->getCalibSrcr(jj,0)->isValid())
		      ...
		      else if(tr->getCalibSrcr(jj,0)->name()=="THitCluster")
			{
			  ...
			  //cout<<"track "<<i<<" hitcluster found : "<<tmp->getNumberOfHits()<<" elements"<<endl;
...
void AGenerateOutput::write_Pixels()
{
  ...
  if(numTrees>numP)expandTree(1,l);
  l=0;
  for(int i=0;i<event.getMaxNumber<TDetector>();i++)
    for(int j=0;j<event.getNumberOfPixels(i);j++)
      {
	//	if(l<maxOfPixel)
	  {
		...
		if(Pix[l/20][l%20].numberOfElements>10)Pix[l/20][l%20].numberOfElements=10;
  ...
  numberOfPixels=l;
  if(l>0)for(int i=0;i<numTrees;i++)pixTree[i]->Fill();
  if(l>0)numPixTree->Fill();
}
...
void AGenerateOutput::write_Hits()
{
  ...
  if(numTrees>numH)expandTree(0,l);
  l=0;
  TCalibHit *th;
  for(int i=0;i<event.getMaxNumber<TDetector>();i++)
    for(int j=0;j<event.getNumberOfHits(i);j++)
  ...
  numberOfHits=l;
  if(l>0)for(int i=0;i<numTrees;i++)hitTree[i]->Fill();
  if(l>0)numHitTree->Fill();
}
void AGenerateOutput::write_Clusters()
{
  ...
  if(numTrees>numC)expandTree(2,l);
  l=0;
  int o=-1;
  TCluster *tc;
  for(int i=0;i<event.getMaxNumber<TDetector>();i++)
	...
	//	if(l<maxOfCluster)
	  {
		...
		if(Clus[l/20][l%20].numberOfPixels>10)Clus[l/20][l%20].numberOfPixels=10;
  ...
  numberOfCluster=l;
  if(l>0)for(int i=0;i<numTrees;i++)cluTree[i]->Fill();
  if(l>0)numCluTree->Fill();
}
...
void AGenerateOutput::getMCTree(string mcFile)
{
  ...
  for(int i=0;i<5;i++)
    {
      ...
      mcTree->Branch(s.data(),&trackIn[i],"phi/D:theta:beta:ID/I:nEl:ss[17]/I:qdc[17]/F:tdc[17]/F:vertex[3]/D:phiIn/D:thetaIn:betaIn:nElIn/I:ssIn[17]/I:vertexIn[3]/D:momentum/D");
    }
  ifstream mcfile;
  ...
  while(mcfile)
	...
	{
	  mcfile>>i>>ID1>>v1>>v2>>v3>>p1>>p2>>p3>>e1>>e2>>e3>>pe1>>pe2>>pe3>>IDfrom1;
	  n=-1;
	  switch(IDfrom1)
	    {
	    case -1:
...
{
  if(event.getNumberOfReactions()==0)return;
  RbaseReaction *rea;
  ...
  for(int i=0;i<rea->getNumberOfSecondaries();i++)
    {
      if(i==5){beg=5;which++;}
      if(i==10){beg=10;which++;}
      if((i%20)==0){beg=i;which++;}
      tr=rea->getTrack(i);
      //      cout<<*tr<<endl;
      if(tr==NULL)return;
...
void AGenerateOutput::expandTree(int which,int upto)
{
  ...
  switch(which)
    ...
    case 0:
      {
	hitFile->cd();
	if(numtrees<=numH)return;
	numtrees=numtrees-numH;
	for(int i=0;i<numtrees;i++)
	  {
	    TTree *tmpTree=new TTree((string("Hits")+string_number(numH+1)).data(),(string("Hits")+string_number(numH+1)).data());
	    ...
	    for(int j=0;j<20;j++)
	      {
		tmpTree->Branch((string("Hit")+string_number(numH*20+j+1)).data(),&tmpHit[j],"qdcR/I:tdcR/I:qdcC/D:tdcC/D:el/I:det/I");
	      }
	...
	for(int i=0;i<numtrees;i++)
	  {
	    TTree *tmpTree=new TTree((string("Pixels")+string_number(numP+1)).data(),(string("Pixels")+string_number(numP+1)).data());
	    tmpTree->Branch("EventNumber",&event.getEventNumber(),"EventNumber/I");
	    tmpTree->Branch("RunNumber",&event.getRunNumber(),"RunNumber/I");
	    pixel *tmpPix=new pixel[20];
	    for(int j=0;j<20;j++)
	      {
		tmpTree->Branch((string("Pixel")+string_number(numP*20+j+1)).data(),&tmpPix[j],"ID/I:numberOfElements/I:elementNumber[10]/I:center[3]/D");
	      }
	...
	for(int i=0;i<numtrees;i++)
	  {
	    TTree *tmpTree=new TTree((string("Cluster")+string_number(numC+1)).data(),(string("Cluster")+string_number(numC+1)).data());
	    tmpTree->Branch("EventNumber",&event.getEventNumber(),"EventNumber/I");
	    tmpTree->Branch("RunNumber",&event.getRunNumber(),"RunNumber/I");
	    cluster *tmpClu=new cluster[20];
	    for(int j=0;j<20;j++)
	      {
		tmpTree->Branch((string("Cluster")+string_number(numC*20+j+1)).data(),&tmpClu[j],"ID/I:numberOfPixels/I:pixelNumber[10]/I:center[3]/D");
	      }
    ...
    case 3:
      {
	if(trackFile!=NULL)trackFile->cd();
	int beg=0;
	int accnum=20;
	if(numT==0)
	  accnum=5;
	...
	while(numT<numtrees)
	  {
	    TTree *tmpTree=new TTree((string("Track")+string_number(numT+1)).data(),(string("Track")+string_number(numT+1)).data());
	    tmpTree->Branch("EventNumber",&event.getEventNumber(),"EventNumber/I");
	    tmpTree->Branch("RunNumber",&event.getRunNumber(),"RunNumber/I");
		...
		tmpTree->Branch((string("Track")+string_number(beg+j+1)).data(),&tmpTrack[j],"qdc[17]/F:tdc[17]/F:ss[17]/I:ssIn[17]/I:vertex[3]/D:vertexIn[3]/D:phi/D:theta:beta:momentum:phiIn:thetaIn:betaIn:ID/I:nEl:nElIn");//ID/I:nEl:phi/D:theta:beta:momentum:phiIn:thetaIn:betaIn:nElIn/I:ss[17]/I:ssIn[17]/I:qdc[17]/F:tdc[17]/F:vertex[3]/D:vertexIn[3]/D");
		tmpTree->Branch((string("hits")+string_number(beg+j+1)).data(),tmpHits[j],(string("hits")+string_number(beg+j+1)+"[30]/I").data());
	      }
...
int AGenerateOutput::getNumTrees(int numEntr, int which)
{
  ...
  switch(which)
    ...
    case 3:
      {
	if(numEntr<=5) return 1;
	if(numEntr<=10) return 2;
	return (numEntr/20)+3;
      }
    }
  return 0;
...
void AGenerateOutput::setReactions(vector<string>reacts,string outName)
{
  ...
  for(unsigned int i=0;i<reacts.size();i++)
	...
	{
	  reactions[j]=new Rpp_pkl_pkppi(NULL,momentum4D(vector3D(0,0,1),Eparticles::getMassG(14)),momentum4D(vector3D(0,0,0),Eparticles::getMassG(14)));
      ...
      else 
	{
      ...
      if(numtrees<numT)expandTree(3,reactions[j]->getNumberOfSecondaries());
      //      f->cd();
      ...
      for(int jj=0;jj<reactions[j]->getNumberOfSecondaries();jj++)
	{
	  ...
	  if(jj==10){k++;b=jj;}
	  if((jj%20)==0){k++;b=jj;}
	  tmpTree->Branch((string("Track")+string_number(jj+1)).data(),&Track[k][jj-b],"qdc[17]/F:tdc[17]/F:ss[17]/I:ssIn[17]/I:vertex[3]/D:vertexIn[3]/D:phi/D:theta:beta:momentum:phiIn:thetaIn:betaIn:ID/I:nEl:nElIn");
...
{
  int common=numT;
  if(common>(int)tTree->size())common=tTree->size();
...
{
  int common=numC;
  if(common>(int)cTree->size())common=cTree->size();
  for(int i=0;i<common;i++)
...
{
  int common=numP;
  if(common>(int)pTree->size())common=pTree->size();
  for(int i=0;i<common;i++)
...
{
  int common=numH;
  if(common>(int)hTree->size())common=hTree->size();
  for(int i=0;i<common;i++)
