#include "AKinFit.h"
#include <stdlib.h>
AKinFit::AKinFit(int maxIterationIn, int maxParticlesIn, float epsilonIn)
    : AFitAlgorithm("WASA kin Fit"), epsilon(epsilonIn), maxParticles(maxParticlesIn),
      maxIteration(maxIterationIn), useCovariant(false)
{
...
void AKinFit::getAddPoint(Vector& data) { add(data); }
...
float AKinFit::fit()
{
  ...
  //  matrixNxM BM_P(4+nC,nMeasured),BU_P(4+nC,nUnmeasured);//matrices
  //  containing derivatives
  matrixNxM GB(4 + nC, 4 + nC);
  matrixNxM GB_1(4 + nC, 4 + nC);
  matrixNxM GM(nMeasured, nMeasured), GM_1(nMeasured, nMeasured),
      GU(nUnmeasured,
         nUnmeasured);                  // error matrix and weighting matrix !diagonal!
  ...
  matrixNxM* matPoint;
  Vector epsM(nMeasured), epsU(nUnmeasured); // corrections to the variables to be introduced by the
                                             // fit
  ...
  if (useCovariant) {
    ...
    for (int i = 0; i < nMeasured; i++) {
      GM.setValue(i, i, dpin[Meas[i] / 10].getValue((int)(Meas[i] % 10)) *
  ...
  for (int i = 0; i < nC; i++) {
    ...
    for (int j = 0; j < nP; j++) {
      ...
      for (int k = 0; k < 4; k++)
        sf[i].setValue(k,
                       sf[i].getValue(k) +
                           sig * ppx[j].getValue(k)); //[k][i]+=in[j][i+5]*ppx[j].getValue(k);
    }
    sum = 0;
    for (int j = 0; j < 3; j++)
      sum += sf[i][j] * sf[i][j];
  ...
  while (condition) {
      ...
      for (int i = 0; i < number; i++) {
        ...
        for (int j = 0; j < nC; j++) {
          if (in[num][j + 5] != 0) {
            ...
            matPoint->setValue(
                j + 4, i, getDerivativeConstraint(
                              in[num][3], pos, p[num], ppx[num], am[num],
                              sf[j])); //-value/sf[4][j]*(in[num][j+5]==0?0:(in[num][j+5]<0?-1:1)));
          } else
    ...
    for (int i = 0; i < nMeasured; i++) {
      n = Meas[i] / 10;
      m = Meas[i] % 10;
      //	  cout<<"Particle "<<n<<" property "<<m<<" old "<<p[n][m]<<"\t
      // new
      //"<<particles[n][m]+epsM[i]<<endl;
      if ((in[n][3] == 2 || in[n][3] == 3 || in[n][3] == 4) && m != 0) {
    ...
    chi2_prev = chi2;
    chi2 = epsM * ((GM)*epsM) + epsU * (GU * epsU) +
           2 * al * (FI + BM * epsM +
                     BU * epsU); // epsM*(GM*epsm)+epsU*(GU*epsU)+2*al*(FI+BM*epsM+BU*epsU)
                                 // ???
    // calculate FM kinematic
    for (int i = 0; i < nP; i++)
      ppx[i] = getMomentum(i, p[i]);
    ...
    for (int i = 0; i < nC; i++) {
      ...
      for (int j = 0; j < nP; j++) {
        if (in[j][i + 5] == 0)
          ...
          sig = 1;
        for (int k = 0; k < 4; k++)
          sf[i].setValue(k, sf[i].getValue(k) +
                                sig * ppx[j].getValue(k)); //[k][i]+=in[j][i+5]*ppx[j].getValue(k);
      }
      sum = 0;
      for (int j = 0; j < 3; j++)
...
momentum4D AKinFit::getMomentum(int num, vector3D pm)
{
  ...
  {
    if (p[num].X() < 0)
      p[num].setValue(0, 0.001);
    float P = sqrt(p[num].X() * (p[num].X() + 2 * am[num]));
    ret.setPM(vector3D(P * sin(p[num].Y()) * cos(p[num].Z()), P * sin(p[num].Y()) * sin(p[num].Z()),
                       P * cos(p[num].Y())),
              am[num]);
    break;
  }
...
vector4D AKinFit::getDerivative(int isIn, int pos, vector3D pm, float mass)
{
  ...
  case 3: // v,th,phi
  {
    ...
    switch (pos) {
    case 0:
      ret.setValues(tmpf * (sin(pm.Y()) * cos(pm.Z())), tmpf * (sin(pm.Y()) * sin(pm.Z())),
                    tmpf * (cos(pm.Y())), 0);
      break;
    case 1:
      ret.setValues(p * (cos(pm.Y()) * cos(pm.Z())), p * (cos(pm.Y()) * sin(pm.Z())),
                    -p * (sin(pm.Y())), 0);
      break;
    case 2:
      ret.setValues(-p * (sin(pm.Y()) * sin(pm.Z())), p * (sin(pm.Y()) * cos(pm.Z())), 0, 0);
      break;
    }
    //      ret.setValue(0,ret.getValue(0)*ppx.W()/(sqrt(p.X()*(p.X()+2*mass))));
    ret.setValue(
        3,
        (pos == 0 ? mass * pm.X() * g * g * g : 0)); //(1-pm.X()*pm.X())/sqrt(1-pm.X()*pm.X()):0));
    break;
  }
  case 4:
    break;
...
float AKinFit::getDerivativeConstraint(int representation, int pos, vector3D pm, momentum4D px,
                                       float mass, Vector sf)
{
...
int AKinFit::getOutInfo() const { return outCode; }
