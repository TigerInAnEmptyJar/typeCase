...
int searchPosInTree(TBranch *bran, int minP, int maxP, Int_t value, Int_t& val, int mode)
{
  ...
  if(mode==0)//search identity
    {
      bran->GetEntry(posH);
      if(val<=value)return posH;
      bran->GetEntry(posL);
      if(val>=value)return posL;
      while(posH-posL>2)
	{
	  posM=(int)((posH+posL)/2);
	  bran->GetEntry(posM);
	  if(val==value)return posM;
	  else if(val<value)posL=posM;
	  else posH=posM;
	}
      return posH;
    }
...
void AReadReactionFromRoot::cleanHit()
{
  ...
  delete []Hits;
  delete []hitTree;
  if(NHit)hitf=numHitTree->GetCurrentFile();
  hitf->Close();
  delete hitf;
  hitf=NULL;
  Hits=NULL;
...
void AReadReactionFromRoot::cleanPix()
{
  ...
  delete []Pix;
  delete []pixTree;
  if(NPix)pixf=numPixTree->GetCurrentFile();
  pixf->Close();
  delete pixf;
  pixf=NULL;
  Pix=NULL;
...
void AReadReactionFromRoot::cleanClu()
{
  ...
  delete []Clus;
  delete []cluTree;
  if(numCluTree)cluf=numCluTree->GetCurrentFile();
  cluf->Close();
  delete cluf;
  cluf=NULL;
  Clus=NULL;
...
void AReadReactionFromRoot::cleanTra()
{
  ...
  delete []fromID;
  delete []traTree;
  if(numTraTree)traf=numTraTree->GetCurrentFile();
  traf->Close();
  delete traf;
  traf=NULL;
  Tracks=NULL;
...
void AReadReactionFromRoot::set_hit(string filename)
{
  if(readHits)
      ...
      while(tmptree!=NULL)
	{
      ...
      if(nextHitEntry)delete []nextHitEntry;
	...
	for(int i=0;i<20;i++)
	  {
	    ...
	    tmptree->SetBranchAddress((string("Hit")+string_number(j*20+i+1)).data(),&tmptracks[i]);
	  }
	Hits[j]=(tmptracks);
	hitTree[j]=(tmptree);
	tmptree->GetEntry(0);
...
void AReadReactionFromRoot::set_pix(string filename)
{
  if(readPixel)
      ...
      while(tmptree!=NULL)
	{
      ...
      for(int i=0;i<numTmp;i++)currentHitEntry[i]=-1;
      if(nextPixEntry)delete []nextPixEntry;
	...
	for(int i=0;i<20;i++)
	  {
	    ...
	    tmptree->SetBranchAddress((string("Pixel")+string_number(j*20+i+1)).data(),&tmptracks[i]);
	  }
	Pix[j]=(tmptracks);
	pixTree[j]=(tmptree);
	tmptree->GetEntry(0);
...
void AReadReactionFromRoot::set_clu(string filename)
{
  if(readCluster)
      ...
      while(tmptree!=NULL)
	{
      ...
      for(int i=0;i<numTmp;i++)currentCluEntry[i]=-1;
      if(nextCluEntry)delete []nextCluEntry;
	...
	for(int i=0;i<20;i++)
	  {
	    ...
	    tmptree->SetBranchAddress((string("Cluster")+string_number(j*20+i+1)).data(),&tmptracks[i]);
	  }
	Clus[j]=(tmptracks);
	cluTree[j]=(tmptree);
	tmptree->GetEntry(0);
...
void AReadReactionFromRoot::set_tra(string filename) 
{
  if(readTracks)
      ...
      while(tmptree!=NULL)
	{
      ...
      if(currentTraEntry)delete []currentTraEntry;
      currentTraEntry=new int[numTmp];
      for(int i=0;i<numTmp;i++)
      if(nextTraEntry)delete []nextTraEntry;
  ...
  for(int i=0;i<numTmp;i++)//while(tmptree!=NULL)
    {
      ...
      reactionTree[num]->SetBranchAddress((string("Track")+string_number(i+1)).data(),&RTra[num][i]);
    }
...
void AReadReactionFromRoot::read_pix()
{
  ...
  for(int j=0;j<numtrees;j++) 
    {
      if(currentPixEntry[j]<0)currentPixEntry[j]++;
	  ...
	  if(event.getMaxNumber<TPixel>()>n)
	    {
	      ...
	      sh->setCenter(point3D(Pix[j/20][j%20].center[0],Pix[j/20][j%20].center[1],Pix[j/20][j%20].center[2]));
	      tp->setShape(sh);
	      if(readHits)
		{
		  for(int kk=0;kk<Pix[j/20][j%20].numberOfElements;kk++)
		    {
		      ...
		      for(int m=0;m<l;m++)
			if(el==event.getHit(id,m).getElement()&&Hits[k/20][k%20].tdcR==event.getHit(id,m).getRawTDC())
...
//   event->getTrigger()=tT;
  ...
  if(numtrees>NTra)numtrees=NTra;
  if(numtrees<0)numtrees=0;
  int tmpev[numtrees+1];
  for(int j=0;j<numtrees;j++) 
    {
      if(currentTraEntry[j]<0)currentTraEntry[j]++;
...
void AReadReactionFromRoot::read_rea()
{
  if(NRea==0)
    return;
  if(!readTracks)event.getNumberOfTracks()=0;
//   event->getTrigger()=tT;
  vector3D v;
  Int_t tmpev[NRea];
  bool val=false;
  ...
  for(int j=0;j<NRea;j++) 
    {
      ...
      //      cout<<j<<" "<<reactionTree[j]->GetEntries()<<" "<<currentReaEntry[j]<<" "<<event.getEventNumber()<<flush;
      ...
      for(int k=0;k<nRtr[j];k++)
	{
      ...
      //      cout<<" use "<<event.getRunNumber()<<"§ "<<rb->getTrack(0)->getNumberOfCalibHits(15)+rb->getTrack(0)->getNumberOfCalibHits(16)<<" "<<rb->getTrack(1)->getNumberOfCalibHits(15)+rb->getTrack(1)->getNumberOfCalibHits(16)<<endl;
    }
  ...
  if(readHits)
      ...
      if(hi!=NULL)
	{
	  ...
	  if(tr->getNumberOfCalibHits()!=n)
	    anaLog<<"AReadFromSimpleTree::readTracks() Track[] wrong number of hits: assigned :"<<tr->getNumberOfCalibHits()<<" named: "<<n<<endli;
	}
      else
	{
	  TCalibHit *ch=NULL;
	  ...
	  for(int j=0;j<17;j++)
	      ...
	      for(int k=0;k<event.getNumberOfHits(det);k++)
		{
	      ...
	      if(ch==NULL)
		{ cout<<"hit not found "<<det<<endl;continue;}
	      if(ch->getElement()!=Track.ss[j])continue;
	      tr->addCalibSrc(ch);
	    }
	}
    }
...
//   cout<<tr<<endl;
//   cout<<Track.ss[0]<<" "<<Track.ss[1]<<" "<<Track.ss[2]<<" "<<Track.ss[3]<<" "<<Track.ss[4]<<" "<<Track.ss[5]<<" "<<Track.ss[6]<<" "<<Track.ss[7]<<" "<<endl;
  for(int i=0;i<17;i++)
    {
//       if(i==8||i==9)
// 	{
...
void AReadReactionFromRoot::searchEvent(int eventNumber, int runNumber, int mode)
{
  ...
  if(mode==4)//search reacktion tree
      ...
      for(int i=0;i<NRea;i++)
	{
	  ...
	  if(ruNR[i]>runNumber)continue;
	  if(ruNR[i]==runNumber)runBegin=0;
	  ...
	  if(ruNR[i]<runNumber)continue;
	  if(ruNR[i]==runNumber)runEnd=reactionTree[i]->GetEntries()-1;
	  else //search end
	    searchPosInTree(runNrBranch,runBegin,reactionTree[i]->GetEntries()-1,runNumber,ruNR[i],2);
	  eventNrBranch->GetEntry(runBegin);
	  nrL=evNR[i];
	  eventNrBranch->GetEntry(runEnd);
	  nrH=evNR[i];
	  //	  cout<<eventNumber<<" "<<runBegin<<"-"<<runEnd<<" "<<nrL<<"-"<<nrH<<" "<<Rfiles[i].size()<<endl;
	  currentReaEntry[i]=runBegin;
	  if(nrL==eventNumber)currentReaEntry[i]=runBegin;
	  else if(nrH==eventNumber)currentReaEntry[i]=runEnd;
	    ...
	    {
	      currentReaEntry[i]=searchPosInTree(eventNrBranch,runBegin,runEnd,eventNumber,evNR[i],0);
	    }
	  hasA=true;
	}
...
//	treeFile=traf;
  ...
  for(int i=0;i<nData;i++)
	...
	searchPosInTree(runNrBranch,runBegin,dataTree[i]->GetEntries()-1,runNumber,*runNr,2);
      ...
      eventNrBranch->GetEntry(runEnd);
      nrH=*evtNr;
      if(nrL==eventNumber)currentEntry[i]=runBegin;
      else if(nrH==eventNumber)currentEntry[i]=runEnd;
      else if(nrL>eventNumber)
	{
	  if(files[i].size()<=1)
	    ...
	    {
	      if(filePos[mode]==0)continue;
	      filePos[mode]--;
	      switch(mode)
		{
		case 0:cleanHit();set_hit(files[0][filePos[0]]);break;
		case 1:cleanPix();set_pix(files[1][filePos[1]]);break;
		case 2:cleanClu();set_clu(files[2][filePos[2]]);break;
		case 3:cleanTra();set_tra(files[3][filePos[3]]);break;
		}
	      searchEvent(eventNumber, runNumber, mode);
	      return;
	    }
...
void AReadReactionFromRoot::sortFiles()
{
  ...
  for(int i=0;i<4;i++)
      ...
      if(files[i].size()<2)
	{
	  ...
	  switch(i)
	    {
	    case 0:set_hit((files[0])[filePos[0]]);break;
	    case 1:set_pix((files[1])[filePos[1]]);break;
	    case 2:set_clu((files[2])[filePos[2]]);break;
	    case 3:set_tra((files[3])[filePos[3]]);break;
	    }
	  continue;
	}
      ...
      while(filesToSort>1)
	{
      ...
      while(!files[i].empty())files[i].pop_back();
      while(!used.empty())used.pop_back();
      while(!begR.empty())begR.pop_back();
      while(!begE.empty())begE.pop_back();
      while(!endR.empty())endR.pop_back();
      while(!endE.empty())endE.pop_back();
  ...
  for(int i=0;i<NRea;i++)
      ...
      while(filesToSort>1)
	{
      ...
      while(!Rfiles[i].empty())Rfiles[i].pop_back();
      for(unsigned int j=0;j<tmp.size();j++)
...
AReadReactionFromRoot::AReadReactionFromRoot(TSetup &setupIn, TEvent &eventIn, bool &validInputIn, const algorithm_parameter &param):AAlgorithm("Read reaction properties from root file"),setup(setupIn),event(eventIn),maxHit(eventIn.getMaxNumber<TCalibHit>()),maxPix(eventIn.getMaxNumber<TPixel>()),maxClu(eventIn.getMaxNumber<TCluster>()),maxTra(eventIn.getMaxNumber<TTrack>()),validInput(validInputIn)
{
...
AReadReactionFromRoot::AReadReactionFromRoot(TSetup &setupIn, TEvent &eventIn, bool &validInputIn, bool readH, bool readP, bool readC, bool readT, bool minimum, bool reconstruct, vector<string> Rnames):AAlgorithm("Read reaction properties from root file"),setup(setupIn),event(eventIn),maxHit(eventIn.getMaxNumber<TCalibHit>()),maxPix(eventIn.getMaxNumber<TPixel>()),maxClu(eventIn.getMaxNumber<TCluster>()),maxTra(eventIn.getMaxNumber<TTrack>()),validInput(validInputIn)
{
  readHits=readH;
  readPixel=readP;
  readCluster=readC;
  ...
  NRea=Rnames.size();
  reactionNames=new string[NRea];
  for(int i=0;i<NRea;i++)reactionNames[i]=Rnames[i];
  init();
}
void AReadReactionFromRoot::init()
{
  ...
  for(int i=0;i<NRea;i++)
    {
      ...
      if(theReaction[i]!=NULL)theReaction[i]->setInitParticles(2,p);
    }
  for(int i=0;i<5;i++)
  ...
  for(int i=0;i<4;i++)filePos[i]=0;
  for(int i=0;i<NRea;i++)RfilePos[i]=0;
  Hits=NULL;
  Pix=NULL;
  Clus=NULL;
  Tracks=NULL;
  RTra=new track1*[NRea];
  for(int i=0;i<NRea;i++)RTra[i]=NULL;
  ...
  if(readHits)
    {
      ...
      for(int i=0;i<NHit;i++)if(currentHitEntry[i]<0)currentHitEntry[i]=0;
      ...
      else
	{
      ...
      if(currentHitEntry[0]<numHitTree->GetEntries())val=true;
    }
  if(readPixel)
    {
      evtBranch=numPixTree->GetBranch("EventNumber");
      numBranch=numPixTree->GetBranch("NumberOfHits");
      for(int i=0;i<NPix;i++)if(currentPixEntry[i]<0)currentPixEntry[i]=0;
      ...
      else
	{
      ...
      if(currentPixEntry[0]<numPixTree->GetEntries())val=true;
    }
  if(readCluster)
    {
      evtBranch=numCluTree->GetBranch("EventNumber");
      numBranch=numCluTree->GetBranch("NumberOfHits");
      for(int i=0;i<NClu;i++)if(currentCluEntry[i]<0)currentCluEntry[i]=0;
      ...
      else
	{
      ...
      if(currentCluEntry[0]<numCluTree->GetEntries())val=true;
    }
  if(readTracks)
    {
      evtBranch=numTraTree->GetBranch("EventNumber");
      numBranch=numTraTree->GetBranch("NumberOfHits");
      for(int i=0;i<NTra;i++)if(currentTraEntry[i]<0)currentTraEntry[i]=0;
      ...
      else
	{
      ...
      if(currentTraEntry[0]<numTraTree->GetEntries())val=true;
    }
  validInput=val;
  read_rea();
  return ptr;
...
//     cout<<currentHitEntry[0]<<" "<<currentReaEntry[0]<<" "<<currentReaEntry[1]<<endl;
  ...
  //cout<<files[0].size()<<" "<<files[1].size()<<" "<<files[2].size()<<" "<<files[3].size()<<" "<<Rfiles[0].size()<<endl;
  event.setRunNumber(r.getRunNumber());
  for(int i=0;i<4;i++)  filePos[i]=0;
  //  cout<<"sortFiles"<<endl;
  sortFiles();
  if(readHits){cleanHit();searchEvent(0,r.getRunNumber(),0);}
  if(readPixel){cleanPix();searchEvent(0,r.getRunNumber(),1);}
  if(readCluster){cleanClu();searchEvent(0,r.getRunNumber(),2);}
  if(readTracks){cleanTra();searchEvent(0,r.getRunNumber(),3);}
  for(int i=0;i<NRea;i++)cleanARea(i);
  //  cout<<"searchEvent-Reactions"<<endl;
  searchEvent(0,r.getRunNumber(),4);
  //cout<<"done"<<endl;
  for(int i=0;i<NRea;i++)currentReaEntry[i]--;
  //cout<<r.getRunNumber()<<" : "<<reactionTree[0]->GetEntries()<<" "<<reactionTree[1]->GetEntries()<<endl;
