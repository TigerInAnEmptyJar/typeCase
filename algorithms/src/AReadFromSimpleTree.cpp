...
AReadFromSimpleTree::AReadFromSimpleTree(const string &fileName, const string &treename, const string &localDirec, bool useLocalD, TTrack **tracksIn, int &numberOfTracksIn, int&eventNumberIn, int maxTracksIn, TEvent &eventIn,int &maxEventsToRead,TSetup *setupIn,bool readT, bool readC, bool readP, bool readH,string TrackFile,string ClusterFile,string PixelFile, string HitFile, bool resetUnreadIn, bool startWithDelayedEventIn, int StartWithEventIn,bool& validInputIn, void* inputmutexIn):AAlgorithm("Read track info from simple tree"), maxTrack(maxTracksIn), numberOfTracks(numberOfTracksIn),maxOfHits(eventIn.getMaxNumber<TCalibHit>()),maxOfPixel(eventIn.getMaxNumber<TPixel>()),maxOfCluster(eventIn.getMaxNumber<TCluster>()),validInput(validInputIn)
{
  if(fileName==treename&&maxEventsToRead==0)currentHitEntry=NULL;
  ...
  if(localDirectory)
    {
      if(localDir=="")localDirectory=false;
  ...
  if(readTracks)
      ...
      else
	{
	  if(localDirectory)
	    {
	      filenames[3]=localDir+"/"+TrackFile.substr(TrackFile.rfind("/")+1,TrackFile.length()-TrackFile.rfind("/")-1);
...
// 		currentTrackEntry[i]++;
  ...
  if(readPixels)
      ...
      else
	{
	  if(localDirectory)
	    {
	      filenames[1]=localDir+"/"+PixelFile.substr(PixelFile.rfind("/")+1,PixelFile.length()-PixelFile.rfind("/")-1);
#if QT_VERSION < 0x040000
	      QProcess copy(QString("cp "));
  ...
  if(readClusters)
      ...
      else
	{
	  if(localDirectory)
	    {
	      filenames[2]=localDir+"/"+ClusterFile.substr(ClusterFile.rfind("/")+1,ClusterFile.length()-ClusterFile.rfind("/")-1);
#if QT_VERSION < 0x040000
	      QProcess copy(QString("cp "));
  ...
  if(readHits)
      ...
      else
	{
	  ...
	  if(localDirectory)
	    {
	      filenames[0]=localDir+"/"+HitFile.substr(HitFile.rfind("/")+1,HitFile.length()-HitFile.rfind("/")-1);
	  ...
	  setHitStruct();
	  anaLog<<(numHitTrees)<<" trees read with "<<((int)numHitTree->GetEntries())<<" Entries"<<endli;
	  if(startWithDelayedEventIn>0)
	    {
...
//cout<<"process()"<<endl;
  if(inputmutex!=NULL)pthread_mutex_lock((pthread_mutex_t*)inputmutex);
  ...
  if(inputmutex!=NULL)
    {
      if(readHits&&hasHit)emit hitRead(numberOfHits);
      if(readPixels&&hasPixel)emit pixelRead(numberOfPixels);
      if(readClusters&&hasCluster)emit clusterRead(numberOfCluster);
      if(readTracks&&hasTrack)emit trackRead(ntr);
  ...
  if(readHits&&hasHit)readHit(c4);
  //  cout<<"done \nread pixels "<<flush;
  if(readPixels&&hasPixel)readPixel(c3);
  //  cout<<"done \nread clusters "<<flush;
  if(readClusters&&hasCluster)readCluster(c2);
  //cout<<"done \nread tracks "<<flush;
  if(readTracks&&hasTrack)readTrack(c1);
  //cout<<"done \nset event number "<<flush;
  event->setEventNumber(tmpevt);
  return ptr;
}
int AReadFromSimpleTree::getCommonEvent(int &iT,bool &ht,int &iC,bool &hc,int &iP,bool &hp,int &iH, bool &hh)
{
  //  cout<<"getCommonEvent()"<<endl;
  int large;
  bool condition;
  if(readTracks)large=evNT;
  if(readClusters)large=evNC;
  if(readPixels)large=evNP;
  if(readHits)large=evNH;
  if(rever)
    {
      evNT=1000000000;
      evNH=1000000000;
  ...
  if(readTracks)
    {
      if(numTrackTree->GetEntries()<=iT+1)return -1;
      numTrackTree->GetBranch("EventNumber")->GetEntry(++iT);
      lowest=evNT;
    }
  if(readClusters)
    {
      if(numCluTree->GetEntries()<=iC+1)return -1;
      numCluTree->GetBranch("EventNumber")->GetEntry(++iC);
      if(lowest==*eventNumber||lowest>evNC)lowest=evNC;
    }
  if(readPixels)
    {
      if(numPixTree->GetEntries()<=iP+1)return -1;
      numPixTree->GetBranch("EventNumber")->GetEntry(++iP);
      if(lowest==*eventNumber||lowest>evNP)lowest=evNP;
    }
  if(readHits)
    {
      if(numHitTree->GetEntries()<=iH+1)return -1;
      iH++;
      numHitTree->GetEntry(iH,0);
      if(lowest==*eventNumber||lowest>evNH)lowest=evNH;
    }
...
// 	  cout<<currentHitEntry[i]<<"\t";	
  ...
  if(rever)
    {
      if(evNP<large)large=evNP;
      if(evNC<large)large=evNC;
      if(evNT<large)large=evNT;
    }
  else
    {
      if(evNP>large)large=evNP;
...
//   event->getTrigger()=tT;
  ...
  if(numtrees>numTrackTrees)numtrees=numTrackTrees;
  if(numtrees<0)numtrees=0;
  int tmpev[numtrees+1];
  for(int j=0;j<numtrees;j++) 
    {
      if(currentTrackEntry[j]<0)currentTrackEntry[j]++;
  ...
  for(int j=0;j<ntr;j++)
    {
      ...
      if(trackBranches[j]->GetEntries()<=currentTrackEntry[w])return;
  ...
  for(int j=0;j<ntr;j++)
    {
      if(j==5){which=1;beg=5;}
      if(j==10){which=2;beg=10;}
      if((j%20)==0&&(j!=0)){which=(j/20)+2;beg=j;}
      th=trackLeaves[j][0]->GetValue();
      ph=trackLeaves[j][1]->GetValue();
      if(th>0.433)ph+=-0.011;
      if(ph<-M_PI)ph+=2*M_PI;
      tracks[j]->reset();
      c.setValues(trackLeaves[j][3]->GetValue(0),trackLeaves[j][3]->GetValue(1),trackLeaves[j][3]->GetValue(2));
	 ...
	 //vector3D(sin(Tracks[which][j-beg].theta)*cos(Tracks[which][j-beg].phi),
      ...
      tracks[j]->getParticler().setVM(tracks[j]->getPath().Direction()*tracks[j]->getSpeed(),Eparticles::getMassG(Tracks[which][j-beg].ID));
      tracks[j]->setCharge(1);
      if(1==0)//readHits&&!reconstructHits)
	{
	  int n=(int)trackLeaves[j][4]->GetValue();
	  if(n==0)tracks[j]->setCharge(0);
	  if(n>30)n=30;
	  for(int k=0;k<n;k++)
	    {
	      pos=(int)trackHitLeaves[j]->GetValue(k);
	      if(pos>numberOfHits||pos<0)continue;
	      int id=Hits[pos/20][pos%20].det;
	      int el=Hits[pos/20][pos%20].el;
	      if(id>setup->getNumberOfDetectors())continue;
	      for(int n=0;n<(*onumberOfHits[id]);n++)
		{
		  if(oHits[id][n]->getElement()==el)//||oHits[id][n]->getRawTDC()==Hits[hitsOfTrack[which][j][k]/20][hitsOfTrack[which][j-beg][k]%20].tdcR)
		    tracks[j]->addCalibSrc(oHits[id][n]);
		}
	     }
	  if(tracks[j]->getNumberOfCalibHits()!=n)
	    anaLog<<"AReadFromSimpleTree::readTracks() Track["<<j<<"] wrong number of hits: assigned :"<<tracks[j]->getNumberOfCalibHits()<<" named: "<<n<<endli;
	}
    }
  which=0;
  beg=0;
  ...
  //  for(int j=0;j<ntr;j++){for(int i=0;i<25;i++)cout<<tracks[j]->getNumberOfCalibHits(i)<<" "<<flush;  cout<<endl;}
  //  cout<<endl;
}
void AReadFromSimpleTree::readPixel(int i)
{
  if(numPixTrees==0)
    return;
  if(i>=numPixTree->GetEntries())return;
  numPixTree->GetEntry(i);
  event->getTrigger()=tP;
  int numtrees=getNumTrees(numberOfPixels,1);
  for(int i=0;i<numtrees;i++) pixTree[i]->GetEntry(currentPixEntry[i]++);
	...
	if(event->getMaxNumber<TPixel>()<(*onumberOfPixels[Pix[j/20][j%20].ID]))
	  {
	    ...
	    ps->setCenter(point3D(Pix[j/20][j%20].center[0],Pix[j/20][j%20].center[1],Pix[j/20][j%20].center[2]));
	    tp->setShape(ps);
	    if(readHits)
	      {
		for(int kk=0;kk<Pix[j/20][j%20].numberOfElements;kk++)
		  {
		    ...
		    for(int n=0;n<(*onumberOfHits[id]);n++)
		      if(el==oHits[id][n]->getElement()&&Hits[k/20][k%20].tdcR==oHits[id][n]->getRawTDC())
...
void AReadFromSimpleTree::readCluster(int i)
{
  if(numCluTrees==0)
    return;
  if(i>=numCluTree->GetEntries())return;
  numCluTree->GetEntry(i);
  event->getTrigger()=tC;
  int numtrees=getNumTrees(numberOfCluster,2);
  for(int i=0;i<numtrees;i++) cluTree[i]->GetEntry(currentCluEntry[i]++);
  for(int j=0;j<event->getMaxNumber<TDetector>();j++)
	...
	if(event->getMaxNumber<TCluster>()<(*onumberOfClusters[Clus[j/20][j%20].ID]))
	  {
	    ...
	    tc->reset();
	    tc->setCenter(point3D(Clus[j/20][j%20].center[0],
				  Clus[j/20][j%20].center[1],
...
void AReadFromSimpleTree::readHit(int i)
{
  ...
  if(numHitTrees==0)
    return;
//   int tmpev[numtrees];
  if(i>=hitTree[0]->GetEntries())return;
  for(int j=0;j<numtrees;j++) 
    {
      if(currentHitEntry[j]<0)currentHitEntry[j]++;
...
bool AReadFromSimpleTree::setTrackTree(TFile *f)
{
  if(readTracks)
	...
	{
	  if(numTmp==1)maxTracks=5;
	  if(numTmp==2)maxTracks=10;
	}
      ...
      for(int i=0;i<maxTracks;i++)trackLeaves[i]=new TLeaf*[9];
      ...
      for(int j=0;j<numTmp;j++)//while(tmptree!=NULL)
	{
	  ...
	  for(int i=0;i<accnum;i++)
	    {
	      ...
	      tmptree->SetBranchAddress((string("Track")+string_number(beg+i+1)).data(),&tmptracks[i]);
	      trackBranches[beg+i]=tmptree->GetBranch((string("Track")+string_number(beg+i+1)).data());
	      trackHitLeaves[beg+i]=tmptree->FindLeaf((string("hits")+string_number(beg+i+1)).data());
	  ...
	  beg+=accnum;
	  if(j==1)accnum=10;
	  else if(j==2)accnum=20;
	  tmptree->GetEntry(1);
	}
      if(currentTrackEntry)delete []currentTrackEntry;
      currentTrackEntry=new int[numTmp];
      for(int i=0;i<numTmp;i++)currentTrackEntry[i]=-1;
...
bool AReadFromSimpleTree::setPixelTree(TFile *f)
{
  if(readPixels)
      ...
      for(int j=0;j<numTmp;j++)
	{
	  ...
	  for(int i=0;i<20;i++)
	    {
	      tmptree->SetBranchAddress((string("Pixel")+string_number(beg+i+1)).data(),&tmptracks[i]);
	    }
      ...
      if(currentPixEntry)delete []currentPixEntry;
      currentPixEntry=new int[numTmp];
      for(int i=0;i<numTmp;i++)currentPixEntry[i]=-1;
...
bool AReadFromSimpleTree::setClusterTree(TFile *f)
{
  if(readClusters)
      ...
      for(int j=0;j<numTmp;j++)
	{
	  ...
	  for(int i=0;i<20;i++)
	    {
	      tmptree->SetBranchAddress((string("Cluster")+string_number(beg+i+1)).data(),&tmptracks[i]);
	    }
      ...
      if(currentCluEntry)delete []currentCluEntry;
      currentCluEntry=new int[numCluTrees];
      for(int i=0;i<numCluTrees;i++)currentCluEntry[i]=-1;
...
bool AReadFromSimpleTree::setHitTree(TFile *f)
{
  if(readHits)
      ...
      while(tmptree!=NULL)
	{
	  ...
	  for(int i=0;i<20;i++)
	    {
	      ...
	      tmptree->SetBranchAddress((string("Hit")+string_number(beg+i+1)).data(),&tmptracks[i]);
	    }
	  Hits[j]=(tmptracks);
	  hitTree[j]=(tmptree);
	  beg+=20;
      ...
      if(currentHitEntry)delete []currentHitEntry;
      currentHitEntry=new int[numTmp];
      for(int i=0;i<numTmp;i++)currentHitEntry[i]=-1;
      anaLog<<(numTmp)<<" hit trees found"<<endli;
      anaLog<<"in file \""<<f->GetName()<<"\" ";
      if(numTmp>0)anaLog<<(int)(hitTree[0]->GetEntries())<<" Entries";
      anaLog<<endli;
      numHitTrees=numTmp;
       c4=-1;
     return true;
...
{
  if(numEntr==0)return 0;
  switch(which)
    {
    case 0:
      if((numEntr/20)+1>numHitTrees)return numHitTrees;
      return (numEntr/20)+1;
    case 1:
      if((numEntr/20)+1>numPixTrees)return numPixTrees;
      return (numEntr/20)+1;
    case 2:
      {
	if((numEntr/20)+1>numCluTrees)return numCluTrees;
	return (numEntr/20)+1;
      }
    case 3:
      {
	int num;
	if(numEntr<=5) num= 1;
	else if(numEntr<=10) num= 2;
	else num=(numEntr/20)+3;
	if(num>numTrackTrees)num=numTrackTrees;
	return num;
      }
    }
  return 0;
...
int AReadFromSimpleTree::searchDelayedEvent(TTree** theTree,int numTrees, int toEvent, int *theCounter, int acounter)
{
  int nev[numTrees];
  ...
  if(toEvent>0)
	...
	{
	  int nl,nh,nm;
	  int pl=0,ph=(toEvent>=theTree[i]->GetEntries()?
		       theTree[i]->GetEntries()-1:toEvent),
	    pm=(pl+ph)/2;
	  ...
	  while(nl<toEvent&&nh>toEvent&&abs(pl-ph)>1)
 	  ...
 	  if(nl>=toEvent){nev[i]=nl;theCounter[i]=pl;}
 	  else if(nh<=toEvent){nev[i]=nh;theCounter[i]=ph;}
 	  else {nev[i]=nl;theCounter[i]=pl+1;}
	  theCounter[i]--;
	}
    }
  for(int i=0;i<numTrees;i++)
  ...
  if(grap)
      ...
      for(int i=0;i<nFiles;i++)
	{
	  switch(type[i])
		...
		if(files[i]==hitf->GetName())continue;
		cleanHits();
		if(!existing(files[i]))
		  readHits=false;
		else
		  {
		    if(localDirectory)
		      {
			filenames[0]=localDir+"/"+files[i].substr(files[i].rfind("/")+1,files[i].length()-files[i].rfind("/")-1);
#if QT_VERSION < 0x040000
			QProcess copy(QString("cp "));
		...
		if(numPixTree)pixelf=numPixTree->GetCurrentFile();
		cleanPixels();
		if(!existing(files[i]))
		  readPixels=false;
		else
		  {
		    if(localDirectory)
		      {
			filenames[1]=localDir+"/"+files[i].substr(files[i].rfind("/")+1,files[i].length()-files[i].rfind("/")-1);
#if QT_VERSION < 0x040000
			QProcess copy(QString("cp "));
			copy.addArgument((files[i].data()));
			copy.addArgument(filenames[1].data());
		...
		if(numCluTree)clusterf=numCluTree->GetCurrentFile();
		cleanCluster();
		if(!existing(files[i]))
		  readClusters=false;
		else
		  {
		    if(localDirectory)
		      {
			filenames[2]=localDir+"/"+files[i].substr(files[i].rfind("/")+1,files[i].length()-files[i].rfind("/")-1);
#if QT_VERSION < 0x040000
		...
		if(numTrackTree)trackf=numTrackTree->GetCurrentFile();
		cleanTracks();
		if(trackf)trackf->Close();
		if(!existing(files[i]))
		  readTracks=false;
		else 
		  {
		    if(localDirectory)
		      {
			filenames[3]=localDir+"/"+files[i].substr(files[i].rfind("/")+1,files[i].length()-files[i].rfind("/")-1);
#if QT_VERSION < 0x040000
			QProcess copy(QString("cp "));
			copy.addArgument((files[i].data()));
			copy.addArgument(filenames[3].data());
...
void AReadFromSimpleTree::cleanTracks()
{
  ...
  if(localDirectory)
    {
      ...
      dir.remove(fn.data());
      cout<<"temporary file \""<<fn.data()<<"\" removed from directory \""<<dir.path().data()<<"\"."<<endl;
    }
  ...
  for(int i=0;i<maxTracks;i++)delete []trackLeaves[i];
  delete []trackLeaves;
}
void AReadFromSimpleTree::cleanHits()
{
  ...
  delete []Hits;
  delete []hitTree;
  if(numHitTree)hitf=numHitTree->GetCurrentFile();
  if(localDirectory)
    {
