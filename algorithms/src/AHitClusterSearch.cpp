...
#include "logger.h"
#include <stdlib.h>
AHitClusterSearch::AHitClusterSearch(TSetup& setupIn, THitCluster ***clustersIn, int **numberOfCluster, TCalibHit*** hitsIn, int** numberOfHits, const algorithm_parameter& ap):AAlgorithm("Search hit-Clusters"), setup(setupIn)
{
  if(ap.getNumberOfParam<vector<int> >()==0)
    {
      numDets=0;
      for(int i=0;i<setup.getNumberOfDetectors();i++)
	if(setup.getDetectorr(i).getNumberOfElements()>0)numDets++;
    }
  else
  ...
  if(ap.getNumberOfParam<vector<int> >()==0)
      ...
      for(unsigned int i=0;i<ap.getParam<vector<int> >(0).getData().size();i++)
	{
	  ...
	  nEl[i]=setup.getDetectorr(ap.getParam<vector<int> >(0).getData().at(i)).getNumberOfElements();
	}
    }
}
...
void *AHitClusterSearch::process(void *ptr)
{
  ...
  for(int i=0;i<numDets;i++)
	...
	{
	  used[j]=-1;
	  if(!hits[i][j]->isValid()||hits[i][j]->getHitShape()==NULL)used[j]=1000;
	}
      //find agglomerations
      for(int j=0;j<*(nHits[i]);j++)
	{
	  if(!hits[i][j]->isValid())continue;
	  if(hits[i][j]->getHitShape()==NULL)continue;
	  ...
	  for(int k=j+1;k<*(nHits[i]);k++)
		...
		{
		  if(abs(hits[i][k]->getElement()-clusters[i][nCl]->getHit(l)->getElement())<=1&&used[k]==-1)
		    {
		      clusters[i][nCl]->addHit(hits[i][k]);
		      used[k]=nCl;
		    }
		  else if(circular[i]&&abs(abs(hits[i][k]->getElement()-clusters[i][nCl]->getHit(l)->getElement())-nEl[i]+1)&&used[k]==-1)
		    {
		      clusters[i][nCl]->addHit(hits[i][k]);
		      used[k]=nCl;
		    }
      ...
      for(int j=0;j<nCl;j++)
	{
	  for(int k=j+1;k<nCl;k++)
		...
		if(!obsolete)
		  for(int n=0;n<clusters[i][k]->getNumberOfHits();n++)
		    {
		      if(abs(clusters[i][j]->getHit(m)->getElement()-clusters[i][k]->getHit(n)->getElement())<=1)
			{
			  obsolete=true;
			  for(int l=0;l<clusters[i][k]->getNumberOfHits();l++)
      ...
      for(int j=0;j<nCl;j++)
	{
	  if(clusters[i][j]->getNumberOfHits()==0)continue;
	  weightedSum=0;
	  sumOfWeights=0;
	  for(int k=0;k<clusters[i][j]->getNumberOfHits();k++)
	    {
	      weightedSum+=clusters[i][j]->getHit(k)->getElement()*clusters[i][j]->getHit(k)->getADC();
