...
AWriteHistogramsToRootFile::AWriteHistogramsToRootFile(const string& filename,
                                                       const string& directory, TEvent& eventIn,
                                                       TSetup& setup, const string& option,
                                                       bool serv, void* histo_mutexIn)
    : AAlgorithm("Write histograms to root file"), event(eventIn), server(serv)
{
...
AWriteHistogramsToRootFile::AWriteHistogramsToRootFile(TFile** rootFileIn, const string& directory,
                                                       TEvent& eventIn, TSetup& setup,
                                                       const string& option, bool serv,
                                                       void* histo_mutexIn)
    : AAlgorithm("Write histograms to root file"), event(eventIn), server(serv)
{
  histo_mutex = histo_mutexIn;
...
void AWriteHistogramsToRootFile::declareHistos(TSetup& setup, const string& option)
{
  ...
  for (int i = 0; i < numF; i++) {
    ...
    if (lis[i].substr(2, 2) == "tr") {
      f[i] = new ATrackHistoAlgorithm(tracks, *numberOfTracks, currentDirectory,
                                      lis[i].substr(5, lis[i].length() - 5), !server);
    } else if (lis[i].substr(2, 2) == "cr") {
      f[i] = new ARawHitCutHistoAlgorithm(hits, numberOfHits, setup, currentDirectory,
                                          lis[i].substr(5, lis[i].length() - 5), !server);
    } else if (lis[i].substr(2, 2) == "rh") {
      f[i] = new ARawHitHistoAlgorithm(raws, numberOfHits, setup, currentDirectory,
                                       lis[i].substr(5, lis[i].length() - 5), !server);
    } else if (lis[i].substr(2, 2) == "ch") {
      cout << "declare calibHistos " << lis[i].substr(5, lis[i].length() - 5).data() << endl;
      f[i] = new ACalibHitHistoAlgorithm(hits, numberOfHits, setup, currentDirectory,
                                         lis[i].substr(5, lis[i].length() - 5), !server);
      cout << "done" << endl;
    } else if (lis[i].substr(2, 2) == "pi") {
      f[i] = new APixelHistoAlgorithm(pixels, numberOfPixels, setup.getNumberOfDetectors(),
                                      currentDirectory, lis[i].substr(5, lis[i].length() - 5),
                                      !server);
    } else if (lis[i].substr(2, 2) == "re") {
      f[i] = new AReactionHistoAlgorithm(&event, currentDirectory,
                                         lis[i].substr(5, lis[i].length() - 5), !server);
    }
    //      else
  }
...
TFile* AWriteHistogramsToRootFile::getFile() { return *rootFile; }
...
TH1** AWriteHistogramsToRootFile::getHistos1() { return histos; }

TH2** AWriteHistogramsToRootFile::getHistos2() { return TDhistos; }
...
vector<string> AWriteHistogramsToRootFile::histogramNames()
{
  ...
  for (int i = 0; i < numberOfHistos; i++)
    if (histos[i] != NULL)
      tmp.push_back(histos[i]->GetName());
  for (int i = 0; i < numberOf2DHistos; i++)
    if (TDhistos[i] != NULL)
      tmp.push_back(TDhistos[i]->GetName());
  return tmp;
}
