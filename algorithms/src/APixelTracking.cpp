...
{
  if(n==0||numberOfTracks==0)return;
  int elnum=0;
  TTrack *ttmp;
  for(int j=0;j<numberOfTracks;j++)
      ...
      for(int i=0;i<n;i++)
	{
	  ...
	  if(elnum>max||(acos(array[i]->getPath().Direction()*tracks[j]->getPath().Direction())<2*M_PI/180.&&
...
int APixelTracking::nElementsInCommon(TTrack *t1,TTrack* t2)
{
  ...
  for(int k=0;k<setup.getNumberOfDetectors();k++)
    {
      if(t1->getNumberOfCalibHits(k)>0&&
	 t2->getNumberOfCalibHits(k)>0&&
	 k!=15&&k!=16&&k!=21)
	{
...
// #endif
  ...
  for(int i=0;i<n;i++)
      ...
      for(int j=i+1;j<n;j++)
	{
	  ...
	  if(elnum>max||(acos(array[i]->getPath().Direction()*array[j]->getPath().Direction())<2*M_PI/180.&&
...
void APixelTracking::eliminateDoubleTracks(int &n1, TTrack **array1,int &n2, TTrack **array2, int max)
{
  if(n2+n1<2)return;
// #ifdef DEBUGGING
//   cout<<"APixelTracking::eliminateDoubleTracks(n="<<n<<",max="<<max<<")"<<endl;
// #endif
  ...
  for(int i=0;i<n1;i++)
      ...
      for(int j=0;j<n2;j++)
	{
	  ...
	  if(elnum>max||acos(array1[i]->getPath().Direction()*array2[j]->getPath().Direction())<2*M_PI/180.)
	    {
	      if(array1[i]->getNumberOfCalibHits()/*ChiSquared()<*/>array2[j]->getNumberOfCalibHits()/*ChiSquared()*/)
		{
		  ttmp=array2[j];
...
void APixelTracking::sortAndCopyPrompt(int &n, TTrack **array)
{
  ...
  for(int i=0;i<n;i++)
    {
      cout<<"\t"<<array[i]->getPath().toString(_spheric_).data()<<array[i]->getChiSquared()<<" "<<array[i]->getNumberOfCalibHits()<<endl<<"\t\t(";
      for(int j=0;j<setup.getNumberOfDetectors();j++)
	if(array[i]->getNumberOfCalibHits(j)>0)cout<<j<<", ";
      cout<<")"<<endl;
    }
#endif
  for(int i=0;i<n;i++)
...
void APixelTracking::sortAndCopyVee(int &n, TTrack **array)
{
  ...
  for(int i=0;i<n;i++)
    {
      dir1=array[i*2]->getPath().Direction();dir1.normalize();
      dir2=array[i*2+1]->getPath().Direction();dir2.normalize();
...
{
  //  cout<<"APixelTracking::searchElement(line="<<line.toString(_spheric_).data()<<", ID="<<ID<<")"<<endl;
  if(ID<0||ID>=nndetpos)return NULL;
  if(checkdets[ID]>=setup.getNumberOfDetectors()||checkdets[ID]<0)return NULL;
  int nEl=setup.getDetectorr(checkdets[ID]).getNumberOfElements();
  if(nEl==0)return NULL;
  //  if((*(numberOfHits[ID]))==0)return NULL;
  if(ndetpos[ID]==0)return NULL;
  int suspect=setup.getDetectorr(checkdets[ID]).getShape(0)->suspect(line,setup.getDetectorr(checkdets[ID]).getStackType());
  if(suspect<0||suspect>=nEl)return NULL;
  int best=0;
  int diff,bestDiff=nEl;
  //  cout<<"suspect "<<suspect<<endl;
  for(int i=0;i<ndetpos[ID];i++)
    {
      if(!hits[checkdets[ID]][detpos[ID][i]]->isValid())continue;
      if(hits[checkdets[ID]][detpos[ID][i]]->getHitShape()==NULL)continue;
...
#ifdef DEBUG_SEARCH
  cout<<"APixelTracking::searchElement(line="<<line.toString(_spheric_).data()<<", ID="<<checkdets[ID]<<") done \n\t(suspect="<<suspect<<", diff="<<bestDiff<<", max="<<max_value<<" ->";
  if(bestDiff<=max_value)cout<<hits[checkdets[ID]][detpos[ID][best]]->getElement()<<")"<<endl;
  else cout<<")"<<endl;
#endif
...
{
#ifdef DEBUG_FIT
  cout<<"APixelTracking::trackFit() "<<track->getNumberOfCalibHits()<<" "<<min_points<<endl;
#endif
  if(track->getNumberOfCalibHits()<min_points)
	...
	if(track->getNumberOfCalibHits(i)>0)cout<<i<<", ";
      cout<<")"<<endl;
#endif
      return -1;
    }
...
#ifdef DEBUG_FIT
	  cout<<"("<<i<<": "<<track->getCalibSrcr(i,j)->getElement()<<")"<<dist.toString(_spheric_).data()<<max_dist<<endl;
#endif
// 	  if(i<=2)
// 	    cout<<"("<<i<<": "<<track->getCalibSrcr(i,j)->getElement()<<")"<<dist.toString(_spheric_).data()<<endl;
 	  sigma=1;//(i<6?10:1);
...
#endif
  if(myFit.Points()<min_points)return -1;
  myFit.fit();
#ifdef DEBUG_FIT
...
#endif
  if(track1->getNumberOfCalibHits()+track2->getNumberOfCalibHits()
     <minPointsVee)return -1;
  vertexFit myFit(3,track1->getNumberOfCalibHits()+
		  track2->getNumberOfCalibHits()+2);
  myFit.clear();
  float sigma;
...
// 		}
  ...
  if(myFit.Points(0)+myFit.Points(1)<minPointsVee)return -1;
  myFit.fit();
...
bool APixelTracking::searchPromptTrack(TTrack *track, int ID)
{
  ...
  for(int i=0;i<nTracking;i++)
    {
      if(track->getNumberOfCalibHits(tracking[i])>0)continue;
...
bool APixelTracking::searchDecayTrack(TTrack *track,int ID)
{
  ...
  if(nst>maxDecayStart)return false;
  for(int i=0;i<nTracking;i++)
    {
      if(track->getNumberOfCalibHits(tracking[i])>0)continue;
      TCalibHit* tmpHit=searchElement(line,tracking[i],maxDiff[ID]);
...
int contains(THitCluster* cl, TCalibHit* hit)
{
  ...
  int n=0;
  for(int i=0;i<cl->getNumberOfHits();i++)
    if(cl->getHit(i)==hit||(cl->getHit(i)->getElement()==hit->getElement()&&cl->getHit(i)->getDetector()==hit->getDetector()))
      n++;
  return n;
}
int contains(TCalibHit* hit1, TCalibHit* hit2)
{
  if(hit1==NULL||hit2==NULL)return 0;
  if(hit1->name()=="THitCluster"||hit2->name()=="THitCluster")
    ...
    {
      if(hit1==hit2||(hit1->getElement()==hit2->getElement()&&hit1->getDetector()==hit2->getDetector()))
	return 1;
      return 0;
    }
}
...
{
  if(tr==NULL||hit==NULL)return 0;
  if(hit->name()=="THitCluster")return contains(tr,(THitCluster*)hit);
  int n=0;
  for(int j=0;j<tr->getNumberOfCalibHits(hit->getDetector());j++)
    {
...
{
  if(pi==NULL||hit==NULL)return 0;
  if(hit->name()=="THitCluster")return contains(pi,(THitCluster*)hit);
  int n=0;
  for(int j=0;j<pi->getNumberOfActualElements();j++)
...
bool APixelTracking::checkWithPrompt(TCalibHit* hit)
{
  for(int i=0;i<numberOfTracks;i++)
	...
	continue;
      if(contains(tracks[i],hit)>0)return false;
    }
  return true;
}
...
void *APixelTracking::process(void* ptr)
{
  ...
  for(int i=0;i<nnpixpos-2;i++)
    {
      if(npixpos[i]==0)continue;
      for(int j=i+1;j<nnpixpos;j++)
	{
	  if(i==j)continue;
	  if(npixpos[j]==0)continue;
	  for(int ii=0;ii<npixpos[i];ii++)
	    {
	      if(tmpNum>=maxTracks)continue;
	      if(pixels[checkpix[i]][pixpos[i][ii]]->getShape()==NULL)continue;
	      p1=pixels[checkpix[i]][pixpos[i][ii]]->getShape()->getCenter();
	      for(int jj=0;jj<npixpos[j];jj++)
		{
		  if(tmpNum>=maxTracks)continue;
		  if(pixels[checkpix[j]][pixpos[j][jj]]->getShape()==NULL)continue;
		  ...
		  for(int k=0;k<nnpixpos;k++)
			...
			{
			  if(pixels[checkpix[k]][pixpos[k][kk]]->getShape()==NULL)continue;
			  distance=pixels[checkpix[k]][pixpos[k][kk]]->getShape()->distance(line);
			  if(distance.R()>0)
			    {
			      if(pixels[checkpix[k]][pixpos[k][kk]]->getShape()->circumscribedRadius()>distance.R())
				distance.setValues(0,0,0);
			      else
				distance=distance*(distance.R()-pixels[checkpix[k]][pixpos[k][kk]]->getShape()->circumscribedRadius())/distance.R();
			    }
		  ...
		  for(int k=0;k<nOnPixel;k++)
		    {
		      if(onPixel[k]->getID()==0){trackID=0;break;}
		      if(onPixel[k]->getID()==1){trackID=1;break;}
		    }
		  if(nOnPixel==2&&trackID==1)continue;
		  if(nOnPixel==2&&trackID==0)
		    if(line.Direction().Theta()>0.15)continue;
		  if(nOnPixel>2)
		    {
		      for(int k=0;k<nOnPixel;k++)
...
// 		    }
		  ...
		  for(int k=0;k<nndetpos;k++)
			...
			if(tmpHit!=NULL)tmptracks[tmpNum]->addCalibSrc(tmpHit);
		      }
		  ...
		  if(1==0&&distance.R()<=maxTargetDist&&
			...
			continue;
		      nPoints=0;
		      for(int k=0;k<setup.getNumberOfDetectors();k++)
			if(tmptracks[tmpNum]->getNumberOfCalibHits(k)>0)nPoints++;
...
// 		      cout<<")"<<endl;
		      ...
		      if((trackID<2&&nPoints<8)||(trackID>=2&&nPoints<7))
			{
		      ...
		      if(tmptracks[tmpNum]->getNumberOfCalibHits(21)>0&&tmptracks[tmpNum]->getNumberOfCalibHits(22)>0)
			{
  ...
  for(int i=0;i<ndecaytracks;i++)
    {
      values[i]=0;
      for(int j=0;j<setup.getNumberOfDetectors();j++)
	if(decayTracks[i]->getNumberOfCalibHits(j)>0)values[i]++;
      values[i]=1./values[i];
    }
...
//   eliminateDoubleTracks(ndecaytracks,decayTracks,maxPromptEqual);
//   for(int i=0;i<ndecaytracks;i++)
//     cout<<decayTracks[i]->getPath().toString(_spheric_).data()<<decayTracks[i]->getChiSquared()<<" "<<decayTracks[i]->getNumberOfCalibHits()<<endl;
  //search vees
  for(int i=0;i<ndecaytracks;i++)
    {
      ...
      for(int j=i+1;j<ndecaytracks;j++)
	{
	  ...
	  cout<<i<<" "<<j<<" "<<poca<<endl;
#endif
	  if(!(poca.length()<maxDistVee)) continue;
	  nincommon=nElementsInCommon(decayTracks[i],decayTracks[j]);
	  //	  cout<<nincommon<<endl;
	  if(nincommon>maxVeeCommon)continue;
	  if(poca.Mid().Z()>100)continue;
...
#include "cylinder.h"
APixelTracking::APixelTracking(TSetup &setupIn, TTrack **tracksIn, TPixel ***pixelsIn, TCluster ***clusterIn, TCalibHit ***hitsIn, int &nTracksIn, int **nPixelsIn, int **nClusterIn, int **nHitsIn, int maxTracksIn, const algorithm_parameter& param): AAlgorithm("pixel track search"), numberOfTracks(nTracksIn), setup(setupIn),maxTracks(maxTracksIn)
{
  tracks=tracksIn;
  pixels=pixelsIn;
  clusters=clusterIn;
  ...
  for(int i=0;i<8;i++)checkdets[i]=i;
  for(int i=8;i<nndetpos;i++)checkdets[i]=i+9;
  for(int i=0;i<nndetpos;i++)
  ...
  checkpix[0]=3;checkpix[1]=4;checkpix[2]=5;checkpix[3]=0;checkpix[4]=1;
  for(int i=0;i<nnpixpos;i++)
  ...
  cout<<"max diff suspect: "<<maxDiff[0]<<" "<<maxDiff[1]<<" "<<maxDiff[2]<<endl;
  cout<<"min Points: "<<minPoints[0]<<" "<<minPoints[1]<<" "<<minPoints[2]<<endl;
  cout<<"max distance: "<<maxDistance[0]<<" "<<maxDistance[1]<<" "<<maxDistance[2]<<endl;
  cout<<"max chi P: "<<maxChiPrompt[0]<<" "<<maxChiPrompt[1]<<" "<<maxChiPrompt[2]<<endl;
  cout<<"max chi D: "<<maxChiDecay[0]<<" "<<maxChiDecay[1]<<" "<<maxChiDecay[2]<<endl;
  */
}
