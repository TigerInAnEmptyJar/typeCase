...
void setTrackErrors(TTrack *tr)
{
  ...
  for(int i=0;i<tr->getNumberOfCalibHits(-2);i++)
    for(int j=0;j<tr->getNumberOfCalibHits(i);j++)
      {
	if(tr->getCalibSrcr(i,j)==NULL)continue;
	if(tr->getCalibSrcr(i,j)->getHitShape()==NULL)continue;
	if(tr->getCalibSrcr(i,j)->getHitShape()->getName()=="wedge")
	  {
	    tmp=abs(((wedge*)tr->getCalibSrcr(i,j)->getHitShape())->getPhiRange()/2.);
	    if(minPhi>tmp&&((wedge*)tr->getCalibSrcr(i,j)->getHitShape())->getDistToBarrelCenter()<0)
	      minPhi=tmp;
	  }
...
bool ALineTrackSearch::searchATrack(int tracknum, int stopI, TBase *pix, TBase *spix)
{
  ...
  if(spix==NULL)p2=vertex;
  else
    {
      if(spix->name()=="TCluster")p2=((TCluster*)spix)->getCenter();
      else p2=((TPixel*)spix)->getShape()->getCenter();
    }
  ...
  for(int i=0;i<numMid;i++)
	...
	{
	  if(!pixels[midIDs[i]][j]->isValid())continue;
  ...
  for(int i=0;i<tak;i++)
    {
      for(int j=0;j<numDetectors;j++)if(tak2[i]==detectorIDs[j])pos=j;
#ifdef DEBUGSEARCH
...
#endif
      if(pos>=0)con=conventional[pos];else con=0;
      found=false;
      ...
      if(!found)
	{
	  ...
	  if(con==1&&pos>=0)//conventional search
		...
		{
		  if(!hits[tak2[i]][j]->isValid())continue;
		  ...
		  if(angularDistance)
		    {
		      ...
		      if(adist<__prec){bestEl=j;bestHit=hitParam;break;}
		      if(adist>maxDistance[offset+stopI])
			continue;
		      if(bestD>adist){bestEl=j;bestD=adist;bestHit=hitParam;}
		    }
		  else
		    {
	  ...
	  else //suspect search
	    {
	      ...
	      closest=0;
#ifdef DEBUGSEARCH
	      cout<<"Suspect element:"<<suspect<<" detector: "<<tak2[i]<<" away: "<<away[(offset+stopI)*numDetectors+pos]<<" "<<shape->getName().ascii()<<endl;
#endif
#ifdef MICROPIXELOUT
	      ...
	      if(suspect>=0&&suspect<det->getNumberOfElements())
		{
		  for(int j=0;j<(*(numberOfHits[tak2[i]]));j++)
		    {
		      if(!hits[tak2[i]][j]->isValid())continue;
		      if(hits[tak2[i]][j]->getHitShape()==NULL)continue;
			...
			{
			  if(abs(hits[tak2[i]][j]->getElement()-suspect)<=away[(offset+stopI)*numDetectors+pos])
			    tmpTracks[tracknum]->addCalibSrc(hits[tak2[i]][j]);
			  else if(det->isCircular())
			    {
			      if(abs(abs(hits[tak2[i]][j]->getElement()-suspect)-det->getNumberOfElements())<=away[(offset+stopI)*numDetectors+pos])
				{
#ifdef DEBUGSEARCH
				  cout<<"circular detector: element taken "<<hits[tak2[i]][j]->getElement()<<endl;
#endif
				  tmpTracks[tracknum]->addCalibSrc(hits[tak2[i]][j]);
				}
			    }
      ...
      if(tmpTracks[tracknum]->getNumberOfCalibHits(tak2[i])<=0)
	{
	  ...
	  if(tmpTracks[tracknum]->getPixel(0,0).getID()==pixid)
	    {
	      cout<<"micro-Pixel: det not found: "<<path.Direction().toString(_spheric_).ascii()<<tak2[i]<<endl;
  ...
  for(int i=0;i<numDetectors;i++)
    {
      if(tmpTracks[tracknum]->getNumberOfCalibHits(detectorIDs[i])>0)continue;
      found=false;
      /*search pixel*/
      for(int k=0;k<tmpTracks[tracknum]->getNumberOfPixels();k++)
	{
      ...
      if(!found)
	{
	  ...
	  if(conventional[i]==1)
		...
		{
		  if(!hits[detectorIDs[i]][j]->isValid())continue;
	  ...
	  if(tmpTracks[tracknum]->getPixel(0,0).getID()==pixid)
	      ...
	      if(suspect>=0&&suspect<det->getNumberOfElements())
		{
		  for(int j=0;j<(*(numberOfHits[detectorIDs[i]]));j++)
		    {
		      if(!hits[detectorIDs[i]][j]->isValid())continue;
		      if(hits[detectorIDs[i]][j]->getHitShape()==NULL)continue;
		      if(abs(hits[detectorIDs[i]][closest]->getElement()-suspect)>abs(hits[detectorIDs[i]][j]->getElement()-suspect))
			closest=j;
		      ...
		      else 
			{
			  if(det->isCircular())
			    {
			      if(abs(hits[detectorIDs[i]][j]->getElement()-suspect)<=away[(offset+stopI)*numDetectors+i])
				tmpTracks[tracknum]->addCalibSrc(hits[detectorIDs[i]][j]);
			      else if(abs(abs(hits[detectorIDs[i]][j]->getElement()-suspect)-det->getNumberOfElements())
  ...
  for(int i=0;i<tmpTracks[tracknum]->getNumberOfCalibHits(-2);i++)
    if(tmpTracks[tracknum]->getNumberOfCalibHits(i)>0)nhits++;
  //  bool condition=tmpTracks[tracknum]->getNumberOfCalibHits(-1)>=(typ==0?minElements[stopI]:minClusterElements[stopI]);
  bool condition=nhits>=(typ==0?minElements[stopI]:minClusterElements[stopI]);
  if(!condition)
    {
#ifdef DEBUGINFO
      cout<<out.ascii()<<" too less elements: "<<nhits/*tmpTracks[tracknum]->getNumberOfCalibHits(-1)*/<<"("<<(typ==0?minElements[stopI]:minClusterElements[stopI])<<")"<<endl;
#endif
#ifdef MICROPIXELOUT
      if(tmpTracks[tracknum]->getPixel(0,0).getID()==pixid)
	{
	  cout<<"Micropixel too less elements: "<<path.Direction().toString(_spheric_).ascii()<<tmpTracks[tracknum]->getNumberOfCalibHits(-1)<<"("<<(typ==0?minElements[stopI]:minClusterElements[stopI])<<") (";
	  for(int i=0;i<tmpTracks[tracknum]->getNumberOfCalibHits(-2);i++)
  ...
  if(tmpTracks[tracknum]->getNumberOfCalibHits(-1)>4)
	...
	{
#ifdef DEBUGINFO
	  cout<<out.ascii()<<" too less elements: "<<tmpTracks[tracknum]->getNumberOfCalibHits(-1)<<"("<<(typ==0?minElements[stopI]:minClusterElements[stopI])<<")"<<endl;
#endif
#ifdef MICROPIXELOUT
	  if(tmpTracks[tracknum]->getPixel(0,0).getID()==pixid)
	    {
	      cout<<"Micropixel too less elements(ac): "<<path.Direction().toString(_spheric_).ascii()<<tmpTracks[tracknum]->getNumberOfCalibHits(-1)<<"("<<(typ==0?minElements[stopI]:minClusterElements[stopI])<<")(";
	  for(int i=0;i<tmpTracks[tracknum]->getNumberOfCalibHits(-2);i++)
	    if(tmpTracks[tracknum]->getNumberOfCalibHits(i)>0)
	      cout<<i<<","<<flush;
	  cout<<")"<<endl;
	...
	{
#ifdef DEBUGINFO
	  cout<<out.ascii()<<" chi² too high: "<<chi<<"("<<(typ==0?maxChi[stopI]:maxClusterChi[stopI])<<")"<<endl;
#endif
#ifdef MICROPIXELOUT
	  if(tmpTracks[tracknum]->getPixel(0,0).getID()==pixid)
	    {
	      cout<<"Micropixel chi² too high: "<<path.Direction().toString(_spheric_).ascii()<<chi<<"("<<(typ==0?maxChi[stopI]:maxClusterChi[stopI])<<")"<<endl;
	    }
#endif
	  return false;
	}
  ...
  if(tmpTracks[tracknum]->getPixel(0,0).getID()==pixid)
    {
      cout<<"micropixel:"<<chi<<path.Direction().toString(_spheric_).ascii()<<tmpTracks[tracknum]->getNumberOfCalibHits(-1)<<" good"<<endl;
    }
#endif
  return true;
}
...
void *ALineTrackSearch::process(void*ptr)
{
  ...
  for(int i=0;i<numStop;i++)
	...
	{
	  if(numTracks>=maxTracks-1)break;
  ...
  for(int i=0;i<numStopCluster;i++)
     ...
     for(int j=0;j<(*(numberOfClusters[stopClusterIDs[i]]));j++)
	{
	  ...
	  if(useVertexAsStart)
	    {
	      if(dir.Theta()<restrictionT[numStop*2+i*2]||dir.Theta()>restrictionT[numStop*2+i*2+1]||
		 dir.Phi()<restrictionP[numStop*2+i*2]||dir.Phi()>restrictionP[numStop*2+i*2+1])
		continue;
	      tmpTracks[numTracks]->reset();
	  ...
	  for(int k=0;k<tmpTracks[i]->getNumberOfCalibHits(-2);k++)
	    {
		 ...
		 tmpTracks[j]->getNumberOfCalibHits(k)>0&&
		 k!=15&&k!=16&&k!=21)
		{
		  n=0;
  ...
  for(int i=0;i<numTracks;i++)//sort by theta
    {
      num=i;
      for(int j=i+1;j<numTracks;j++)
	if(tmpTracks[num]->getPath().Direction().Theta()>tmpTracks[j]->getPath().Direction().Theta())
	  num=j;
      if(num==i)continue;
      ttmp=tmpTracks[num];
      for(int j=num-1;j>=i;j--)
...
algorithm_parameter ALineTrackSearch::getDescription()
{
  ...
  ret.addParam<vector<int> >(single_parameter<vector<int> >("essential detector IDs for Cluster",tmp));  
  ret.addParam<vector<float> >(single_parameter<vector<float> >("max chi squared for pixel track",tmp2));
  ret.addParam<vector<float> >(single_parameter<vector<float> >("theta restriction for pixel/cluster",tmp2));
  ret.addParam<vector<float> >(single_parameter<vector<float> >("phi restriction for pixel/cluster",tmp2));
  ret.addParam<vector<float> >(single_parameter<vector<float> >("max distance",tmp2));
  ret.addParam<vector<float> >(single_parameter<vector<float> >("max chi squared for cluster track",tmp2));
  return ret;
}
ALineTrackSearch::ALineTrackSearch(TSetup &setupIn, TTrack **tracksIn, TCluster ***clustersIn, TPixel ***pixelsIn, TCalibHit ***hitsIn, int maxTracksIn, int &numberOfTracksIn, int **numberOfClustersIn, int **numberOfPixelsIn, int **numberOfHitsIn, const algorithm_parameter &descr):AAlgorithm("Line Track search"),setup(setupIn), maxTracks(maxTracksIn),numberOfTracks(numberOfTracksIn),myLineFit(100)
{
  hits=hitsIn;
  pixels=pixelsIn;
  clusters=clustersIn;
  ...
  for(int i=0;i<numStopCluster;i++)
    {
      ...
      restrictionT[numStop*2+i*2]=descr.getParam<vector<float> >(1).getData().at(numStop*2+i*2);
      restrictionT[numStop*2+i*2+1]=descr.getParam<vector<float> >(1).getData().at(numStop*2+i*2+1);
      restrictionP[numStop*2+i*2]=descr.getParam<vector<float> >(2).getData().at(numStop*2+i*2);
      restrictionP[numStop*2+i*2+1]=descr.getParam<vector<float> >(2).getData().at(numStop*2+i*2+1);
  ...
  for(int i=0;i<numStop;i++)
    {
      ...
      cout<<"\t"<<"essential detectors:"<<endl<<"\t";
      for(int j=0;j<numEssentials[i];j++)
	cout<<essentials[i][j]<<", "<<flush;
      cout<<endl;
      cout<<"\t"<<"distance to suspect:"<<endl<<"\t";
  ...
  for(int i=0;i<numStopCluster;i++)
    {
      ...
      cout<<"\t"<<restrictionT[2*numStop+2*i]<<"< theta <"<<restrictionT[2*numStop+2*i+1]<<endl;
      cout<<"\t"<<restrictionP[2*numStop+2*i]<<"< phi   <"<<restrictionP[2*numStop+2*i+1]<<endl;
      cout<<"\t"<<"essential detectors:"<<endl<<"\t";
