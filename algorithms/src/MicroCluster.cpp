...
AMicroCluster::AMicroCluster(int IDIn, int& numberOfPixelsIn, TPixel **pixelIn, int maxClusterIn, int& numberOfClustersIn, TCluster **clusterIn, int patternIn):AAlgorithm("Cluster search in Micro strip detector"),ID(IDIn),maxCluster(maxClusterIn),numberOfClusters(numberOfClustersIn),numberOfPixels(numberOfPixelsIn)
{
  pixel=pixelIn;
  cluster=clusterIn;
  pattern=patternIn;
  for(int i=0;i<maxCluster;i++)cluster[i]->setID(ID);
}
  ...
  while(stillPixelLeft)
      ...
      else
	{
	  ...
	  pixelLeft-=searchRing(cluster[numberOfClusters], normer, mean, used, energy, 
				(pattern&1?1:(pattern&2?2:0)),
				maxPixel);
	  if(pattern&4)
	    {
	      for(int i=1;i<cluster[numberOfClusters]->getNumberOfPixels();i++)
		{
		  if(pixelLeft<=0)break;
		  n1=-1;
		  for(int j=0;j<numberOfPixels;j++)
		    if(&cluster[numberOfClusters]->getPixelr(i)==pixel[j])
		      n1=j;
		  if(n1>=0)
		    pixelLeft-=searchRing(cluster[numberOfClusters], normer, mean, used, 
					  energy,(pattern&1?1:(pattern&2?2:0)),
					  n1);
		}
	    }
	  mean=mean*(1/normer);
	  //pixelLog<<" "<<pixelLeft<<" left; center point of cluster: "<<mean<<endli;
	  cluster[numberOfClusters]->setCenter(point3D(0,0,0)+mean);
	  //	  cout<<ID<<" "<<cluster[numberOfClusters]->getPixelr(0).getID()<<cluster[numberOfClusters]->getCenter().Z()<<" "<<cluster[numberOfClusters]->getPixelr(0).getShape()->getCenter().Z()<<" "<<cluster[numberOfClusters]->getPixelr(0).getElement(0).getHitShape()->getCenter().Z()<<endl;
...
/*searchRing searches in all pixels the ones which are within an element
 ...
 to the cluster.*/
int AMicroCluster::searchRing(TCluster* clust,float &normer_f, vector3D &mean_f, bool *used_f, float *energy_f, int size, int centerP)
{
  bool take;
  int n1,n2;
  float d=0;
  ...
  for(int i=0;i<numberOfPixels;i++)
	...
	for(int j=0;j<pixel[i]->getNumberOfActualElements();j++)
	  for(int k=0;k<pixel[centerP]->getNumberOfActualElements();k++)  
	    if(pixel[i]->getElementr(j).getDetector()==pixel[centerP]->getElementr(k).getDetector())
	      {
