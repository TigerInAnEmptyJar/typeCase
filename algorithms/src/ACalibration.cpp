...
#include "logger.h"
#include <vector>
ACalibration::ACalibration(TCalibHit** hitsIn, int& numberOfHitsIn, const TDetector& det,
                           const vector<string>& filenames, int& runNrIn, int& evtNrIn, bool cA,
                           bool cT)
    : AAlgorithm("Calibration"), numberOfHits(numberOfHitsIn), DetID(det.getID()), correctADC(cA),
      correctTDC(cT), runNumber(runNrIn), eventNumber(evtNrIn)
{
  ...
  for (int i = 0; i < numberOfHits; i++) {
    ...
    if (numberOfRanges[3] > 0) {
      if ((((rawADC >= parameter[3][actualRange[3]]->getParameter(numElem, 0)) &&
            (rawADC <= parameter[3][actualRange[3]]->getParameter(numElem, 1))) ||
           (!correctADC)))
      //&&
    ...
    if ((numberOfRanges[2] > 0) && (numberOfRanges[0] > 0) &&
        (rawTDC != -1)) { //"[0]/(x-[1])+[2]*log(|x-[3]|)+[4]" x=rADC-adcOffs
...
bool ACalibration::checkRange()
{
  ...
  for (int i = 0; i < 6; i++) {
    if (actualRange[i] < 0 || numberOfRanges[i] == 0)
      continue;
    if ((!correctADC) && (i == 0))
      continue;
    if ((!correctTDC) && (i == 1 || i == 2 || i == 5))
      continue;
...
void ACalibration::getFromFiles(const vector<string>& calibrationFiles)
{
  ...
  for (unsigned int i = 0; i < calibrationFiles.size(); i++) {
    CommonCalibrationParser::getFileContent(calibrationFiles[i],
                                            tmp); //<<" "<<calibrationFiles[i].data()<<endl;
  }
  while (!tmp.empty()) {
    tmpParser = tmp.back();
    tmp.pop_back();
    if (tmpParser->getDetector() == DetID && tmpParser->getParamID() < 6 &&
        tmpParser->getParamID() >= 0) {
  ...
  for (int i = 0; i < 6; i++) {
    if (!(correctADC && correctTDC)) {
      if (!correctADC && (i == 0 || i == 4))
        continue;
      if (!correctTDC && (i == 2 || i == 5 || i == 1))
        continue;
    }
    anaLog << "detID-" << DetID << " PID-" << i << " #-ranges:" << numberOfRanges[i];
    for (int k = 0; k < numberOfRanges[i]; k++)
      anaLog << " from run " << parameter[i][k]->getRange(0) << " to "
             << parameter[i][k]->getRange(1) << endli;
    if (numberOfRanges[i] == 0)
      anaLog << endli;
  }
  anaLog << endli;
}
