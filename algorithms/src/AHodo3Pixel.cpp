...
AHodo3Pixel::AHodo3Pixel(TPixel** pixelIn, int& numberOfPixelsIn, int maxOfPixelsIn,
                         int** numberOfHitsIn, int** numberOfHitClustersIn, TCalibHit*** hitsIn,
                         THitCluster*** hitClusters, TSetup& setup, const algorithm_parameter& ap)
    : AAlgorithm("Pixel calculation for 3-layered hodoscope"), numberOfPixels(numberOfPixelsIn),
      maxOfPixels(maxOfPixelsIn)
{
  ...
  fiber* shxn =
      (fiber*)setup.getDetectorr(idx).getShape(setup.getDetectorr(idx).getNumberOfElements() - 1);
  fiber* shy0 = (fiber*)setup.getDetectorr(idy).getShape(0);
  fiber* shyn =
      (fiber*)setup.getDetectorr(idy).getShape(setup.getDetectorr(idy).getNumberOfElements() - 1);
...
AHodo3Pixel::AHodo3Pixel(TPixel** pixelIn, int& numberOfPixelsIn, int maxOfPixelsIn,
                         int& numberOfHitsXIn, int& numberOfHitsYIn, int& numberOfHitsDIn,
                         TCalibHit** hitsXIn, TCalibHit** hitsYIn, TCalibHit** hitsDIn,
                         TDetector& dx, TDetector& dy, TDetector& dz)
    : AAlgorithm("Pixel calculation for 3-layered hodoscope"), numberOfPixels(numberOfPixelsIn),
      maxOfPixels(maxOfPixelsIn), numberOfHitsX(&numberOfHitsXIn), numberOfHitsY(&numberOfHitsYIn),
      numberOfHitsD(&numberOfHitsDIn)
{
  hitsX = hitsXIn;
  hitsY = hitsYIn;
  hitsD = hitsDIn;
...
void AHodo3Pixel::process()
{
  ...
  for (int i = 0; i < *numberOfHitsD; i++) {
    ...
    if (dsh != NULL)
      dline[i] = sLine3D(dsh->getCorner() + dsh->getDirection(1) * 0.5 + dsh->getDirection(2) * 0.5,
                         dsh->getDirection(0));
  }
  for (int i = 0; i < *numberOfHitsX; i++) {
    dsh = (fiber*)((hitsX[i])->getHitShape());
    if (dsh != NULL)
      xline[i] = sLine3D(dsh->getCorner() + dsh->getDirection(1) * 0.5 + dsh->getDirection(2) * 0.5,
                         dsh->getDirection(0));
  }
  for (int i = 0; i < *numberOfHitsY; i++) {
    dsh = (fiber*)((hitsY[i])->getHitShape());
    if (dsh != NULL)
      yline[i] = sLine3D(dsh->getCorner() + dsh->getDirection(1) * 0.5 + dsh->getDirection(2) * 0.5,
                         dsh->getDirection(0));
  }
  float dx, dy;
  point3D hit, h2, h3;
