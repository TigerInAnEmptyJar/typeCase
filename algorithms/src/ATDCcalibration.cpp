...
// void fitTheHisto(TH2* h2d, TH1* h1d,TF1 *fitfunc,CommonCalibrationParser
// *output,int element,bool
// fixAtMax,int ptt,string opt);
void nextPad(int nPads, TPostScript* ps, TCanvas* canv, int& nc, TObject** pointers);
ATDCcalibration::ATDCcalibration(int& evtnr, int& rnr, TTrack** trIn, TPixel*** pixIn,
                                 TCalibHit*** hitIn, int& numTrack, int** numPix, int** numHits,
                                 TSetup& setin, const algorithm_parameter& descr)
    : AAlgorithm("Calibration algorithm for beam, geometry and TDC"), eventNumber(evtnr),
      runNumber(rnr), numberOfTracks(numTrack), setup(setin)
{
  gStyle->SetPalette(1);
  ...
  if (doGeometry && (doBeam || doWalk || doOffset)) {
    cout << "Error in ATDCcalibration: walk, offset or beam calibration "
            "selected with geometry "
            "calibration!"
         << endl;
  ...
  for (int i = 0; i < nCalibDets; i++) {
    if (pixelBased[i] >= 0)
      continue;
    if (measuresAgainst[i] < 0 || measuresAgainst[i] >= nCalibDets)
      continue;
  ...
  if (doGeometry) {
    ...
    for (int i = 0; i < nCalibDets; i++) {
      geoHistos[i] = new TH2F(
          (string("distance of element to track: det") + string_number(detectors[i])).data(),
          (string("distance of element to track: det") + string_number(detectors[i])).data(), 100,
          -5, 5, numElements[i], 0, numElements[i] - 1);
  ...
  if (doBeam) {
    ...
    for (int i = 0; i < 40; i++) {
      ...
      for (int j = 0; j < 40; j++) {
        vector3D tmp((i - 19.5) * 0.001, (j - 19.5) * 0.001, 1);
        tmp.normalize();
        beamHistos[i][j] =
            new TH1F((string("beam ") + string_number(i) + " " + string_number(j)).data(),
                     (string("beam at \\theta=") + string_number(tmp.Theta() * 180. / M_PI) +
                      "deg, \\phi=" + string_number(tmp.Phi() * 180. / M_PI) + "deg")
                         .data(),
                     100, -1, 1);
  ...
  if (doTwoSided) {
    ...
    for (int i = 0; i < nTwoSided; i++) {
      ...
      for (int j = 0; j < numElements[twoDets[i]]; j++) {
        twoSidedHisto[i][j] =
            new TH2F((string("pos vs \\DeltaTDC:") + string_number(detectors[twoDets[i]]) + "," +
                      string_number(j))
                         .data(),
                     (string("pos vs \\DeltaTDC:det ") + string_number(detectors[twoDets[i]]) +
                      ", el" + string_number(j))
                         .data(),
                     100, -10 + 6, 10 + 6, 100, thmin / 10. + 70, thmax / 10. + 10);
  ...
  if (doWalk || doOffset) {
    ...
    for (int i = 0; i < nCalibDets; i++) {
      ...
      float mm = 0;
      resolutionHisto[i * 2] = new TH2F(
          (string("resolution vs el, det ") + string_number(detectors[i])).data(),
          (string("tdc resolution: det ") + string_number(detectors[i]) + " versus close det")
              .data(),
          numElements[i], 0, numElements[i], 100, mm + (detectors[i] > 10 ? -2 : -3),
          mm + (detectors[i] > 10 ? 2 : 3));
      resolutionHisto[i * 2]->SetDirectory(NULL);

      resolutionHisto[i * 2 + 1] = new TH2F(
          (string("resolution vs light run path, det ") + string_number(detectors[i])).data(),
          (string("tdc resolution: det ") + string_number(detectors[i]) + " light run path").data(),
          (i < 2 ? 100 : i < 5 ? 25 : 30), thmin * 0.9, thmax * 1.02, 100,
          mm + (detectors[i] > 10 ? -2 : -3), mm + (detectors[i] > 10 ? 2 : 3));
      resolutionHisto[i * 2 + 1]->SetDirectory(NULL);
      resolutionHisto1[i] = new TH1F(
          (string("resolution det ") + string_number(detectors[i])).data(),
          (string("tdc resolution: det ") + string_number(detectors[i]) + " versus close det")
              .data(),
          100, mm + (detectors[i] > 10 ? -2 : -3),
          mm + (detectors[i] > 10 ? 2 : 3)); //,100,thmin*0.9,thmax*1.1);

      resolutionHisto1[i]->SetDirectory(NULL);
      offsetHistos[i] = new TH1*[numElements[i]];
      for (int j = 0; j < numElements[i]; j++) {
        offsetHistos[i][j] = new TH1F(
            (string("det ") + string_number(detectors[i]) + ",el " + string_number(j)).data(),
            (string("det ") + string_number(detectors[i]) + ",el " + string_number(j)).data(), 100,
    ...
    if (!readFile) {
      outFile =
          new TFile((calibrationOutputPath + string_number(getpid()) + ".root").data(), "recreate");
    ...
    if (doWalk) {
      if (hasPix) {
        if (!readFile) {
          ...
          pixTree->Branch("pix", &outpix,
                          "tdc[3]/F:qdc[3]/F:pointx/F:pointy/F:pointz/F:el[3]/I:ID/I");
        } else {
      ...
      if (hasTrack) {
        if (!readFile) {
          ...
          trackTree->Branch("tr", &outtrack_el,
                            "pointz[2]/F:tdc[2]/F:qdc[2]/F:theta/F:phi/F:el[2]/I:det/I");
        } else {
...
float ATDCcalibration::thMin(int id) { return 0; }
void ATDCcalibration::defineWalkStuff(bool justClean, bool redefine, bool mkehistos, int histoptt,
                                      bool mkefunctions, float rngmod)
{
      ...
      for (int i = 0; i < nCalibDets; i++)
        for (int j = 0; j < numElements[i]; j++) {
          if (redefine || (histoptt & 1) > 0)
            delete walkHistos[i][j];
          if (redefine || (histoptt & 2) > 0)
            delete walkHistos[i][j + numElements[i]];
          if (redefine || (histoptt & 4) > 0)
            delete walkHistos[i][j + 2 * numElements[i]];
          if (doLRT[i])
            if (redefine || (histoptt & 8) > 0)
              delete walkHistos[i][j + 3 * numElements[i]];
        }
  }
  ...
  if (redefine || justClean) {
    for (int i = 0; i < nCalibDets; i++) {
      if (walkHistos != NULL)
        delete[] walkHistos[i];
      if (walkFunctions != NULL)
        delete[] walkFunctions[i];
    }
    if (walkHistos != NULL)
      delete[] walkHistos;
    if (walkFitFunctions != NULL)
      delete[] walkFitFunctions;
    if (walkFunctions != NULL)
      delete[] walkFunctions;
  }
  if (justClean) {
    walkHistos = NULL;
    walkFitFunctions = NULL;
  ...
  if (redefine) {
    ...
    for (int i = 0; i < nCalibDets; i++) {
      ...
      for (int j = 0; j < numElements[i]; j++)
        walkFunctions[i][j] =
            new TF1("walkfunction", "[0]/(x-[1])+[2]*log(abs(x-[3]))+[4]", 0, 4000);
    }
  }
  if (redefine || mkehistos) {
    ...
    for (int i = 0; i < nCalibDets; i++) {
      ...
      for (int j = 0; j < numElements[i]; j++) {
        ...
        if (redefine || (histoptt & 4) > 0)
          walkHistos[i][j + 2 * numElements[i]] = new TH1F(
              (s[2] + s[3]).data(), (s[2] + s[3]).data(),
          ...
          if (doLRT[i]) {
            walkHistos[i][j + 3 * numElements[i]] =
                new TH2F((string("tdc vs \\theta:") + s[3]).data(),
                         (string("tdc vs \\theta:") + s[3]).data(), 30, thmin * 0.9, thmax * 1.02,
  ...
  if (redefine || mkefunctions) {
    for (int i = 0; i < nCalibDets; i++) {
      for (int j = 0; j < 5; j++)
        walkFitFunctions[i]->SetParameter(j, 0);
...
TH1F* getSlice(TH2F* histo, int bin, int axis)
{
  if (histo == NULL)
    return NULL;
  if (bin < 0 || (axis == 0 && bin > histo->GetNbinsX() + 1) ||
      (axis == 1 && bin > histo->GetNbinsY() + 1))
    return NULL;
  ...
  for (int i = 1; i <= ret->GetNbinsX(); i++)
    ret->Fill(ret->GetBinCenter(i),
              histo->GetBinContent((axis == 0 ? bin : i), (axis == 0 ? i : bin)));
  return ret;
}
void ATDCcalibration::defineOffsetStuff(bool justClean, bool redefine, bool mkehistos,
                                        bool mkeparser, float rngmod)
{
  ...
  if (redefine) {
    ...
    offsetRanges[0] = 0;
    for (unsigned int i = 0; i < raa.size(); i++)
      offsetRanges[i + 1] = raa[i];
    evtNr = (int)2.4e06;
    if (trackTree != NULL)
      if (trackTree->GetEntries() > 10)
        trackTree->GetEntry(trackTree->GetEntries() - 1);
    if (pixTree != NULL)
      if (pixTree->GetEntries() > 10)
        pixTree->GetEntry(pixTree->GetEntries() - 1);
    if (raa.size() == 0)
      offsetRanges[1] = evtNr;
  ...
  if (redefine || mkehistos) {
    ...
    canv->cd(1);
    for (int j = 0; j < nCalibDets; j++) {
      TH2F tmphisto("tmpHisto", "tmpHisto", numElements[j], -0.5, numElements[j] - 0.5, 2000, 0,
                    200);
    ...
    for (int i = 0; i < noffsetRanges; i++)
      for (int j = 0; j < nCalibDets; j++) {
        ...
        if (ny < 5)
          ny = 5;
        for (int k = 0; k < numElements[j]; k++) {
          offsetHistos2[i][j][k] = new TH2F(
              (s1 + string_number(detectors[j]) + "el" + string_number(k) + " " + string_number(i))
                  .data(),
              (s1 + string_number(detectors[j]) + " el " + string_number(k)).data(), nx,
              offsetRanges[i], offsetRanges[i + 1], ny,
              meanPosition[j][k] -
                  meanOff[j * 2 + 1] * rngmod * (twoSided[j] == measuresAgainst[j] ? 3 : 1),
              meanPosition[j][k] +
                  meanOff[j * 2 + 1] * rngmod * (twoSided[j] == measuresAgainst[j] ? 3 : 1));
        }
      }
...
{
  ofstream output;
  output.open((calibrationOutputPath + "run" + string_number(runNumber) + "_" +
               string_number(runCounter) + ".cal")
                  .data());
  cout << "write data to \""
       << (calibrationOutputPath + "run" + string_number(runNumber) + "_" +
           string_number(runCounter) + ".cal")
              .data()
       << "\"" << endl;
  if (printPattern > 0) {
    postscript = new TPostScript((calibrationOutputPath + "run" + string_number(runNumber) + "_" +
                                  string_number(runCounter) + "_control.ps")
                                     .data(),
                                 112);
  ...
  if (doBeam)
    doCalibrationBeam();
  defineOffsetStuff(false, /*redefine*/ true, /*mkehistos*/ true,
                    /*mkeparser*/ true, /*rngmod*/ 1);
  if (doTwoSided)
  ...
  if (doWalk || doOffset) {
    ...
    // 	      offsetHistos2[0][i][k]=new
    // TH2F((s1+string_number(detectors[i])+"el"+string_number(k)+" 0").data(),
    // 					      (s1+string_number(detectors[i])+"
    // el
    // "+string_number(k)).data(),
    ...
    for (int i = 0; i < nCalibDets; i++) {
      ...
      // 	  float
      // x=f1->GetParameter(2)*2;//resolutionHisto1[i]->GetFunction("gaus")->GetParameter(2)*3.;
      // 	  for(int k=0;k<numElements[i];k++)
      // 	    {
      // 	      canvas->cd(nc+1);
      ...
      for (int k = 0; k < numElements[i]; k++) {
        ...
        float x1 = ff->GetX(0.01 * ff->GetParameter(0),
                            walkHistos[i][k + numElements[i] * 2]->GetXaxis()->GetXmin(),
                            ff->GetParameter(1));
        float x2 = ff->GetX(0.01 * ff->GetParameter(0), ff->GetParameter(1),
                            walkHistos[i][k + numElements[i] * 2]->GetXaxis()->GetXmax());
  ...
  while (!tmp.empty()) {
    ...
    for (int j = 0; j < nCalibDets; j++)
      if (tmp.back()->getDetector() == detectors[j] &&
          (tmp.back()->getParamID() == 0 || tmp.back()->getParamID() == 1 ||
           tmp.back()->getParamID() == 2 || tmp.back()->getParamID() == 3))
        used = j;
    if (used >= 0) {
...
void ATDCcalibration::fillGeoHisto()
{
  ...
  for (int j = 0; j < nRefPix; j++)
    for (int k = 0; k < *(numberOfPixels[refPixels[j]]); k++) {
      sLine3D line(point3D(0, 0, 0),
                   pixels[refPixels[j]][k]->getShape()->getCenter() - point3D(0, 0, 0));
      for (int i = 0; i < nCalibDets; i++)
        for (int l = 0; l < *(numberOfHits[detectors[i]]); l++) {
          currHit = hits[detectors[i]][l];
          if (!currHit->isValid())
            continue;
          if (currHit->getHitShape() == NULL)
            continue;
          ...
          if (hitSh->getName() == "fiber") {
            ...
            if (tmpv.R() > 0)             // left of shape+ , right of shape -
              geoHistos[l]->Fill(currHit->getElement(),
                                 tmpv.R() * (tmpv * ((fiber*)hitSh)->getDirection(1) < 0 ? 1 : -1));
            ...
            if (tmpv.R() < ((ring*)hitSh)->getInnerRadius())
              geoHistos[l]->Fill(currHit->getElement(),
                                 tmpv.R() - ((ring*)hitSh)->getInnerRadius());

            else if (tmpv.R() > ((ring*)hitSh)->getOuterRadius())
              geoHistos[l]->Fill(currHit->getElement(),
                                 tmpv.R() - ((ring*)hitSh)->getOuterRadius());
            ...
            if (tmpv.R() > 0)
              geoHistos[l]->Fill(
                  ...
                  tmpv.R() * (((wedge*)hitSh)
                                          ->getNormal()
                                          .angle(tmpv, ((wedge*)hitSh)->getLowerPhiEdge()) < 0
                                  ? 1
                                  : -1));
          }
        }
...
void ATDCcalibration::fillBeamHisto()
{
  ...
  for (int i = 0; i < 40; i++)
    for (int j = 0; j < 40; j++) {
      beam.setValues((i - 19.5) * 0.001, (j - 19.5) * 0.001, 1);
      beam.normalize();
...
float ATDCcalibration::getCalibratedTDC(const TRawHit& h, int dn, const point3D& p)
{
  float tdc, qdc;
  int el = h.getElement();
  if (el < 0 || el > numElements[dn])
    return -1;
  qdc = h.getADC();
  tdc = h.getTDC();
  if (numRanges[dn][0] > 0)
    qdc = qdc - params[dn][0][actualRanges[dn][0]]->getParameter(el, 0);
  float wal = 0;
  if (numRanges[dn][2] > 0) {
    ...
    if (use) {
      if (*(numberOfPixels[i]) > 0)
        for (int k = 0; k < pixels[i][0]->getNumberOfActualElements(); k++)
          for (int j = 0; j < nCalibDets; j++)
            if (pixels[i][0]->getElementr(k).getDetector() == detectors[j])
              dets[k] = j;
      outpix.ID = i;
      for (int j = 0; j < *(numberOfPixels[i]); j++) {
        ...
        for (int l = 0; l < pixels[i][j]->getNumberOfActualElements(); l++) {
          ...
          outpix.qdcs[l] =
              pixels[i][j]->getElementr(l).getRawADC() -
    ...
    for (int k = 0; k < ntrackElstructs; k++) {
      if (currentTrack->getNumberOfCalibHits(detectors[trackEls[0][k]]) > 0 &&
        ...
        for (int j = 0; j < 2; j++) {
          ...
          if (currentHit == NULL)
            continue;
          if (currentHit->getHitShape() == NULL)
            continue;
          if (currentHit->getElement() >= numElements[trackEls[j][k]] ||
              currentHit->getElement() < 0)
            continue;
          p = getPoint(currentHit->getHitShape(), currentTrack->getPath());
          outtrack_el.pointz[j] = p.Z();
          outtrack_el.el[j] = currentHit->getElement();
          outtrack_el.tdcs[j] = getCalibratedTDC(currentHit->getRaw(), trackEls[j][k],
                                                 p); // currentHit->getRawTDC();
          outtrack_el.qdcs[j] =
              currentHit->getRawADC() -
              params[trackEls[j][k]][0][actualRanges[trackEls[j][k]][0]]->getParameter(
                  outtrack_el.el[j], 0);
        }
        trackTree->Fill();
      }
  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (int k = 0; k < nCalibDets; k++) {
      ...
      if (currentTrack->getNumberOfCalibHits(detectors[k]) > 0) {
        ...
        if (currentHit->getHitShape() == NULL)
          continue;
        if (currentHit->getElement() >= numElements[k] || currentHit->getElement() < 0)
          continue;
        p = getPoint(currentHit->getHitShape(), currentTrack->getPath());
        outtrack[i].el[k] = currentHit->getElement();
        outtrack[i].tdcs[k] = getCalibratedTDC(currentHit->getRaw(), k,
                                               p); // currentHit->getRawTDC();
...
void ATDCcalibration::fillWalkHistos(int pattern) //, float ***offsets,float*** slopes)
{
  ...
  if (resolutionHisto != NULL)
    for (int i = 0; i < nCalibDets * 2; i++)
      if (resolutionHisto[i] != NULL)
        resolutionHisto[i]->Reset();
  if (resolutionHisto1 != NULL)
    for (int i = 0; i < nCalibDets; i++)
      if (resolutionHisto1[i] != NULL)
        resolutionHisto1[i]->Reset();
  ...
  for (int i = 0; i < nCalibDets; i++) {
    ...
    zfront[i] =
        sh->getCenter().Z() - sqrt(sh->getOuterRadius() * sh->getOuterRadius() -
                                   sh->getDistToBarrelCenter() * sh->getDistToBarrelCenter());
    //      float
    //      z1=sqrt(sh->getOuterRadius()*sh->getOuterRadius()-sh->getDistToBarrelCenter()*sh->getDistToBarrelCenter());
  ...
  if (hasTrack) {
    for (int j = 0; j < trackTree->GetEntries() /*500000*/; j++) {
      ...
      while (evtNr > offsetRanges[cor + 1] && cor < noffsetRanges)
        cor++;
      el[0] = -1;
      el[1] = -1;
      for (int i = 0; i < 2; i++) {
        ...
        if (el[i] < numElements[det[i]] && el[i] >= 0) {
          lrp[i] = getLightWay(setup.getDetectorr(detectors[det[i]]).getShape(el[i]),
                               point3D(z[i] * tan(outtrack_el.theta) * cos(outtrack_el.phi),
                                       z[i] * tan(outtrack_el.theta) * sin(outtrack_el.phi), z[i]),
                               false);
          tdcs[0][i] =
              getTDC(evtNr, det[i], el[i], outtrack_el.tdcs[i], outtrack_el.qdcs[i], lrp[i],
                     cor /*offsets[cor],slopes[cor]*/, c1, true, (!twoSided[det[i]]));
          tdcs[1][i] =
              getTDC(evtNr, det[i], el[i], outtrack_el.tdcs[i], outtrack_el.qdcs[i], lrp[i],
                     cor /*offsets[cor],slopes[cor]*/, c2, true, (!twoSided[det[i]]));
        }
      }
      if (pixelBased[trackEls[0][outtrack_el.det]] >= 0 || el[0] < 0)
        continue;
      if (pixelBased[trackEls[1][outtrack_el.det]] >= 0 || el[1] < 0)
        continue;
      twoadd = 0;
      if (calibPos[3][det[0]] >= 0) {
        twoadd = -(((outtrack_el.pointz[0] /*+zmid[det[0]]*0.5*/ - zfront[det[0]]) / 10. +
                   ...
                   outParser[det[0]][calibPos[3][det[0]]].getParameter(el[0], 0)) *
                 2.;
        // 	      cout<<outtrack_el.pointz[0]<<" "<<det[0]<<"
        // "<<zfront[det[0]]<<"
        // "<<outParser[det[0]][calibPos[3][det[0]]].getParameter(el[0],1)<<"
        // "<<outParser[det[0]][calibPos[3][det[0]]].getParameter(el[0],0)<<"
        // "<<flush;
      ...
      if ((pattern & 4) > 0)
        for (int i = 0; i < 2; i++) {
          if (qdcs[i] > 1e8 || el[i] < 0)
            continue;
          walkHistos[det[i]][numElements[det[i]] + el[i]]->Fill(qdcs[i]);
          if (lrp[i] >= min_maxLRP[det[i] * 2] && lrp[i] <= min_maxLRP[det[i] * 2 + 1]) {
            walkHistos[det[i]][numElements[det[i]] * 2 + el[i]]->Fill(-tdcs[0][i] -
                                                                      0.5 * twoadd * (2 * i - 1));
            offsetHistos2[cor][det[i]][el[i]]->Fill(evtNr,
                                                    -tdcs[0][i] - 0.5 * twoadd * (2 * i - 1));
          }
        }
      if (twoSided[det[0]] == det[1]) {
        for (int i = 0; i < 2; i++) {
          if (lrp[i] >= min_maxLRP[det[i] * 2] && lrp[i] <= min_maxLRP[det[i] * 2 + 1]) {
            walkHistos[det[i]][el[i]]->Fill(qdcs[i],
                                            tdcs[1][i] - tdcs[0][1 - i] + twoadd * (2 * i - 1));
          }
          resolutionHisto[det[i] * 2]->Fill(el[i],
                                            tdcs[1][i] - tdcs[0][1 - i] + twoadd * (2 * i - 1));
          resolutionHisto[det[i] * 2 + 1]->Fill(lrp[i],
                                                tdcs[1][i] - tdcs[0][1 - i] + twoadd * (2 * i - 1));
          resolutionHisto1[det[i]]->Fill(tdcs[1][i] - tdcs[0][1 - i] + twoadd * (2 * i - 1));
          if (doLRT[det[i]])
            walkHistos[det[i]][el[i] + 3 * numElements[det[i]]]->Fill(
                lrp[i], tdcs[1][i] - tdcs[0][1 - i] + twoadd * (2 * i - 1));
        }
      } else {
        for (int i = 0; i < 2; i++) {
          if (lrp[i] >= min_maxLRP[det[i] * 2] && lrp[i] <= min_maxLRP[det[i] * 2 + 1])
          ...
          if (doLRT[det[i]])
            walkHistos[det[i]][el[i] + 3 * numElements[det[i]]]->Fill(lrp[i],
                                                                      tdcs[1][i] - tdcs[0][1 - i]);
        }
      }
    }
    ...
    for (int j = 0; j < pixTree->GetEntries() /*50000*/; j++) {
      ...
      for (int i = 0; i < nCalibDets; i++) {
        ...
        if (outpix.el[pixelDets[i]] < numElements[i] && outpix.el[pixelDets[i]] >= 0) {
          ...
          tdcs[0][i] = getTDC(evtNr, i, el[i], outpix.tdcs[pixelDets[i]], outpix.qdcs[pixelDets[i]],
                              lrp[i], cor /*offsets[cor],slopes[cor]*/, c1, true, true);
          tdcs[1][i] = getTDC(evtNr, i, el[i], outpix.tdcs[pixelDets[i]], outpix.qdcs[pixelDets[i]],
                              lrp[i], cor /*offsets[cor],slopes[cor]*/, c2, true, true);
        }
      }
      if ((pattern & 4) > 0)
        for (int i = 0; i < nCalibDets; i++) {
      ...
      for (int i = 0; i < nCalibDets; i++) {
        ...
        if (!((detectors[i] % 3) > 0 && lrp[i] >= min_maxLRP[i * 2] &&
              lrp[i] <= min_maxLRP[i * 2 + 1])) // lrp[i]>100))
        ...
        if (doLRT[i])
          walkHistos[i][el[i] + 3 * numElements[i]]->Fill(
              lrp[i], tdcs[1][i] - tdcs[0][detectors[i] < 3 ? 8 : 9]);
        if ((detectors[i] % 3) == 0)
...
void ATDCcalibration::fillOffsetHistos(
    /*float ***offsets, float ***slopes,*/ TH1*** absOffsetHistos, int pattern)
{
  cout << "x" << flush;
  for (int i = 0; i < nCalibDets; i++) {
    (*absOffsetHistos)[i] = NULL;
    if (!isStop[i]) {
      (*absOffsetHistos)[i] = new TH1F(
          (string("difference starts") + string_number(detectors[i])).data(),
          (string("\\Delta TOF start1-start2") + string_number(detectors[i])).data(), 100, -2, 2);
    } else {
      float thmin = 0, thmax = thMax(i);
      ...
      if (doLRT[i])
        ...
        tofrange = 2;
        if (pattern < 5)
          tofrange = 2 + 0.1 * (5 - pattern);
      }
      if (calibPos[2][i] >= 0 && (pattern <= 1 || (pattern < 5 && twoSided[i] >= 0))) {
        (*absOffsetHistos)[i] = new TH2F(
            (string("difference ") + string_number(detectors[i])).data(),
            (string("\\Delta TOF calculated-measured: front-det") + string_number(detectors[i]))
                .data(),
            (detectors[i] < 6 ? 20 : 100), thmin * 0., thmax * 1., 100, tofmean - tofrange,
            tofmean + tofrange);
      } else {
        (*absOffsetHistos)[i] = new TH1F(
            (string("difference ") + string_number(detectors[i])).data(),
            (string("\\Delta TOF calculated-measured: front-det") + string_number(detectors[i]))
                .data(),
            100, tofmean - tofrange, tofmean + tofrange);
      }
    }
    if ((*absOffsetHistos)[i] != NULL)
      (*absOffsetHistos)[i]->SetDirectory(0);
  }
  float tdcF[2], tdcB[2], /*tofT[2],*/ tdcSum;
  int nsum;
  vector3D pF[2], pB[2];
  float pb = setup.getBeamr().getParticle().Momentum().R(),
        mass = Eparticles::getMassG(14); // setup.getBeamr().getParticle().Mass();
  float c = 299.792;
  ...
  for (int i = 0; i < elTree->GetEntries(); i++) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      v1.setValues(cos(outtrack[j].phi) * sin(outtrack[j].theta),
                   sin(outtrack[j].phi) * sin(outtrack[j].theta), cos(outtrack[j].theta));
    ...
    for (int j = 0; j < 2; j++) {
      nsum = 0;
      tdcSum = 0;
      pF[j].setValues(0, 0, 0);
      for (int k = 0; k < nCalibDets; k++) {
        if (isStop[k]) {
          continue;
        }
        if ((outtrack[j].el[k] < 0 || outtrack[j].el[k] >= numElements[k]))
          continue;
        nsum++;
        tdcSum =
            tdcSum + getTDC(evtNr, k, outtrack[j].el[k], outtrack[j].tdcs[k], outtrack[j].qdcs[k],
                            0, cor /*offsets[cor],slopes[cor]*/, true, true, false);

        pF[j] = pF[j] +
                getEntrance(setup.getDetectorr(detectors[k]).getShape(outtrack[j].el[k]), path[j]);
      }
      ...
      for (int k = 0; k < nCalibDets; k++) {
        ...
        lrp = abs(getLightWay(setup.getDetectorr(detectors[k]).getShape(outtrack[j].el[k]), pB[j],
                              false));

        tdcB[j] = getTDC(evtNr, k, outtrack[j].el[k], outtrack[j].tdcs[k], outtrack[j].qdcs[k], lrp,
                         cor /*offsets[cor], slopes[cor]*/, true, true, calibPos[2][k] >= 0);

        if (twoSided[k] >= 0) {
          if (outtrack[j].el[k] != outtrack[j].el[twoSided[k]])
            continue;
          tdcB[j] = (tdcB[j] + getTDC(evtNr, twoSided[k], outtrack[j].el[twoSided[k]],
...
void ATDCcalibration::fillOnlineOffsetHisto()
{
  // TCalibHit *currentHit;
  int cor = 0;
  while (cor < noffsetRanges && offsetRanges[cor] < eventNumber)
    cor++;
  for (int i = 0; i < numberOfTracks; i++) {
...
void ATDCcalibration::doCalibrationGeo()
{
  ...
  for (int j = 0; j < nCalibDets; j++) {
    ...
    if (setup.getDetectorr(detectors[j]).getShape(0)->getName() == "fiber") {
      ...
      p = sh->getCenter();
      TF1 func("func", functionString[j].data(), 0,
               setup.getDetectorr(detectors[j]).getNumberOfElements());
      prf->Fit(&func, "QR0");
      f = prf->GetFunction("func");
      cout << detectors[j] << ":" << f->GetParameter(0) << "*sin(" << f->GetParameter(1) << "*(x-"
           << f->GetParameter(2) << "))+" << f->GetParameter(3) << endl;
...
void ATDCcalibration::doCalibrationBeam()
{
  ...
  for (int i = 0; i < 40; i++)
    for (int j = 0; j < 40; j++) {
      ...
      if (beamHistos[i][j]->GetFunction(fun.GetName())->GetParameter(2) < minv) {
        mini = i;
        minj = j;
        minv = beamHistos[i][j]->GetFunction(fun.GetName())->GetParameter(2);
      }
      if (printMe) {
        beamHistos[i][j]->Draw();
        if (nc == 19) {
  ...
  vector3D tmp((mini - 19.5) * 0.001, (minj - 19.5) * 0.001, 1);
  tmp.normalize();
...
void ATDCcalibration::resetWalkHistos()
{
  for (int i = 0; i < nCalibDets; i++)
    for (int j = 0; j < numElements[i]; j++) {
      ...
      if (doLRT[i])
        walkHistos[i][j + 3 * numElements[i]]->Reset();
    }
}
void ATDCcalibration::doCalibrationWALK()
{
  ...
  if (trackTree != NULL)
    cout << "Track-Entries:" << trackTree->GetEntries() << flush;
  if (pixTree != NULL)
    cout << " Pix-Entries:" << pixTree->GetEntries() << flush;
  TCanvas* canvas = NULL;
  int nc = 0;
  ...
  bool printMe = (printIteration || printBefore || printAfter || printLastIteration ||
                  ((printPattern & 1) > 0));
  if (printMe) {
    canvas = new TCanvas();
    canvas->Divide(4, 3);
  ...
  if (hasSingle) {
    for (int i = 1; i < nCalibDets; i++)
      if (!single[i] && single[i - 1])
        nRR++;
    if (single[nCalibDets - 1] && !single[nCalibDets - 2])
      nRR--;
    beg = new int[nRR];
    ennd = new int[nRR];
    nRR = 0;
  ...
  for (int i = 0; i < nCalibDets; i++) {
    if (!doLRT[i])
      continue;
    for (int j = 0; j < numElements[i]; j++)
      fitTheHisto(((TH2*)walkHistos[i][j + 3 * numElements[i]]), NULL, &func,
                  &outParser[i][calibPos[2][i]], j, /*fixAtMax=*/false, -1, "0");
  }
  ...
  for (int rr = 0; rr < nRR + 1; rr++) {
    for (int j = (rr == 0 ? 0 : ennd[rr - 1]); j < (rr == nRR ? nCalibDets : beg[rr]); j++) {
      ...
      for (int k = 0; k < numElements[j]; k++) {
        fitTheHisto(((TH2*)walkHistos[j][k]), walkHistos[j][k + numElements[j]],
                    walkFitFunctions[j], &outParser[j][calibPos[0][j]], k, /*fixAtMax=*/false, -1,
                    "0");
        for (int l = 0; l < walkFunctions[j][k]->GetNumberFreeParameters(); l++)
          walkFunctions[j][k]->SetParameter(l, outParser[j][calibPos[0][j]].getParameter(k, l));
      }
    ...
    if (printIteration) {
      printLine = false;
      printWalkHistos(canvas, 12, nc, (rr == 0 ? 0 : ennd[rr - 1]),
                      (rr == nRR ? nCalibDets : beg[rr]));
    }
  }
    ...
    for (int i = 0; i < nCalibDets; i++) {
      ...
      if (calibPos[2][i] >= 0)
        for (int j = 0; j < numElements[i]; j++) {
          ...
          // 	      fitTheHisto(((TH2*)walkHistos[i][j+3*numElements[i]]),
          // NULL,
          ...
          if (tmpf1 != NULL) {
            for (int k = 0; k < 4; k++)
              outParser[i][calibPos[2][i]].setParameter(
                  j, k, outParser[i][calibPos[2][i]].getParameter(j, k) + tmpf1->GetParameter(k));
          }
          delete tmp;
        }
    }
    ...
    for (int j = 0; j < nCalibDets; j++) {
      if (doLRT[j])
        for (int k = 0; k < numElements[j]; k++) {
          ...
          walkHistos[j][k + 3 * numElements[j]]->Draw("col");
          tmpH[nc]->Draw("same");
          line.DrawLine(walkHistos[j][k + 3 * numElements[j]]->GetXaxis()->GetXmin(), 0,
                        walkHistos[j][k + 3 * numElements[j]]->GetXaxis()->GetXmax(), 0);
          nextPad(12, postscript, canvas, nc, ((TObject**)tmpH));
        }
    }
  }
  ...
  if (printMe) {
    ...
    for (int j = 0; j < nCalibDets; j++) {
      ...
      if ((j % 4) == 3) {
        canvas->Update();
        postscript->NewPage();
      } //	  nextPad(12,postscript,canvas,nc,NULL);
    }
    if ((nCalibDets % 4) > 0) {
      canvas->Update();
      postscript->NewPage();
...
void ATDCcalibration::doCalibrationOffset()
{
  ...
  if (!doWalk) {
    ...
    for (int i = 0; i < noffsetRanges; i++)
      for (int j = 0; j < nCalibDets; j++)
        for (int k = 0; k < numElements[j]; k++) {
          getProfileFit(offsetHistos2[i][j][k], (isStop[j] ? "landau" : "gaus"), 1, line,
                        offsets[i][j][k], slopes[i][j][k], 0, 1);
        }
    ...
    if (printMe)
      delete canvas;
    for (int i = 0; i < nCalibDets; i++)
      if (absOffsetHisto[i] != NULL)
        delete absOffsetHisto[i];
  ...
  //////////
  for (int j = 0; j < nCalibDets; j++) {
    if (referencesTo[j] < 0 || calibPos[1][j] < 0)
      continue;
    if (absOffsetHisto[j]->IsA() == TH1F::Class() && absOffsetHisto[j]->GetEntries() < 10) {
  ...
  for (int i = 0; i < numberOfTracks; i++) {
    for (int j = 0; j < nTwoSided; j++) {
      for (int k = 0; k < tracks[i]->getNumberOfCalibHits(detectors[twoDets[j]]); k++) {
        ...
        if (!h1->isValid())
          continue;
        if (h1->getElement() > 0 || h1->getElement() >= numElements[twoDets[j]])
          continue;
        h2 = NULL;
        for (int l = 0; l < tracks[i]->getNumberOfCalibHits(detectors[twoSided[twoDets[j]]]); l++)
          if (tracks[i]->getCalibSrcr(detectors[twoSided[twoDets[j]]], l) != NULL)
            if (tracks[i]->getCalibSrcr(detectors[twoSided[twoDets[j]]], l)->getElement() ==
                h1->getElement())
        ...
        twoSidedHisto[j][h1->getElement()]->Fill(
            (h2->getTDC() - h1->getTDC()) / 2,
            (getPoint(h1->getHitShape(), tracks[i]->getPath()).Z() - zfront[j]) / 10);
      }
    }
  }
}
    ...
    for (int k = 0; k < elTree->GetEntries(); k++) {
      ...
      for (int j = 0; j < 2; j++)
        for (int i = 0; i < nTwoSided; i++) {
          ...
          if (el < 0 || el != outtrack[j].el[twoSided[twoDets[i]]] || el >= numElements[twoDets[i]])
            continue;
          tdc1 = getTDC(evtNr, twoDets[i], el, outtrack[j].tdcs[twoDets[i]],
                        outtrack[j].qdcs[twoDets[i]], 0, cor /*offsets[cor],slopes[cor]*/, true,
                        true, false);
          tdc2 = getTDC(evtNr, twoSided[twoDets[i]], el, outtrack[j].tdcs[twoSided[twoDets[i]]],
                        outtrack[j].qdcs[twoSided[twoDets[i]]], 0, cor /*offsets[cor],slopes[cor]*/,
                        true, true, false);
          twoSidedHisto[i][el]->Fill((tdc2 - tdc1) / 2,
                                     (outtrack[j].pointz[twoDets[i]] - zfront[i]) / 10);
        }
    }
    break;
  }
  case 0: {
    ...
    for (int k = 0; k < trackTree->GetEntries(); k++) {
      ...
      while (evtNr > offsetRanges[cor + 1] && cor < noffsetRanges)
        cor++;
      for (int i = 0; i < nTwoSided; i++) {
        if (!(trackEls[0][outtrack_el.det] == twoDets[i] ||
              trackEls[1][outtrack_el.det] == twoDets[i]))
          continue;
        if (trackEls[0][outtrack_el.det] == twoDets[i])
          num = 0;
        else
        ...
        if (el < 0 || el != outtrack_el.el[1 - num] ||
            el >= numElements[trackEls[num][outtrack_el.det]])
          continue;
        tdc1 = getTDC(evtNr, twoDets[i], el, outtrack_el.tdcs[num], outtrack_el.qdcs[num], 0,
                      cor /*offsets[cor],slopes[cor]*/, !true, true, false);
        tdc2 = getTDC(evtNr, twoSided[twoDets[i]], el, outtrack_el.tdcs[1 - num],
                      outtrack_el.qdcs[1 - num], 0, cor /*offsets[cor],slopes[cor]*/, !true, true,
                      false);
  ...
  case 1: {
    ...
    for (int k = 0; k < pixTree->GetEntries(); k++) {
      ...
      for (int i = 0; i < nTwoSided; i++) {
        ...
        if (el < 0 || el != outpix.el[pixelDets[twoSided[twoDets[i]]]] ||
            el >= numElements[twoDets[i]])
          continue;
        tdc1 = getTDC(evtNr, twoDets[i], el, outpix.tdcs[pixelDets[twoDets[i]]],
                      outpix.qdcs[pixelDets[twoDets[i]]], 0, cor /* offsets[cor],slopes[cor]*/,
                      true, true, false);
        tdc2 = getTDC(evtNr, twoSided[twoDets[i]], el, outpix.tdcs[pixelDets[twoSided[twoDets[i]]]],
                      outpix.qdcs[pixelDets[twoSided[twoDets[i]]]], 0,
                      cor /*offsets[cor],slopes[cor]*/, true, true, false);
    ...
    if (calibPos[3][twoDets[i]] >= 0)
      for (int j = 0; j < numElements[twoDets[i]]; j++) {
        ...
        for (int ll = 0; ll < 3; ll++) {
          ...
          switch (ll) {
          case 2:
            tmp[nc] = getMaxHisto((TH2*)twoSidedHisto[i][j]);
            break;
          case 1:
            tmp[nc] = ((TH2*)twoSidedHisto[i][j])->ProfileX();
            break;
          case 0:
            tmp[nc] = getProfile((TH2*)twoSidedHisto[i][j], "gaus", 1);
            break;
          }
          ...
          if (ff->GetChisquare() / ff->GetNDF() > 15) {
            for (int l = 0; l < 10; l++) {
              mmin =
                  twoSidedHisto[i][j]->GetMean() - twoSidedHisto[i][j]->GetRMS() * (2. - 0.11 * l);
              mmax =
                  twoSidedHisto[i][j]->GetMean() + twoSidedHisto[i][j]->GetRMS() * (2. - 0.11 * l);
              line.SetParameters(14, 140);
              tmp[nc]->Fit(&line, (printMe ? "Q" : "Q0"), (printMe ? "same" : ""), mmin, mmax);
            ...
            if (ff->GetChisquare() / ff->GetNDF() > 15) {
              for (int l = 0; l < 10; l++) {
                mmin = twoSidedHisto[i][j]->GetMean() -
                       twoSidedHisto[i][j]->GetRMS() * (2. - 0.11 * l);
                mmax = twoSidedHisto[i][j]->GetMean() +
                       twoSidedHisto[i][j]->GetRMS() * (2. - 0.11 * l);
                line.SetParameters(14, 140);
        ...
        if (printMe)
          nextPad(12, postscript, canvas, nc, ((TObject**)tmp));
        else
          delete tmp[nc];
      }
  }
...
void ATDCcalibration::calculateOffsets(int ptt)
{
  ...
  for (int i = 0; i < noffsetRanges; i++)
    for (int j = 0; j < nCalibDets; j++) {
      for (int k = 0; k < numElements[j]; k++) {
        line.SetParameters(
            offsetHistos2[i][j][k]->GetBinCenter(offsetHistos2[i][j][k]->GetNbinsX()) -
                offsetHistos2[i][j][k]->GetBinCenter(1),
            0);
        getProfileFit(offsetHistos2[i][j][k], (isStop[j] ? "landau" : "gaus"), 1, line,
                      offsets[i][j][k], slopes[i][j][k], 0, 1);
      }
...
TH1* getMaxHisto(TH2* hist)
{
  if (hist == NULL)
    return NULL;
  TH1* histo1 =
      new TH1F((string(hist->GetName()) + "_Max").data(), hist->GetTitle(), hist->GetNbinsX(),
  ...
  for (int i = 1; i < hist->GetNbinsX() + 1; i++) {
    ...
    sum = hist->GetBinContent(i, maxBin) / hist->GetBinError(i, maxBin) /
          hist->GetBinError(i, maxBin);
...
void ATDCcalibration::getProfileFit(TH2* histo, const string& functionName, int parnum,
                                    TF1& function, float& value, float& value2, int parnum1,
                                    int parnum2)
{
  TH1* tmpH;
  ...
  //   else
  {
    ...
    if (ff->GetChisquare() / ff->GetNDF() > 10)
      tmpH->Fit(&function, "Q0+", "", tmpH->GetBinCenter((int)(tmpH->GetNbinsX() * 0.1) + 1),
...
TH1* ATDCcalibration::getProfile(TH2* histo, string functionName, int parnum)
{
  if (histo == NULL)
    return NULL;
  if (functionName == "")
    return histo->ProfileX();
  TF1 function("f1", functionName.data());
  if (function.GetNumberFreeParameters() <= parnum)
    parnum = function.GetNumberFreeParameters() - 1;
  //   if(histo->GetEntries()/histo->GetNbinsX()<histo->GetNbinsY()*1.5)
  //     {
  ...
  TH1F* ret =
      new TH1F((string(histo->GetName()) + "_PFx").data(), histo->GetTitle(), histo->GetNbinsX(),
               histo->GetXaxis()->GetXmin(), histo->GetXaxis()->GetXmax());
  TH1F tmp("tmpHisto", "tmpHisto", histo->GetNbinsY(), histo->GetYaxis()->GetXmin(),
  ...
  for (int i = 1; i < histo->GetNbinsX() + 1; i++) {
    ...
    if (ne > 20) {
      ...
      ret->SetBinContent(
          i,
          tmp.GetBinCenter(tmp.GetNbinsX() /
                           2)); // tmp.GetFunction(function.GetName())->GetParameter(parnum));
      ret->SetBinError(i,
                       (tmp.GetBinCenter(1) + tmp.GetBinCenter(tmp.GetNbinsX())) /
                           2.); // tmp.GetFunction(function.GetName())->GetParError(parnum));
      if (tmph != NULL) {
                                  ...
                                  int element, bool fixAtMax, int ptt, string opt)
{
  ...
  if (h1d != NULL) {
    float rmsfac = 1;
    if (output != NULL) {
      if (output->getParamID() == 6)
        rmsfac = 3.;
      else if ((output->getDetector() == 6 || output->getDetector() == 7))
        rmsfac = 0.1;
    }
    ffmin = h1d->GetBinCenter(h1d->GetMaximumBin()) -
            h1d->GetRMS() * rmsfac; //(/*(j<2||j>3)?*/160/*:h1d->GetRMS()*/);
    ...
    if (output != NULL) {
      if (output->getDetector() == 6 || output->getDetector() == 7) {
        ...
        while (h1d->GetBinContent(mb) < 100 && mb < xb)
          mb++;
...
void ATDCcalibration::printWalkHistos(TCanvas* canv, int npads, int& ncanv, int begin, int end)
{
  ...
  lin.SetLineColor(1);
  lin.SetLineWidth(2);
  for (int j = begin; j < end; j++) { // if(detectors[j]!=6)continue;
    for (int k = 0; k < numElements[j]; k++) {
      canv->cd(ncanv + 1);
      ...
      obj[ncanv]->Draw("same");
      if (printLine)
        lin.DrawLine(walkHistos[j][k]->GetXaxis()->GetXmin(), 0,
                     walkHistos[j][k]->GetXaxis()->GetXmax(), 0);
      else {
...
float ATDCcalibration::calculateLRT(int detectorNumber, int ElementNumber, float lrp)
{
  if (calibPos[2][detectorNumber] < 0 /*||!(doLRT[detectorNumber]||twoSided[detectorNumber]>=0)*/) {
    cout << "wrong detector for LRT:" << detectors[detectorNumber] << endl;
    return 0;
  }
  return outParser[detectorNumber][calibPos[2][detectorNumber]].getParameter(ElementNumber, 0) +
         outParser[detectorNumber][calibPos[2][detectorNumber]].getParameter(ElementNumber, 1) *
             lrp +
         outParser[detectorNumber][calibPos[2][detectorNumber]].getParameter(ElementNumber, 2) *
             lrp * lrp +
         outParser[detectorNumber][calibPos[2][detectorNumber]].getParameter(ElementNumber, 3) *
             lrp * lrp * lrp;
  //    +outParser[detectorNumber][pos].getParameter(ElementNumber,4)*lrp*lrp*lrp*lrp;
}
float ATDCcalibration::getTDC(int evtnr, int detectorNumber, int elementNumber, float tdc,
                              float qdc, float path, int corr, bool hwalk, bool hoff, bool hlrt)
{
  float ret = tdc;
  if (hoff)
    ret = ret - (offsets[corr][detectorNumber][elementNumber] +
                 slopes[corr][detectorNumber][elementNumber] * evtNr * 1e-6);
  if (hlrt && calibPos[2][detectorNumber] >= 0 && path > 0)
    ret = ret - calculateLRT(detectorNumber, elementNumber, path);
  if (hwalk)
    ret = ret - calculateWALK(detectorNumber, elementNumber, qdc);
  return ret;
}
point3D ATDCcalibration::getPoint(volumeShape* sh, const sLine3D& path)
{
  ...
  if (sh->getName() == "wedge") {
    if (((wedge*)sh)->getDistToBarrelCenter() > 0) {
      ...
      //	  cout<<p<<"\t"<<w->getCenter()<<"\t"<<alpha<<"\t"<<path.Direction().Theta()<<"\t"<<cos(M_PI-alpha-path.Direction().Theta())<<"
      //"<<cos(path.Direction().Theta())<<" "<<w->getOuterRadius()<<"
      //"<<w->getCenter().Z()-sqrt(w->getOuterRadius()*w->getOuterRadius()-w->getDistToBarrelCenter()*w->getDistToBarrelCenter())<<endl;
      // 	  p=path.Foot()+path.Direction()*
      // 	    (sh->getCenter().Z()*cos(alpha)/
      // 	     cos(M_PI-alpha-path.Direction().Theta()));//
      // (entrance(path);
    }
  }
  return p;
...
void ATDCcalibration::writeCalibration(CommonCalibrationParser* pa, ostream& out)
{
  if (pa == NULL)
    return;
  if (pa->getDetector() < 0 || pa->getParamID() < 0)
    return;
  //   cout<<"writeCalibration() "<<pa->getDetector()<<"
  //   "<<pa->getParamID()<<endl;
  string detname = "", paramname = string_number(pa->getParamID());
  switch (pa->getDetector()) {
  case 0:
    detname = "QUIRL_GERADE";
    break;
  case 1:
    detname = "QUIRL_LINKS";
    break;
  case 2:
    detname = "QUIRL_RECHTS";
    break;
  case 3:
    detname = "RING_GERADE";
    break;
  ...
  case 5:
    ...
    break;
  case 6:
    detname = "BARREL_VORNE";
    break;
  case 7:
    detname = "BARREL_HINTEN";
    break;
  case 15:
    detname = "TORTE_A";
    break;
  ...
  case 6:
    ...
    break;
  case 7:
    paramname = "tdcRadialPix";
    break;
  case 8:
    paramname = "tdcRadialPol";
    break;
  case 9:
    paramname = "tdcRadialSpPix";
    break;
  case 10:
    paramname = "tdcRadialSpPol";
    break;
  case 11:
    paramname = "qdcRadialPix";
    break;
  case 12:
    paramname = "qdcRadialPol";
    break;
  case 13:
    paramname = "tdcBarrelAdd";
    break;
  }
  out << "#======================new calibration type==========================" << endl;
  out << "" << endl;
  out << "==>               " << detname.data() << " " << pa->getDetector() << " "
      << pa->getNumElements() << "  " << paramname.data() << "  " << pa->getParamID() << "  "
      << pa->getNumParameters() << endl;
  out << "" << endl;
  out << "#---------------------------------------------------" << endl;
  ...
  if (eventBased || (((pa->getDetector() > 14 && pa->getParamID() == 3) || pa->getParamID() == 5) &&
                     noffsetRanges > 1)) {
