...
void AMultipleTreeOutput::cleanTree()
{
  ...
  if(outFile!=NULL)
    ...
    if(countTree!=NULL)delete countTree;
  countTree=NULL;
  for(int j=0;j<3;j++)
    {
      for(int i=0;i<nTrees[j];i++)
...
{
  if(tp<0||tp>2)return;
  if(nTrees[tp]>=maxtrees)return;
  readWriteLog<<"addATree(int tp="<<tp<<") nTrees[tp]="<<nTrees[tp]<<" "<<maxtrees<<endli;
  /**********************************************
  ...
  for(int i=0;i<nTracks_per_tree;i++)
    {
      ...
      trees[tp][nTrees[tp]]->Branch(branchname.data(),&trackOut[tp][nTrees[tp]][i],"vertex[3]/D:fitted[3]/D:el[30]/I:det[30]/I:phi/D:theta:beta:energy:momentum:ID/I:nEl");
    }
  if(tp==1)
    {
      for(int i=0;i<nTracks_per_tree;i++)
	{
	  ...
	  trees[tp][nTrees[tp]]->Branch(branchname.data(),&trackOut[tp][nTrees[tp]][i+nTracks_per_tree],"vertex[3]/D:fitted[3]/D:el[30]/I:det[30]/I:phi/D:theta:beta:energy:momentum:ID/I:nEl");
	}
    }
  else if(tp==2)
    {
      ...
      for(int i=0;i<nTracks_per_tree;i++)
	{
	  ...
	  trees[tp][nTrees[tp]]->Branch(branchname.data(),&trackOut[tp][nTrees[tp]][i+nTracks_per_tree*2],"vertex[3]/D:fitted[3]/D:el[30]/I:det[30]/I:phi/D:theta:beta:energy:momentum:ID/I:nEl");
	}
    }
  nTrees[tp]++;
}
...
void AMultipleTreeOutput::setTree(TTrack *in,trackT *out)
{
  ...
  int n=0;
  for(int i=0;i<30;i++){out->det[n]=-1;out->el[n]=-1;}
...
void AMultipleTreeOutput::writeTracks()
{
  ...
  for(int i=0;i<numberOfTracks;i++)
    {
      ...
      if(tracks[i]->getNumberOfSecondaryTracks()==0)np++;
      else if(tracks[i]->getNumberOfSecondaryTracks()==1)nk++;
      else if(tracks[i]->getNumberOfSecondaryTracks()==2)nv++;
    }
  ...
  for(int i=0;i<numberOfTracks;i++)
    {
      if(tracks[i]->isSecondary())continue;
      if(tracks[i]->getNumberOfSecondaryTracks()==0)pp[np++]=i;
      else if(tracks[i]->getNumberOfSecondaryTracks()==1)kk[nk++]=i;
      else if(tracks[i]->getNumberOfSecondaryTracks()==2)vv[nv++]=i;
    }
  nPTracks=np;
  nVTracks=nv;
  nKTracks=nk;
  ...
  for(int i=0;i<nk;i++)
    {
      ...
      setTree(tracks[kk[i]]->getSecondary(0),&trackOut[1][i/nTracks_per_tree][i%nTracks_per_tree+nTracks_per_tree]);
    }
  for(int i=0;i<nv;i++)
    {
  ...
  if((np%nTracks_per_tree)>0)npt++;
  if((nk%nTracks_per_tree)>0)nkt++;
  if((nv%nTracks_per_tree)>0)nvt++;
  for(int i=0;i<npt;i++)
    {
...
void AMultipleTreeOutput::newRun(run_parameter &rp)
{
  if(newFileEachRun)
    {
      cleanTree();
      string nam1=rp.getName();string nam="";
      for(unsigned int i=0;i<nam1.length();i++)
	if(nam1[i]!=' ')nam=nam+nam1;
...
{
  if(outFile==NULL)return ptr;
  writeTracks();
  return ptr;
}
AMultipleTreeOutput::AMultipleTreeOutput(int max, int &evtNr,int &rnNr, int &triggerIn, int &nTrack, TTrack **tracksIn, const algorithm_parameter &param):AAlgorithm("Track output into multiple trees"),maxtrees(max),numberOfTracks(nTrack),eventNr(evtNr),runNr(rnNr),trgg(triggerIn)
{
  tracks=tracksIn;
  countTree=NULL;
  entryNumbers=NULL;
