...
#include <qdatetime.h>
#include "logger.h"
ACalibrationGeneration::ACalibrationGeneration(TEvent &eventIn, TSetup &setupIn, const algorithm_parameter &descr):AAlgorithm("Generate calibration constants"),event(eventIn),setup(setupIn)
{
  author="typeCase";
  if(descr.getNumberOfParam<string>()>0)author=descr.getParam<string>(0).getData();
  beamtime="Oct_04";
  if(descr.getNumberOfParam<string>()>1)beamtime=descr.getParam<string>(1).getData();
  fromRun=0;
  uptoRun=10000;
  if(descr.getNumberOfParam<int>()>0)fromRun=descr.getParam<int>(0).getData();
  if(descr.getNumberOfParam<int>()>1)uptoRun=descr.getParam<int>(1).getData();
  useEventBase=false;
  if(descr.getNumberOfParam<bool>()>0)useEventBase=descr.getParam<bool>(0).getData();
  if(useEventBase)
    {
      eventBase=10000;
      if(descr.getNumberOfParam<int>()>2)eventBase=descr.getParam<int>(2).getData();
      eventCounter=0;
    }
  outFileName="testCal.data";
  if(descr.getNumberOfParam<string>()>2)outFileName=descr.getParam<string>(2).getData();
...
void ACalibrationGeneration::writeCalibrationToFile(CommonCalibrationParser* parameter)
{
  ...
  out<<endl;
  out.close();
  anaLog<<"Calibration \""<<calibrationNames[parameter->getParamID()]<<"\" for detector-ID-"<<parameter->getDetector()<<" written to file \""<<outFileName<<"\""<<endli;
}
void ACalibrationGeneration::declareCalibs(const algorithm_parameter &descr)
{
  ...
  for(int i=0;i<numberOfCalibrationAlgorithms;i++)calibrationAlgorithms[i]=NULL;
  ...
  for(int i=0;i<numberOfCalibrationAlgorithms;i++)
      ...
      if(calibrationAlgorithms[algoNum-1]!=NULL)
	{
 	  ...
 	  connect(calibrationAlgorithms[algoNum-1],SIGNAL(evaluated(CommonCalibrationParser*)),this,SLOT(writeCalibrationToFile(CommonCalibrationParser*)));
 	}
    }
}
