...
ATofPixCorrection::ATofPixCorrection(int &eventNr, int &runNr, TTrack ** tracksIn, int &numberOfTracksIn, TSetup& setup, const algorithm_parameter &descr, const vector<string> &calibrationFiles):AAlgorithm("TDC correction pixel wise"), numberOfTracks(numberOfTracksIn), numPixelTypes(descr.getParam<vector<int> >(0).getData().size()), EventNumber(eventNr), RunNumber(runNr)
{
  tracks=tracksIn;
  ...
  if(descr.getNumberOfParam<bool>() >0) runTime=descr.getParam<bool>(0).getData();
  else runTime=true;
  if(descr.getNumberOfParam<bool>() >1) calib=descr.getParam<bool>(1).getData();
  else calib=true;
//   for(int i=0;i<numPixelTypes;i++)
//     cout<<"light velocity in detector: "<<pixelTypes[i]<<": "<<CinSzinti[i]<<endl;
}
ATofPixCorrection::ATofPixCorrection(int &eventNr, int &runNr, TTrack **tracksIn, int &numberOfTracksIn, TSetup& setup, vector<int> pixelTypesIn, const vector<string> &calibrationFiles):AAlgorithm("TDC correction pixel wise"), numberOfTracks(numberOfTracksIn), numPixelTypes(pixelTypesIn.size()), EventNumber(eventNr), RunNumber(runNr)
{
  tracks=tracksIn;
  pixelTypes=new int[numPixelTypes];
  numRanges7=new int[numPixelTypes];
...
{
  if(track->getNumberOfCalibHits(0)>0&&track->getNumberOfCalibHits(1)>0&&track->getNumberOfCalibHits(2)>0)
    {
      tmpPixel.reset();
  ...
  else if(track->getNumberOfCalibHits(3)>0&&track->getNumberOfCalibHits(4)>0&&track->getNumberOfCalibHits(5)>0)
    {
      tmpPixel.reset();
...
{
  if(calib)
    if(!checkRanges()){cout<<"Range not found!!! run: "<<RunNumber<<" event: "<<EventNumber<<endl;return ptr;}
  int pt;
  int pixpos;
  int left=0,right=0;
  TPixel *pix;
      ...
      for(int j=0;j<tracks[i]->getNumberOfPixels();j++)
	{
	  ...
	  for(int k=0;k<pix->getNumberOfActualElements();k++)
		...
		if(pix->getElementr(k).getDetector()==pixelTypes[l])
	      ...
	      if(calib)
		tdc+=(calibrations7[pt][actualRange7[pt]]->getParameter(pix->getElementr(k).getElement(),pixpos)+
		  (use9[pt]?
		   calibrations9[pt][actualRange9[pt]]->getParameter(pix->getElementr(k).getElement(),pixpos):
		   0))/10.;
	      if(runTime)
...
bool ATofPixCorrection::checkRanges()
{
  ...
  for(int i=0;i<numPixelTypes;i++)
    {
      if(actualRange7[i]<0||numRanges7[i]<=actualRange7[i])return false;
      ...
      for(int j=0;j<numRanges7[i];j++)
	{
      ...
      actualRange7[i]=num;
      if(!use9[i])continue;
      if((actualRange9[i]<0||numRanges9[i]<=actualRange9[i]))return false;
...
{
  if(calibrationFiles.size()==0)return;
