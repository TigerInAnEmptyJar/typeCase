...
ATrackTreeInput::ATrackTreeInput(int &evtNrIn, int &runNrIn, int &triIn, TTrack **trackIn, int &numTrackIn, int maxTrackIn, int numDetsIn, int **numHitsIn, TCalibHit ***hitIn, int **numPixIn, TPixel ***PixIn, bool &readInValid, const algorithm_parameter &param):AAlgorithm("Track tree input"),numberOfTracks(numTrackIn),maxtracks(maxTrackIn),numDets(numDetsIn),valid(readInValid),eventNumber(evtNrIn),runNumber(runNrIn),trigger(triIn)
{
  valid=false;
  trackTree=NULL;
  currentEntry=0;
...
void *ATrackTreeInput::process(void* ptr)
{
  ...
  numberOfTracks=0;
  if(trackTree->GetEntries()<=currentEntry)return ptr;
  if(currentEntry<0)return ptr;
  if(searchForEvent)
...
void ATrackTreeInput::prepareNextEntry()
{
  if(doPattern)
	...
	{
	  useEvent=false;
	  for(int i=0;i<3;i++)numbers[i]->GetBranch()->GetEntry(currentEntry);
	  bool patterntrue;
	  for(int i=0;i<numPattern;i++)
	      ...
	      for(int j=0;j<3;j++)
		{
	      ...
	      if(patterntrue)useEvent=true;
	    }
	  if(!useEvent)
	    {
// 	      if(numbers[0]->GetValue()==2&&numbers[1]->GetValue()==0&&numbers[2]->GetValue()==1)
// 		cout<<"skip event "<<numbers[0]->GetValue()<<" "<<numbers[1]->GetValue()<<" "<<numbers[2]->GetValue()<<" "<<currentEntry<<endl;
	      currentEntry++;
	    }
	}
...
void ATrackTreeInput::readEvent(int EvtNr)
{
  ...
  if(header[0]==NULL||header[1]==NULL||header[2]==NULL||
     numbers[0]==NULL||numbers[1]==NULL||numbers[2]==NULL)return;
  numberOfTracks=0;
  header[0]->GetBranch()->GetEntry(currentEntry);
  ...
  if(header[0]->GetValue()<EvtNr&&trackTree->GetEntries()>currentEntry+1)
    {
      readWriteLog<<"ATrackTreeInput: Event not found: EventNumber "<<EvtNr<<" last Entry "<<currentEntry<<endli;
      return;
    }
  else{
...
void ATrackTreeInput::readEntry(int entry)
{
  ...
  if(entry<0||entry>=trackTree->GetEntries())
    {valid=false;return;}
  for(int i=0;i<14;i++)if(prompt[i]==NULL)return;
  for(int i=0;i<22;i++)if(vee[i]==NULL)return;
  for(int i=0;i<25;i++)if(kink[i]==NULL)return;
  for(int i=0;i<3;i++)if(header[i]==NULL)return;
  for(int i=0;i<3;i++)if(numbers[i]==NULL)return;
...
//     cout<<"bad! event "<<numbers[0]->GetValue()<<" "<<numbers[1]->GetValue()<<" "<<numbers[2]->GetValue()<<" "<<currentEntry<<endl;
//   if(numbers[0]->GetValue()+numbers[2]->GetValue()*3!=numberOfTracks)
//     cout<<"bad event "<<numbers[0]->GetValue()<<" "<<numbers[1]->GetValue()<<" "<<numbers[2]->GetValue()<<" "<<currentEntry<<" "<<numberOfTracks<<endl;
  ...
  for(int j=0;j<nel;j++)
    {
      ...
      if(d<0||d>=numDets)continue;
      for(int l=0;l<(*(numberOfCalibHits[d]));l++)
	{
	  if(hits[d][l]->getDetector()==d&&
	     hits[d][l]->getElement() ==e
	     &&hits[d][l]->isValid())
	    tmpHit=hits[d][l];
	}
      if(tmpHit!=NULL)
	...
	{
// 	  cerr<<"ERROR: hit not found in ATrackTreeInput! ("<<d<<","<<e<<")"<<endl;
// 	  for(int l=0;l<(*(numberOfCalibHits[d]));l++)
// 	    {
// 	      cout<<*hits[d][l]<<endl;
// 	    }
  ...
  for(int i=0;i<numDets;i++)
      ...
      for(int j=0;j<(*(numberOfPixels[i]));j++)
	{
	  ...
	  for(int k=0;k<pixels[i][j]->getNumberOfActualElements();k++)
	    {
	      has=false;
	      for(int l=0;l<tr->getNumberOfCalibHits(pixels[i][j]->getElementr(k).getDetector());l++)
		if(tr->getCalibSrcr(pixels[i][j]->getElementr(k).getDetector(),l)->getElement()==pixels[i][j]->getElementr(k).getElement())
		  has=true;
	      if(!has)take=false;
	    }
	  if(take)
...
void ATrackTreeInput::readPrompt()
{
  ...
  for(int i=0;i<numbers[0]->GetValue();i++)
    {
      if(numberOfTracks>=maxtracks)continue;
      ...
      assignHitsToTrack(prompt[9],prompt[10],tracks[numberOfTracks],(int)prompt[8]->GetValue(i),ntmp);
	 ...
	 ((tracks[numberOfTracks]->getNumberOfCalibHits(0)>0&&
	  ...
	  (tracks[numberOfTracks]->getNumberOfCalibHits(6)>0&&
	   tracks[numberOfTracks]->getNumberOfCalibHits(7)>0))))
...
void ATrackTreeInput::readVee()
{
  ...
  for(int i=0;i<numbers[2]->GetValue();i++)
    {
      if(numberOfTracks+2>=maxtracks)continue;
      tracks[numberOfTracks]->reset();
      ...
      v1.setValues(sin(vee[0]->GetValue(i))*cos(vee[1]->GetValue(i)),
		  sin(vee[0]->GetValue(i))*sin(vee[1]->GetValue(i)),
		  cos(vee[0]->GetValue(i)));
      v2.setValues(sin(vee[3]->GetValue(i))*cos(vee[4]->GetValue(i)),
		  sin(vee[3]->GetValue(i))*sin(vee[4]->GetValue(i)),
		  cos(vee[3]->GetValue(i)));
      ...
      assignHitsToTrack(vee[14],vee[15],tracks[numberOfTracks+1],(int)vee[12]->GetValue(i),ntmp);
      ntmp=ntmp+(int)vee[12]->GetValue(i);
      assignHitsToTrack(vee[14],vee[15],tracks[numberOfTracks+2],(int)vee[13]->GetValue(i),ntmp);
...
void ATrackTreeInput::readKink()
{
  ...
  for(int i=0;i<numbers[1]->GetValue();i++)
    {
      if(numberOfTracks+1>=maxtracks)continue;
      tracks[numberOfTracks]->reset();
      tracks[numberOfTracks+1]->reset();
      ...
      v1.setValues(sin(kink[0]->GetValue(i))*cos(kink[1]->GetValue(i)),
		  sin(kink[0]->GetValue(i))*sin(kink[1]->GetValue(i)),
		  cos(kink[0]->GetValue(i)));
      v2.setValues(sin(kink[3]->GetValue(i))*cos(kink[4]->GetValue(i)),
		  sin(kink[3]->GetValue(i))*sin(kink[4]->GetValue(i)),
		  cos(kink[3]->GetValue(i)));
...
void ATrackTreeInput::clearTree()
{
  ...
  currentEntry=-1;
  for(int i=0;i<14;i++)prompt[i]=NULL;
  for(int i=0;i<22;i++)vee[i]=NULL;
  for(int i=0;i<25;i++)kink[i]=NULL;
  for(int i=0;i<3;i++)header[i]=NULL;
  for(int i=0;i<6;i++)numbers[i]=NULL;
  valid=false;
  if(localDirectory)
    {
      int ret;
...
void ATrackTreeInput::onNewRun(run_parameter &run)
{
	  ...
	  while(tmps.find("/")<tmps.npos&&tmps.rfind("/")>0)
	    {
	      ...
	      if(tmps.length()<1){tmps=parentdirs.back();parentdirs.pop_back();}
	    }
  ...
  readWriteLog<<"Open file \""<<filename.data()<<"\" for track tree input with "<<(int)trackTree->GetEntries()<<" Entries"<<endli;
  ...
  bool ready=true;
  for(int i=0;i<14;i++)if(prompt[i]==NULL)ready=false;
  for(int i=0;i<22;i++)if(vee[i]==NULL)ready=false;
  for(int i=0;i<25;i++)if(kink[i]==NULL)ready=false;
  for(int i=0;i<3;i++)if(header[i]==NULL)ready=false;
  for(int i=0;i<3;i++)if(numbers[i]==NULL)ready=false;
  valid=ready;
  currentEntry=0;
}
