...
ALineFitAlgorithm::ALineFitAlgorithm(int max):AFitAlgorithm("line fit algorithm"),
  maxLines(max), numLines(0), re(4)
{
  A_=new Vector[maxLines];
  for(int i=0;i<maxLines;i++)A_[i].reDimension(4) ;
...
void *ALineFitAlgorithm::process(void*ptr)
{
  return ptr;
}

void ALineFitAlgorithm::clear()
{
  numLines=0;
}
void ALineFitAlgorithm::add(Vector &data)
{
  if((numLines==maxLines)||(data.getDimension()<5))return;
...
Vector ALineFitAlgorithm::getResult()
{
  return re;
}
...
float ALineFitAlgorithm::fit()
{
  ...
  A_1=-A_1;
  //  cout<<endl;
  if(A_1.Lines()==0) return 100000000;
  ret=(A_1)*(At*b);
  re=ret;
  double chi2 = 0;
  //  double tmp;
...
// 	chi2 += tmp * tmp / abs(A.Columns () - A.Lines ());//strange normalization
...
//       sLine3D ml(sh->getCorner()+sh->getDirection(1)*0.5+sh->getDirection(2)*0.5,sh->getDirection(0));
//       atm=ml-coord;
