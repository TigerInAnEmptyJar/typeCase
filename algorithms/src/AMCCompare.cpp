...
AMCCompare::AMCCompare(TEvent &eventIn, string inputFile, string outFileName, string reaction):AAlgorithm("MC-Compare"), event(eventIn)
{
  outFile=outFileName;
  ...
  for(int i=0;i<NumberOfTracks;i++)
    {
      ...
      MCTree->Branch(s.ascii(),&MCTracks[i],"phi/D:theta:beta:ID/I:nEl:ss[17]/I:qdc[17]/F:tdc[17]/F:vertex[3]/D:phiIn/D:thetaIn:betaIn:nElIn/I:ssIn[17]/I:vertexIn[3]/D:momentum/D");
      eventTree->Branch(s.ascii(),&Tracks[i],"phi/D:theta:beta:ID/I:nEl:ss[17]/I:qdc[17]/F:tdc[17]/F:vertex[3]/D:phiIn/D:thetaIn:betaIn:nElIn/I:ssIn[17]/I:vertexIn[3]/D:momentum/D");
    }
  ifstream in;
...
{
  if(event->getNumberOfReactions()<1)return ptr;
  if(event->getReaction(0)->IsA()!=reactiontype::IsA())return ptr;
  setEvent();
...
{
  if(treename==MCTree->GetName())return MCTree;
  if(treename==eventTree->GetName())return eventTree;
  return NULL;
}
void AMCCompare::readMCEvent(istream &input)
{
  ...
  for(int i=0;i<NumberOfTracks;i++)done[i]=false;
  float f[12];
  in>>n;
  for(int i=0;i<n;i++)
    {
      in>>m[0]>>m[1]>>f[0]>>f[1]>>f[2]>>f[3]>>f[4]>>f[5]>>f[6]>>f[7]>>f[8]>>f[9]>>f[10]>>f[11]>>m[2];
      pos=-1;
      int j=0;
...
int AMCCompare::setMCEvent(int eventNumber)
{
  ...
  if(entry>MCTree->GetEntries())entry=MCTree->GetEntries()-1;
  ...
  if(EventNumberMC!=eventNumber)return -1;
  for(int i=0;i<NumberOfTracks;i++)
    {
