...
AtdcRadialPolCalibration::AtdcRadialPolCalibration(TEvent &eventIn, TSetup &setup, algorithm_parameter & descr):AAlgorithm("Generate tdcRadialPol calibration"), event(eventIn), minEntriesPerHisto(descr.getParam<int>(0).getData())
{
  postScriptHistos=false;
  if(descr.getNumberOfParam<bool>()>0)postScriptHistos=descr.getParam<bool>(0).getData();
  ...
  if(descr.getNumberOfParam<int>()>3)ymin=descr.getParam<int>(3).getData();
  if(descr.getNumberOfParam<int>()>4)ymax=descr.getParam<int>(4).getData();
  eventStart=0;
  string nn;
  ...
  for(int i=0;i<numberOfDetectors;i++)
	...
	{
	  nn=(string("tdc-difference: detID-")+string_number(detectorIDs[i])+" el-"+string_number(j));
	  qdcHistograms[i][j]=new /*TProfile(nn.data(),nn.data(),100,xmin,xmax);/*/TH2F(nn.data(),nn.data(),100,xmin,xmax,100,ymin,ymax);
	}
    }
...
// 	{
// 	  nn=(string("tdc-difference: detID-")+string_number(detectorIDs[i])+" el-"+string_number(j));
// 	  qdcHistograms1[i][j]=new TH2F(nn.data(),nn.data(),100,xmin,xmax,100,ymin,ymax);
// 	}
//     }
...
AtdcRadialPolCalibration::AtdcRadialPolCalibration(TEvent &eventIn, TSetup &setup, vector<int> &detectors, vector<int> &Stdetectors,vector<int> readSide, vector<string> reactionNames, int minEntriesPerHistoIn):AAlgorithm("Generate tdcRadialPix calibration"), event(eventIn), minEntriesPerHisto(minEntriesPerHistoIn)
{
  ...
  for(int i=0;i<numberOfDetectors;i++)
      ...
      else
	{
      ...
      for(int j=0;j<numberOfElements[i];j++)
	{
	  nn=(string("tdc-difference: detID-")+string_number(detectorIDs[i])+" el-"+string_number(j)).data();
...
void* AtdcRadialPolCalibration::process(void *ptr)
{
  ...
  //cout<<"AtdcRadialPolCalibration::process(void *ptr) "<<(event.getReaction()->name()=="Rppelastic"?"1a":(event.getReaction()->name()=="Rpp_dpiPlus"?"2a":"no"))<<endl;
  //check for reactions:
  int found=-1;
  for(int i=0;i<numberOfReactions;i++)
  ...
  for(int i=0;i<react->getNumberOfTracks();i++)
    {
      ...
      ints[2]=i;
#endif
      if(react->name()=="Rpp_dpiPlus"&&i==0)continue;
      ...
      nh=0;
      //      for(int j=0;j<25;j++)cout<<track->getNumberOfCalibHits(j)<<" "<<flush;
      //      cout<<endl;
      for(int j=0;j<numberOfStartDetectors;j++)
	{
	  //	  cout<<startDetectors[j]<<" "<<track->getNumberOfCalibHits(startDetectors[j])<<flush;
      ...
      for(int j=0;j<numberOfDetectors;j++)
	{
	  ...
	  if(track->getNumberOfCalibHits(detectorIDs[j])<=0)continue;
  ...
  //  cout<<"next"<<endl;
  //  cout<<event.getRunNumber()<<"-"<<event.getEventNumber()<<": "<<tree->GetEntries()<<endl;
  return ptr;
}
...
//   string s;
  for(int i=0;i<numberOfElements[num];i++)
    {
      prof[i]=/*(TProfile*)qdcHistograms[num][i];/*/((TH2*)qdcHistograms[num][i])->ProfileX();
//       s=(string("tmpH-")+string_number(num)+"-"+string_number(i));
//       hists[i]=new TH1F(s.data(),s.data(),qdcHistograms[num][i]->GetNbinsX(),qdcHistograms[num][i]->GetXaxis()->GetXmin(),qdcHistograms[num][i]->GetXaxis()->GetXmax());
  ...
  if(postScriptHistos)
    {
      ...
      canv->Print((fn+"[").data());
      //TPostScript *ps=new TPostScript((string("histosDet-")+string_number(detectorIDs[num])+".ps").data(),112);
      int canvasCount=0;
      for(int i=0;i<numberOfElements[num];i++)
	{
	  if(canvasCount==12)
