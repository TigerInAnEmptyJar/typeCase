...
#include <stdlib.h>
AVertexFit::AVertexFit(int numberOflines)
    : AFitAlgorithm("vertex fit algorithm"), maxLines(numberOflines), numLines1(0), numLines2(0),
      re(7), re1(4), re2(4)
{
...
void AVertexFit::add(Vector& data)
{
  ...
  if ((num == 0 && numLines1 == maxLines) || (num == 1 && numLines2 == maxLines))
    return;
  if (num == 0) {
...
Vector AVertexFit::getResult() { return re; }
void AVertexFit::getAddPoint(Vector& data) { add(data); }

float AVertexFit::fit()
{
  if (numLines1 < 4 || numLines2 < 4)
    return -1;
