...
Q_OBJECT
 private:
  ...
  bool calib;
 public:
  ATofPixCorrection(int &eventNr, int &runNr, TTrack **tracksIn, int &numberOfTracksIn, TSetup& setup, const algorithm_parameter &descr, const vector<string> &calibrationFiles);
  ATofPixCorrection(int &eventNr, int &runNr, TTrack **tracksIn, int &numberOfTracksIn, TSetup& setup, vector<int> pixelTypes, const vector<string> &calibrationFiles);
