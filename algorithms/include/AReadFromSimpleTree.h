...
class AReadFromSimpleTree:public AAlgorithm
{
 ...
 private:
  ...
  void *inputmutex;//!
  int searchDelayedEvent(TTree** thetree,int numTree, int eventNumber, int* theCounter, int acounter);
  ...
  void cleanCluster();
 public:
  AReadFromSimpleTree(const string &fileName, const string &treename, const string &localDirec, bool useLocalD, TTrack **tracksIn, int &numberOfTracksIn, int&eventNumberIn, int maxTrackIn, TEvent &eventIn,int &maxEventsToRead,TSetup *setupIn,bool readT, bool readC, bool readP, bool readH,string TrackFile,string ClusterFile,string PixelFile, string HitFile, bool resetUnreadIn, bool startWithDelayedEventIn, int StartWithEventIn, bool &validInputIn, void* inputmutexIn);
