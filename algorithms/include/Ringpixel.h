...
class ARingpixel: public AAlgorithm
{
 private:
	...
	vector3D zeroPhiDir;//!
 public:
	ARingpixel(bool ParallelprojectionIn, bool SpecialprojectionplaneIn, bool OriginforprojectionpointIn, bool TargetforprojectionpointIn, bool UsetimedifferenceIn, bool CorrectlightflighttimeIn, bool TriangleapproximationIn, point3D projectionpointIn, point3D pointofprojectionplaneIn, vector3D normaltoprojectionplaneIn, TDetector& detS, TDetector& detL, TDetector& detR, int minimumdeltaphiIn, int maximumdeltaphiIn, int minimumnumberofelementsIn, int numberOfSpiralSpiralCrossingsIn, int pixelIDIn, float maxChiIn, float maxDeltaTIn, int *NumberOfPixelsIn, TCalibHit**straightE, TCalibHit**leftE,TCalibHit**rightE, int& nStraight, int &nLeft, int &nRight, TPixel** PixelsIn, int maxOfPixelsIn);
	virtual ~ARingpixel();
