...
class ATDCcalibration:public AAlgorithm
{
  ...
  void defineWalkStuff(bool justClean,bool redefine, bool mkehistos,int histoptt, bool mkefunctions, float rngmod);
  void defineOffsetStuff(bool justClean,bool redefine,bool mkehistos, bool mkeparser, float rngmod);
  float thMax(int id);
  float thMin(int id);
  ...
  float getTDC(int evtnr,int detectorNumber, int elementNumber, float tdc, float qdc, float path, int corr/*float **offsets, float **offsetSlope*/, bool hwalk, bool hoff, bool hlrt);
  void findRanges();
  ...
  void getProfileFit(TH2*histo,const string &functionName, int parnum, 
 ...
 public:
  ATDCcalibration(int &evtnr, int &rnr, TTrack **trIn, TPixel ***pixIn, TCalibHit*** hitIn, int &numTrack, int **numPix, int **numHits, TSetup &setin,const algorithm_parameter &descr);
  ~ATDCcalibration();
