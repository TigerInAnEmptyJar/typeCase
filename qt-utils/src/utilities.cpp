...
string getOpenFileName(QWidget *parent, const string &caption, const string &startWith, const string &filter, string *selectedFilter)
{
  QString sf, fn;
#if QT_VERSION < 0x040000
  fn=QFileDialog::getOpenFileName(startWith.data(), filter.data(),parent , 0, caption.data(), &sf);
...
string getSaveFileName(QWidget *parent, const string &caption, const string &startWith, const string &filter, string *selectedFilter)
{
  QString sf, fn;
#if QT_VERSION < 0x040000
  fn=QFileDialog::getSaveFileName(startWith.data(), filter.data(),parent , 0, caption.data(), &sf);
  if(selectedFilter!=NULL)
    *selectedFilter=sf.ascii();
  if(fn=="")return "";
  return fn.ascii();
...
string getSaveFileName(QWidget *parent, const string &caption, const string &startWith, const string &filter, bool reAsk, string *selectedFilter)
{
  ...
  if(!reAsk||s==""||!existing(s))return s;
  int ret=QMessageBox::No;
  while(existing(s)&&ret!=QMessageBox::Yes)
    {
      ret=QMessageBox::warning(parent,"File exists",(string("File \"")+s+"\" exists.\nOverwrite?").data(),QMessageBox::Yes|QMessageBox::Default,QMessageBox::No,QMessageBox::Cancel|QMessageBox::Escape);
...
vector<string> getOpenFileNames(QWidget *parent, const string &caption, const string &startWith, const string &filter, string *selectedFilter)
{
  QString sf;
  QStringList fn;
  vector<string> ret;
#if QT_VERSION < 0x040000
  fn=QFileDialog::getOpenFileNames(startWith.data(), filter.data(),parent , 0, caption.data(), &sf);
  if(selectedFilter!=NULL)
    *selectedFilter=sf.ascii();
...
{if(c!=NULL)c->insertItem(input);}
#endif
#ifdef QLISTWIDGET_H
void add2ListBox(QListWidget* c, const QString &input)
{if(c!=NULL)c->addItem(input);}
#endif
