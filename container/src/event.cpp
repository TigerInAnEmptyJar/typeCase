...
#include <typeinfo>
TEvent::TEvent():TBase("TEvent"),maxHit(0), maxPixel(0), maxCluster(0),maxTrack(0),maxDet(0),maxReactions(0)
{
  hits=NULL;
...
TEvent::~TEvent()
{
  ...
  for(int i=0;i<maxDet;i++)
    ...
    delete []numberOfHitCluster;
    for(int i=0;i<maxReactions;i++)
      if(reactions[i]!=NULL)delete reactions[i];
    delete []reactions;
}

void TEvent::setEventNumber(int evtNr)
{
    eventNumber=evtNr;
}

void TEvent::setRunNumber(int RnNr)
{
    runNumber=RnNr;
}
...
template<class X>int TEvent::getMaxNumber()
{
  if(typeid(X)==typeid(TRawHit)||typeid(X)==typeid(TCalibHit)||typeid(X)==typeid(THitCluster))
    return maxHit;
...
template<class X>int TEvent::getNumberOf(int num)
{
...
template<class X>int& TEvent::numberOf(int num)
{
  if(typeid(X)==typeid(TRawHit)||typeid(X)==typeid(TCalibHit))
    return getNumberOfHits(num);
  if(typeid(X)==typeid(TPixel))
...
//   if(det==-2) return ret;
//   ret=-1;
  if((det<0)||(det>maxDet)) return numberOfHitCluster[0];
...
//   if(det==-2) return ret;
//   ret=-1;
  if((det<0)||(det>maxDet)) return numberOfHits[0];
...
//   if(det==-1)return ret;
//   ret=-1;
  if((det<0)||(det>maxDet)) return numberOfCluster[0];
...
//   if(det==-1)return ret;
//   ret=-1;
  if((det<0)||(det>=maxDet)) return numberOfPixels[0];
...
int &TEvent::getNumberOfTracks()
{
    return numberOfTracks;
}

int& TEvent::getEventNumber()
{
    return eventNumber;
}

int &TEvent::getRunNumber()
{
    return runNumber;
}
...
int &TEvent::getTrigger()
{
  return trigger;
}

vector3D &TEvent::getPolarisation()
{
  return polarisation;
}
...
{
  if(r==NULL)return;
  if(numReactions==maxReactions)return;
  if(numReactions==0)
    {
      reactions[0]=r;
    }
...
{
  if(num<0||num>=numReactions)return NULL;
  return (reactions[num]);
}
int TEvent::getNumberOfReactions()
{
  return numReactions;
}
