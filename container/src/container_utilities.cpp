...
vector3D getPath(int nstart, int* start, int nstop, int* stop, int nhits, TCalibHit** hits,
                 const sLine3D& path)
{
  if (start == NULL || stop == NULL)
    return vector3D();
...
vector3D getPath(int nstart, TCalibHit** start, int nstop, TCalibHit** stop, const sLine3D& path)
{
  ...
  int ns = 1, nt = 1;
  vector3D sa, so;
  sa.setValues(0, 0, 1e20);
  so.setValues(0, 0, 1e20);
  vector3D tmp;
  for (int i = 0; i < nstart; i++) {
    if (start[i] == NULL)
      continue;
    if (start[i]->getHitShape() == NULL)
      continue;
