...
THitCluster::THitCluster(const THitCluster& cls)
    : TCalibHit((const TCalibHit&)cls), numElem(cls.getMaxNumberOfHits())
{
  accElem = 0;
...
THitCluster::~THitCluster() { delete[] hits; }
int THitCluster::getNumberOfHits() const { return accElem; }
int THitCluster::getMaxNumberOfHits() const { return numElem; }
...
THitCluster& THitCluster::operator=(const THitCluster& hc)
{
  ...
  for (int i = 0; i < hc.getNumberOfHits(); i++)
    addHit(hc.getHit(i));
  return *this;
}
