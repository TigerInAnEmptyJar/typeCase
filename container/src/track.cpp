...
TTrack::TTrack(int max, int dets)
    : TBase("TTrack"), maxhits(max), maxDets(dets), particle(0, 0, 0, 0)
{
  defined = false;
  ...
  for (int i = 0; i < maxDets; i++)
    calibSrc[i] = new TCalibHit*[maxhits];
...
void TTrack::addCalibSrc(TCalibHit* calibIn)
{
  ...
  if (!(calibIn->isValid()))
    return;
  if (calibIn->getDetector() >= maxDets)
    return;
  if ((numhits[calibIn->getDetector()] > maxhits - 1))
    return;
  defined = true;
  bool isIn = false;
...
TRawHit& TTrack::getRawSrcr(int det, int num)
{
  ...
  return calibSrc[det][num]
      ->getRaw(); //((TCalibHit*)calibSrc[det].At(num))->getRaw();//.GetObject())->getRaw();
}
...
{
  o << "(" << t.getPath().Direction().Theta() << ", " << t.getPath().Direction().Phi()
    << "),beta: " << t.getSpeed() << " Vertex " << t.getPath().Foot() << endl;
  for (int i = 0; i < t.getNumberOfCalibHits(-2); i++) {
    for (int j = 0; j < t.getNumberOfCalibHits(i); j++) {
...
sLine3D TTrack::getPath() const { return path; }
TCalibHit TTrack::getCalibSrc(int det, int num) const
{
  if ((det < 0) || (det >= maxDets))
    return TCalibHit();
  if ((num < 0) || (num >= numhits[det]))
    return TCalibHit();
...
void TTrack::removeCalibSrc(TCalibHit* calibIn)
{
  if (calibIn == NULL)
    return;
  if (calibIn->getDetector() < 0 || calibIn->getDetector() >= maxDets)
    return;
  if (numhits[calibIn->getDetector()] <= 0)
    return;
...
momentum4D TTrack::getParticle() const { return particle; }
...
bool TTrack::isDefined() { return defined; }
...
int TTrack::getNumberOfPixels() const { return numPix; }
...
float TTrack::getChiSquared() { return chiQ; }

float& TTrack::getChiSquaredr() { return chiQ; }

void TTrack::setChiSquared(float chi) { chiQ = chi; }
bool TTrack::isValid() const { return valid; }

void TTrack::setValid(bool val) { valid = val; }

float TTrack::getKineticEnergy() const { return kineticEnergy; }

void TTrack::setKineticEnergy(float val) { kineticEnergy = val; }

float TTrack::getSpeed() const { return speed; }

void TTrack::setSpeed(float val) { speed = val; }
void TTrack::reset()
{
  clearCalibSrc();
  clearPixel();
...
bool TTrack::isSecondary() const { return order; }

void TTrack::setOrder(bool o) { order = o; }
int TTrack::getNumberOfSecondaryTracks() const { return numSec; }
...
int TTrack::getCharge() const { return charge; }
int& TTrack::getCharger() { return charge; }
void TTrack::setCharge(int c) { charge = c; }
