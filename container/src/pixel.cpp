...
int TPixel::getNumberOfElements() const { return numDet; }

int TPixel::getNumberOfActualElements() const { return accNum; }
...
bool TPixel::isDefined() { return defined; }

int TPixel::getID() const { return ID; }

void TPixel::setID(int IDIn) { ID = IDIn; }

float TPixel::getMaxChiSquared() const { return maxChi; }

void TPixel::setMaxChiSquared(float value) { maxChi = value; }
void TPixel::reset()
{
  valid = false;
...
{
  if (accNum != p.getNumberOfActualElements())
    return false;
  if (ID != p.getID())
    return false;
  bool tmp[accNum];
  for (int i = 0; i < accNum; i++) {
    ...
    for (int j = 0; j < accNum; j++) {
      if ((elements[(i)]) /*.GetObject()*/ == &p.getElementr(j))
        tmp[i] = true;
    }
  }
  for (int i = 0; i < accNum; i++)
...
bool TPixel::isValid() const { return valid; }

void TPixel::setValid(bool v) { valid = v; }
