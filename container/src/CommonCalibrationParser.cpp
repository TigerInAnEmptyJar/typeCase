...
CommonCalibrationParser::CommonCalibrationParser(int nE, int ID): numElements(nE),DetID(ID)
{
  ...
  for(int i=0;i<numElements;i++)parameter[i]=new float[numParameters];
  range=new int[4];
  for(int i=0;i<4;i++)range[i]=-1;
  paramID=-1;
  valid=false;
}
...
{
  for(int i=0;i<numElements;i++) delete []parameter[i];
  delete []parameter;
  delete []range;
}
...
bool CommonCalibrationParser::isValid()const
{
  return valid;
}
void CommonCalibrationParser::setValid(bool value)
{
  valid=value;
}
...
int CommonCalibrationParser::getNumParameters()const
{
  return numParameters;
}

int CommonCalibrationParser::getNumElements()const
{
  return numElements;
}

int CommonCalibrationParser::getDetector()const
{
  return DetID;
}
...
{
  if((!valid)||(num>3)) return -1;
  if((num==0||num==2)&&range[num]==-1)return 0;
  if(range[num]==-1)return 9999999;
  return range[num];
}
...
{
  if(!valid) return -1;
  if(element>=numElements) return -1;
  if(num>=numParameters) return -1;
  return parameter[element][num];
}

int CommonCalibrationParser::getParamID()const
{
  return paramID;
}
...
{
  if(DetID!=pars.getDetector())return false;
  if(valid!=pars.isValid())return false;
  if(numElements!=pars.getNumElements())return false;
  if(numParameters!=pars.getNumParameters())return false;
  for(int i=0;i<4;i++)
    {
      if((i==0||i==2)&&range[i]==-1)
	{
	  if(pars.getRange(i)!=0)return false;
	}
      else if(range[i]==-1)
	{
	  if(pars.getRange(i)!= 9999999)return false;
	}
      else if(range[i]!=pars.getRange(i))return false;
    }
//   for(int i=0;i<numElements;i++)
...
void CommonCalibrationParser::write(ostream &output,const string &author,const string &beamTime)
{
  ...
  switch(DetID)
    {
    case 0:det="QUIRL_GERADE";break;
    case 1:det="QUIRL_LINKS";break;
    case 2:det="QUIRL_RECHTS";break;
    case 3:det="RING_GERADE";break;
    case 4:det="RING_LINKS";break;
    case 5:det="RING_RECHTS";break;
    case 6:det="BARREL_VORNE";break;
    case 7:det="BARREL_HINTEN";break;
    case 15:det="TORTE_A";break;
    case 16:det="TORTE_B";break;
    case 17:det="HODO_X";break;
    case 18:det="HODO_Y";break;
    case 19:det="LARGE_HODO_X";break;
    case 20:det="LARGE_HODO_Y";break;
    case 21:det="DS_MIKRO_STRIP_RADIAL";break;
    case 22:det="DS_MIKRO_STRIP_PHI";break;
    case 23:det="HODO_D";break;
    case 25:det="CALORIMETER";break;
    }
  switch(paramID)
    {
    case 0:param="PEDESTAL";break;
    case 1:param="TdcFactor";break;
    case 2:param="WALK";break;
    case 3:param="CUTS";break;
    case 4:param="QdcFactor";break;
    case 5:param="TdcOffset";break;
    case 6:param="zBarrel";break;
    case 7:param="tdcRadialPix";break;
    case 8:param="tdcRadialPol";break;
    case 9:param="tdcRadialSpPix";break;
    case 10:param="tdcRadialSpPol";break;
    case 11:param="qdcRadialPix";break;
    case 12:param="qdcRadialPol";break;
    case 13:param="tdcBarrelAdd";break;
    }
  output<<"==>               "<<det.data()<<" "<<DetID<<" "<<numElements<<"  "<<param.data()<<"  "<<paramID<<"  "<<numParameters<<endl;
  output<<endl;
  output<<"#---------------------------------------------------"<<endl;
...
int CommonCalibrationParser::getCalibrationTypesInFile(string filename, vector<int> &pIDs, int detID)
{
  vector<int> v1, v2;
  getCalibrationTypesInFile(filename,v1,v2);
...
int CommonCalibrationParser::getCalibrationTypesInFile(string filename, vector<int> &pIDs, vector<int> &dets)
{
  ifstream input;
  ...
  while(input.good()&&!input.eof())
      ...
      while(input.good()&&!input.eof()&&!(hasA(line,"==>   ")||hasA(line,"-->  ")))
	{
      ...
      if(!(input.good()&&!input.eof()))return pIDs.size();
      c=' ';
      while(c==' '||c=='\t')input.get(c);
      while(c!=' '&&c!='\t')input.get(c);
      input>>n1>>n2;
      input.get(c);
      line=c;
...
void CommonCalibrationParser::setParameter(int element, int num, float value)
{
  ...
  if(num<0||num>=numParameters)return;
...
void CommonCalibrationParser::setParamID(int value)
{
  paramID=value;
}
...
void CommonCalibrationParser::setDetID(int value)
{
  DetID=value;
}
int CommonCalibrationParser::getFileContent(string filename, vector<CommonCalibrationParser*> &content)
{
  ifstream input;
  input.open(filename.data());
  char li[1000],c;
  ...
  while(input.good()&&!input.eof())
      ...
      while(input.good()&&!input.eof()&&!(hasA(line,"==>")||hasA(line,"-->")))
	{
      ...
      if(!(input.good()&&!input.eof()))return content.size();
      c=' ';
