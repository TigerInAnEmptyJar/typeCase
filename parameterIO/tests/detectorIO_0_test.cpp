...
/*!
 * \brief ParameterIoTest.detector_write_read_0
 * This test checks whether parameters written to a file can be restored again.
 */
TEST_F(ParameterIoTest, detector_write_read_0)
{
  ...
  std::filesystem::path path{tempPath.path().toStdString() + "/data/detectors_0.data"};
  ...
  io.writeParameter(path, parameters, version_0, ParameterIO::FileType::DETECTOR);
  auto result = io.readParameter(path, version_0, ParameterIO::FileType::DETECTOR);
...
/*!
 * \brief ParameterIoTest.detector_read_0
 ...
 */
TEST_F(ParameterIoTest, detector_read_0)
{
  auto result =
      io.readParameter("data/detectors_0.data", version_0, ParameterIO::FileType::DETECTOR);
  std::vector<std::shared_ptr<base_parameter>> parameters;
...
/*!
 * \brief ParameterIoTest.detector_write_read_1
 * This test checks whether parameters written to a file can be restored again.
 */
TEST_F(ParameterIoTest, detector_write_read_1)
{
  QTemporaryDir tempPath;
  // Adding the intermediate directory "/data/" on purpose: the IO must be able to create the file
  // even if the directory doesn't exist yet.
  std::filesystem::path path{tempPath.path().toStdString() + "/data/detectors_1.data"};
  std::vector<std::shared_ptr<base_parameter>> parameters;
  appendSomeDetectors(parameters);
  // 2 materials, 2 shapes, 2 detectors, 1 reaction
  EXPECT_THAT(parameters, testing::SizeIs(7));

  io.writeParameter(path, parameters, version_1, ParameterIO::FileType::DETECTOR);
  auto result = io.readParameter(path, version_1, ParameterIO::FileType::DETECTOR);
...
/*!
 * \brief ParameterIoTest.detector_read_1
 * This test checks whether a previously written detector file is still legible. This test is
 * different from the test above, since it explicitly checks backward compatibility.
 */
TEST_F(ParameterIoTest, detector_read_1)
{
  auto result =
      io.readParameter("data/detectors_1.data", version_1, ParameterIO::FileType::DETECTOR);
  std::vector<std::shared_ptr<base_parameter>> parameters;
  appendSomeShapes(parameters);
...
/*!
 * \brief ParameterIoTest.detector_write_read_2
 * This test checks whether parameters written to a file can be restored again.
 */
TEST_F(ParameterIoTest, detector_write_read_2)
{
  QTemporaryDir tempPath;
  // Adding the intermediate directory "/data/" on purpose: the IO must be able to create the file
  // even if the directory doesn't exist yet.
  std::filesystem::path path{tempPath.path().toStdString() + "/data/detectors_2.data"};
  std::vector<std::shared_ptr<base_parameter>> parameters;
  appendSomeDetectors(parameters);
  // 2 materials, 2 shapes, 2 detectors, 1 reaction
  EXPECT_THAT(parameters, testing::SizeIs(7));

  io.writeParameter(path, parameters, version_2, ParameterIO::FileType::DETECTOR);
  auto result = io.readParameter(path, version_2, ParameterIO::FileType::DETECTOR);
...
/*! \brief ParameterIoTest.detector_read_2
 * This test checks whether a previously written detector file is still legible. This test is
 * different from the test above, since it explicitly checks backward compatibility.
 */
TEST_F(ParameterIoTest, detector_read_2)
{
  auto result =
      io.readParameter("data/detectors_2.data", version_2, ParameterIO::FileType::DETECTOR);
  std::vector<std::shared_ptr<base_parameter>> parameters;
  appendSomeShapes(parameters);
...
TEST_F(ParameterIoTest, detector_id_recovery_0)
{
  installRecovery();
  QTemporaryDir tempPath;
  std::filesystem::path path{tempPath.path().toStdString() + "/data/detectors_0.data"};
  ...
  ASSERT_THAT(result, testing::SizeIs(3));
  ASSERT_NE(nullptr, std::dynamic_pointer_cast<reaction_parameter>(result[0]));
  ASSERT_NE(nullptr, std::dynamic_pointer_cast<reaction_parameter>(parameters[2]));
  EXPECT_EQ(std::dynamic_pointer_cast<reaction_parameter>(parameters[2])->getTargetShape().id(),
            std::dynamic_pointer_cast<reaction_parameter>(result[0])->getTargetShape().id());
