...
public:
    reaction_parameter(const shape_parameter &target=shape_parameter(), bool twoBeamIn=false, int first=0, int sec=0);
    ~reaction_parameter();
...
template<>bool takeItemFromVector<detector_parameter>(vector<detector_parameter> &vect,detector_parameter &item);
template<>bool moveItemToPosVector<detector_parameter>(vector<detector_parameter>& vect,detector_parameter&item,int pos);
template<>bool takeItemFromVector<reaction_parameter>(vector<reaction_parameter> &vect,reaction_parameter &item);
template<>bool moveItemToPosVector<reaction_parameter>(vector<reaction_parameter>& vect,reaction_parameter&item,int pos);
#endif
