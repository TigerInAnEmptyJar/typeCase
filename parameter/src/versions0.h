  ...
  for(int j=0;j<zahl;j++)
    {
      ...
      a.addParam<vector3D>(single_parameter<vector3D>(string(_tmp),tmpVector));//vector3D(fz1,fz2,fz3)));
    }
  i >> zahl;
  //    cout<<"int:"<<zahl<<endl;
  for(int j=0;j<zahl;j++)
...
void write_algorithm_parameter_0(ostream& o,const algorithm_parameter &a)
{
  ...
  for(int i =0;i<a.getNumberOfParam<algorithm_parameter>();i++)
    o<<a.getParam<algorithm_parameter>(i).getData()<<" "<<(a.getParam<algorithm_parameter>(i).getName().data())<<endl;    
