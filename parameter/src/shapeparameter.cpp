...
shape_parameter::shape_parameter(const shape_parameter& sp):base_parameter(sp), fcompleteWrite(sp.completeWrite())
{
  setName(sp.getName());
  setDescription(sp.getDescription());
  for(int i=0;i<sp.NumberOfParams<point3D>();i++)addParam<point3D>(sp.getParam<point3D>(i),sp.getParamName<point3D>(i));
  for(int i=0;i<sp.NumberOfParams<vector3D>();i++)addParam<vector3D>(sp.getParam<vector3D>(i),sp.getParamName<vector3D>(i));
  for(int i=0;i<sp.NumberOfParams<int>();i++)addParam<int>(sp.getParam<int>(i),sp.getParamName<int>(i));
  for(int i=0;i<sp.NumberOfParams<float>();i++)addParam<float>(sp.getParam<float>(i),sp.getParamName<float>(i));
  for(int i=0;i<sp.NumberOfParams<string>();i++)addParam<string>(sp.getParam<string>(i),sp.getParamName<string>(i));
}
...
template<class T>T shape_parameter::getParam(int i) const 
{
  if(i<0||i>=NumberOfParams<T>())return T();
  const void* pt=0;
...
template<class T>string shape_parameter::getParamName(int i)const
{
  if(i<0||i>=NumberOfParams<T>())return "";
  if(typeid(T)==typeid(int))
...
template<class T>void shape_parameter::setParam(int i, const T &p)
{
  if(i<0||i>=NumberOfParams<T>())return;
  void* pt=0;
  if(typeid(T)==typeid(int))
...
template<class T>void shape_parameter::setParamName(int i,string n)
{
  if(i<0||i>=NumberOfParams<T>())return;
  void* p=0;
  if(typeid(T)==typeid(int))
...
template<class T>void shape_parameter::addParam(T p,string n)
{
  void* pt=0;
  if(typeid(T)==typeid(int))
    pt=&integers;
  ...
  if(typeid(T)==typeid(string))
    pt=&strings_;
  if(pt!=0) ((vector<single_parameter<T> >*)pt)->push_back(single_parameter<T>(n,p));
}

template<class T>int shape_parameter::NumberOfParams() const
{
  if(typeid(T)==typeid(int))
...
void shape_parameter::clearProperties()
{
  ...
  while(!vectors.empty())vectors.pop_back();
  while(!integers.empty())integers.pop_back();
  while(!floats.empty())floats.pop_back();
  while(!strings_.empty())strings_.pop_back();
}
	...
	case 'V':
	  {
		...
		i.unget();
		for(int j=0;j<3;j++)i>>f[j];
		p.setValues(sin(f[0])*cos(f[1])*f[2],
			    sin(f[0])*sin(f[1])*f[2],
			    cos(f[0])*f[2]);
		sh.setCompleteWrite(true);
	      }
...
ostream &operator <<(ostream &o, const shape_parameter &sh)
{
  ...
  if(sh.completeWrite())
	...
	o<<"P"<<sh.getParam<point3D>(k).x()<<" "<<sh.getParam<point3D>(k).y()<<" "<<sh.getParam<point3D>(k).z()<<" "<<sh.getParamName<point3D>(k).data()<<endl;
      for(int k=0;k<sh.NumberOfParams<vector3D>();k++)
	o<<"v"<<sh.getParam<vector3D>(k).Theta()*180./M_PI<<" "<<sh.getParam<vector3D>(k).Phi()*180./M_PI<<" "<<sh.getParam<vector3D>(k).R()<<" "<<sh.getParamName<vector3D>(k).data()<<endl;
