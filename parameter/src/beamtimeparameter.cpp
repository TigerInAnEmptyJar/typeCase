...
beamTime_parameter::beamTime_parameter(string n)
{
    setName(n);
}
...
string beamTime_parameter::getFileName() const
{
    return fileName;
}

void beamTime_parameter::setFileName(string fn)
{
    fileName=fn;
}

string beamTime_parameter::getSetupFile()const
{
    return setupFile;
}

void beamTime_parameter::setSetupFile(string fn)
{
    setupFile=fn;
}
...
{
    if((int)calibFileNames1.size()<=det) return "";
    if((int)calibFileNames1[det].size()<=typ) return "";
...
void beamTime_parameter::addDetectorCalibration(vector<string> fn)
{
    ...
    {
	int sz=fn.size();
	if (sz>(int)calibFileNames1[0].size()) sz=calibFileNames1[0].size();
	for(int i=0;i<sz;i++)
	{
	    tmp.push_back(fn[i]);
	    if(fn[i]=="")
...
{
    if(calibFileNames1.size()==0) return;
...
void beamTime_parameter::addTypCalibration(vector<string> fn)
{
    ...
    {
	int sz=fn.size();
	if(sz>(int)calibFileNames1.size())sz=calibFileNames1.size();
	for(int i=0;i<sz;i++)
	{
	    if(fn[i]=="")
	    {
...
{
    if((int)calibFileNames1.size()<=d) return false;
    if((int)calibFileNames1[d].size()<=t) return false;
    return (hasA[d])[t];
}

void beamTime_parameter::setHasACalib(int d, int t, bool us)
{
    if((int)calibFileNames1.size()<=d) return;
    if((int)calibFileNames1[d].size()<=t) return;
    (hasA[d])[t]=us;
}


int beamTime_parameter::getNumCalibDets()const
{
    return calibFileNames1.size();
}
...
{
  if(num<0||num>=(int)calibFileNames.size())return "";
...
int beamTime_parameter::getNumberOfCalibrationFiles()const
{
  return calibFileNames.size();
}
void beamTime_parameter::addCalibrationFile(string filename)
{
...
int beamTime_parameter::getNumberOfDetectors()const
{
  return hasQ.size();
}
...
{
  if(num<0||num>(int)hasQ.size())return false;
  return hasQ[num];
}
...
int beamTime_parameter::getMonth() const
{
    return month;
}

int beamTime_parameter::getYear()const
{
    return year;
}
...
int run_parameter::getType()const
{
    return type;
}

void run_parameter::setType(int t)
{
    type=t;
}

int run_parameter::getParentNumber()const
{
    return parent;
}

void run_parameter::setParentNumber(int p)
{
    parent=p;
}

bool run_parameter::hasOwnSetup()const
{
    return setup;
}

void run_parameter::setOwnSetup(bool s)
{
    setup=s;
}

bool run_parameter::hasOwnCalibration() const
{
    return calib;
}

void run_parameter::setOwnCalibration(bool c)
{
    calib=c;
}
...
void run_parameter::addDetectorCalibration(vector<string> fn)
{
    ...
    {
	int sz=fn.size();
	if (sz>(int)enabeles[0].size()) sz=enabeles[0].size();
	for(int i=0;i<sz;i++)
	{
	    if(fn[i]=="")
		t1.push_back(false);
...
void run_parameter::addTypCalibration(vector<string> fn)
{
    ...
    {
	int sz=fn.size();
	if(sz>(int)enabeles.size())sz=enabeles.size();
	for(int i=0;i<sz;i++)
	{
	    if(fn[i]=="")
		enabeles[i].push_back(false);
...
{
    if((int)enabeles.size()<=d) return false;
    if((int)enabeles[d].size()<=t) return false;
    return (enabeles[d])[t];
}

void run_parameter::setUseOwn(int d, int t, bool us)
{
    if((int)enabeles.size()<=d) return;
    if((int)enabeles[d].size()<=t) return;
    (enabeles[d])[t]=us;
}

int run_parameter::getRunNumber() const
{
    return runNumber;
}

void run_parameter::setRunNumber(int rn)
{
    runNumber=rn;
}
...
int run_parameter::getNumberOfFiles()const
{
  return files.size();
}
...
{
  if(num<0||num>=(int)files.size())return "";
  return files[num];
}
int run_parameter::getFileType(int num)const
{
  if(num<0||num>=(int)files.size())return -1;
...
beamTime_parameter *run_parameter::getParent()const
{
  return par;
}

void run_parameter::setParent(beamTime_parameter* p)
{
  par=p;
}
bool run_parameter::hasAdditionalCalibration() const
{
  return addiC;
}
void run_parameter::setAdditionalCalibration(bool c)
{
  addiC=c;
}
...
ostream &operator<<(ostream &o, const run_parameter &d)
{
  ...
  for(int i=0;i<d.getNumberOfFiles();i++)
    o<<"F"<<d.getFileType(i)<<" "<<d.getFileEvents(i)<<" "<<contractPath(d.getFile(i)).data()<<endl;
...
#ifdef USE_QT
  o<<"T"<<d.getStartTime().date().year()<<" "<<d.getStartTime().date().month()<<" "<<d.getStartTime().date().day()<<" "<<d.getStartTime().time().hour()<<" "<<d.getStartTime().time().minute()<<" "<<d.getStartTime().time().second()<<endl;
  o<<"P"<<d.getStopTime().date().year()<<" "<<d.getStopTime().date().month()<<" "<<d.getStopTime().date().day()<<" "<<d.getStopTime().time().hour()<<" "<<d.getStopTime().time().minute()<<" "<<d.getStopTime().time().second()<<endl;
#else
  o<<"T"<<d.getStartTime().tm_year+1900<<" "<<d.getStartTime().tm_mon<<" "<<d.getStartTime().tm_mday<<" "<<d.getStartTime().tm_hour<<" "<<d.getStartTime().tm_min<<" "<<d.getStartTime().tm_sec<<endl;
  o<<"P"<<d.getStopTime().tm_year+1900<<" "<<d.getStopTime().tm_mon<<" "<<d.getStopTime().tm_mday<<" "<<d.getStopTime().tm_hour<<" "<<d.getStopTime().tm_min<<" "<<d.getStopTime().tm_sec<<endl;
#endif
  o<<"="<<endl;
  return o;
}
