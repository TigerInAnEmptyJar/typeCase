...
template <class X>
void single_parameter<X>::setName(const string& n)
{
  name = n;
}

template <class X>
X single_parameter<X>::getData() const
{
  return data;
}

template <class X>
void single_parameter<X>::setData(X d)
{
  data = d;
}
...
template single_parameter<vector3D> algorithm_parameter::getParam<vector3D>(
    const string& nme) const;
...
// template void algorithm_parameter::changeParam<QBitArray>(string name,
// QBitArray data);
// template void algorithm_parameter::changeParam<QBitArray>(string name,
// QBitArray data, int pos);
...
template single_parameter<vector<int>> algorithm_parameter::getParam<vector<int>>(
    const string& nme) const;
...
template single_parameter<vector<float>> algorithm_parameter::getParam<vector<float>>(
    const string& nme) const;
...
template single_parameter<vector<string>> algorithm_parameter::getParam<vector<string>>(
    int i) const;
template single_parameter<vector<string>> algorithm_parameter::getParam<vector<string>>(
    const string& nme) const;
template void algorithm_parameter::addParam<vector<string>>(single_parameter<vector<string>> data);

template void algorithm_parameter::changeParam<algorithm_parameter>(string name,
                                                                    algorithm_parameter data);
template void algorithm_parameter::changeParam<algorithm_parameter>(string name,
                                                                    algorithm_parameter data,
                                                                    int pos);
...
template single_parameter<algorithm_parameter> algorithm_parameter::getParam<algorithm_parameter>(
    int i) const;
template single_parameter<algorithm_parameter> algorithm_parameter::getParam<algorithm_parameter>(
    const string& nme) const;
template void algorithm_parameter::addParam<algorithm_parameter>(
    single_parameter<algorithm_parameter> data);
#endif
...
template <class X>
void algorithm_parameter::changeParam(string name, X data)
{
  void* vec = NULL;
...
template <class X>
void algorithm_parameter::changeParam(string name, const bool data[], int num)
{
...
template <class X>
void algorithm_parameter::changeParam(string name, X data, int pos)
{
  if (pos < 0)
    return;
  void* vec = NULL;
  ...
  //     vec=&bitmaps;
    ...
    if (vec == NULL)
      return;
  if (pos >= (int)((vector<single_parameter<X>>*)vec)->size())
    return;
...
template <class X>
int algorithm_parameter::getNumberOfParam() const
{
  if (typeid(X) == typeid(int))
...
template <class X>
single_parameter<X> algorithm_parameter::getParam(const string& nme) const
{
  const void* p = 0;
  if (typeid(X) == typeid(int)) {
  ...
  if (p != 0)
    return *((single_parameter<X>*)p);
  return single_parameter<X>();
}
template <class X>
single_parameter<X> algorithm_parameter::getParam(int i) const
{
  if (i < 0 || i >= getNumberOfParam<X>())
    return single_parameter<X>();
  const void* p = 0;
  if (typeid(X) == typeid(int))
    p = &integers.at(i);
...
template <class X>
void algorithm_parameter::addParam(single_parameter<X> data)
{
  void* p = 0;
  if (typeid(X) == typeid(int))
    p = &integers;
  ...
  //     p=&bitmaps;
    ...
    p = &algos;
  if (p != 0)
    ((vector<single_parameter<X>>*)p)->push_back(data);
}

template <class X>
void algorithm_parameter::popParam()
{
  if (typeid(X) == typeid(int))
    integers.pop_back();
  else if (typeid(X) == typeid(float))
...
bool algorithm_parameter::operator<(algorithm_parameter p) { return ID < p.getID(); }
bool algorithm_parameter::operator<=(algorithm_parameter p) { return ID <= p.getID(); }
bool algorithm_parameter::operator>(algorithm_parameter p) { return ID > p.getID(); }
bool algorithm_parameter::operator>=(algorithm_parameter p) { return ID >= p.getID(); }
...
bool algorithm_parameter::operator==(const algorithm_parameter& ap) const
{
  ...
  if (getName() != ap.getName())
    return false;
  if (getNumberOfParam<int>() != ap.getNumberOfParam<int>())
    return false;
  if (getNumberOfParam<float>() != ap.getNumberOfParam<float>())
    return false;
  if (getNumberOfParam<bool>() != ap.getNumberOfParam<bool>())
    return false;
  if (getNumberOfParam<point3D>() != ap.getNumberOfParam<point3D>())
    return false;
  if (getNumberOfParam<vector3D>() != ap.getNumberOfParam<vector3D>())
    return false;
  //     if(getNumberOfParam<QBitArray>()!=ap.getNumberOfParam<QBitArray>())
  //     return false;
  if (getNumberOfParam<vector<int>>() != ap.getNumberOfParam<vector<int>>())
    return false;
  if (getNumberOfParam<vector<float>>() != ap.getNumberOfParam<vector<float>>())
    return false;
  if (getNumberOfParam<vector<string>>() != ap.getNumberOfParam<vector<string>>())
    return false;
  if (getNumberOfParam<algorithm_parameter>() != ap.getNumberOfParam<algorithm_parameter>())
    return false;
  return true;
}
...
ostream& operator<<(ostream& o, const algorithm_parameter& a)
{
  if (&o == &cout) {
    ...
    for (int i = 0; i < a.getNumberOfParam<algorithm_parameter>(); i++)
      o << "A" << a.getParam<algorithm_parameter>(i).getData(); //<<"
...
template single_parameter<> algorithm_parameter::getParam<>(const string
&nme)const;
*/
#endif
