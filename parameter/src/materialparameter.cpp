...
{
  if(e.getMass()!=mass)return false;
  if(e.getCharge()!=charge)return false;
  if(e.getName()!=getName())return false;
  return true;
}
...
float element_parameter::getMass()const
{
    return mass;
}

float element_parameter::getCharge()const
{
    return charge;
}

float element_parameter::getWeight()const
{
    return weight;
}

void element_parameter::setMass(float m)
{
    mass=m;
}

void element_parameter::setCharge(float c)
{
    charge=c;
}

void element_parameter::setWeight(float w)
{
    weight=w; 
}
...
ostream &operator <<(ostream &o, const element_parameter &ep)
{
    ...
    {
	o<<ep.getMass()<<" "<<ep.getCharge()<<" "<<ep.getWeight()<<" "<<ep.getName().data();
    }
    return o;
}
...
material_parameter &material_parameter::operator=(const material_parameter &m)
{
  ...
  ClearEnergyLoss();
  radiationLength=m.RadiationLength();
  while(!elements.empty())elements.pop_back();
  for(int i=0;i<m.NumberOfElements();i++)
    elements.push_back(m.Element(i));
  for(int i=0;i<m.NumberOfEnergyLossParams();i++)
    energyloss.push_back(m.EnergyLoss(i));
...
bool material_parameter::IsActive() const
{
    return active;
}
...
{
    if ((num>(int)energyloss.size())||(num<0))return -1;
    return energyloss[num];
}

float material_parameter::Density() const
{
    return density;
}

float material_parameter::RadiationLength() const
{
...
{
    if((num<0)||(num>=(int)elements.size())) return element_parameter();
    return elements[num];
}
void material_parameter::setElement(int pos, const element_parameter &ep)
{
  if(pos>=(int)elements.size()||pos<0)return;
...
void material_parameter::addElement(const element_parameter &ep)
{
    elements.push_back(ep);
}

int material_parameter::NumberOfElements()const
{
...
{
    if((num<0)||(num>=(int)elements.size())) return;
...
void material_parameter::setRadiationLength(float value)
{
    radiationLength=value;
}

int material_parameter::NumberOfEnergyLossParams()const
{
    return energyloss.size();
}

void material_parameter::setActive(bool is)
{
    active=is;
}
...
{
    if (num==(int)energyloss.size()) energyloss.push_back(value);
    else
	energyloss.at(num)=value;
}

void material_parameter::setDensity(float value)
{ 
    density=value;
}
...
float material_parameter::Speed()const
{
    return speed;
}
void material_parameter::setSpeed(float val)
{
    speed=val;
}
...
ostream &operator<<(ostream &o, const material_parameter &m)
{
	...
	for(int j=0;j<m.NumberOfElements();j++)
	  o<<m.Element(j).getMass()<<" "<<m.Element(j).getCharge()<<" "<< m.Element(j).getWeight()<<" "<<m.Element(j).getName().data()<<endl;
    }
   //o<<"\n";
    return o;
}
