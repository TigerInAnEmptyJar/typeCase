...
bool parameterManager::readAlgorithmParameter_ascii(string filename,
                                                    vector<algorithm_parameter>& ret, int version)
{
  ...
  if (version == -1 || version == versionNumbers[0][1]) {
    ...
    string line(li);
    if (!((line.find("version") < line.npos || line.find("Version") < line.npos) &&
          line.find("parameter") < line.npos && line.find("algorithm") < line.npos))
      return false;
    input1 >> num;
    for (int j = 0; j < num; j++) {
...
bool parameterManager::readUsedAlgorithmParameter_ascii(string filename,
                                                        ...
                                                        int version)
{
  ...
  if (check_version_type(filename.data(), v, t)) {
    if (t != 0)
      return false;
  } else if (!(version == versionNumbers[0][0] || version == -1))
    return false;
  if (version == -1 || version == versionNumbers[0][1]) {
    while (!ret.empty())
      ret.pop_back();
...
bool parameterManager::writeAlgorithmParameter_ascii(string filename,
...
bool parameterManager::readDetectorParameter_ascii(string filename, vector<detector_parameter>& ret,
                                                   int version)
{
...
bool parameterManager::readDetectorParameter_ascii(string filename, vector<detector_parameter>& ret,
                                                   reaction_parameter& col, int version)
{
  if (!existing(filename)) {
    return false;
  }
  ...
  if (check_version_type(filename.data(), v, t)) {
    if (t != 5)
      return false;
  } else if (!(version == versionNumbers[5][0] || version == -1 ||
               version == versionNumbers[5][1] || version == versionNumbers[5][2]))
    return false;
...
bool parameterManager::writeDetectorParameter_ascii(string filename,
                                                    const vector<detector_parameter>& ret,
                                                    const reaction_parameter& col, int version)
{
  if (version == -1 || version == versionNumbers[5][2]) {
    ofstream out;
    out.open(filename.data());
    out << "########## detector parameter ####### file version :" << versionNumbers[5][2]
        << " #######" << endl;
    out << col << " ";
    out << ret.size() << endl;
    for (unsigned int i = 0; i < ret.size(); i++)
    ...
    ofstream out;
    out.open(filename.data());
    out << "########## detector parameter ####### file version :" << versionNumbers[5][1]
        << " #######" << endl;
    out << col << " ";
    out << ret.size() << endl;
    for (unsigned int i = 0; i < ret.size(); i++)
...
bool parameterManager::readMaterialParameter_ascii(string filename, vector<material_parameter>& ret,
                                                   int version)
{
  if (!existing(filename))
    return false;
  ...
  if (check_version_type(filename.data(), v, t)) {
    if (t != 6)
      return false;
  } else if (!(version == versionNumbers[6][0] || version == -1))
    return false;
...
bool parameterManager::writeMaterialParameter_ascii(string filename,
                                                    const vector<material_parameter>& ret,
                                                    int version)
{
  if (version == -1 || version == versionNumbers[6][0]) {
    ofstream output;
...
bool parameterManager::readSetup_ascii(string filename, vector<detector_parameter>& ret,
                                       vector<material_parameter>& mret, reaction_parameter& col,
                                       int version, string filename1)
{
  if (!existing(filename)) {
    return false;
  }
...
bool parameterManager::writeSetup_ascii(string filename, string filename1,
                                        const vector<detector_parameter>& ret,
...
bool parameterManager::readShapeParameter_ascii(string filename, vector<shape_parameter>& ret,
                                                int version)
{
  if (!existing(filename))
    return false;
  ...
  if (check_version_type(filename.data(), v, t)) {
    if (t != 1)
      return false;
  } else if (!(version == versionNumbers[1][0] || version == -1))
    return false;
...
bool parameterManager::writeShapeParameter_ascii(string filename,
                                                 const vector<shape_parameter>& ret, int version)
{
  if (version == versionNumbers[1][0]) {
    ofstream out;
    out.open(filename.data());
...
bool parameterManager::readPaintParameter_ascii(string filename,
                                                vector<vector<paint_parameter>>& ret, int version)
{
  if (!existing(filename))
    return false;
  int v, t;
  if (check_version_type(filename.data(), v, t)) {
    if (t != 4)
      return false;
  } else if (!(version == versionNumbers[4][0] || version == -1))
    return false;
...
bool parameterManager::writePaintParameter_ascii(string filename,
                                                 const vector<vector<paint_parameter>>& ret,
                                                 int version)
{
  if (version == -1 || version == versionNumbers[4][0]) {
    ofstream output;
    output.open(filename.data());
    output << "########## paint parameter ####### file version :" << versionNumbers[4][0]
           << " #######" << endl;
    output << ret.size() << " ";
    for (unsigned int i = 0; i < ret.size(); i++) {
      output << ret[i].size() << " ";
...
bool parameterManager::readBeamTimeParameter_ascii(string filename, vector<beamTime_parameter>& ret,
                                                   int version)
{
  if (!existing(filename))
    return false;
  ...
  if (check_version_type(filename.data(), v, t)) {
    if (t != 2)
      return false;
  } else if (!(version == versionNumbers[2][0] || version == -1 ||
               version == versionNumbers[2][1] || version == versionNumbers[2][2]))
    return false;
  if (version == -1 || version == versionNumbers[2][0] || version == versionNumbers[2][1] ||
      version == versionNumbers[2][2]) {
    while (!ret.empty())
      ret.pop_back();
    ifstream input;
    input.open(filename.data());
...
bool parameterManager::readRunParameter_ascii(string filename, vector<run_parameter>& ret,
                                              int version)
{
  if (!existing(filename))
    return false;
  ...
  if (version == versionNumbers[3][0]) {
    ...
    string line(li);
    if (!((line.find("version") < line.npos || line.find("Version") < line.npos) &&
          line.find("parameter") < line.npos && line.find("run") < line.npos))
      return false;
    input1 >> numRuns;
    for (int j = 0; j < numRuns; j++) {
      run_parameter r;
...
bool parameterManager::addRunParameter_ascii(string filename, vector<run_parameter>& ret,
                                             int version)
{
  int v, t;
  if (check_version_type(filename.data(), v, t)) {
...
bool parameterManager::readDataBasis_ascii(string filename, vector<beamTime_parameter>& bret,
                                           vector<run_parameter>& rret, int version)
{
  if (!existing(filename))
    return false;
  if (version == -1 || version == versionNumbers[2][0] || version == versionNumbers[2][1] ||
...
bool parameterManager::readDataBasis_ascii(string filename, vector<beamTime_parameter>& bret,
                                           vector<vector<run_parameter>>& rret, int version)
{
  if (!existing(filename))
    return false;
  if (version == -1 || version == versionNumbers[2][0] || version == versionNumbers[2][1] ||
...
bool parameterManager::writeBeamTimeParameter_ascii(string filename,
                                                    const vector<beamTime_parameter>& ret,
                                                    int version)
{
  if (version == -1 || version == versionNumbers[2][0] || version == versionNumbers[2][1] ||
      version == versionNumbers[2][2]) {
...
bool parameterManager::writeRunParameter_ascii(string filename, const vector<run_parameter>& ret,
                                               int beamTime, int version)
{
  if (version == versionNumbers[3][0]) {
    ofstream output;
    int nums = 0;
...
bool parameterManager::writeRunParameter_ascii(string filename, const vector<run_parameter>& ret,
                                               beamTime_parameter* beamTime, int version)
{
  if (version == versionNumbers[3][0]) {
    ofstream output;
    int nums = 0;
...
bool parameterManager::writeDataBasis_ascii(string filename, const vector<beamTime_parameter>& bret,
                                            const vector<run_parameter>& rret, int version)
{
  if (version == -1 || version == versionNumbers[2][0] || version == versionNumbers[2][1] ||
      version == versionNumbers[2][2]) {
    ...
    if (!bea)
      return false;
    for (unsigned int i = 0; i < bret.size(); i++)
      bea = (bea && writeRunParameter_ascii(bret[i].getFileName(), rret,
                                            (beamTime_parameter*)(&(bret[i])), version));
    return bea;
  }
  return false;
}
...
bool parameterManager::writeDataBasis_ascii(string filename, const vector<beamTime_parameter>& bret,
                                            const vector<vector<run_parameter>>& rret, int version)
{
  if (version == -1 || version == versionNumbers[2][0] || version == versionNumbers[2][1] ||
      version == versionNumbers[2][2]) {
    bool bea = writeBeamTimeParameter_ascii(filename, bret, version);
    ...
    for (unsigned int i = 0; i < bret.size(); i++) {
      ...
      bea = (bea && writeRunParameter_ascii(bret[i].getFileName(), rret[i],
                                            (beamTime_parameter*)(&(bret[i])), version));
    }
    return bea;
  }
...
template <class X>
int parameterManager::numberOfVersions()
{
  if (!isInit)
    init();
...
template <class X>
int parameterManager::version(int num)
{
  if (!isInit)
    init();
...
template <class X>
tm parameterManager::versionDate(int num)
{
  if (!isInit)
    init();
  int max = numberOfVersions<X>();
...
bool parameterManager::IsInit() { return isInit; }
...
bool check_version_type(const char* filename, int& version, int& type)
{
  ...
  int t = -1;
  for (int i = 0; i < 7; i++)
    if (line.find(names[i]) < line.npos)
      t = i;
  if (t < 0)
    return false;
  type = t;
