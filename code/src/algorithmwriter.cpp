...
string algorithmWriter::fiomakefile=string(getenv("KTOFPACK"))+"/gui/io_algorithm_widgets/makefile";
string algorithmWriter::fheader=string(getenv("KTOFPACK"))+"/algorithms/include/algorithms.h";
string algorithmWriter::fw_header=string(getenv("KTOFPACK"))+"/gui/io_algorithm_widgets/include/ioalgorithms.h";
string algorithmWriter::fiwidget=string(getenv("KTOFPACK"))+"/gui/typeCase/src/addInputAlgoWidget.h";
string algorithmWriter::fowidget=string(getenv("KTOFPACK"))+"/gui/typeCase/src/addOutputAlgoWidget.h";
string algorithmWriter::fanalyser=string(getenv("KTOFPACK"))+"/analyser/src/tofAnalysis_algorithm_t.cpp";

bool algorithmWriter::libAvail()const
{
  return false;
}
algorithmWriter::algorithmWriter(): fheader_file(""),fclass_in_header(0),fneeds_moc(false),fcopy_or_link(0),fis_io(false),finput_algo(false), fwidget_name(""), fwidget_header(""), fwidget_source(""),fversion_dependent_header(false),fioversion_dependent_header(false),ffrequency(0),ffrequency_string(0), fnuncall(0), fid(0),fcomment_on_init("")
{
}
algorithmWriter::~algorithmWriter()
{
}
void algorithmWriter::init(string makefileIn, string iomakefileIn, string headerIn, string widgetHeaderIn, string iwidgetIn, string owidgetIn, string analyserIn)
{
...
vector<string> algorithmWriter::makeFileCall()
{
  ...
  if(shortHeader.find("/")<shortHeader.npos)
    shortHeader=shortHeader.substr(shortHeader.rfind("/")+1,shortHeader.length()-shortHeader.rfind("/")-1);
...
vector<string> algorithmWriter::ioCall()
{
  ...
  ret.push_back(string("connect(w,SIGNAL(insertClick(algorithm_parameter*,bool)),order,SLOT(algorithmAdd(algorithm_parameter *)));"));
  return ret;
}
...
string algorithmWriter::getFrequency()
{
  ...
  switch(ffrequency)
    {
    case 0:fs="1";break;
    case 1:fs="NumberOfTracks";break;
    case 2:fs="NumberOfDetectors";break;
    case 3:
      {
	...
	if(ffrequency_string<fparameter.getNumberOfParam<vector<int> >())
	...
	else if(ffrequency_string<fparameter.getNumberOfParam<vector<float> >()+fparameter.getNumberOfParam<vector<int> >())
	  {
	    fs+="float";
	  ...
	  fs="1";
	if(n>=0)fs+=string("> >(")+string_number(n)+").size()";
	break;
      }
    }
  return fs;
...
vector<string> algorithmWriter::constrCall()
{
  ...
  for(unsigned int i=0;i<fconnections.size()/4;i++)
	...
	sender="stearIt";
      if(receiver=="algorithm"||receiver=="ALGORITHM"||receiver=="algo"||receiver==fdefinition.Name())
	receiver="(*out)[0]";
      ret.push_back(pre+"connect("+sender+", SIGNAL("+signal+"), "+receiver+", SLOT("+slot+"));");
    }
...
{
  cout<<"insert to io "<<algorithmWriter::fiwidget.data()<<" "<<algorithmWriter::fowidget.data()<<endl;
  if(ioAvail())return;
  vector<string> tmp;
  ...
  while(!input.eof())
      ...
      if((hasA(s1,"case ")||hasA(s2,"case "))&&hasA(s,"}"))
	{
	  switch(function)
	    {
	    case 1://count
	      tmp.push_back(pre+"case "+string_number(fid)+": numf=numf+"+getFrequency()+";break;//"+fdefinition.Name());
	      function=0;
	      break;
	    case 2://parameter
	      tmp.push_back(pre+"case "+string_number(fid)+": return "+fdefinition.Name()+"::getDescription();");
	      function=0;
	      break;
	    case 3://definition
...
ostream &operator <<(ostream &o, const algorithmWriter &wr)
{
  ...
  o<<wr.classInHeader()<<" "<<wr.needsMOC()<<" "<<wr.versionDependentHeader()<<" "<<wr.copyOrLink()<<endl;//Class name of algorithm, index of class in header file
  string sSource;
  ...
  o<<wr.frequency()<<" "<<wr.frequencyString()<<" "<<wr.nunCall()<<" "<<wr.nConnections();
  ...
  for(int i=0;i<n[0];i++)
    {
      s="";o.get(c);while(!(c=='\n'||o.eof())){s=s+c;o.get(c);}
      tmp.push_back(s);
      if(o.eof()){wr.setConstructorCall(tmp);
      return o;}
    }
