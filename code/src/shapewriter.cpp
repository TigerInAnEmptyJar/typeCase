...
bool shapeWriter::libAvail()const
{
  ...
  if(bs->getName()==fparameter.getName()||bs->getName()!="none")
    {delete bs;return true;}
  delete bs;return false;
}
...
void shapeWriter::insertToShapes()
{
  ...
  while(!input.eof())
	...
	{s=s+c;input.get(c);}
      backup.push_back(s);
      if(s.find("if(sp.getName()==")<s.npos&&((!hasA(s,"else"))||s.find("if(sp.getName()==")<s.find("else")))
	{
	  ...
	  tmp.push_back(pre+"  }");
	  s=pre+"else "+s.substr(s.find("if(sp.getName()=="),s.length()-s.find("if(sp.getName()=="));
	}
      tmp.push_back(s);
    }
  input.close();
...
ostream &operator <<(ostream &o, const shapeWriter &wr)
{
  ...
  o<<wr.classInHeader()<<" "<<wr.constructorInClass()<<" "<<wr.copyOrLink()<<endl;//Class name of shape, index of class in header file
  string sSource;
