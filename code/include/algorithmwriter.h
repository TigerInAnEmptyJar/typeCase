...
class algorithmWriter
{
 ...
 public:
  algorithmWriter();
  ~algorithmWriter();
  static void init(string makefileIn, string iomakefileIn, string headerIn, string widgetHeaderIn, string iwidgetIn, string owidgetIn, string analyserIn);
  ...
  int frequencyString()const{return ffrequency_string;;}
  int nunCall()const{return fnuncall;}
  string connections(int num)const {if(num<0||num>(int)fconnections.size())return "";return fconnections[num];}

  int nConnections()const {return fconnections.size()/4;}
  string callParameter(int num)const {if(num<0||num>(int)fcall_parameter.size())return "";return fcall_parameter[num];}
  ...
  void clearConnections() { while(!fconnections.empty())fconnections.pop_back(); }
  void clearCallParameter() {while(!fcall_parameter.empty())fcall_parameter.pop_back();}
  ...
  vector<string> makeFileCall();  vector<string> iomakeFileCall();
