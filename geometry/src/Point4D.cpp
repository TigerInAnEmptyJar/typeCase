...
void point4D::setValues(double x, double y, double z, double w)
{
    ...
    if ((!(x<0||x>=0))||(!(y<0||y>=0))||(!(z<0||z>=0))||(!(w<0||w>=0)))
      {
 	Infinite();
 	infinityPos = ((!(x<0||x>=0))?0:
 		       ((!(y<0||y>=0))?1:
 			((!(z<0||z>=0))?2:3)));

	f_phi=(infinityPos==0?0:M_PI/2.);
	f_r=(infinityPos==3?sqrt(value1*value1+value2*value2+value3*value3):nanf("nan"));
	f_rho=(infinityPos==0||infinityPos==1?nanf("nan"):sqrt(value1*value1+value2*value2));
...
{
    if ((p.getDimension()!=4)||(p.getDimension()!=3)) return;
    setValues(p[0], p[1], p[2],  
	      (p.getDimension()==3?W():p.getValue(3)));    
}
...
{
  if(getState()!=_regular_&&getState()!=_zero_)return point::toString();
  rep3D re=repre;
  switch(pt)
    {
    case _cartesic3_:re=_cartesic3_;break;
    case _spheric_:re=_spheric_;break;
    case _cylindric_:re=_cylindric_;break;
    }
...
// {
//     return ((value1-p.X()<__prec)&&(value2-p.Y()<__prec)&&(value3-p.Z()<__prec)&&(W()-p.W()<__prec));
// }
