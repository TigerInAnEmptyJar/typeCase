...
void Eparticles::init()
{
  ...
  if(lifeTimes)delete []lifeTimes;
  if(names)delete[]names;
  if(GeantIDs)delete []GeantIDs;
...
bool Eparticles::IsInit()
{
  return isInit;
}

void Eparticles::setParticle(int ID, string name, float mass, float charge, float lifetime, int GeantID)
{
  name=upper(name);
  remove(name," ");
  if(ID<0||ID>numberOfParticles) return;
  names[ID]=name;
  masses[ID]=mass;
  charges[ID]=charge;
...
int Eparticles::getNumberOfParticles()
{
...
{
  if(ID<0||ID>numberOfParticles) return -1;
  return GeantIDs[ID];
}
...
{
  if(ID<0||ID>numberOfParticles) return 0;
  return charges[ID];
}
