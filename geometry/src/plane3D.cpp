...
#include <TObject.h>
#endif
plane3D::plane3D(const plane3D &p):geomObject(),f(p.Foot()),dir1(p.Direction(0)),dir2(p.Direction(1))
{
    dir1=p.Direction(0);
    dir2=p.Direction(1);
    dir1.normalize();
    ...
    switch(p.getState())
      {
      case _undefined_:Undefined();break;
      case _regular_:Regular();break;
      case _infinite_:Infinite();break;
      case _zero_:Zero();break;
      }
}

/*plane3D::plane3D(triangle3D t):point(3),point3D(t.A()), dir1(t.A()-t.B()),dir2(t.A()-t.C())
{
  dir1.normalize();
  dir2.normalize();
...
plane3D::plane3D(const point3D &a, const point3D &b, const point3D &c):geomObject(),f(a),dir1(a-b),dir2(a-c)
{
  f=a;
  vector3D v(dir1),w(dir2);
  v.normalize();
  ...
  dir1=v;
  dir2=w;
  if ((f.getState()==_infinite_)||(dir1.getState()==_infinite_)||(dir2.getState()==_infinite_)) 
    Infinite();
  else if ((f.getState()==_undefined_)||(dir1.getState()==_undefined_)||(dir2.getState()==_undefined_))
    Undefined();
...
plane3D::plane3D(const point3D &foot, const vector3D &d1, const vector3D &d2):geomObject(),f(foot),dir1(d1),dir2(d2)
{
  f=foot;
  dir1.normalize();
  dir2.normalize();
...
{
    if(!(n_==p.Normal()))return 0;
    if((p&f)==vector3D(0,0,0))return 1;
    return 0;
}
vector3D plane3D::operator & (const point3D &p)const
{
    if((p-f)*n_==0) return vector3D(0,0,0);
    vector3D temp=(p-f);
    temp=n_*(temp*n_);
    return temp;
...
{
    if(l.Direction()*n_==0) return point3D();
...
point3D plane3D::operator - (const lLine3D &l)const
{
    ...
    vector3D v=l.P()-p;
    if (v*l.Direction()>0) v=l.Q()-p;
    if(v.length()>(l.P()-l.Q()).length()) return point3D();
    return p;
}//hit point line-plane
...
{
    if (*this==p) return sLine3D(f,f);
    if (n_==p.Normal()) return sLine3D();
    vector3D temp1;
  ...
  if(pt>=0)
    {
      ...
      ptt3=pt&3;
      pt=pt<<2;
      if(pt==_parametric_||pt==_normal_)plr=(planeRep)pt;
    }
  if(plr==_parametric_)
    return (string("( P = ")+f.toString(ptt1)+" + t * "+dir1.toString(ptt2)+" * s * "+dir2.toString(ptt3)+")");
  else
