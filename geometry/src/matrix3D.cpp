...
{
  if(num>2||num<0)return vector3D();
...
{
  string ret="(";
  ret+=string_number(getValue(0,0))+", "+string_number(getValue(0,1))+", "+string_number(getValue(0,2))+"\n ";
  ret+=string_number(getValue(1,0))+", "+string_number(getValue(1,1))+", "+string_number(getValue(1,2))+"\n ";
  ret+=string_number(getValue(2,0))+", "+string_number(getValue(2,1))+", "+string_number(getValue(2,2))+")";
  return ret;
}
//#endif
...
{
  if(line<0||line>2||column<0||column>2)return 0;
...
{
  if(line<0||line>2||column<0||column>2)return;
...
int matrix3D::Unitary() const
{
    return unit;
}

int matrix3D::Orthogonal() const
{
    return (unit&&reel);
}

matrix3D matrix3D::Rotate(const vector3D &fromX, const vector3D &fromY, const vector3D &fromZ, const vector3D &toX, const vector3D &toY, const vector3D &toZ)
{
    vector3D l1,l2,l3,c;
