...
{
    if (point::getState()!=_regular_)return;
    double tmp = length();
    if(dimension<=20)
      for (int i=0; i < dimension; i++)
...
{
    if (point::getState()==_zero_)return 0;
    if (point::getState()!=_regular_) return -1;
    double tmp = 0;
...
{
    if (point::getDimension() != p.getDimension()) return;
...
{
    if (point::dimension != w.getDimension()) return;
    kind = w.isLine();
    if (w.getState()==_zero_) 
    {
	Zero();
...
{
    if (point::dimension!=w.getDimension()) return Vector(1);
    Vector temp(dimension);
    if(isColumn())temp.transpose();
    for (int i = 0; i < point::dimension; i++)
...
{
    if (w.getDimension()!=point::dimension) return 0;
    double temp = 0;
    for (int i = 0; i < point::dimension; i++)
...
{
    if (point::dimension!=p.getDimension()) return 0;
    int j = 1;
    for (int i = 0; i < point::dimension; i++)
