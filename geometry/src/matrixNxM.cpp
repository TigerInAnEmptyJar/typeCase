...
matrixNxM::~matrixNxM() { delete[] data; }
...
int matrixNxM::Columns() const { return numberOfColumns; }

int matrixNxM::Lines() const { return numberOfLines; }
...
Vector operator*(const Vector& v, const matrixNxM& m)
{
  ...
  if (v.isColumn()) {
    if ((numberOfLines > 1) || (numberOfLines == 0))
      return Vector(0);
    if (v.getDimension() != numberOfColumns)
      return Vector(0);
    double tmp = v * m(0);
    Vector ret(1);
    ret.setValue(0, tmp);
    ...
    if (v.getDimension() != numberOfLines)
      return Vector(0);
    Vector ret(numberOfColumns);
    for (int i = 0; i < numberOfColumns; i++) {
...
double matrixNxM::Determinant() const
{
  ...
  for (int i = 0; i < numberOfColumns; i++) {
    ...
    for (int j = 0; j < numberOfLines; j++) {
      ...
      n1++;
      n2--;
      if (n1 >= numberOfColumns)
        n1 = 0;
      if (n2 < 0)
        n2 = numberOfColumns - 1;
    }
    sum = sum + sum1 - sum2;
  }
  return sum;
...
{
  if (lineBegin > lineEnd || columnBegin > columnEnd || columnBegin < 0 || columnEnd < 0 ||
      lineBegin < 0 || lineEnd < 0 || columnBegin >= numberOfColumns ||
      columnEnd >= numberOfColumns || lineBegin >= numberOfLines || lineEnd >= numberOfLines)
    return matrixNxM(0, 0);
...
bool matrixNxM::UpperTriangular() const
{
  for (int i = 0; i < numberOfLines; i++)
    for (int j = 0; j < numberOfColumns; j++)
      if (i > j && abs(data[i * numberOfColumns + j]) > __prec)
        return false;
  return true;
}
...
{
  if (Q.Columns() != numberOfLines)
    return;
  if (R.Lines() != numberOfColumns)
    return;
}
//#include  "LU_Decomposition.cpp"
// extern "C" void dinv_(int*,void*,int*,double[],int*);
extern "C" int LUDecomposition(int lines, int columns, long double* matIn, long double* outL,
                               long double* outU, long double* permutation);
extern "C" int LDUDecomposition(int lines, int columns, long double* matIn, long double* outL,
                                long double* outD, long double* outU, long double* permutation);
...
{
  long double LL[numberOfLines * numberOfLines], UU[numberOfLines * numberOfLines],
      MM[numberOfLines * numberOfLines], PP[numberOfLines * numberOfLines];
  for (int i = 0; i < numberOfLines; i++)
    for (int j = 0; j < numberOfLines; j++)
      MM[i * numberOfLines + j] = data[i * numberOfLines + j];
  //  matrixNxM P(numberOfLines,numberOfLines);
  LUDecomposition(numberOfLines, numberOfColumns, MM, LL, UU, PP);
  for (int i = 0; i < numberOfLines; i++)
    for (int j = 0; j < numberOfLines; j++)
      L.setValue(i, j, LL[i * numberOfLines + j]);
  for (int i = 0; i < numberOfLines; i++)
    for (int j = 0; j < numberOfLines; j++)
      U.setValue(i, j, UU[i * numberOfLines + j]);
}
void matrixNxM::decomposeLDU(matrixNxM& L, matrixNxM& D, matrixNxM& U) const
{
  long double LL[numberOfLines * numberOfLines], UU[numberOfLines * numberOfLines],
      MM[numberOfLines * numberOfLines], PP[numberOfLines * numberOfLines],
      DD[numberOfLines * numberOfLines];
  for (int i = 0; i < numberOfLines; i++)
    for (int j = 0; j < numberOfLines; j++)
      MM[i * numberOfLines + j] = data[i * numberOfLines + j];
  //  matrixNxM P(numberOfLines,numberOfLines);
  ...
  for (int i = 0; i < numberOfLines; i++)
    for (int j = 0; j < numberOfLines; j++)
      D.setValue(i, j, DD[i * numberOfLines + j]);
  //   matrixNxM P(numberOfLines,numberOfLines);
  //   LDUDec(*this,L, D, U, P);
}
matrixNxM matrixNxM::operator-() const
{
  ...
  if (UpperTriangular()) {
    ...
    for (int i = 0; i < numberOfLines; i++)
      for (int j = 0; j < numberOfLines; j++)
        U_1.setValue(i, j, UU[i * numberOfLines + j]);
    // cout<<"-"<<endl;
    return U_1;
  }
  if (LowerTriangular()) {
    ...
    for (int i = 0; i < numberOfLines; i++)
      for (int j = 0; j < numberOfLines; j++)
        L_1.setValue(i, j, UU[i * numberOfLines + j]);
    // cout<<"-"<<endl;
    return L_1;
  }
  ...
  long double LL[numberOfLines * numberOfLines], UU[numberOfLines * numberOfLines],
      MM[numberOfLines * numberOfLines], PP[numberOfLines * numberOfLines],
      L_L[numberOfLines * numberOfLines], U_U[numberOfLines * numberOfLines];
  for (int i = 0; i < numberOfLines; i++)
    for (int j = 0; j < numberOfLines; j++)
      MM[i * numberOfLines + j] = data[i * numberOfLines + j];
  //  matrixNxM P(numberOfLines,numberOfLines);
  ...
  //  cout<<"."<<flush;
  matrixNxM L_1(numberOfLines, numberOfLines), U_1(numberOfLines, numberOfLines),
      P(numberOfLines, numberOfLines);
  matrixNxM ret(numberOfLines, numberOfLines);
  for (int i = 0; i < numberOfLines; i++)
    for (int j = 0; j < numberOfLines; j++)
      L_1.setValue(i, j, L_L[i * numberOfLines + j]);
  for (int i = 0; i < numberOfLines; i++)
    for (int j = 0; j < numberOfLines; j++)
      U_1.setValue(i, j, U_U[i * numberOfLines + j]);
  for (int i = 0; i < numberOfLines; i++)
    for (int j = 0; j < numberOfLines; j++)
      P.setValue(i, j, PP[i * numberOfLines + j]);
  //   ret=P*U_1*L_1;
  //   cout<<"-"<<flush;
