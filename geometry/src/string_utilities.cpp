...
{
  if(!isNumber(str[0])&&str[0]!='-')return false;
...
{
  if(contains(str,'.')>1)return false;
  if(!isNumber(str[0])&&str[0]!='-'&&str[0]!='.')return false;
  for(unsigned int i=1;i<str.length();i++)
