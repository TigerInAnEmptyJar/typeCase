...
{
    if (dimension != p.getDimension()&&dimension>0) return;
    if (p.getState()==_zero_) 
    {
	Zero();
	return;
...
{
  if(getState()==_undefined_||getState()==_infinite_)return geomObject::toString();
  string ret="(";
  for(int i=0;i<dimension;i++)
...
{
    if (getDimension()!=q.getDimension()) return Vector(1);
...
{
    if (getDimension()!=w.getDimension()) return point(1);
...
{
    if (getDimension()!=p.getDimension()) return 0;
    int j = 1;
    for (int i = 0; i < getDimension(); i++)
...
void point::setValues(double f1,double f2,double f3,double f4,double f5,
  ...
  if(dimension>20)
    {
      ...
      v[4]=f5;v[5]=f6;v[6]=f7;v[7]=f8;
      v[8]=f9;v[9]=f10;v[10]=f11;v[11]=f12;
      v[12]=f13;v[13]=f14;v[14]=f15;v[15]=f16;
      v[16]=f17;v[17]=f18;v[18]=f19;v[19]=f20;
      return;
    }
  switch(dimension)
    {
