...
{
    double t1 = (pPoint.Y()-l.P().Y())-((pPoint.X()-l.P().X())*
					  l.Direction().Y()/l.Direction().X());
    double t2 = (pPoint.Z()-l.P().Z())-((pPoint.X()-l.P().X())*
					  l.Direction().Z()/l.Direction().X());
    if ((t1-t2>__prec)||((t1<0)||(t1>1))||((t2<0)&&(t2>1))) return point3D();
...
double lLine3D::operator & (const lLine3D & l)const
{
	...
	if(t>1) p=Q();
	else p= pPoint + Direction() * t;
    if (s<0) q=l.P();
    else
	if(s>1) q=l.Q();
	else q = l.P() + l.Direction() * s;
    vector3D pq = p-q;
    return sqrt(pq*pq);
}
