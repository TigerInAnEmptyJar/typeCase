...
enum geomState { _zero_, _regular_, _infinite_, _undefined_};
//representation of the 2D points in output
enum repTD {_cartesic_, _polar_};
//representation of the 3D points in output
enum rep3D {_cartesic3_, _spheric_, _cylindric_};
//representation of planes in 3D in output
enum planeRep {_normal_, _parametric_};
...
class point: public geomObject
{
 ...
 public:
    ...
    virtual ~point(){if(dimension>=20)delete[]value;}
    virtual void reDimension(int newDim);            
    void setValues(double values[]);
    void setValues(double f1=0,double f2=0,double f3=0,double f4=0,
		   double f5=0,double f6=0,double f7=0,double f8=0,
		   double f9=0,double f10=0,double f11=0,double f12=0,
		   double f13=0,double f14=0,double f15=0,double f16=0,
		   double f17=0,double f18=0,double f19=0,double f20=0);
    virtual void setValue(int pos, double valueIn);
    virtual void Zero();          
    virtual double getValue(int pos) const
      {
	if(pos>=dimension||pos<0)return 0;
	if(pos<20) return v[pos];
	else return value[pos-20];
      }   
...
class point2D:  public point
{
 ...
 public:
    ...
    virtual void toggleRep(){
      if (repre == _cartesic_ ) repre = _polar_;
      else repre = _cartesic_;
    }
...
class vector2D: public point2D, public Vector
{
 ...
 public:
    ...
    vector2D operator !()const
      {	vector2D v(*this);v.transpose();return v;}
    virtual string toString(int pt=-1)const;
};
...
class point3D:  public point
{
 ...
 public:
    ...
    point3D(double x, double y, double z):point(0){repre=_cartesic3_;setValues(x,y,z);}
    virtual void setValues(double x, double y, double z);
    virtual void setThetaPhiR(double th, double ph, double rIN)
      {	value1=rIN*sin(th)*cos(ph);value2=rIN*sin(th)*sin(ph);value3=rIN*cos(th);
	f_theta=th;f_phi=ph;f_r=rIN;f_rho=rIN*sin(th);}
    virtual double X() const{return value1;}
    virtual double Y() const{return value2;}
    ...
    vector3D operator - (const point3D &q)const;//{return vector3D(value1-q.X(), value2-q.Y(), value3-q.Z());}
    point3D operator - (const vector3D &v)const;//{return point3D(value1-v.X(), value2-v.Y(), value3-v.Z());}
    point3D operator + (const vector3D &v)const;//{return point3D(value1+v.X(), value2+v.Y(), value3+v.Z());}
...
class vector3D: public point3D, public Vector
{
 ...
 public:
    ...
    virtual void normalize(){if(point3D::f_r==0)return;
    ...
    vector3D operator + (const vector3D &w)const{return vector3D(point3D::value1+w.X(), point3D::value2+w.Y(), point3D::value3+w.Z());}
    vector3D operator - (const vector3D &w)const{return vector3D(point3D::value1-w.X(), point3D::value2-w.Y(), point3D::value3-w.Z());}
    double operator * (const vector3D &w)const{return (point3D::value1*w.X()+point3D::value2*w.Y()+point3D::value3*w.Z());}
    ...
    vector3D operator * (double f)const{return vector3D(f*point3D::value1, f*point3D::value2, f*point3D::value3);}
    vector3D operator / (double f)const{return vector3D(point3D::value1/f, point3D::value2/f, point3D::value3/f);}
    int operator ==(const vector3D &v)const;
    virtual double operator[](int pos)const{return point3D::getValue(pos);}  
    vector3D operator !()const{ vector3D vec(*this); vec.transpose(); return vec;}
...
class point4D: public point
{
 ...
 public:
    ...
    point4D(double x, double y, double z, double w): point(0)
      {repre = _cartesic3_;setValues(x, y, z, w);}
    ...
    int operator == (const point4D &p){return ((value1-p.X()<__prec)&&(value2-p.Y()<__prec)&&(value3-p.Z()<__prec)&&(W()-p.W()<__prec));}
    virtual double operator[](int pos)const{
      switch(pos)
	{
	case 0:return value1;
...
class vector4D: public point4D, public Vector
{
 ...
 public: 
    ...
    virtual int getDimension() const{return 4;}           
    virtual void normalize(){ double t=length(); if(t==0)return;
    setValues(point4D::value1/t,point4D::value2/t,point4D::value3/t,point4D::value4/t);}
    virtual double length() const{return sqrt(point4D::f_r*point4D::f_r+point4D::value4*point4D::value4);}      
    void operator = (const point &p);
    void operator = (const Vector &v);
    void operator = (const point4D &p);
    ...
    void operator = (const point3D &p);
    void operator = (const vector3D &v);
    int operator ==(const vector4D &v)const{return ((point4D::value4-v.W()<__prec)&&(point4D::value1-v.X()<__prec)&&(point4D::value2-v.Y()<__prec)&&(point4D::value3-v.Z()<__prec));}
    vector4D operator !()const{vector4D v(*this);v.transpose();return v;}
 ...
 public:
  momentum4D():vector4D(){Undefined();}
    ...
    void setPE(const vector3D &p, double e)
      {
	mass=sqrt(abs(e*e-p.R()*p.R()));velocity=p*(1/e);
	gamma=1/sqrt(1-velocity.R()*velocity.R());
	point4D::setValues(p.X(), p.Y(), p.Z(), e);
      }
    void setValues(double x,double y, double z, double e)
      {setPE(vector3D(x,y,z),e);}
    double Energy() const{return point4D::value4;}
    double Mass() const{return mass;}
    double Beta() const{return velocity.R();}
    double Gamma() const{return gamma;}
    double Rapidity()const{return 0.5*(log((point4D::value4+point4D::value3)/(point4D::value4-point4D::value3)));}
    ...
    vector3D Direction() const{vector3D ret(point4D::value1,point4D::value2,point4D::value3);  ret.normalize(); return ret;}
    momentum4D boost(const vector3D &beta)const;
    static momentum4D CM(const momentum4D &v1,const momentum4D &v2,const momentum4D &v3=momentum4D(),const momentum4D &v4=momentum4D(),const momentum4D &v5=momentum4D());
    ...
    momentum4D operator !()const{ momentum4D m(*this); m.transpose(); return m;}
    ...
    momentum4D operator*(double factor)const{return momentum4D(Momentum()*factor,point4D::value4*factor);}
...
class sLine2D:  public geomObject
{
 ...
 public:
  sLine2D():geomObject(){}
    ...
    void Zero(){geomObject::Zero();d.setValues(0,0);foot.Zero();}
...
class lLine2D: public geomObject
{
 ...
 public:
  lLine2D(): geomObject(),qPoint(){geomObject::Undefined(); }
    ...
    int isIn(const point2D &p) const{vector2D vv=(*this)-p;return ((vv.R()<__prec)?1:0);}
    ...
    geomState getState() const{return geomObject::getState();}
    int getDimension() const{return 2;}
    void Zero(){geomObject::Zero();pPoint.Zero();qPoint.Zero();}
    void operator =(const lLine2D &l);
    vector2D operator -(const point2D &p)const;
    lLine2D operator -(const lLine2D &l)const;
    point2D operator && (const lLine2D &l)const{lLine2D tmpc=(*this)-l;return tmpc.Mid();}
...
class sLine3D:  public geomObject
{
 ...
 protected:
    void Infinite(){geomObject::Infinite();}              
    void Undefined(){geomObject::Undefined(); foot=point3D(); d=vector3D();}     
    void Regular(){geomObject::Regular();}               
 public:
    sLine3D(): geomObject(){}
    ...
    void setRep(rep3D r){d.setRep(r); foot.setRep(r);}
    int isLine() const{return d.isLine();}
    int isColumn() const{return d.isColumn();}
    void transpose(){d.transpose();}
    geomState getState() const{return geomObject::getState();}
    int getDimension() const{return 3;}
    void Zero(){geomObject::Zero();d.setValues(0,0,0);foot.Zero();}
    vector3D operator -(const point3D &p)const //distance line - point
      {vector3D v=p-foot; vector3D n=v-d*(d*v); return n;}
...
class lLine3D: public geomObject
{
 private:
  point3D pPoint;
    point3D qPoint;
 protected:
    void Undefined(){ pPoint.Undefined(); qPoint=point3D(); geomObject::Undefined();}
 public:
    ...
    void setRep(rep3D r){ pPoint.setRep(r); qPoint.setRep(r);}
    geomState getState() const{return geomObject::getState();}
    int getDimension() const{return 3;}
    void Zero(){geomObject::Zero();pPoint.Zero();qPoint.Zero();}
...
class plane3D: public geomObject
{
 ...
 public:
  ...
  geomState getState() const{return geomObject::getState();}
  void Zero(){geomObject::Zero();dir1.setValues(0,0,0);dir2.setValues(0,0,0);}
  rep3D getRep() const{return f.getRep();}
  void setRep(rep3D r){f.setRep(r);dir1.setRep(r);dir2.setRep(r); n_.setRep(r);}
...
class matrix3D:public geomObject
{
 ...
 public:
  ...
  matrix3D Rotate(const vector3D &fromX, const vector3D &fromY, const vector3D &fromZ, const vector3D &toX, const vector3D &toY, const vector3D &toZ);
...
/*     matrixNxMT<elements> subset(int lineBegin, int lineEnd, int columnBegin, int columnEnd) const; */
...
class projector:public geomObject
{
 ...
 public:
  ...
  static point2D project(const plane3D &plane, const point3D &eye, const point3D &point, int mode=predefined);
