...
vector3D getEntrance(base_shape *shape, const sLine3D &path)
{
  ...
  if(shape->getName()=="wedge"||shape->getName()=="spiral"||shape->getName()=="ring"||
     shape->getName()=="fiber"||shape->getName()=="hexPrism"||shape->getName()=="cylinder"||
     shape->getName()=="strawTube"||shape->getName()=="volumeShape")
    return getEntrance((volumeShape*)shape,path);
  if(shape->getName()=="triangle"||shape->getName()=="rectangle"||shape->getName()=="circle"||
     shape->getName()=="sphericTriangle"||shape->getName()=="quadrangle"||
     shape->getName()=="hexagon"||shape->getName()=="sphericRectangle")
...
vector3D getEntrance(volumeShape *shape, const sLine3D &path)
{
  ...
  if(shape->getName()=="wedge")
      ...
      if(((wedge*)shape)->getDistToBarrelCenter()>0)
	{
	  ...
	  //	  float h=sqrt(w->getOuterRadius()*w->getOuterRadius()-w->getDistToBarrelCenter()*w->getDistToBarrelCenter());
	  float s=w->getCenter().Z()*(w->getDistToBarrelCenter()/w->getOuterRadius()/sin(M_PI-alpha-path.Direction().Theta()));
  ...
  else if(shape->getName()=="strawTube")
    {
      ...
      foot=((strawTube*)shape)->getCenter()+normal*(((strawTube*)shape)->getRadius()/normal.R());
    }
  ...
  if(shape->getName()=="wedge")
      ...
      if(((wedge*)shape)->getDistToBarrelCenter()>0)
	{
      ...
      else return shape->entrance(path);
    }
  else
    return shape->entrance(path);
  return vector3D();
}
vector3D ConeLine(const point3D &tip, const vector3D &rotaxis, const float &alpha, const float &height, const sLine3D& path)
{
  ...
  double b=2*(v.X()*(c.X()-f.X())+v.Y()*(c.Y()-f.Y()))/tana2 -2*v.Z()*(c.Z()-f.Z());
  double cc=(c.Z()-f.Z())*(c.Z()-f.Z())-(((c.X()-f.X())*(c.X()-f.X()))+((c.Y()-f.Y())*(c.Y()-f.Y())))/tana2;
  vector3D d(0,0,0);
  ...
  //double ph[2]={acos((c.X()-f.X()-t[0]*v.X())/z[0]/tan(alpha)),acos((c.X()-f.X()-t[1]*v.X())/z[1]/tan(alpha))};
  point3D p[2]={f+t[0]*v,f+t[1]*v};
  if(abs((tip-p[0]).Z())>height)return p[1];
  if(abs((tip-p[1]).Z())>height)return p[0];
  int outInd=0;
  if(t[0]<0&&t[1]<0)
    {
      if(t[0]<t[1])outInd=1;else outInd=0;
    }
  else if(t[0]<0||t[1]<0)
    {
      if(t[0]<0)
...
float getLightWay(shape_parameter& sp, sLine3D &path, bool from)
{
  if(sp.getName()=="wedge"||sp.getName()=="spiral")
    ...
    {
      float z=sp.getParam<float>(2)*sp.getParam<float>(0)/(sp.getParam<float>(0)-sp.getParam<float>(1));
...
float getLightWay(volumeShape *sh, const sLine3D &path, bool from)
{
  ...
  if(onPoint.getState()!=_regular_)return -1;
