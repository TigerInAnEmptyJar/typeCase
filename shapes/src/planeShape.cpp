...
vector3D planeShape::distancePlane(const plane3D &p)
{
  return vector3D(p.Foot());
}
int planeShape::getNumberOfPoints()const
{
  return 0;
}
vector3D planeShape::distance(const sLine3D &line)
{
  return vector3D(line.Direction());
}
float planeShape::circumscribedRadius()const
{
  return circRadius;
}
