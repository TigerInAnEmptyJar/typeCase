...
strawTube::strawTube(const strawTube& tube):cylinder(tube.getCenter(),tube.getDirection(),tube.getRadius())
{
...
strawTube::strawTube(point3D centerIn, vector3D lineDirection, float rad, vector3D stackDirection, vector3D shiftDirection, float isoChroneR, int halv, int halvAt):cylinder(centerIn,lineDirection,rad)
{
...
float strawTube::getIsoChroneRadius()const
{
  return isoChrone;
}
vector3D strawTube::getStackingDirection()const
{
  return stackingDirection;
}
vector3D strawTube::getShift()const
{
  return shift;
}
void strawTube::setIsoChroneRadius(float value)
{
  if(value>radius)isoChrone=radius;
  else if(value<0)isoChrone=0;
  else isoChrone=value;
}
void strawTube::setStackingDirection(vector3D value)
{
  stackingDirection=value;
}
void strawTube::setShift(vector3D value)
{
  shift=value;
}
void strawTube::setNumberOfHalvedElements(int value)
{
  nHalvedElements=value;
}
void strawTube::setHalvedAt(int value)
{
  halvedAt=value;
}
int strawTube::getNumberOfHalvedElements()const
{
  return nHalvedElements;
}
int strawTube::getHalvedAt()const
{
  return halvedAt;
}
...
volumeShape *strawTube::getNext(int times, int stackType)
{
  ...
  switch (stackType)
    {
    case 0:ret->setCenter(ret->getCenter()+ret->getStackingDirection()*times);break;
    case 1:
      {
	if(times<halvedAt)
	  ...
	  {
	    ret->setLine(ret->getCenter()+ret->getStackingDirection()*times+ret->getDirection()*0.5,
			 ret->getDirection()*0.5);
	  }
	else
	  {
	    ret->setLine(ret->getCenter()
			 +ret->getStackingDirection()*(times-nHalvedElements)
			 -ret->getDirection()*0.5,
			 ret->getDirection()*0.5);
	  }
	break;
    ...
    case 2:
      {
	if(times<halvedAt)
	  ret->setCenter(ret->getCenter()+ret->getStackingDirection()*(times/2.)+ret->getShift()*(times%2));
	else if(times>halvedAt+2*nHalvedElements)
	  ret->setCenter(ret->getCenter()+ret->getStackingDirection()*((times-nHalvedElements)/2.)+ret->getShift()*(times%2));
	else if(times<halvedAt+nHalvedElements)
	  {
	    ret->setLine(ret->getCenter()
			 +ret->getStackingDirection()*(times/2.)
...
void strawTube::Draw(const point3D &eye, const plane3D &plane, vector4D* boundingBox, int lColor, int fColor, int fStyle)const
{
  strawTube::Draw(eye,plane,boundingBox,NULL,lColor,fColor,fStyle);
}
void strawTube::Draw(const point3D &eye, const plane3D &plane, vector4D* boundingBox, TObject **ident, int lColor, int fColor, int fStyle)const
{
