...
rectangle::rectangle(point3D Ce, vector3D N, point3D A, point3D B, point3D C, point3D D):planeShape("rectangle")
{
  center=Ce;
  normal=N;
  normal.normalize();
...
{
  if(description.getName()!="rectangle")return;
  if(description.NumberOfParams<point3D>()<4)return;
  ...
  normal.normalize();
  vector3D v1(description.getParam<point3D>(0)-center),v2(description.getParam<point3D>(1)-center);
  circRadius=v1.R();
  if(v2.R()>circRadius)circRadius=v2.R();
}
...
point3D rectangle::getPoint(int num)const
{
  switch(num)
    {
    case 0:return left.P();
    case 1:return left.Q();
    case 2:return right.Q();
    case 3:return right.P();
    }
  return point3D();
}

point3D rectangle::A()const
{
  return left.P();
}

point3D rectangle::B()const
{
  return left.Q();
}

point3D rectangle::C()const
{
  return right.Q();
}

point3D rectangle::D()const
{
  return right.P();
}
...
{
  return left.length()*(left.P()-right.P()).length();
}
...
{
  return(left.P()-right.P()).length();
}

lLine3D rectangle::AB() const
{
  return left;
}

lLine3D rectangle::BC() const
{
  return lLine3D(left.Q(),right.Q());
}

lLine3D rectangle::CD() const
{
  return lLine3D(right.Q(),right.P());
}

lLine3D rectangle::DA() const
{
  return lLine3D(right.P(),left.P());
}
...
int rectangle::getNumberOfPoints()const
{
  return 4;
}
...
void rectangle::Draw(const point3D &eye, const plane3D &plane, vector4D* boundingBox, TObject **ident, int lColor, int fColor, int fStyle)const
{
  point3D ps[4]={getPoint(0),getPoint(1),getPoint(2),getPoint(3)};
  DrawPoints(4, ps,eye, plane, boundingBox, ident, lColor, fColor, fStyle);
}
void rectangle::Draw(const point3D &eye, const plane3D &plane, vector4D* boundingBox, int lColor, int fColor, int fStyle)const
{
...
vector3D rectangle::distance(const sLine3D &line)
{
  ...
  float a=acos((aa*bb)/aa.R()/bb.R()),b=acos((bb*cc)/bb.R()/cc.R()),c=acos((cc*dd)/cc.R()/dd.R()),d=acos((aa*dd)/aa.R()/dd.R());
  if((a+b+c+d-M_PI*2.)>-0.01)return vector3D(0,0,0);
  if((a+b+c+d-M_PI)<0)
    {
      if(aa.R()<bb.R()&&aa.R()<cc.R()&&aa.R()<dd.R())return aa;
      if(bb.R()<cc.R()&&bb.R()<dd.R())return bb;
      if(cc.R()<dd.R())return cc;
      return dd;
    }
  if(a>b&&a>c&&a>d)
    return (bb+aa)*0.5;
