...
base_shape* getShape(const shape_parameter& sp)
{
  ...
  if (sp.getName() == "wedge") {
    if (sp.NumberOfParams<float>() > 4)
      shape = new wedge(sp.getParam<float>(0), sp.getParam<float>(1), sp.getParam<float>(2),
                        sp.getParam<float>(3), sp.getParam<point3D>(0), sp.getParam<vector3D>(0),
    ...
    shape = new cylinder(sp.getParam<point3D>(0), sp.getParam<vector3D>(0), sp.getParam<float>(0));
  else if (sp.getName() == "spiral")
    shape = new spiral(sp.getParam<float>(0), sp.getParam<float>(1), sp.getParam<float>(4),
                       sp.getParam<float>(2), sp.getParam<float>(3), sp.getParam<point3D>(0),
    ...
    shape = new fiber(
        sp.getParam<point3D>(0) -
            shift, // 0.5*sp.getParam<vector3D>(0)-sp.getParam<vector3D>(1)*(sp.getParam<int>(1)+((int)(1.5*sp.getParam<int>(0)))),
        sp.getParam<vector3D>(0),
        sp.getParam<vector3D>(1), sp.getParam<vector3D>(2), sp.getParam<int>(0),
        sp.getParam<int>(1));
  } else if (sp.getName() == "hexPrism")
    shape = new hexPrism(sp.getParam<point3D>(0), sp.getParam<vector3D>(0),
                         sp.getParam<vector3D>(1), sp.getParam<vector3D>(2), sp.getParam<int>(0));
  else if (sp.getName() == "ring")
    shape = new ring(sp.getParam<point3D>(0), sp.getParam<vector3D>(0), sp.getParam<float>(0),
                     sp.getParam<float>(0) + sp.getParam<float>(1), sp.getParam<float>(2));
  else if (sp.getName() == "strawTube")
    shape = new strawTube(sp.getParam<point3D>(0), sp.getParam<vector3D>(0), sp.getParam<float>(0),
                          sp.getParam<vector3D>(1), sp.getParam<vector3D>(2), sp.getParam<float>(1),
    ...
    shape = new triangle(sp.getParam<point3D>(0), sp.getParam<point3D>(1), sp.getParam<point3D>(2));
