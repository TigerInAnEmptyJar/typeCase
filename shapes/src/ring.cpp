...
ring::ring(point3D centerIn,vector3D normalIn,float innerRadius,float outerRadius,float thicknessIn):volumeShape("ring")
{
  center=centerIn;
  normal=normalIn;
  normal.normalize();
...
float ring::getOuterRadius()const
{
  return outerR;
}
...
volumeShape *ring::getNext(int times,int stackType)
{
  ...
  switch(stackType)
	...
	//	cout<<"ring: "<<times<<" : "<<atan((innerR+add)/center.Z())*180./M_PI<<"° -> "<<innerR+add<<endl;
	break;
      }
    case 2:
      {
...
//   pitch=enter-center;
  if(abs(dist*normal)<__prec)
      ...
      else
	{
	  ...
	  if(pitch.R()>(innerR+outerR)/2.)pitch=pitch*-1;
	}
    }
  else
    pitch=normal;
...
Vector ring::Hitting(const sLine3D &line)
{
  ...
  ret.setValues(platePoint.X(),platePoint.Y(),platePoint.Z(),dis.X(),dis.Y(),dis.Z(),plate.Normal().X(),plate.Normal().Y(),plate.Normal().Z(),sigma);
  return ret;
  //end acceleration
  ...
  if(r==0)
    {
      ret.setValues(platePoint.X(),platePoint.Y(),platePoint.Z(),innerR*normal.X(),innerR*normal.Y(),innerR*normal.Z(),0,0,0,sigma);
      return ret;
    }
  else if(r<innerR)
...
void ring::Draw(const point3D &eye, const plane3D &plane, vector4D* boundingBox, int lColor, int fColor, int fStyle)const
{
...
int ring::suspect(const sLine3D &line, int stackType)
{
  switch(stackType)
	...
	if(l.R()<=outerR&&l.R()>=innerR)return 0;
	sLine3D centerLine(center,normal);
	lLine3D poca=centerLine-line;
	if(poca.length()>outerR)return -1;
	//////ka	
      }
    }
  return -1;
