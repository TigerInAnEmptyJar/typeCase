...
{
  if ((s.getName() != "cylinder") && (s.getName() != "wedge") && (s.getName() != "spiral") &&
      (s.getName() != "circular"))
    return;
  center = s.getCenter();
...
float cylinder::getRadius() const { return radius; }
...
void cylinder::setRadius(float r) { radius = (r < 0 ? -r : r); }

#include "fiber.h"
bool cylinder::cut(const fiber& f)
{
  if (f.getCorner() == center)
    return true;
  return false;
}
volumeShape* cylinder::getNext(int times, int stackType)
{
  if (times == stackType)
    return this;
  return this;
}
volumeShape* cylinder::getEnvelope(int stackType)
{
  if (stackType == 0)
    return this;
  return this;
...
Vector cylinder::Hitting(const sLine3D& line)
{
  ...
  if (l > 0) {
    ...
    if (v2_ * no < 0 && v2_.length() <= direction.length()) {
      ret.setValues(p.X(), p.Y(), p.Z(), 0, 0, 0, v1_.X(), v1_.Y(), v1_.Z(),
                    0.5 * direction.length());
      return ret;
    }
  }
  ...
  if (dist1.length() < d || d < 0)
    ret.setValues(platePoint.X(), platePoint.X(), platePoint.X(), dist1.X(), dist1.Y(), dist1.Z(),
                  0, 0, 0, sigma);
  else
    ret.setValues(conn.P().X(), conn.P().X(), conn.P().Y(), dist2.X(), dist2.Y(), dist2.Z(), 0, 0,
                  0, sigma);
  return ret;
}
...
void cylinder::Draw(const point3D& eye, const plane3D& plane, vector4D* boundingBox,
                    TObject** ident, int lColor, int fColor, int fStyle) const
{
  ...
  if (viewDir * direction == 0) {
    ...
    v2.normalize();
    quadrangle q(center + direction + v2 * radius, center + direction - v2 * radius,
                 center - v2 * radius, center + v2 * radius);
...
void cylinder::Draw(const point3D& eye, const plane3D& plane, vector4D* boundingBox, int lColor,
