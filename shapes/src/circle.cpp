...
//#include <TPolyLine.h>
void circle::Draw(const point3D& eye, const plane3D& plane, vector4D* boundingBox, TObject** ident,
                  int lColor, int fColor, int fStyle) const
{
  ...
  //      for(int i=0;i<numPoints;i++)
  //	{
  //	  dp[i]=projector::project(center+start*rad);//getPoint(i)+v[i]*(((double)j)/((double)nPointsPerEdge)))
  // ps[i]=center+start*rad;
  //	  //ps[i]=plane-sLine3D(eye,ps[i]-eye);
  //	  //dp[i].setValues(plane.Direction(0)*ps[i],plane.Direction(1)*ps[i]);
  //	  pox[i]=dp[i][0];
  //	  poy[i]=dp[i][1];
...
void circle::Draw(const point3D& eye, const plane3D& plane, vector4D* boundingBox, int lColor,
