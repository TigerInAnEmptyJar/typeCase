...
triangle::triangle(point3D a, vector3D v1, vector3D v2):planeShape("triangle")
{
  ...
  circRadius=v_1.R();
  if(v_2.R()>circRadius)circRadius=v_2.R();
  if(v_3.R()>circRadius)circRadius=v_3.R();
}
triangle::triangle(const shape_parameter &description):planeShape("triangle")
{
  if(description.getName()!="triangle")return;
  if(description.NumberOfParams<point3D>()<3)return;
...
vector3D triangle::distancePlane(const plane3D &p)
{
  ...
  if(v1*v2<0)return vector3D(0,0,0);
  if(v2*v3<0)return vector3D(0,0,0);
  if(v3*v1<0)return vector3D(0,0,0);
  if(v1.length()<v2.length()&&v1.length()<v3.length())return v1;
  if(v2.length()<v3.length()&&v1.length()>v2.length())return v2;
  return v3;
}
...
//     }
// }
int triangle::getNumberOfPoints()const
{
  return 3;
}
//#include <TPolyLine.h>
...
void triangle::Draw(const point3D &eye, const plane3D &plane, vector4D* boundingBox, TObject **ident, int lColor, int fColor, int fStyle)const
{
  point3D ps[3]={getPoint(0),getPoint(1),getPoint(2)};
  DrawPoints(3, ps,eye, plane, boundingBox, ident, lColor, fColor, fStyle);
}
void triangle::Draw(const point3D &eye, const plane3D &plane, vector4D* boundingBox, int lColor, int fColor, int fStyle)const
{
