...
fiber::fiber(point3D cor, vector3D d1, vector3D d2,vector3D d3, int halved, int halvedat): volumeShape("fiber")
{
...
{
    if((s.getName()!="fiber")&&(s.getName()!="rectangular")) return;
    if(s.getName()=="fiber")
    {
...
{
  if(description.getName()!="fiber")return;
  if(description.NumberOfParams<point3D>()<1||
     description.NumberOfParams<vector3D>()<3||
     description.NumberOfParams<int>()<2)return;
  dir1=description.getParam<vector3D>(0);
  dir2=description.getParam<vector3D>(1);
  dir3=description.getParam<vector3D>(2);
  numberOfHalvedElements=description.getParam<int>(0);
  halvedAt=description.getParam<int>(1);
  corner=description.getParam<point3D>(0)-dir1*0.5-dir2*(halvedAt+(numberOfHalvedElements%2)*0.5+numberOfHalvedElements);
}
...
bool fiber::cut(const fiber &f)
{
    ...
    yp=(cTc*dir2)/yl/yl;
    zp=(cTc*dir3)/zl/zl;
    if((abs(xp)<1)||(abs(yp)<1)||(abs(zp)<1)) return true;
...
int fiber::getHalved() const
{
    return numberOfHalvedElements;
}

void fiber::setHalved(int num)
{
    numberOfHalvedElements=num;
}

int fiber::getHalvedAt()const
{
    return halvedAt;
}

void fiber::setHalvedAt(int num)
{
    halvedAt=num;
}

point3D fiber::getCorner() const
{
    return corner;
}
...
void fiber::setDirection(int num, vector3D v)
{
    switch(num)
    {
    case 0: dir1.setValues(v.X(),v.Y(),v.Z()); return;
    case 1: dir2.setValues(v.X(),v.Y(),v.Z()); return;
    case 2: dir3.setValues(v.X(),v.Y(),v.Z()); return;
    }
    center=corner+dir1*0.5+dir2*0.5+dir3*0.5;
    if(dir1.R()>dir2.R()&&dir1.R()>dir3.R())
      res=dir2;
...
volumeShape *fiber::getNext(int times,int stackType)
{
	...
	{ 
	  ...
	  else if(times>=2*numberOfHalvedElements+halvedAt)//times>=halvedAt+numberOfHalvedElements)
...
Vector fiber::Hitting(const sLine3D &line)
{
  ...
  //  else if(poca.length()>diag.R())//out of volume
      ...
      if(ll1>=0&&ll1<=1&&ll2>=0&&ll2<=1)
	{
      ...
      vector3D d1=dir1*(ll1<0?ll1:(ll1<1?0:ll1-1))+dir2*(ll2<0?ll2:(ll2<1?0:ll2-1));
      ...
      if(ll1>=0&&ll1<=1&&ll2>=0&&ll2<=1)
	{
      ...
      vector3D d2=dir1*(ll1<0?ll1:(ll1<1?0:ll1-1))+dir3*(ll2<0?ll2:(ll2<1?0:ll2-1));
      ...
      if(ll1>=0&&ll1<=1&&ll2>=0&&ll2<=1)
	{
      ...
      vector3D d3=dir2*(ll1<0?ll1:(ll1<1?0:ll1-1))+dir3*(ll2<0?ll2:(ll2<1?0:ll2-1));
      if(d1.R()<=d2.R()&&d1.R()<=d3.R())
	{
	  ret.setValues(hit1.X(),hit1.Y(),hit1.Z(),d1.X(),d1.Y(),d1.Z(),dir3.X(),dir3.Y(),dir3.Z(),sigma2);
	  return ret;
	}
      if(d3.R()>d2.R())
	{
	  ret.setValues(hit2.X(),hit2.Y(),hit2.Z(),d2.X(),d2.Y(),d2.Z(),dir2.X(),dir2.Y(),dir2.Z(),sigma2);
	  return ret;
	}
      ret.setValues(hit3.X(),hit3.Y(),hit3.Z(),d3.X(),d3.Y(),d3.Z(),dir1.X(),dir1.Y(),dir1.Z(),sigma2);
      return ret;
    }
	...
	case 3:plane=plane3D(corner,dir3*-1);break;
      ...
      //      cout<<plane<<pl<<" "<<plane.Normal()*line.Direction()<<line<<hitP<<endl;
...
//Hit with 3 planes only with planes that can have an entrance => normal_to_plane*line_direction<0
    plane3D p1;
    plane3D p2;
    plane3D p3;
...
void fiber::Draw(const point3D &eye, const plane3D &plane, vector4D* boundingBox, TObject **ident, int lColor, int fColor, int fStyle) const
{
  ...
  for(int i=0;i<3;i++)
    c[i]=point3D(0,0,0)+(vector3D(cornersi[i][0])+vector3D(cornersi[i][1])+vector3D(cornersi[i][2])+vector3D(cornersi[i][3]))*0.25;
  ...
  if(boundingBox!=NULL)
    {
      ...
      for(int i=0;i<2;i++)
	{
	  if(boundingBox->X()>bb[i].X())boundingBox->setValue(0,bb[i].X());
	  if(boundingBox->Y()>bb[i].Y())boundingBox->setValue(1,bb[i].Y());
	  if(boundingBox->Z()<bb[i].Z())boundingBox->setValue(2,bb[i].Z());
	  if(boundingBox->W()<bb[i].W())boundingBox->setValue(3,bb[i].W());
	}
    }
}
void fiber::Draw(const point3D &eye, const plane3D &plane, vector4D* boundingBox, int lColor, int fColor, int fStyle)const
{
...
int fiber::suspect(const sLine3D &line, int stackType)
{
  switch(stackType)
	...
	if(has2)//only first or last element
	  {
	    if((c2-center).R()<dir2.R()/10)return 0;
	    return nE-1;
	  }
	return -1;
      }
	...
	float l3_2=l3*dir2/(dir2.R()*dir2.R());
	bool has3=(l3_1<=1+l3_2*dir2.R()/dir1.R()&&
		   l3_1>=-l3_2/dir1.R()*dir2.R()
		   &&l3_2<=nE&&l3_2>=0);
...
shape_parameter fiber::description()const
{
  shape_parameter sh;
  sh.setName("fiber");
  sh.addParam<point3D>(corner+dir1*0.5+dir2*(halvedAt+(numberOfHalvedElements%2)*0.5+numberOfHalvedElements),"center of envelope");
  sh.addParam<vector3D>(dir1,"length");
  sh.addParam<vector3D>(dir2,"width");
  sh.addParam<vector3D>(dir3,"thickness");
  sh.addParam<int>(numberOfHalvedElements,"number of halved elements");
...
shape_parameter fiber::getDescription()
{
  ...
  sh.addParam<vector3D>(vector3D(),"thickness");
