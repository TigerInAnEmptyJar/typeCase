...
hexPrism::hexPrism(point3D c, vector3D thickness,vector3D toFirstPoint, vector3D keyWidth, int st):volumeShape("hexPrism")
{
  stacki=st;
  center=c;
  dir1=thickness;
...
{
  if(description.getName()!="hexPrism") return;
  if(description.NumberOfParams<point3D>() <1||
     description.NumberOfParams<int>() <1||
     description.NumberOfParams<vector3D>() <3)return;
	...
	if(stacki>0)
	  {
	    ...
	    if((times<ntot))
	      return (new hexPrism(center+dir3*(2*(times-ntot+n))+(shiftUp+shiftLeft)*(-lev-stacki-1),dir1,dir2,dir3,stacki));
	    ...
	    if((times<ntot))
	      return (new hexPrism(center+dir3*(2*(times-ntot+n))+shiftUp*(-lev-stacki-1)+shiftLeft*(abs(stacki)+lev+1),dir1,dir2,dir3,stacki));
	    return getClone();
	  }
	else
	  {
	    ...
	    if((times<ntot))
	      return (new hexPrism(center+dir3*(2*(times-ntot+n))+(shiftUp+shiftLeft)*(-lev-stacki-1)+shiftUp*2,dir1,dir2,dir3,stacki));
	    ntot=ntot+n-1;
	    if((times<ntot))
	      return (new hexPrism(center+dir3*(2*(times-ntot+n-1))+(shiftUp+shiftLeft)*abs(stacki),dir1,dir2,dir3,stacki));
	    ntot=ntot+n;
	    if((times<ntot))
	      return (new hexPrism(center+dir3*(2*(times-ntot+n))+shiftUp*(-lev-stacki+1)+shiftLeft*(-lev+abs(stacki)+1),dir1,dir2,dir3,stacki));
	    ...
	    if(times<ntot+abs(stacki)-1)
	      return (new hexPrism(center+dir3*(2*(times-ntot+1))+shiftUp*(-lev-stacki)+shiftLeft*(abs(stacki)+lev+2),dir1,dir2,dir3,stacki));
	    return getClone();
	  }
      }
    case -3:
...
{
  if(f.getCorner()==center)return true;
  return false;
}
...
void hexPrism::setDirection(int num, vector3D v)
{
  switch(num)
    {
    case 0:dir1=v; return;
    case 1:dir2=v; return;
    case 2:dir3=v; return;
    }
}
...
int hexPrism::getStackingOrder()const
{
  return stacki;
}
void hexPrism::setStackingOrder(int st)
{
  stacki=st;
}
...
{
  Vector ret(10);
//Hit with 4 planes only with planes that can have an entrance => normal_to_plane*line_direction<0
    plane3D p1;
    plane3D p2;
    plane3D p3;
    plane3D p4;
    ...
    if(toce.length()<=dir3.length())
      {
	ret.setValues(hit4.X(),hit4.Y(),hit4.Z(),0,0,0,p4.Normal().X(),p4.Normal().Y(),p4.Normal().Z(),sigma4);
	return ret;
      }
    int i=0;
    ...
    sLine3D sl(c4+matrix3D(d4,i*M_PI/3.0)*dir2,matrix3D(d4,i*M_PI/3.0+M_PI/2)*dir2);
    ...
    switch(outcode1)
      {
      case 5:dist1=hit1-c1;break;
      case 6:dist1=hit1-(c1+dir1);break;
      case 9:dist1=hit1-(c1+dh1*thickness);break;
      case 10:dist1=hit1-(c1+dir1+dh1*thickness);break;
      case 4:dist1=lLine3D(c1,dir1)-hit1;break;
      case 1:dist1=lLine3D(c1,dh1*thickness)-hit1;break;
      case 2:dist1=lLine3D(c1+dir1,dh1*thickness)-hit1;break;
      case 8:dist1=lLine3D(c1+dh1*thickness,dir1)-hit1;break;
      }
    //second plane
    switch(outcode2)
      {
      case 5:dist2=hit2-c2;break;
      case 6:dist2=hit2-(c2+dir1);break;
      case 9:dist2=hit2-(c2+dh2*thickness);break;
      case 10:dist2=hit2-(c2+dir1+dh2*thickness);break;
      case 4:dist2=lLine3D(c2,dir1)-hit2;break;
      case 1:dist2=lLine3D(c2,dh2*thickness)-hit2;break;
      case 2:dist2=lLine3D(c2+dir1,dh2*thickness)-hit2;break;
      case 8:dist2=lLine3D(c2+dh2*thickness,dir1)-hit2;break;
      }
    //third plane
    switch(outcode3)
      {
      case 5:dist3=hit3-c3;break;
      case 6:dist3=hit3-(c3+dir1);break;
      case 9:dist3=hit3-(c3+dh3*thickness);break;
      case 10:dist3=hit3-(c3+dir1+dh3*thickness);break;
      case 4:dist3=lLine3D(c3,dir1)-hit3;break;
      case 1:dist3=lLine3D(c3,dh3*thickness)-hit3;break;
      case 2:dist3=lLine3D(c3+dir1,dh3*thickness)-hit3;break;
      case 8:dist3=lLine3D(c3+dh3*thickness,dir1)-hit3;break;
      }
    if((dist1.length()<=dist2.length())&&(dist1.length()<=dist3.length())&&(dist1.length()<=dist4.length()))
      {
...
#include "planeShape.h"
void hexPrism::Draw(const point3D &eye, const plane3D &plane, vector4D* boundingBox, TObject **ident, int lColor, int fColor, int fStyle)const
{
...
void hexPrism::Draw(const point3D &eye, const plane3D &plane, vector4D* boundingBox, int lColor, int fColor, int fStyle)const
{
...
int hexPrism::suspect(const sLine3D &line, int stackType)
{
  switch(stackType)
	...
	vector3D l_t=l-l_l;
	if(l_t.R()>dir2.R())return -1;
	if(l_l*dir3<0)
	  {
	    if(l_l.R()<dir2.R())return 0;
	    return -1;
	  }
	...
	point3D hit=pl-line;
	vector3D l=hit-c;
	if(l.R()>(abs(stacki)*2+2)*dir2.R())return -1;
