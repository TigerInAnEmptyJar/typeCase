...
{
  if(description.getName()!="hexagon")return;
  if(description.NumberOfParams<point3D>()<2||
     description.NumberOfParams<vector3D>()<1)return;
...
int hexagon::getNumberOfPoints()const
{
  return 6;
}
...
point3D hexagon::D()const
{
  return (center-A_)+center;
}
...
lLine3D hexagon::AB()const
{
  return lLine3D(A_,B());
}

lLine3D hexagon::BC()const
{
  return lLine3D(B(),C());
}

lLine3D hexagon::CD()const
{
  return lLine3D(C(),D());
}

lLine3D hexagon::DE()const
{
  return lLine3D(D(),E());
}

lLine3D hexagon::EF()const
{
  return lLine3D(E(),F());
}

lLine3D hexagon::FA()const
{
  return lLine3D(F(),A_);
}
...
point3D hexagon::getPoint(int num)const
{
  ...
  switch(num)
    ...
    case 2:tmp=turn*tmp;
    case 1:tmp=turn*tmp;
    case 0:return center+tmp;
    }
  return point3D();
}

void hexagon::setA(point3D p)
{
  A_=p;
}

point3D hexagon::A()const
{
  return A_;
}
...
vector3D hexagon::distance(const sLine3D &line)
{
  ...
  l=l*(l.R()-(center-A_).R());
  if(l*(A_-center)<0)return vector3D(0,0,0);
  return l;
}
...
void hexagon::Draw(const point3D &eye, const plane3D &plane, vector4D* boundingBox, TObject **ident, int lColor, int fColor, int fStyle)const
{
...
void hexagon::Draw(const point3D &eye, const plane3D &plane, vector4D* boundingBox, int lColor, int fColor, int fStyle)const
{
