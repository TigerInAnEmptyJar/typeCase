...
wedge::wedge(float i, float a, float d,float t, point3D c,vector3D n, vector3D l, float dis): volumeShape("wedge")
{
    innerR=i;
    outerR=a;
    dphi=d;
...
{
  if(description.getName()!="wedge")return;
...
{
  if((f.getCorner()-center).R()<0)return false;
  return false;
}
...
float wedge::getInnerRadius()const
{
    return innerR;
}

float wedge::getOuterRadius()const
{
    return outerR;
}

float wedge::getPhiRange() const
{
    return dphi;
}

float wedge::getThickness() const
{
    return thickness;
}

vector3D wedge::getNormal() const
{
    return normal;
}

vector3D wedge::getLowerPhiEdge() const
{
    return lpe;
}
vector3D wedge::getHigherPhiEdge() const
{
    return hpe;
}

float wedge::getDistToBarrelCenter()const
{
  return distBarrel;
}

void wedge::setDistToBarrelCenter(float v)
{
  distBarrel=v;
}

void wedge::setInnerRadius(float v)
{
    innerR=(v<0?-v:v);
}

void wedge::setOuterRadius(float v)
{
    outerR=(v<0?-v:v);
}
...
void wedge::setThickness(float v)
{
    thickness=(v<0?-v:v);
}

void wedge::setCenter(point3D c)
{
    center=c;
}
...
volumeShape *wedge::getNext(int times,int stackType)
{
  ...
  switch(stackType)
    ...
    case 4:
      {
	double l=sqrt(outerR*outerR-distBarrel*distBarrel);
	vector3D rot_vec=vector3D(0,0,-1);//lpe*(outerR-distBarrel*distBarrel/l)+outerR*distBarrel/l*normal;
	vector3D lpeBase=lpe-rot_vec*(lpe*rot_vec);lpeBase.normalize();
	double angle=2*atan(l/outerR*tan(dphi/2.));
	rot_vec.normalize();
	matrix3D rotm(rot_vec,angle*times);
	lpeBase=rotm*lpeBase;
	vector3D lpeNew=distBarrel*lpeBase+rot_vec*sqrt(outerR*outerR-distBarrel*distBarrel);lpeNew.normalize();
...
Vector wedge::Hitting(const sLine3D &line)
{
  ...
  if(distBarrel<=0)
  ...
  else
    {
      ...
      double cc=(c[2]-f[2])*(c[2]-f[2])-(((c[0]-f[0])*(c[0]-f[0]))+((c[1]-f[1])*(c[1]-f[1])))/tana2;
      vector3D d(0,0,0);
      ...
      else//(b*b-4*a*cc>=0)
	{
	  ...
	  double ph[2]={acos((c[0]-f[0]-t[0]*v[0])/z[0]/tan(alpha)),acos((c[0]-f[0]-t[1]*v[0])/z[1]/tan(alpha))};
	  point3D p[2]={f+t[0]*v,f+t[1]*v};
	  int outInd=0;
	  if(t[0]<0&&t[1]<0)
	    {
	      if((ph[0]>=lpe.Phi()&&ph[0]<=hpe.Phi()&&z[0]<=outerR*ca&&z[0]>=innerR*ca)
		 ||(ph[1]>=lpe.Phi()&&ph[1]<=hpe.Phi()&&z[1]<=outerR*ca&&z[1]>=innerR*ca))
		{
	      ...
	      else
		{
		  ...
		  d1=ll1-p[0];d2=ll2-p[0];d3=ll1-p[1];d4=ll2-p[1];
		  if(d1*d2<0||d3*d4<0)
		    {
		      if(d1*d2<0)outInd=0;
		      else {d1=d3;d2=d4;outInd=1;}
		    }
		  else
		    {
		      if(d1.R()<d2.R()&&d1.R()<d3.R()&&d1.R()<d4.R()){outInd=0;d=d1;}
		      else if(d2.R()<d1.R()&&d2.R()<d3.R()&&d2.R()<d4.R()){outInd=0;d=d2;}
		      else if(d3.R()<d2.R()&&d3.R()<d1.R()&&d3.R()<d4.R()){outInd=1;d=d3;}
		      else {outInd=1;d=d4;}
		    }
		}
	      ret.setValues(p[outInd].X(),p[outInd].Y(),p[outInd].Z(),d.X(),d.Y(),d.Z(),normal.X(),normal.Y(),normal.Z(),sigma*(center-p[outInd]).R());
	      return ret;
	    }
	  if(t[0]<0||t[1]<0)
	    {
		  ...
		  if(ph[outInd]<lpe.Phi()||ph[outInd]>hpe.Phi())
		    {
		      ...
		      d1=ll1-p[outInd];d2=ll2-p[outInd];
		      if(d1*d2>0)
			{
			  if(d1.R()<d2.R())d=d1;
			  else d=d2;
	    ...
	    {
	      if(t[0]<t[1])outInd=0;else outInd=1;
	      if(z[outInd]<outerR*ca&&z[outInd]>innerR*ca)
		{
		  if(ph[outInd]<lpe.Phi()||ph[outInd]>hpe.Phi())
		    {
	  ...
	  if(z[0]>outerR*ca&&z[1]>outerR*ca)
	    {
	      ret.setValues(p[0].X(),p[0].Y(),p[0].Z(),0,0,0,normal.X(),normal.Y(),normal.Z(),sigma*(center-p[0]).R());
	      return ret;
	    }
	}
    }
  ...
  if(!(platePoint.getState()==_undefined_))
      ...
      if(distBarrel<=0)
	{
	  if((r<=outerR)&&(r>=innerR)&&pph<=ddphi&&pph>=0)//(pl<=dphi)&&(ph<=dphi))
	    {
	      ret.setValues(platePoint.X(),platePoint.Y(),platePoint.Z(),0,0,0,plate.Normal().X(),plate.Normal().Y(),plate.Normal().Z(),sigma*(center-platePoint).R());
	      return ret;
	    }
	}
      else
	{
	  if((r<=outerR)&&(r>=innerR)&&pph<=ddphi&&pph>=0)//(v1_*v2_<=0))
	    {
	      ret.setValues(platePoint.X(),platePoint.Y(),platePoint.Z(),0,0,0,plate.Normal().X(),plate.Normal().Y(),plate.Normal().Z(),sigma*platePoint.Rho());
	      //	      cout<<ret<<endl;
	      return ret;
	    }
	}
  ...
  if(dis.R()>dis1.R())dis=dis1;
  ret.setValues(platePoint.X(),platePoint.Y(),platePoint.Z(),dis.X(),dis.Y(),dis.Z(),plate.Normal().X(),plate.Normal().Y(),plate.Normal().Z(),sigma*(center-platePoint).R());
  return ret;
  //end acceleration
  //check side planes
  ...
  if(((distBarrel<=0)&&(pl<=dphi)&&(ph<=dphi))||((distBarrel>0)&&(v1_*v2_<=0)))
    {
      if (r<innerR)//check if hit from inside
	{
	  ...
	  if(l>0)
		...
		p=line.Foot()+t1*line.Direction();
	      ...
	      v1_a.normalize();
	      if(v2_a*normal<0&&v2_a.length()<=thickness&&(acos(v1_a*lpe)<dphi&&(acos(v1_a*hpe)<dphi)))
		{
		  ret.setValues(p.X(),p.Y(),p.Z(),0,0,0,v1_a.X(),v1_a.Y(),v1_a.Z(),0.5*thickness);
		  return ret;
		}
	    }
	}
  ...
  if((dist1.length()<dist2.length()&&(!(platePoint.getState()==_undefined_)))||(sidePoint.getState()==_undefined_))
    {
...
// 	cout<<line<<b*b-4*a*cc<<endl;
	if(b*b-4*a*cc<0)
	...
	else//(b*b-4*a*cc>=0)
	  {
	    ...
	    int s[2];
	    double phii=lpe.Phi();
	    if(phii<ph[0])s[0]= (int)(48.*(ph[0]-phii)/M_PI);
	    else s[0]=(int)(48.*(ph[0]-phii+2*M_PI)/M_PI);
	    if(phii<ph[1])s[1]= (int)(48.*(ph[1]-phii)/M_PI);
	    else s[1]= (int)(48.*(ph[1]-phii+2*M_PI)/M_PI);
	    if(t[0]<0&&t[1]<0)return -1;
	    if((z[0]<innerR*ca||z[0]>outerR*ca)&&(z[1]<innerR*ca||z[1]>outerR*ca))return -1;
