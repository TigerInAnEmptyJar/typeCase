...
sphericRectangle::sphericRectangle(point3D a, point3D b, point3D c, point3D d):planeShape("sphericRectangle")
{
  vector3D a_b(b-a);
  vector3D c_d(c-d);
  float l1=a_b.R(),l2=c_d.R();
  if(l1-l2>__prec) return;
  sLine3D AB(a,a_b);
  sLine3D DC(d,c_d);
  lLine3D hit=AB-DC;
  if((hit.P()-hit.Q()).R()>__prec) return;
  A_=a;
  B_=b;
  C_=c;
  D_=d;
...
sphericRectangle::sphericRectangle(point3D mid, vector3D lp, vector3D hp, float ra, float ri):planeShape("sphericRectangle")
{
  lp.normalize();
  hp.normalize();
  normal=lp^hp;
...
sphericRectangle::sphericRectangle(point3D mid, vector3D lp, vector3D nor, float ra, float ri, float dp):planeShape("sphericRectangle")
{
//   nor.normalize();
  lp.normalize();
//   normal=nor^lp;
...
sphericRectangle::sphericRectangle(const shape_parameter &description):planeShape("sphericRectangle")
{
  if(description.getName()!="sphericRectangle")return;
  if(description.NumberOfParams<point3D>()<5)return;
...
point3D tmp;
  ...
  if (tmp.getState()==_undefined_) return;
...
void sphericRectangle::setPoints(point3D mid, vector3D lp, vector3D nor, float ra, float ri, float dp)
{
  normal=nor;
  normal.normalize();
  lp.normalize();
...
{
  if((b-a).length()!=(c-d).length()) return;
  sLine3D AB(a,b-a);
  sLine3D DC(d,c-d);
  lLine3D hit=AB-DC;
  if(!(hit.P()==hit.Q())) return;
  A_=a;
  B_=b;
  C_=c;
  D_=d;
...
point3D sphericRectangle::A()const
{
  return A_;
}

point3D sphericRectangle::B()const
{
  return B_;
}

point3D sphericRectangle::C()const
{
  return C_;
}

point3D sphericRectangle::D()const
{
  return D_;
}
...
int sphericRectangle::getNumberOfPoints()const
{
  return 4;
}
