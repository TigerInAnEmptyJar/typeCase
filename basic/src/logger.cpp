...
logMan decI(4);
logMan kOf(5);
logMan::logMan(int idIn)
{
    id=idIn;
}

int logMan::getID()const
{
    return id;
}
...
logger &logger::operator<<(const logMan &man)
{
	...
	break;
    case 1: level++;break;
    case 2: level--;*output<<endl;break;
    case 3: 
	*output<<setw(man.getTer());
	break;
    case 4: level--;break;
    case 5: delete output;
...
logger &logger::operator<<(const vector3D &val)
{
  ...
  *output<<val;
  output->flush();
  emit textAdded("("+string_number(val.X())+", "+string_number(val.Y())+", "+string_number(val.Z())+")");
  return *this;
}
...
logger &logger::operator<<(const lLine3D &val)
{
  ...
  *output<<val;
  output->flush();
  emit textAdded("(p = ("+string_number(val.P().X())+", "+string_number(val.P().Y())+", "+string_number(val.P().Z())+"), q = ("+string_number(val.Q().X())+", "+string_number(val.Q().Y())+", "+string_number(val.Q().Z())+"))");
  return *this;
}
...
logger &logger::operator<<(const sLine3D &val)
{
  ...
  *output<<val;
  output->flush();
  emit textAdded("(p = ("+string_number(val.Foot().X())+", "+string_number(val.Foot().Y())+", "+string_number(val.Foot().Z())+") + t * ("+string_number(val.Direction().X())+", "+string_number(val.Direction().Y())+", "+string_number(val.Direction().Z())+"))");
  return *this;
}
...
logger &logger::operator<<(const plane3D &val)
{
  ...
  *output<<val;
  output->flush();
  emit textAdded("(p - ("+string_number(val.Foot().X())+", "+string_number(val.Foot().Y())+", "+string_number(val.Foot().Z())+") ) * ("+string_number(val.Normal().X())+", "+string_number(val.Normal().Y())+", "+string_number(val.Normal().Z())+"))");
  return *this;
}
...
logger &logger::operator<<(const vector4D &val)
{
  ...
  *output<<val;
  output->flush();
  emit textAdded("("+string_number(val.X())+", "+string_number(val.Y())+", "+string_number(val.Z())+", "+string_number(val.W())+")");
  return *this;
}
