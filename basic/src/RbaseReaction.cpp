...
RbaseReaction::RbaseReaction(AFitAlgorithm *kinFitIn, int nTracks, int nInits):TBase("RbaseReaction"),numberOfInitParticles(nInits),numberOfTracks(nTracks)
{
  fkinFit=kinFitIn;
...
RbaseReaction::RbaseReaction(const RbaseReaction &rIn):TBase("RbaseReaction"),numberOfInitParticles(rIn.getNumberOfPrimaries()),numberOfTracks(rIn.getNumberOfTracks())
{
...
int RbaseReaction::getNumberOfTracks() const
{
  return numberOfTracks;
}

int RbaseReaction::getNumberOfPrimaries() const
{
  return -1;
}

int RbaseReaction::getNumberOfSecondaries() const
{
  return -1;
}
...
{
  if(num<0||num>=numberOfParticles) return momentum4D();
  else
...
{
  if(num<0||num>numberOfTracks)return;
  tracks[num]=track;
}
TTrack* RbaseReaction::getTrack(int num)
{
  if(num<0||num>numberOfTracks)return NULL;
...
// float RbaseReaction::justCheck(TTrack **tracksIn, int num, int *indexes, momentum4D *results)
// {
//   return -1;
// }
...
float RbaseReaction::getChiLimit()const
{
  return chiLimit;
}
...
{
  if(num<0||num>numberOfInitParticles)return momentum4D();
  return initParticles[num];
}

float RbaseReaction::getExcessEnergy()const
{
  return excessEnergy;
}

momentum4D RbaseReaction::getCenterOfMass()const
{
  return cm;
}
...
float RbaseReaction::getChiSquared() const
{
  return chi2;
}
AFitAlgorithm *RbaseReaction::getKinFit()const
{
  return fkinFit;
}
...
RbaseReaction *RbaseReaction::getClone()
{
  return new RbaseReaction(*this);
}
...
{
  if(num>=numberOfParticles||num<0)return -1;
  return particleIDs[num];
}
void RbaseReaction::calculate(int justGeometry)
{
  if(justGeometry<1000)calculateCM();
}
float RbaseReaction::check(TTrack **tracks_nd, int num_nd, TTrack **tracks_1s, int num_1s, TTrack **tracks_2s, int num_2s)
{
  if(tracks_nd==NULL||tracks_1s==NULL||tracks_2s==NULL)return -1;
  if(num_nd==0&&num_1s==0&&num_2s==0)return -1;
  return -1;
}
float RbaseReaction::justCheck(TTrack **tracks_nd, int num_nd, TTrack **tracks_1s, int num_1s, TTrack **tracks_2s, int num_2s, int *indexes, momentum4D *results){  
  if(tracks_nd==NULL||tracks_1s==NULL||tracks_2s==NULL)return -1;
  if(num_nd==0&&num_1s==0&&num_2s==0)return -1;
  if(indexes==NULL||results==NULL)return -1;
  return -1;
}
RbaseReaction* RbaseReaction::checkNew(TTrack **tracks_nd, int num_nd, TTrack **tracks_1s, int num_1s, TTrack **tracks_2s, int num_2s)
{
  if(tracks_nd==NULL||tracks_1s==NULL||tracks_2s==NULL)return NULL;
  if(num_nd==0&&num_1s==0&&num_2s==0)return NULL;
  return NULL;
}
vector3D solve(const vector3D &d1, const vector3D &d2, const vector3D &d3, const vector3D &s)
{
  Vector l[3];
  for(int i=0;i<3;i++)l[i].reDimension(4);
  for(int i=0;i<3;i++)
    {
...
//   cout<<endl;
  for(int i=0;i<3;i++)
    if(l[0][i]==0&&l[1][i]==0&&l[2][i]==0)return vector3D();	
  Vector *ll[3]={NULL,NULL,NULL};
  for(int i=0;i<3;i++)ll[i]=&l[i];
  ...
  ret.setValue(0,(ll[0]->getValue(3)-ret[2]*ll[0]->getValue(2)-ret[1]*ll[0]->getValue(1))/ll[0]->getValue(0));
  return ret;
}
