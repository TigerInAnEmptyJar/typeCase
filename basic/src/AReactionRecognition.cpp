...
int nCandidates=0;
int nPassed=0;
AReactionRecognition::AReactionRecognition(TSetup &setup,TEvent &eventIn,AFitAlgorithm *kinfitIn,string options):AAlgorithm("reaction recognition for "+options),event(eventIn)
{
...
AReactionRecognition::AReactionRecognition(TSetup &setup,TEvent &eventIn,AFitAlgorithm *kinfitIn,vector<algorithm_parameter> processes):AAlgorithm("reaction recognition"),event(eventIn)
{
  if(!Eparticles::IsInit())Eparticles::init();
  kinfit=kinfitIn;
...
void *AReactionRecognition::process(void *ptr)
{
  ...
  for(int i=0;i<event.getNumberOfTracks();i++)
	...
	switch(tracks[i]->getNumberOfSecondaryTracks())
	  {
	  case 0: tr_p[np]=tracks[i];np++;break;
	  case 1: tr_k[nk]=tracks[i];nk++;break;
	  case 2: tr_v[nv]=tracks[i];nv++;break;
	  }
      }
  //  cout<<np<<" "<<nv<<" "<<nk<<endl;
  if(np==2&&nv==1){nCandidates++;}//cout<<"candidate "<<event.getEventNumber()<<endl;}
  for(int i=0;i<numberOfReactions;i++)
    {
  ...
  //  if(np==2&&nv==1)cout<<"\t "<<event.getEventNumber()<<" "<<event.getNumberOfReactions()<<endl;
  return ptr;
}
