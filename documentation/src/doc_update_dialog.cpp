...
#if QT_VERSION < 0x040000
doc_update_dialog::doc_update_dialog(QWidget* parent, bool modal, WFlags fl)
    : QDialog(parent, "update dialog", modal, fl)
{
...
doc_update_dialog::doc_update_dialog(QWidget* parent, Qt::WindowFlags fl) : QDialog(parent, fl)
{
  ...
  connect(togetherView, SIGNAL(rightButtonClicked(QListViewItem*, const QPoint&, int)), this,
          SLOT(onTogeterClick(QListViewItem*)));
#else
  setWindowTitle("Update Class");
  setWindowState(Qt::WindowNoState);
  togetherView->setContextMenuPolicy(Qt::CustomContextMenu);
  connect(togetherView, SIGNAL(customContextMenuRequested(const QPoint&)), this,
          SLOT(onTogeterClick(const QPoint&)));
#endif
...
void doc_update_dialog::checkBoxes()
{
  ...
  for (unsigned int i = 0; i < oldBox->count(); i++) {
    ...
    for (unsigned int j = 0; j < newBox->count(); j++)
      if (newBox->text(j) == oldBox->text(i))
        m = j;
    if (m >= 0) {
...
nameTypeDoc* doc_update_dialog::getDoc(int tp, const QString& nme)
{
  ...
  if (cls == NULL)
    return NULL;
  for (int j = 0; j < cls->nConstructors(); j++)
    if (nme == cls->Constructor(j).toString().data())
      return &cls->Constructor(j);
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < cls->nSlots(i); j++)
      if (nme == cls->Slot(j, i).toString().data())
        return &cls->Slot(j, i);
    for (int j = 0; j < cls->nMethods(i); j++)
      if (nme == cls->Method(j, i).toString().data())
        return &cls->Method(j, i);
    for (int j = 0; j < cls->nVariables(i); j++)
      if (nme == cls->Variable(j, i).toString().data())
        return &cls->Variable(j, i);
  }
  for (int j = 0; j < cls->nSignals(); j++)
    if (nme == cls->Signal(j).toString().data())
      return &cls->Signal(j);
  return NULL;
}
void doc_update_dialog::accept()
{
  ...
  while (it != NULL) {
    ...
    if (ndoc != NULL && odoc != NULL) {
      ...
      for (int i = 0; i < odoc->DescriptionLines(); i++)
        ndoc->addDescriptionLine(odoc->Description(i));
    }
    it = it->nextSibling();
  }
  QMessageBox::information(this, "Property assignment",
                           QString::number(num) + " properties assigned", QMessageBox::Ok);
  if (newBox->count() > 0)
    QMessageBox::information(this, "Property assignment",
                             QString::number(newBox->count()) + " new properties unassigned",
                             QMessageBox::Ok);
  if (oldBox->count() > 0)
    QMessageBox::information(this, "Property assignment",
                             QString::number(oldBox->count()) + " old properties unassigned",
                             QMessageBox::Ok);
...
bool doc_update_dialog::classesMatch()
{
  if (oldClass == NULL || newClass == NULL)
    return false;
  if (oldBox->count() != 0 || newBox->count() != 0)
    return false;
  accept();
  return true;
}
