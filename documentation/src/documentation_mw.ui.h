...
{
  string s = getOpenFileName(this, "Open file", (string(getenv("KTOFPACK")) + "/doc/").data(),
                             "Class description files (*.data)");
  if (s == "")
    return;
  if (!existing(s))
    return;
  ...
  input.close();
  OnClassClick();
  s = string("file: ") + s + " opened, " + string_number((long)classes.size() - ncl) +
      "classes read";
#if QT_VERSION < 0x040000
...
{
  string s = getOpenFileName(this, "Open file", (string(getenv("KTOFPACK")) + "/include").data(),
                             "C++ Header  files (*.h)");
  if (s == "")
    return;
  if (!existing(s))
...
void documentation_mw::OnFileSaveAs()
{
  ...
  if (n == -1)
    return;
  string s =
      getSaveFileName(this, "save class description", (string(getenv("KTOFPACK")) + "/doc/").data(),
                      "Class description files (*.data)");
  if (s == "")
    return;
  if (existing(s)) {
...
void documentation_mw::OnFileExit() { close(); }
void documentation_mw::OnFileClose()
{
  ...
  if (!existing(files[n]) || files[n] == "") {
    ...
    int res = QMessageBox::question(this, "save file",
                                    "Do you want to save this class before closing it?",
...
void documentation_mw::OnFileOpenDirectory()
{
  ...
  for (int i = 0; i < (int)dir.count(); i++) {
    if ((int)QS2s(dir[i]).rfind(".data") != (int)dir[i].length() - 5)
      continue;
    nF++;
    ifstream input;
  ...
  s = string("directory: ") + s + " opened, " + string_number(nF) + " files read, " +
      string_number(ncl) + "classes read";
#if QT_VERSION < 0x040000
...
void documentation_mw::OnClassClick()
{
  ...
  it = new QListViewItem(
      listView1, "Slots",
      (string_number(classes[n].nSlots(0) + classes[n].nSlots(1) + classes[n].nSlots(2))).data());
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < classes[n].nSlots(i); j++) {
      ac = new QListViewItem(it, classes[n].Slot(j, i).Name(), classes[n].Slot(j, i).Type(),
                             (i == 0 ? "public" : (i == 1 ? "protected" : "private")));
      addParameterToListView(ac, classes[n].Slot(j, i));
    }
  }
  it = new QListViewItem(
      listView1, "Functions",
      (string_number(classes[n].nMethods(0) + classes[n].nMethods(1) + classes[n].nMethods(2)))
          .data());
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < classes[n].nMethods(i); j++) {
      ac = new QListViewItem(it, classes[n].Method(j, i).Name(), classes[n].Method(j, i).Type(),
                             (i == 0 ? "public" : (i == 1 ? "protected" : "private")));
      addParameterToListView(ac, classes[n].Method(j, i));
    }
  }
  it = new QListViewItem(listView1, "Constructors",
                         (string_number(classes[n].nConstructors())).data());
  for (int j = 0; j < classes[n].nConstructors(); j++) {
    ac = new QListViewItem(it, classes[n].Constructor(j).Name(), classes[n].Constructor(j).Type(),
                           "public");
    addParameterToListView(ac, classes[n].Constructor(j));
  }
  it = new QListViewItem(listView1, "Variables",
                         (string_number(classes[n].nVariables(0) + classes[n].nVariables(1) +
                                        classes[n].nVariables(2)))
                             .data());
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < classes[n].nVariables(i); j++)
      new QListViewItem(it, classes[n].Variable(j, i).Name(), classes[n].Variable(j, i).Type(),
                        (i == 0 ? "public" : (i == 1 ? "protected" : "private")),
                        classes[n].Variable(j, i).Default());
  listView2->clear();
  ...
  it = new QTreeWidgetItem(
      listView1, QStringList() << "Signals" << (string_number(classes[n].nSignals())).data());
  for (int j = 0; j < classes[n].nSignals(); j++) {
    ac = new QTreeWidgetItem(it, QStringList() << classes[n].Signal(j).Name().data()
                                               << classes[n].Signal(j).Type().data() << visi[0]);
    addParameterToListView(ac, classes[n].Signal(j));
  }
  it = new QTreeWidgetItem(
      listView1, QStringList() << "Slots"
                               << (string_number(classes[n].nSlots(0) + classes[n].nSlots(1) +
                                                 classes[n].nSlots(2)))
                                      .data());
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < classes[n].nSlots(i); j++) {
      ac = new QTreeWidgetItem(it, QStringList() << classes[n].Slot(j, i).Name().data()
                                                 << classes[n].Slot(j, i).Type().data() << visi[i]);
      addParameterToListView(ac, classes[n].Slot(j, i));
    }
  }
  it = new QTreeWidgetItem(
      listView1, QStringList() << "Functions"
                               << (string_number(classes[n].nMethods(0) + classes[n].nMethods(1) +
                                                 classes[n].nMethods(2)))
                                      .data());
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < classes[n].nMethods(i); j++) {
      ac = new QTreeWidgetItem(it, QStringList() << classes[n].Method(j, i).Name().data()
                                                 << classes[n].Method(j, i).Type().data()
                                                 << visi[i]);
      addParameterToListView(ac, classes[n].Method(j, i));
    }
  }
  it = new QTreeWidgetItem(listView1, QStringList()
                                          << "Constructors"
                                          << (string_number(classes[n].nConstructors())).data());
  for (int j = 0; j < classes[n].nConstructors(); j++) {
    ac = new QTreeWidgetItem(it, QStringList() << classes[n].Constructor(j).Name().data()
                                               << classes[n].Constructor(j).Type().data()
                                               << visi[0]);
    addParameterToListView(ac, classes[n].Constructor(j));
  }
  it = new QTreeWidgetItem(listView1, QStringList() << "Variables"
                                                    << (string_number(classes[n].nVariables(0) +
                                                                      classes[n].nVariables(1) +
                                                                      classes[n].nVariables(2)))
                                                           .data());
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < classes[n].nVariables(i); j++)
      new QTreeWidgetItem(it, QStringList() << classes[n].Variable(j, i).Name().data()
                                            << classes[n].Variable(j, i).Type().data() << visi[i]
                                            << classes[n].Variable(j, i).Default().data());
  listView2->clear();
  for (int i = 0; i < classes[n].nSuperClasses(); i++) {
    new QTreeWidgetItem(listView2, QStringList() << classes[n].SuperClass(i).data()
                                                 << classes[n].SuperClassDoc(i).data());
  }
...
void documentation_mw::addParameterToListView(QListViewItem* it, functionNameTypeDoc& var)
{
  for (int k = var.nParameters() - 1; k >= 0; k--)
    new QListViewItem(it, var.Parameter(k).Name(), var.Parameter(k).Type(), "",
                      var.Parameter(k).Default());
}
#else
void documentation_mw::addParameterToListView(QTreeWidgetItem* it, functionNameTypeDoc& var)
{
  for (int k = var.nParameters() - 1; k >= 0; k--)
    new QTreeWidgetItem(it, QStringList() << var.Parameter(k).Name().data()
                                          << var.Parameter(k).Type().data() << ""
                                          << var.Parameter(k).Default().data());
}
#endif
void documentation_mw::OnEditUpdate()
{
  ...
  if (n == -1)
    return;
  QMessageBox::information(this, "Check header",
                           (string("Check header-file \"") + classes[n].HeaderFile()).data(),
                           QMessageBox::Ok);
  vector<TMyClass> ret;
  TMyClass::fileContent(1, classes[n].HeaderFile(), ret);
  int m = -1;
  for (unsigned int i = 0; i < ret.size(); i++)
    if (classes[n].Name() == ret[i].Name())
      m = i;
  if (m < 0) {
    QMessageBox::critical(this, "Class update", "Update not possible. Class not found in header.",
                          QMessageBox::Abort, 0);
    return;
  }
  ...
  if (!checkBox8->isChecked()) {
    ...
    if (whicht >= 0) {
      int ret = QMessageBox::question(
          this, QString("Remove ") + tp,
          QString("Remove ") + tp + "\"" + property->toString().data() + "\"?",
          QMessageBox::Yes | QMessageBox::Default, QMessageBox::No | QMessageBox::Escape);
      if (ret == QMessageBox::Yes) {
        switch (whicht) {
        case 0:
          classes[n].removeMethod(l, m);
          break;
        case 1:
          classes[n].removeConstructor(m);
          break;
        case 2:
          classes[n].removeVariable(l, m);
          break;
        }
        removeIt = true;
#if QT_VERSION < 0x040000
...
void documentation_mw::OnEditHTML()
{
  ...
  if (n == -1)
    return;
  string s = getSaveFileName(this, "Write html", (string(getenv("KTOFPACK")) + "/doc/").data(),
                             "Web files (*.html)");
  if (s == "")
    return;
...
void documentation_mw::OnEditHTMLAll()
{
  ...
  for (unsigned int i = 0; i < index.size(); i++) {
    string s = getSaveFileName(this, string("Write html for class ") + classes[index[i]].Name(),
                               string(getenv("KTOFPACK")) + "/html/", "Web files (*.html)");
    if (s != "")
  ...
  string s = string("HTML document generated for ") + string_number((long)index.size()) +
             (index.size() > 1 ? " classes " : " class ");
#if QT_VERSION < 0x040000
...
void documentation_mw::OnEditNextItem()
{
  ...
  if (ind >= 0) {
    ...
    while (listView1->topLevelItem(cathe)->childCount() == 0 && iter < 1) {
      while (listView1->topLevelItem(cathe)->childCount() == 0 &&
             cathe < listView1->topLevelItemCount() - 1)
        cathe++;
      if (listView1->topLevelItem(cathe)->childCount() == 0)
        cathe = 0;
    }
    if (listView1->topLevelItem(cathe)->childCount() == 0)
      return;
    ...
    while (listView1->indexOfTopLevelItem(it->parent()) < 0)
      it = it->parent();
    listView1->collapseItem(it);
    cathe = listView1->indexOfTopLevelItem(it->parent());
    num = it->parent()->indexOfChild(it);
    if (num < it->parent()->childCount())
      num++;
    else {
      num = 0;
      int iter = 0;
...
#endif
{
  ...
  if (it->text(0) == tmp->Name().data()) {
    ...
    if (tmp->Doc() != "")
      s += string("Doc: ") + tmp->Doc() + "\n";
    if (tmp->TypeDoc() != "")
      s += string("TypeDoc: ") + tmp->TypeDoc() + "\n";
    if (tmp->Specifier() != "")
      s += string("Specifier: ") + tmp->Specifier() + "\n";
    if (tmp->Default() != "")
      s += string("Default: ") + tmp->Default();
...
#endif
{
  if (it == NULL || prp == NULL)
    return false;
  if (it->text(0) != prp->Name().data() || prp->nParameters() != it->childCount())
    return false;
#if QT_VERSION < 0x040000
  if (prp->nParameters() == 0)
    return true;
  QListViewItem* ac = it->firstChild();
  if (ac->text(1) != prp->Parameter(0).Type().data())
    return false;
  ...
  if (it->parent()->text(0) == "Variables") {
    for (int j = 0; j < 3; j++)
      for (int i = 0; i < classes[n].nVariables(j); i++)
        if (it->text(0) == classes[n].Variable(i, j).Name().data() &&
            it->text(1) == classes[n].Variable(i, j).Type().data())
...
#endif
{
  ...
  else if (getKnownClass(classes[n].SuperClass(m)) != NULL)
...
#endif
{
  ...
  if (it == NULL)
    return;
  string s = QStringInputDialog::getLine(
      this, (string("Class ") + QS2s(it->text(0)) + " documented in").data());
  if (s != "")
    it->setText(1, s.data());
}
...
void documentation_mw::OnKnownFunctionsClick()
{
  if (checkBox8->isChecked()) {
    ...
    for (unsigned int i = 0; i < knownFunctions.size(); i++) {
      addParameterToListView(new QListViewItem(it, knownFunctions[i].Name(),
                                               knownFunctions[i].Type(), "",
                                               knownFunctions[i].Default()),
                             knownFunctions[i]);
    }
  } else {
    OnClassClick();
...
void documentation_mw::OnClassShow() { classShow->show(); }
