...
functionNameTypeDoc::functionNameTypeDoc(const functionNameTypeDoc& n)
    : nameTypeDoc(n.Name(), n.Type(), n.TypeId(), n.Specifier(), n.Doc(), n.TypeDoc())
{
  limiter = n.Limiter();
  clearDescription();
...
functionNameTypeDoc& functionNameTypeDoc::operator=(const functionNameTypeDoc& fn)
{
  ...
  clearDescription();
  for (int i = 0; i < fn.DescriptionLines(); i++)
    addDescriptionLine(fn.Description(i));
  limiter = fn.Limiter();
  clearParameter();
  for (int i = 0; i < fn.nParameters(); i++)
    addParameter(fn.getParameter(i));
  return *this;
}
bool functionNameTypeDoc::operator==(const functionNameTypeDoc& fn)
{
  ...
  if (fn.Type() != Type())
    return false;
  if (fn.Specifier() != Specifier())
    return false;
  if (fn.Limiter() != limiter)
    return false;
  if (nParameters() != fn.nParameters())
    return false;
...
int functionNameTypeDoc::nParameters() const { return parameter.size(); }
...
void functionNameTypeDoc::parse(const string& s1)
{
  ...
  if (hasA(s, "operator")) {
    ...
    while (isSpace(s[opPos + 8]))
      s = s.substr(0, opPos + 8) + s.substr(opPos + 9, s.length() - opPos - 9);
  }
