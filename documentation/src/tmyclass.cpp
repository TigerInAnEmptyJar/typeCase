...
TMyClass::TMyClass(const TMyClass& n)
{
  ...
  for (int j = 0; j < 3; j++)
    for (int i = 0; i < n.nVariables(j); i++)
      addVariable(j, n.getVariable(i, j));
  for (int j = 0; j < 7; j++)
    for (int i = 0; i < n.nMethods(j); i++)
      addMethod(j, n.getMethod(i, j));
  for (int i = 0; i < n.nConstructors(); i++)
    addConstructor(n.getConstructor(i));
...
string TMyClass::Name() const { return className; }
void TMyClass::setName(const string& s) { className = s; }
string TMyClass::Doc() const { return classDoc; }
void TMyClass::setDoc(const string& s) { classDoc = s; }
...
int TMyClass::DescriptionLines() const { return description.size(); }
...
void TMyClass::addDescriptionLine(const string& s) { description.push_back(s); }
string TMyClass::HeaderFile() const { return headerfile; }
void TMyClass::setHeaderFile(const string& s) { headerfile = s; }
int TMyClass::nSuperClasses() const { return supers.size(); }
string TMyClass::SuperClass(int i) const
{
  if (i < 0 || i > (int)supers.size())
    return "";
...
int TMyClass::nVariables(int t) const
{
  switch (t) {
  case 0:
    return variablesPublic.size();
  case 1:
    return variablesProtected.size();
  case 2:
    return variablesPrivate.size();
  }
  return 0;
}
nameTypeDoc& TMyClass::Variable(int i, int t)
{
  switch (t) {
  case 0:
    if (i < 0 || i >= (int)variablesPublic.size())
      return variablesPublic[0];
    return variablesPublic[i];
  case 1:
    if (i < 0 || i >= (int)variablesProtected.size())
      return variablesProtected[0];
    return variablesProtected[i];
  case 2:
    if (i < 0 || i >= (int)variablesPrivate.size())
      return variablesPrivate[0];
...
int TMyClass::nMethods(int t) const
{
  switch (t) {
  case 0:
    return functionsPublic.size();
  case 1:
    return functionsProtected.size();
  case 2:
    return functionsPrivate.size();
  case 3:
    return slotsPublic.size();
  case 4:
    return slotsProtected.size();
  case 5:
    return slotsPrivate.size();
  case 6:
...
functionNameTypeDoc& TMyClass::Method(int i, int t)
{
  switch (t) {
  case 0:
    if (i < 0 || i >= (int)functionsPublic.size())
      return functionsPublic[0];
    return functionsPublic[i];
  case 1:
    if (i < 0 || i >= (int)functionsProtected.size())
      return functionsProtected[0];
    return functionsProtected[i];
  case 2:
    if (i < 0 || i >= (int)functionsPrivate.size())
      return functionsPrivate[0];
    return functionsPrivate[i];
  case 3:
    if (i < 0 || i >= (int)slotsPublic.size())
      return slotsPublic[0];
    return slotsPublic[i];
  case 4:
    if (i < 0 || i >= (int)slotsProtected.size())
      return slotsProtected[0];
    return slotsProtected[i];
  case 5:
    if (i < 0 || i >= (int)slotsPrivate.size())
      return slotsPrivate[0];
    return slotsPrivate[i];
  case 6:
    if (i < 0 || i >= (int)thesignals.size())
      return thesignals[0];
    return thesignals[i];
  }
  return functionsPublic[0];
...
int TMyClass::nSignals() const { return thesignals.size(); }
...
int TMyClass::nConstructors() const { return constructors.size(); }
functionNameTypeDoc TMyClass::getConstructor(int i) const
{
  if (i < 0 || i > (int)constructors.size())
    return functionNameTypeDoc();
...
void TMyClass::addSuperClass(const string& superClassName, const string& superClassDoc, int derived,
                             TMyClass* super)
{
...
void TMyClass::clearVariable(int t)
{
  ...
  switch (t) {
  case 0:
    while (!variablesPublic.empty())
      variablesPublic.pop_back();
    break;
  case 1:
    while (!variablesProtected.empty())
      variablesProtected.pop_back();
    break;
  case 2:
    while (!variablesPrivate.empty())
      variablesPrivate.pop_back();
    break;
  }
}
void TMyClass::addVariable(int t, const nameTypeDoc& var)
{
  ...
  case 0:
    ...
    variablesPublic.back().setDoc(
        string("#v") + string_number((int)(variablesPublic.size() + variablesProtected.size() +
                                           variablesPrivate.size() + 1)));
    break;
  case 1:
    variablesProtected.push_back(var);
    variablesProtected.back().setDoc(
        string("#v") + string_number((int)(variablesPublic.size() + variablesProtected.size() +
                                           variablesPrivate.size() + 1)));
    break;
  case 2:
    variablesPrivate.push_back(var);
    variablesPrivate.back().setDoc(
        string("#v") + string_number((int)(variablesPublic.size() + variablesProtected.size() +
                                           variablesPrivate.size() + 1)));
    break;
  }
}
void TMyClass::addVariable(int t, string nameIn, string typeIn, int typeIdIn, string specifier,
                           string docIn, string typeDocIn)
{
  ...
  case 0:
    ...
    break;
  case 1:
    variablesProtected.push_back(
        nameTypeDoc(nameIn, typeIn, typeIdIn, specifier, docIn, typeDocIn));
    variablesProtected.back().setDoc(
        string("#v") + string_number((int)(variablesPublic.size() + variablesProtected.size() +
                                           variablesPrivate.size() + 1)));
    break;
...
void TMyClass::clearMethod(int t)
{
  if (t < 0 || t > 7)
    return;
  while (!functionType[t].empty())
    functionType[t].pop_back();
  switch (t) {
  case 0:
    while (!functionsPublic.empty())
      functionsPublic.pop_back();
    break;
  case 1:
    while (!functionsProtected.empty())
      functionsProtected.pop_back();
    break;
  case 2:
    while (!functionsPrivate.empty())
      functionsPrivate.pop_back();
    break;
  case 3:
    while (!slotsPublic.empty())
      slotsPublic.pop_back();
    break;
  case 4:
    while (!slotsProtected.empty())
      slotsProtected.pop_back();
    break;
  case 5:
    while (!slotsPrivate.empty())
      slotsPrivate.pop_back();
    break;
  case 6:
    while (!thesignals.empty())
      thesignals.pop_back();
    break;
  }
}
void TMyClass::addMethod(int t, const functionNameTypeDoc& var)
{
  ...
  case 0:
    ...
    functionsPublic.back().setDoc(
        string("#f") + string_number((int)(functionsPublic.size() + functionsProtected.size() +
                                           functionsPrivate.size() + 1)));
    break;
  case 1:
    functionsProtected.push_back(var);
    functionsProtected.back().setDoc(
        string("#f") + string_number((int)(functionsPublic.size() + functionsProtected.size() +
                                           functionsPrivate.size() + 1)));
    break;
  case 2:
    functionsPrivate.push_back(var);
    functionsPrivate.back().setDoc(
        string("#f") + string_number((int)(functionsPublic.size() + functionsProtected.size() +
                                           functionsPrivate.size() + 1)));
    break;
  case 3:
    slotsPublic.push_back(var);
    slotsPublic.back().setDoc(
        string("#l") +
        string_number((int)(slotsPublic.size() + slotsProtected.size() + slotsPrivate.size() + 1)));
    break;
  case 4:
    slotsProtected.push_back(var);
    slotsProtected.back().setDoc(
        string("#l") +
        string_number((int)(slotsPublic.size() + slotsProtected.size() + slotsPrivate.size() + 1)));
    break;
  case 5:
    slotsPrivate.push_back(var);
    slotsPrivate.back().setDoc(
        string("#l") +
        string_number((int)(slotsPublic.size() + slotsProtected.size() + slotsPrivate.size() + 1)));
    break;
  case 6:
...
void TMyClass::addMethod(int t, string nameIn, string typeIn, int typeIdIn, string specifier,
                         string docIn, string typeDocIn)
{
  if (t < 0 || t > 7)
    return;
...
int TMyClass::MethodType(int i, int t) const
{
  if (t < 0 || t > 7)
    return -1;
  if (i < 0 || i >= (int)functionType[t].size())
    return -1;
...
void TMyClass::clearSlots(int t)
{
  if (t < 0 || t > 4)
    return;
  while (!functionType[t + 3].empty())
    functionType[t + 3].pop_back();
  switch (t) {
  case 0:
    while (!slotsPublic.empty())
      slotsPublic.pop_back();
...
void TMyClass::addSlot(int t, string nameIn, string typeIn, int typeIdIn, string specifier,
                       string docIn, string typeDocIn)
{
  if (t < 0 || t > 3)
    return;
...
{
  while (!functionType[6].empty())
    functionType[6].pop_back();
  while (!thesignals.empty())
    thesignals.pop_back();
}
...
void TMyClass::addSignal(string nameIn, string typeIn, int typeIdIn, string specifier, string docIn,
                         string typeDocIn)
{
...
void TMyClass::addConstructor(string typeIn, int typeIdIn, string specifier, string docIn,
                              string typeDocIn)
{
  constructors.push_back(
      functionNameTypeDoc(className, typeIn, typeIdIn, specifier, docIn, typeDocIn));
  constructors.back().setDoc(string("#c") + string_number((int)(constructors.size() + 1)));
}
void TMyClass::removeConstructor(int i) { removeFromVector<functionNameTypeDoc>(constructors, i); }
...
void TMyClass::checkFunctionType()
{
  bool found;
  for (int i = 0; i < 3; i++)
    for (unsigned int j = 0; j < functionType[i].size(); j++)
      functionType[i][j] = 0;
  for (unsigned int k = 0; k < functionsPublic.size(); k++) {
    ...
    cout << "method \"" << functionsPublic[k].Name().data() << "\" is "
         << (found ? "overwritten" : "new") << endl;
    if (found)
      functionType[0][k] = 1;
  }
  for (unsigned int k = 0; k < functionsProtected.size(); k++) {
    ...
    cout << "method \"" << functionsProtected[k].Name().data() << "\" is "
         << (found ? "overwritten" : "new") << endl;
    if (found)
      functionType[1][k] = 1;
  }
  while (functionType.size() > 3)
  ...
  for (unsigned int i = 0; i < supers.size(); i++) {
    ...
    for (int j = 0; j < supers[i]->nMethods(0); j++) {
      ...
      for (unsigned int k = 0; k < functionsProtected.size(); k++)
        if (functionsProtected[k] == supers[i]->Method(j, 0))
          found = true;
      cout << "method \"" << supers[i]->Name().data()
           << "::" << supers[i]->Method(j, 0).Name().data() << "\" is "
           << (found ? "overwritten" : "inherrited") << endl;
    ...
    for (int j = 0; j < supers[i]->nMethods(1); j++) {
      ...
      for (unsigned int k = 0; k < functionsProtected.size(); k++)
        if (functionsProtected[k] == supers[i]->Method(j, 1))
          found = true;
      cout << "method \"" << supers[i]->Name().data()
           << "::" << supers[i]->Method(j, 1).Name().data() << "\" is "
           << (found ? "overwritten" : "inherrited") << endl;
      tmp.push_back((found ? 1 : 0));
    }
...
void TMyClass::addSuperDescriptions()
{
  ...
  for (int j = 0; j < 7; j++) {
    for (unsigned int k = 0; k < allSuperClasses.size(); k++) {
      if (allSuperClasses[k] == NULL)
        continue;
      for (int i = 0; i < allSuperClasses[k]->nMethods(j); i++) {
        if (allSuperClasses[k]->Method(i, j).Name() == string("~") + allSuperClasses[k]->Name())
          continue;
        int found = -1;
        for (int m = 0; m < 7; m++)
        ...
        if (found != -1) {
          for (int l = 0; l < allSuperClasses[k]->Method(i, j).DescriptionLines(); l++)
            Method(found / 8, found % 8)
                .addDescriptionLine(allSuperClasses[k]->Method(i, j).Description(l));
        }
      }
    }
  }
...
vector<string> TMyClass::toHTML(const vector<functionNameTypeDoc>& associatedFunctions) const
{
  ...
  if (hasA(headerfile, "/"))
    ret.push_back(string("<H1><A NAME=\"top\">Class ") + className + " in \"" +
                  headerfile.substr(headerfile.rfind("/") + 1,
                                    headerfile.length() - headerfile.rfind("/") - 1) +
                  "\"</A></H1>");
  else
    ret.push_back(string("<H1><A NAME=\"top\">Class ") + className + " in \"" + headerfile +
                  "\"</A></H1>");
  ...
  for (int i = 0; i < 2; i++)
    nF +=
        newFunctions[i + 3].size() + inherrited[i + 3].size() + inherritedOverwritten[i + 3].size();
  if (nF > 0) {
  ...
  for (unsigned int i = 0; i < functionsPublic.size(); i++) {
    ...
    for (unsigned int j = 0; j < tmp.size(); j++)
      ret.push_back(tmp[j]);
    if (i == functionsPublic.size() - 1 && functionsProtected.size() == 0 &&
        slotsPublic.size() == 0 && slotsProtected.size() == 0 && thesignals.size() == 0)
  ...
  for (unsigned int i = 0; i < functionsProtected.size(); i++) {
    ...
    for (unsigned int j = 0; j < tmp.size(); j++)
      ret.push_back(tmp[j]);
    if (i == functionsProtected.size() - 1 && slotsPublic.size() == 0 &&
        slotsProtected.size() == 0 && thesignals.size() == 0)
      ret.push_back("<BR/><ttop><a href=\"#top\">top</a></ttop>");
    else
      ret.push_back("<BR/><ttop><a href=\"#top\">top</a></ttop><hr/>");
  }
  ...
  ret.push_back("<a rel=\"license\" "
                "href=\"http://creativecommons.org/licenses/by-nc-sa/3.0/"
                "\"><img");
  ret.push_back("alt=\"Creative Commons License\" style=\"border-width:0\"");
  ret.push_back("src=\"http://i.creativecommons.org/l/by-nc-sa/3.0/88x31.png\" "
                "/></a><br />This");
  ret.push_back("work is licensed under a <a rel=\"license\"");
  ret.push_back("href=\"http://creativecommons.org/licenses/by-nc-sa/3.0/\">cc "
                "by-nc-sa 3.0");
...
template <class X>
void TMyClass::removeFromVector(vector<X>& v, int pos)
{
...
ostream& operator<<(ostream& o, const TMyClass& t)
{
  ...
  for (int i = 0; i < t.nSuperClasses(); i++) {
    o << "S" << t.InherritanceType(i) << " " << (t.Super(i) == NULL ? 0 : 1) << " "
      << t.SuperClass(i).data() << endl
      << smallerPath(t.SuperClassDoc(i).data()) << endl;
    if (t.Super(i) != NULL)
...
istream& operator>>(istream& o, TMyClass& t)
{
  ...
  while (o.good() && c != '=') {
    ...
    nameTypeDoc tmpn;
    ty = 0;
    if (c == 'C' || c == 'F' || c == 'f' || c == 'G' || c == 'L' || c == 'l' || c == 'n' ||
        c == 'N')
      o >> tmpf;
    else if (c == 'V' || c == 'v' || c == 'g')
      o >> tmpn;
    ...
    case 'S': {
      ...
      TMyClass* tmp = NULL;
      if (m != 0)
        tmp = getKnownClass(li);
      else
        tmp = getKnownClass(l1);
      if (tmp == NULL && l2 == "") {
...
{
  vector<TMyClass> tmp;
  if (TMyClass::content(datatype, data, tmp) <= 0)
    return false;
  if (nameIn == "") {
    (*this) = tmp[0];
    return true;
...
public:
  datastreamer(const vector<string>& dataIn)
  {
    for (unsigned int i = 0; i < dataIn.size(); i++)
      fdata.push_back(dataIn[i]);
    line = 0;
    col = 0;
  }
  bool eof()
  {
    if (line >= (int)fdata.size())
      return true;
  ...
  string getline()
  {
    if (eof())
      return "";
    while (!eof() && fdata[line][col] == '#' && col == 0)
      line++;
    string ret = "";
    char c = get();
    while (c != '\n' && !eof()) {
...
string eliminateCommentPreproc(datastreamer& dta, string* comment) { return dta.getline(); }
string eliminateCommentPreproc(string dta, string* comment)
{
  ...
  while (hasA(s, "//")) {
    ...
    s = s.substr(s.find("//") + 2, s.length() - s.find("//") - 2);
    if (comment != NULL)
      *comment = (*comment) + s.substr(0, s.find('\n') + 1);
    s = s.substr(s.find('\n'), s.length() - s.find('\n'));
    s = s1 + s;
  }
  while (hasA(s, "#")) {
...
int TMyClass::content(int dataType, const vector<string>& data, vector<TMyClass>& contens)
{
  ...
  while (!streamer.eof()) {
    ...
    while (!(hasA(tmp, '{')                         // class definition
             ...
             || (hasA(tmp, "<") && hasA(tmp, ">") && tmp.find("<") < tmp.find("class") &&
                 tmp.find(">") > tmp.find("class")) // template definition
             || streamer.eof())) {
    ...
    if (hasA(classNameString, ":")) {
      superString = classNameString.substr(
          classNameString.find(":") + 1, classNameString.length() - classNameString.find(":") - 1);
    ...
    if (tmp1 != "")
      newClass.addDescriptionLine(tmp1);
    ...
    while (!finished && !streamer.eof()) {
      ...
      tmp = eliminateCommentPreproc(tmp, NULL);
      minimizeString(tmp);
      if (hasA(tmp, "Q_OBJECT")) // Q_OBJECT is a makro for objects derived from
                                 // QObject. it
      ...
      if (hasA(tmp, "ClassDef")) // TObject is a makro for objects derived from
                                 // TObject. it provides
      ...
      if (hasA(tmp, ':') && !hasA(tmp, "(")) // visibility change
      {
        ...
        if (hasA(tmp1, "public"))
          visibility = 0;
        if (hasA(tmp1, "protected"))
          visibility = 1;
        if (hasA(tmp1, "private"))
          visibility = 2;
        if (hasA(tmp1, "slots"))
          ftype = 1;
        if (hasA(tmp1, "signal"))
          ftype = 2;
        tmp = remove(remove(remove(remove(remove(tmp, "public"), "protected"), "private"), "slots"),
                     "signal");
  ...
  while (i < (int)data.size()) {
    ...
    while (!hasA(tmp, "class ") && (int)data.size() > i + 1) {
      ...
      if (hasA(tmp, "(") && hasA(tmp, ")") && hasA(tmp, ";")) {
        ...
        if (hasA(tmp, "{")) {
          if (contains(tmp, "{") == contains(tmp, "}"))
            tmp = tmp.substr(0, tmp.find("{")) +
                  tmp.substr(tmp.rfind("}") + 1, tmp.length() - tmp.rfind("}") - 1);
          else if (contains(tmp, "{") < contains(tmp, "}"))
            tmp = tmp.substr(tmp.rfind("}") + 1, tmp.length() - tmp.rfind("}") - 1);
          else
            tmp = tmp.substr(0, tmp.find("{"));
    ...
    if (i >= (int)data.size() || !hasA(tmp, "class"))
      return nfound;
    ...
    TMyClass newClass(classNameString);
    if (hasA(superString, ",")) {
      while (superString.rfind(",") == superString.length() - 1 && curvedBrackets <= 0 &&
             (int)data.size() > i + 1) {
        i++;
        tmp = data[i];
        if (tmp[0] == '#')
    ...
    while (!finished && i < (int)data.size()) {
      ...
      if (hasA(tmp, ";")) {
        ...
        if (hasA(propertyString, ":") && !hasA(propertyString, "::")) {
          ...
          if (hasA(tmp1, "signal"))
            ftype = 2;
          propertyString = propertyString.substr(
              propertyString.find(":") + 1, propertyString.length() - propertyString.find(":") - 1);
        }
        if (hasA(propertyString, ",") || (hasA(propertyString, "(") && hasA(propertyString, ")"))) {
          if (hasA(propertyString,
                   ",")) // more than one variable of that type or function
          {
            if (hasA(propertyString, "(") == 0) // more than one variable
            {
              ...
              while (hasA(tmp1, ",")) {
                ...
                remove(tmp, "[");
                remove(tmp, "]");
                nameTypeDoc tmpNN(tmp, tmpN.Type() + cn, tmpN.TypeId(), tmpN.Specifier(),
                                  tmpN.Doc(), tmpN.TypeDoc());
        ...
        tmp = tmp.substr(tmp.find(":") + 1, tmp.length() - tmp.find(":") - 1);
        minimizeString(tmp);
        if (hasA(propertyString, "public"))
          visibility = 0;
        if (hasA(propertyString, "protected"))
          visibility = 1;
        if (hasA(propertyString, "private"))
          visibility = 2;
        if (hasA(propertyString, "slots"))
          ftype = 1;
        if (hasA(propertyString, "signal"))
          ftype = 2;
        if (tmp == "") {
          i++;
          tmp = data[i];
        ...
        while (tmp[i + 1] == '#' && (int)data.size() > i + 2) {
          i++;
        }
        tmp = tmp + data[++i];
        if (hasA(tmp, "//"))
      ...
      if (curvedBrackets <= 0 && hasA(tmp, ";"))
        finished = true;
      if (curvedBrackets - hasA(tmp, "}") <= 0 && tmp.rfind(";") > tmp.rfind("}"))
        finished = true;
    }
    if (finished) {
      newClass.addSuperDescriptions();
...
{
  s = remove(s, '\n');
  while (isSpace(s[0]) && s.length() > 0)
    s = s.substr(1, s.length() - 1);
  while (isSpace(s[s.length() - 1]) && s.length() > 0)
...
bool addSuperFromString(TMyClass& cls, string str)
{
  ...
  while (hasA(str, ",")) {
    ...
    if (hasA(tmp, "public")) {
      tmp = remove(tmp, "public");
      visibility = 0;
    }
    if (hasA(tmp, "protected")) {
      tmp = remove(tmp, "protected");
      visibility = 1;
    }
    if (hasA(tmp, "private")) {
      tmp = remove(tmp, "private");
      visibility = 2;
    }
    tmp = remove(tmp, " ");
  ...
  if (str != "") {
    if (hasA(str, "public")) {
      str = remove(str, "public");
      visibility = 0;
    }
    if (hasA(str, "protected")) {
      str = remove(str, "protected");
      visibility = 1;
    }
    if (hasA(str, "private")) {
      str = remove(str, "private");
      visibility = 2;
    }
    str = remove(str, " ");
    TMyClass* tmpClass = NULL;
...
void init_Q_and_T_object()
{
  ...
  classN.addDescriptionLine("The className() function that returns the class "
                            "name as a string at runtime, ");
  classN.addDescriptionLine("without requiring native runtime type information "
  ...
  inher.addDescriptionLine("the inherits() function that returns whether an "
  ...
  tr1.addDescriptionLine("Returns a translated version of sourceText, or "
                         "sourceText itself if there is no ");
  tr1.addDescriptionLine("appropriate translated version. The translation "
                         "context is QObject with comment ");
  tr1.addDescriptionLine("(0 by default). All QObject subclasses using the "
                         "Q_OBJECT macro automatically have ");
  tr1.addDescriptionLine("a reimplementation of this function with the subclass name as context.");
  tr1.addDescriptionLine("Warning: This method is reentrant only if all "
                         "translators are installed before calling ");
  tr1.addDescriptionLine("this method. Installing or removing translators "
                         "while performing translations is not ");
  tr1.addDescriptionLine("supported. Doing so will probably result in crashes "
                         "or other undesirable behavior. ");
  functionNameTypeDoc tr2("static QString trUtf8( const char* sourceText, const char* comment)");
  tr2.addDescriptionLine("Returns a translated version of sourceText, or "
                         "QString::fromUtf8(sourceText) if there ");
  tr2.addDescriptionLine("is no appropriate version. It is otherwise identical "
                         "to tr(sourceText, comment). ");
  tr2.addDescriptionLine("Warning: This method is reentrant only if all "
                         "translators are installed before calling ");
  tr2.addDescriptionLine("this method. Installing or removing translators "
                         "while performing translations is not ");
  tr2.addDescriptionLine("supported. Doing so will probably result in crashes "
                         "or other undesirable behavior. ");
  ...
  prop1.addDescriptionLine("Returns TRUE if the operation was successful; "
                           "otherwise returns FALSE. ");
  prop1.addDescriptionLine("Information about all available properties is "
                           "provided through the metaObject()");
  ...
  prop2.addDescriptionLine("Information about all available properties are "
                           "provided through the metaObject(). ");
  ...
  metaO.addDescriptionLine("A meta object contains information about a class "
                           "that inherits QObject, e.g. class name,");
  metaO.addDescriptionLine("superclass name, properties, signals and slots. "
                           "Every class that contains the ");
  metaO.addDescriptionLine("Q_OBJECT macro will also have a meta object. ");
  metaO.addDescriptionLine("The meta object information is required by the "
                           "signal/slot connection mechanism ");
  metaO.addDescriptionLine("and the property system. The functions isA() and inherits() also make "
                           "use of the meta object. ");
