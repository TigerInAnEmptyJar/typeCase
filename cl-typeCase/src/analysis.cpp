...
int main(int argc, char** argv)
{
  ...
  srandom(now.date().year() + now.date().day() + now.date().month() + now.time().hour() +
  ...
  if (existing("analysisInit.init")) {
    ifstream configfile;
    configfile.open("analysisInit.init");
  ...
  if (!parameter.rootD) {
    if (parameter.completeDataInput) {
      ...
      if (!parameter.showParams)
        cout << "read data from file \"" << parameter.dataFile.data() << "\"" << endl;
      ...
      if (rparams.size() > 0) {
        ...
        cout << "Warning: no run with this name found: \"" << parameter.runName.data() << "\""
             << endl;
        return 0;
      }
      if (rparams.size() > 20)
  ...
  if (!parameter.showParams)
    cout << mparams.size() << " materials read" << endl;
  if (!parameter.showParams)
  ...
  int nE = modAlgos(aparams, rparams, parameter); // completeDataInput, shiftEvent, EventShift,
  ...
  if (nE == -1 || rparams.size() > 1)
    logg.setEventsToAnalyse(parameter.numberOfEvents);
  else
    logg.setEventsToAnalyse(nE);
  tofAnalysis analyser(200, 30, 1); // nThreads);
  pointer = &analyser;
  QObject::connect(&analyser, SIGNAL(newRun(run_parameter&, beamTime_parameter&)), &logg,
  ...
  if (parameter.showInitState) {
    QObject::connect(&analyser, SIGNAL(initStateChanged(const char*)), &logg,
                     SLOT(onInitState(const char*)));
    QObject::connect(&analyser, SIGNAL(finishStateChanged(const char*)), &logg,
                     SLOT(onCleanUpState(const char*)));
  }
  QTime timer;
  timer.start();
    ...
    } else if (ap[i]->getID() == 63) // track tree output
    {
      ...
      if (param.outpath != "") {
        ...
        if (count > 1)
          ...
          case 0:
            pattern = pattern + "prompt_";
            break;
          case 1:
            pattern = pattern + "vee_";
            break;
          case 2:
...
void loggingClass::setData(string d) { dataAnalysed = d; }
void loggingClass::newRun(run_parameter& r)
{
  dataAnalysed = r.getName();
  cout << "new run: \"" << r.getName().data() << "\"" << endl;
}
void loggingClass::setStartingEvent(int value) { startingEvent = value; }
void loggingClass::setEventsToAnalyse(int value) { eventsTo = value; }
void loggingClass::onInitState(const char* state) { cout << state << endl; }
void loggingClass::onCleanUpState(const char* state) { cout << state << endl; }
                            ...
                            reaction_parameter react)
{
  ...
  out << "############# ANALYSIS: " << loc.tm_mday << "." << loc.tm_mon + 1 << "."
      << 1900 + loc.tm_year << " ##################" << endl;
  out << "############# DATA-BASIS ###########################" << endl;
...
void setAllowedOptions(po::options_description& desc, parameters_t& param)
{
  desc.add_options()("help", "produce help message")("n", po::value<string>(),
      ...
      //    ("M",po::value<string>(),"specify a file containing the used
      //    materials, ROOT version")
      //    ("D",po::value<string>(),"specify a file which Contains the
      //    description of the data and
      //    their calibration, ROOT version")
      ("r", po::value<vector<string>>(),
       "select single run(s) out of data basis, data basis will them be read "
       "with typeCase format. "
       ...
       "513) will be used")("runs", po::value<string>(),
          ...
          "directory to use for local copy of in- and output-files")("nonLocal",
                                                                     "no local directory")(
          "paint", "do printout of event pattern")("H", "show more information about analysis")(
...
bool checkOptions(po::options_description& desc, po::variables_map& vam, parameters_t& param)
{
  ...
  bool show = false;
  if (vam.count("s")) {
    param.setupFile = vam["s"].as<string>();
    param.rootS = false;
  }
  if (vam.count("S")) {
    param.setupFile = vam["S"].as<string>();
  ...
  if (vam.count("a")) {
    param.algoFile = vam["a"].as<string>();
    param.rootA = false;
  }
  if (vam.count("A")) {
    param.algoFile = vam["A"].as<string>();
  ...
  if (vam.count("d")) {
    param.dataFile = vam["d"].as<string>();
    param.rootD = false;
  }
  if (vam.count("D")) {
    param.dataFile = vam["D"].as<string>();
  ...
  if (vam.count("j"))
    param.nThreads = vam["j"].as<int>();
  if (vam.count("l")) {
    param.shiftEvent = true;
    param.EventShift = vam["l"].as<int>();
  }
  if (vam.count("o")) {
    param.outpath = vam["o"].as<string>();
  }
  if (vam.count("H")) {
    show = true;
  }
  if (vam.count("I"))
    param.showInitState = true;
  if (vam.count("local"))
    param.local = true;
  if (vam.count("nonLocal"))
    param.local = false;
  if (vam.count("local-directory")) {
    param.local = true;
    param.localDirectory = vam["local-directory"].as<string>();
  }
  if (vam.count("paint"))
    param.paintIt = true;
  if (vam.count("runs"))
    param.runsFile = vam["runs"].as<string>();
  ...
  if (!existing(param.algoFile)) {
    cout << "algorithms file: \"" << param.algoFile.data() << "\" doesn\'t exist." << endl;
    return false;
  }
  if (!existing(param.setupFile)) {
    cout << "setup file: \"" << param.setupFile.data() << "\" doesn\'t exist." << endl;
    return false;
  }
  if (!existing(param.dataFile)) {
    cout << "data-basis file: \"" << param.dataFile.data() << "\" doesn\'t exist." << endl;
    return false;
  }
  if (!existing(param.matFile)) {
    cout << "material file: \"" << param.matFile.data() << "\" doesn\'t exist." << endl;
    return false;
  }
  if (param.runsFile != "" && existing(param.runsFile)) {
    ifstream runin;
    runin.open(param.runsFile.data());
    string s;
    char c;
    while (!runin.eof()) {
      s = "";
  ...
  if (show) {
    ...
    cout << "start at event " << param.EventShift << " " << (param.shiftEvent ? "on" : "off")
         << endl;
    cout << "use " << param.nThreads << " Threads" << endl;
  }
  if (!param.showParams)
    cout << "Do analysis with " << param.nThreads << " thread(s). For " << param.numberOfEvents
         << " events" << endl;
  return true;
}
