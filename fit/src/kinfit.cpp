...
kinFit::kinFit(const kinFit &inFit):geomObject(),maxParticles(inFit.MaxParticles()),maxIteration(inFit.MaxIterations()),epsilon(inFit.Epsilon())
{
  in =new int*[maxParticles];
  for(int i=0;i< maxParticles;i++)
    in[i]=new int[maxParticles+5];
...
kinFit::kinFit(int maxIterationIn,int maxParticlesIn,float epsilonIn):geomObject(),maxParticles(maxParticlesIn),maxIteration(maxIterationIn),epsilon(fabs(epsilonIn))
{
  in =new int*[maxParticles];
  for(int i=0;i< maxParticles;i++)
    in[i]=new int[maxParticles+5];
...
float kinFit::Epsilon()const
{
  return epsilon;
}
void kinFit::setEpsilon(float value)
{
  epsilon=fabs(value);
}
int kinFit::NParticles()const
{
  return nP;
}
int kinFit::MaxIterations()const
{
  return maxIteration;
}
int kinFit::MaxParticles()const
{
  return maxParticles;
}
void kinFit::clear()
...
void kinFit::add(const vector3D &properties, const vector3D &errors, const vector3D& doFit,float mass, int representation, int in_out_going, int *additionalConstraints)
{
  if(nP>=maxParticles)return;
  ...
  if(additionalConstraints!=NULL)
    ...
    for(int i=0;i<nC;i++)in[nP][i+5]=0;
  nP++;
}
void kinFit::addMomentum(const vector3D &properties, const vector3D &errors, const vector3D& doFit, float mass, int in_out_going, int *additionalConstraints)
{
  if(nP>=maxParticles)return;
  add(properties, errors, doFit, mass, 1, in_out_going, additionalConstraints);
//   cout<<"added Px/Py/Pz :"<<properties<<" error:"<<errors<<", :"<<getMomentum(properties, 1,mass).toString(_cartesic_+4).data()<<endl;
}
void kinFit::addEnergyThetaPhi(const vector3D &properties, const vector3D &errors, const vector3D& doFit, float mass, int in_out_going, int *additionalConstraints)
{
  if(nP>=maxParticles)return;
  add(properties, errors, doFit, mass, 2, in_out_going, additionalConstraints);
//   cout<<"added E/th/ph :"<<properties<<" error:"<<errors<<", :"<<getMomentum(properties, 2,mass).toString(_spheric_+8).data()<<endl;
}
void kinFit::addBetaThetaPhi(const vector3D &properties, const vector3D &errors, const vector3D& doFit, float mass, int in_out_going, int *additionalConstraints)
{
  if(nP>=maxParticles)return;
  add(properties, errors, doFit, mass, 3, in_out_going, additionalConstraints);
//   cout<<"added ß/th/ph :"<<properties<<" error:"<<errors<<", :"<<getMomentum(properties, 3,mass).toString(_spheric_+12).data()<<endl;
}
void kinFit::addMomentumThetaPhi(const vector3D &properties, const vector3D &errors, const vector3D& doFit, float mass, int in_out_going, int *additionalConstraints)
{
  if(nP>=maxParticles)return;
  add(properties, errors, doFit, mass, 4, in_out_going, additionalConstraints);
//   cout<<"added P/th/ph :"<<properties<<" error:"<<errors<<", :"<<getMomentum(properties, 4,mass).toString(_spheric_+4).data()<<endl;
}
void kinFit::addEnergyDxDy(const vector3D &properties, const vector3D &errors, const vector3D& doFit, float mass, int in_out_going, int *additionalConstraints)
{
  if(nP>=maxParticles)return;
  add(properties, errors, doFit, mass, 5, in_out_going, additionalConstraints);
//   cout<<"added E/dx/dy :"<<properties<<" error:"<<errors<<", :"<<getMomentum(properties, 5,mass).toString(_spheric_+4).data()<<endl;
}
void kinFit::addBetaDxDy(const vector3D &properties, const vector3D &errors, const vector3D& doFit, float mass, int in_out_going, int *additionalConstraints)
{
  if(nP>=maxParticles)return;
  add(properties, errors, doFit, mass, 7, in_out_going, additionalConstraints);
//   cout<<"added ß/dx/dy :"<<properties<<" error:"<<errors<<", :"<<getMomentum(properties, 7,mass).toString(_spheric_+12).data()<<endl;
}
void kinFit::addMomentumDxDy(const vector3D &properties, const vector3D &errors, const vector3D& doFit, float mass, int in_out_going, int *additionalConstraints)
{
  if(nP>=maxParticles)return;
  add(properties, errors, doFit, mass, 6, in_out_going, additionalConstraints);
//   cout<<"added P/dx/dy :"<<properties<<" error:"<<errors<<", :"<<getMomentum(properties, 6,mass).toString(_spheric_+4).data()<<endl;
}

vector3D kinFit::Errors(int num)const
{
  if(num<0||num>=nP)return vector3D();
  return dpin[num];
}
...
{
  if(constraint<0||constraint>=nC)return -1;
  return ain[constraint];
}
int kinFit::Constraints()const
{
  return nC;
}
float kinFit::Chi2()const
{
  return chi;
}
int kinFit::OutInfo()const
{
  return outCode;
}
...
float kinFit::fit()
{
  ...
  epsM.transpose();
  epsU.transpose();
  for(int i=0;i<nmea;i++)  epsM.setValue(i,0);
  for(int i=0;i<numea;i++)  epsU.setValue(i,0);
  ...
  if(false)//useCovariant)
      ...
      for(int i=0;i<nMeasured;i++)
	{
	  GM_1.setValue(i,i,
			...
			dpin[Meas[i]/10].getValue((int)(Meas[i]%10)));
	  GM.setValue(i,i,
 		      1./dpin[Meas[i]/10].getValue((int)(Meas[i]%10))*
		      1./dpin[Meas[i]/10].getValue((int)(Meas[i]%10)));
	}
    }
...
//      chi2_prev=chi2;
	...
	chi2=epsM*((GM)*epsM)+epsU*(GU*epsU)+2*al*(FI+BM*epsM+BU*epsU);
      ...
      calculateCinematics(ppx, FM, sf, d);
//       cout<<iteration<<"<"<<maxIteration<<":"<<FM<<" \t"<<d<<" "<<chi2<<endl;
//      iteration++;
	...
	{
	  outCode=iteration;
	  //	  cout<<"well done "<<iteration<<" "<<outCode<<" "<<d_last<<endl;
	  break;
	}
      if(d>d_prev)
	{
      ...
      if(iteration>maxIteration)
	{
	  outCode=iteration;
	  outCode=outCode*-1;
	  //	  cout<<"maximum number of iterations exceeded "<<iteration<<" "<<outCode<<" "<<d_last<<endl;
	  break;
	}
    }
  chi=chi2;
...
void kinFit::checkValues(int rep,vector3D &pm,bool m1, bool m2, bool m3)
{
  ...
  if(m1)
	...
	if(abs(kin)>1)kin=(kin>0?1:-1)*0.9995;
    }
  switch(rep)
    {
    case 1:return;
    case 2:pm.setValues(kin,th,ph);return;
    case 3:pm.setValues(kin,th,ph);return;
    case 4:pm.setValues(kin,th,ph);return;
    case 5:pm.setValues(kin,dx,dy);return;
    case 7:pm.setValues(kin,dx,dy);return;
    }
}
...
vector4D kinFit::getPDerivative(int isIn,int pos,vector3D pm,float mass)
{
  ...
  switch(abs(isIn))
    {
    case 1://px,py,pz
      ret.setValues((pos==0?1:0),(pos==1?1:0),(pos==2?1:0),pm.getValue(pos)/sqrt(pm.R()*pm.R()+mass*mass));
      break;
    case 2://E,th,ph
      {
	...
	switch(pos)
	  ...
	  case 0:		
	    ret.setValues((kin+mass)/tmpf*(sin(th)*cos(ph)),
			  (kin+mass)/tmpf*(sin(th)*sin(ph)),
			  (kin+mass)/tmpf*(cos(th)),1);
	    break;
	  case 1:		  
	    ret.setValues(tmpf*(cos(th)*cos(ph)),
			 tmpf*(cos(th)*sin(ph)),
			 -tmpf*(sin(th)),0);
	    break;
	  case 2:	  
	    ret.setValues(-tmpf*(sin(th)*sin(ph)),
			 tmpf*(sin(th)*cos(ph)),
			 0,0);
	    break;
	  }
	...
	switch(pos)
	  {
	  case 0:		
	    ret.setValues(tmpf*(sin(th)*cos(ph)),
			  tmpf*(sin(th)*sin(ph)),
			  tmpf*(cos(th)),mass*kin*g*g*g);
	    break;
	  case 1:		  
	    ret.setValues(p*(cos(th)*cos(ph)),
			  p*(cos(th)*sin(ph)),
			  -p*(sin(th)),0);
	    break;
	  case 2:	  
	    ret.setValues(-p*(sin(th)*sin(ph)),
			  p*(sin(th)*cos(ph)),0,0);
	    break;
	  }
	break;
	...
	switch(pos)
	  {
	  case 0:	
	    ret.setValues(sin(th)*cos(ph),
			  sin(th)*sin(ph),
			  cos(th),
			  kin/sqrt(kin*kin+mass*mass));
	    break;
	  case 1:
	    ret.setValues(kin*cos(th)*cos(ph),
			  kin*cos(th)*sin(ph),
			  -kin*sin(th),0);
	    break;
	  case 2:
	    ret.setValues(-kin*sin(th)*sin(ph),
			  kin*sin(th)*cos(ph),
			  0,0);
	    break;
	  }
	...
	switch(pos)
	  {
	  case 0:		
	    ret.setValues((kin+mass)/tmpf*(dx/l),
			  (kin+mass)/tmpf*(dy/l),
			  (kin+mass)/tmpf*(1/l),1);
	    break;
	  case 1:		  
	    ret.setValues(tmpf*(l*l-dx*dx)/l/l/l,
			  -tmpf*(dx*dy/l/l/l),
			  -tmpf*(dx/l/l/l),0);
	    break;
	  case 2:	  
	    ret.setValues(-tmpf*(dx*dy/l/l/l),
			  tmpf*(l*l-dy*dy)/l/l/l,
			  -tmpf*(dy/l/l/l),0);
	    break;
	  }
	...
	switch(pos)
	  ...
	  case 0:		
	    ...
	    break;
	  case 1:		  
	    ret.setValues(kin*(l*l-dx*dx)/l/l/l,
			  -kin*(dx*dy/l/l),
			  -kin*(pm.Y()/l/l/l),0);
	    break;
	  case 2:	  
	    ret.setValues(-kin*(dx*dy/l/l/l),
			  kin*(l*l-dy*dy)/l/l/l,
			  -kin*(dy/l/l/l),0);
	    break;
	  }
	...
	switch(pos)
	  {
	  case 0:		
	    ret.setValues(tmpf*(dx/l),
			  tmpf*(dy/l),
			  tmpf*(1/l),
			  mass*kin/g/g/g);
	    break;
	  case 1:		  
	    ret.setValues(p*(l*l-dx*dx)/l/l/l,
			  -p*(dx*dy/l/l/l),
			  -p*(dx/l/l/l),0);
	    break;
	  case 2:	  
	    ret.setValues(-p*(dx*dy/l/l/l),
			  p*(l*l-dy*dy)/l/l/l,
			  -p*(dy/l/l/l),0);
	    break;
	  }
...
float kinFit::getMDerivative(int representation, int pos, vector3D pm, float mass, Vector sf)
{
  ...
  for(int i=0;i<3;i++)sum+=sf[i]*der[i];
...
momentum4D kinFit::getMomentum(int num,vector3D pm)
{
  ...
  switch(abs(in[num][3]))
	...
	ret.setPM(vector3D(P*sin(th)*cos(ph),
			   P*sin(th)*sin(ph),
			   P*cos(th)),
		  am[num]);
	break;
    ...
    case 3://v,th,ph
      {
	ret.setVM(kin*vector3D(cos(ph)*sin(th),
			       sin(ph)*sin(th),
			       cos(th)),am[num]);
	break;
      }
    case 4://p,th,ph
      {
	ret.setPM(kin*vector3D(cos(ph)*sin(th),
			       sin(ph)*sin(th),
			       cos(th)),am[num]);break;
      }
    case 5://E,dx,dy
...
momentum4D kinFit::getMomentum(vector3D pm, int rep,float mass)
{
  ...
  switch(rep)
    {
    case 1:ret.setPM(pm,mass);break;
    case 2://E,th,ph
      {
	float P=sqrt(kin*(kin+2*mass));
	ret.setPM(P*vector3D(cos(ph)*sin(th),
			     sin(ph)*sin(th),
			     cos(th)),
		  mass);
	break;
    ...
    case 3://v,th,ph
      {
	ret.setVM(pm.X()*vector3D(cos(ph)*sin(th),
				  sin(ph)*sin(th),
				  cos(th)),mass);
	break;
      }
...
Vector kinFit::FM_last()const
{return fm;}
float kinFit::D_last()const
{return d_last;}
