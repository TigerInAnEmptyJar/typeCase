...
int lineFit::MaxPoints() const { return maxPoints; }
int lineFit::Points() const { return numPoints; }
...
bool lineFit::Fitted() const { return fitted; }
float lineFit::Chi() const { return chi2; }
...
float lineFit::fit()
{
  ...
  for (int i = 0; i < numPoints; i++) {
    b.setValue(i, (resolution[i][0] * closestPointToLine[i][0] +
                   resolution[i][1] * closestPointToLine[i][1]) /
                      sigma[i]);
  ...
  chi2 = chi2 * chi2;
  if (numPoints > 4)
    chi2 = chi2 / abs(4 - numPoints);
  newLine(point3D(ret[0], ret[1], 0), vector3D(ret[2], ret[3], 1));
  Regular();
  fitted = true;
  return chi2;
}
float lineFit::fit(int nPoints, vector3D* points, vector3D* resolutions, float* sigmas,
                   sLine3D& result)
{
  if (nPoints <= 3)
    return -1;
  ...
  for (int i = 0; i < nPoints; i++) {
    b.setValue(i,
               (resolutions[i][0] * points[i][0] + resolutions[i][1] * points[i][1]) / sigmas[i]);
