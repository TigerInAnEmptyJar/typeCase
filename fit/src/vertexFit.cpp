...
#include <stdlib.h>
vertexFit::vertexFit(int maxLinesIn, int maxPointsIn)
    : geomObject(), maxLines(maxLinesIn), maxPoints(maxPointsIn)
{
...
vertexFit::vertexFit(const vertexFit& v)
    : geomObject(), maxLines(v.MaxLines()), maxPoints(v.MaxPoints())
{
  numPoints = new int[maxLines];
  points = new vector3D*[maxLines];
  res = new vector3D*[maxLines];
...
bool vertexFit::Fitted() const { return fitted; }
float vertexFit::Chi() const { return chi2; }
int vertexFit::MaxPoints() const { return maxPoints; }
int vertexFit::MaxLines() const { return maxLines; }
...
void vertexFit::addPoint(const vector3D& p, const vector3D& r, float s, int line)
{
  ...
  if (fabs(s) > 1e-20)
    ...
    sigma[line][numPoints[line]] = 1e-20;
  numPoints[line]++;
  if (numLines <= line)
    numLines = line + 1;
}
...
float vertexFit::fit()
{
  ...
  for (int j = 0; j < numLines; j++)
    for (int i = 0; i < numPoints[j]; i++) {
      b.setValue(n,
                 (points[j][i][0] * res[j][i][0] + points[j][i][1] * res[j][i][1]) / sigma[j][i]);
