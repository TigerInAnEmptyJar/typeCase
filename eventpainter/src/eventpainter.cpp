...
void EventPainter::setEvent(TEvent *e)
{
    Event=e;
}

void EventPainter::setCanvas(TPad *c)
{
    canvas=c;
}
...
{
    if(upperLeftIn.getState()==_regular_) upperLeft=upperLeftIn;
    if(lowerRightIn.getState()==_regular_) lowerRight=lowerRightIn;
}
...
void EventPainter::addTrackColor(int color)
{
...
void EventPainter::setTrackSorted(bool sort)
{
    detSorted=sort;
}
void EventPainter::setTrackLike(bool like)
{
    trackLike=like;
}
vector<TBase*> EventPainter::getDrawnObjects(double x,double y)
{
	...
	if(drawings[i]->IsA()==TCrown::Class())
	{
	    ...
	    if(v.length()>=((TCrown*)drawings[i])->GetR1()&&v.length()<=((TCrown*)drawings[i])->GetR2())
	...
	else if(drawings[i]->IsA()==TPolyLine::Class())
	{
	  if(drawnObjects[i]->name()=="TTrack")
	    {
	      lLine2D line(point2D(((TPolyLine*)drawings[i])->GetX()[0],
				   ((TPolyLine*)drawings[i])->GetY()[0]),
			   point2D(((TPolyLine*)drawings[i])->GetX()[1],
...
// 	      }
	    ...
	    {
	      TCutG cut("cut1",((TPolyLine*)drawings[i])->GetN(),((TPolyLine*)drawings[i])->GetX(),((TPolyLine*)drawings[i])->GetY());
	      if(cut.IsInside(x,y))
		ret.push_back(drawnObjects[i]);
	    }
	}
...
//     projector::setProjector(plane3D(point3D(0,0,1),vector3D(1,0,0),vector3D(0,1,0)),point3D(0,0,0),projector::fishEye);
    ...
    double x1,y1,x2,y2;
    canvas->GetRange(x1,y1,x2,y2);
    TText *text=new TText(x1,y1+75,(string("#hits: ")+string_number(Event->getTotalNumberOfHits())).data());
    text->Draw();
    drawings.push_back(text);
    text=new TText(x1,y1+50,(string("#pixels: ")+string_number(Event->getTotalNumberOfPixels())).data());
    text->Draw();
    drawings.push_back(text);
    text=new TText(x1,y1+25,(string("#tracks: ")+string_number(Event->getNumberOfTracks())).data());
    text->Draw();
    drawings.push_back(text);
...
void EventPainter::paintHits(/*float zoom*/)
{
    ...
    {
      if(detectors[j]<0||Event->getMaxNumber<TDetector>()<=detectors[j])continue;
      ...
      for(int i=0;i<Event->getNumberOfHits(detectors[j]);i++)
	{
	  ...
	  if(Event->getHit(detectors[j],i).isValid())
	      ...
	      if(sh!=NULL)
		{
		  ...
		  if(boundingBox[0]<maxBoundingBox[0])maxBoundingBox.setValue(0,boundingBox[0]);
		  if(boundingBox[1]<maxBoundingBox[1])maxBoundingBox.setValue(1,boundingBox[1]);
		  if(boundingBox[2]>maxBoundingBox[2])maxBoundingBox.setValue(2,boundingBox[2]);
		  if(boundingBox[3]>maxBoundingBox[3])maxBoundingBox.setValue(3,boundingBox[3]);
		}
	    }
	}
    }
...
void EventPainter::paintPixels(/*float zoom*/)
{
    ...
    {
      if(pixels[j]<0||pixels[j]>=Event->getMaxNumber<TDetector>())continue;
      if(Event->getNumberOfPixels(pixels[j])<=0)continue;
      point3D center;
      point2D p;
...
void EventPainter::setPaintMode(int mode)
{
  switch(mode)
    {
    case 0:
      projector::setProjector(plane3D(point3D(0,0,100000),vector3D(1,0,0),vector3D(0,1,0)),point3D(0,0,0),projector::parallel);break;
    case 1:
      projector::setProjector(plane3D(point3D(0,0,100000),vector3D(1,0,0),vector3D(0,1,0)),point3D(0,0,0),projector::central);break;
    case 2:
      projector::setProjector(plane3D(point3D(0,0,100000),vector3D(1,0,0),vector3D(0,1,0)),point3D(0,0,0),projector::fishEye);break;
    }
}
...
{
  while(!particleColors.empty())particleColors.pop_back();
}
void EventPainter::setParticleMode(bool value)
{
  particleLike=value;
}    
