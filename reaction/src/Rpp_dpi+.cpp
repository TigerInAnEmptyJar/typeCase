...
Rpp_dpiPlus::Rpp_dpiPlus(AFitAlgorithm *kinFitIn, momentum4D p1, momentum4D p2):RbaseReaction(kinFitIn,2,2)
{
  fname="Rpp_dpiPlus";
  if(!Eparticles::IsInit())Eparticles::init();
  numberOfParticles=2;
...
Rpp_dpiPlus::~Rpp_dpiPlus()
{
  delete []cmparticles;
}
int Rpp_dpiPlus::getNumberOfPrimaries() const
{
  return 2;
}
int Rpp_dpiPlus::getNumberOfSecondaries() const
{
  return 2;
}
void Rpp_dpiPlus::setCoplanarityDifference(double value)
{
  coplanarity=value;
}
void Rpp_dpiPlus::setAngleLimit(double value)
{
  angleLimit=value;
}
void Rpp_dpiPlus::setCrossoverMomentum(double value)
{
  crossover=value;
}
...
//   cout<<"gs: "<<g3<<" "<<g4<<endl;
  ...
  thetaDmax=(g3<1.0?M_PI:(g3==1.0?M_PI/2.0:atan(1.0/sqrt(g3*g3-1.0)/gamma)));
  thetaPimax=(g4<1.0?M_PI:(g4==1.0?M_PI/2.0:atan(1.0/sqrt(g4*g4-1.0)/gamma)));
  //  cout<<"theta_deuteron_max: "<<thetaDmax<<" , theta_pion_max: "<<thetaPimax<<endl;
}
...
//   return bestChi;
// }
// float Rpp_dpiPlus::justCheck(TTrack **tracksIn, int num, int *indexes, momentum4D *results)
// {
//   bool useTime=false;
//   int Npri=0;
//   for(int i=0;i<num;i++)
...
// //   float p_d_cm=sqrt((s-m_d*m_d-m_pi*m_pi)*(s-m_d*m_d-m_pi*m_pi)-4*m_d*m_d*m_pi*m_pi)/(2*sqrt(s));
...
void Rpp_dpiPlus::calculate(int justGeometry)
{
  ...
  switch(justGeometry)
    ...
    case 1:
      {
	vector3D dir1=(tracks[0])->getPath().Direction(),
	  dir2=(tracks[1])->getPath().Direction();
...
float Rpp_dpiPlus::check(TTrack **tracks_nd, int num_nd, TTrack **tracks_1s, int num_1s, TTrack **tracks_2s, int num_2s)
{
  chi2=1e+5;
  if((tracks_nd==NULL&&tracks_1s==NULL&&tracks_2s==NULL)||num_nd+num_1s+num_2s<2)return chi2;
  int inde[2]={-1,-1};
  float chi=justCheck(tracks_nd, num_nd, tracks_1s,num_1s, tracks_2s,num_2s,inde,NULL);
  for(int i=0;i<2;i++)
    if(inde[i]<0||inde[i]>num_nd+num_1s+num_2s)return chi2;
...
float Rpp_dpiPlus::justCheck(TTrack **tracks_nd, int num_nd, TTrack **tracks_1s, int num_1s, TTrack **tracks_2s, int num_2s, int *indexes, momentum4D *results)
{
  if(tracks_nd==NULL&&tracks_1s==NULL&&num_nd+num_1s<2)return 1e+5;
  if(num_2s==0||tracks_2s==NULL)chi2=1e+5;
  ...
  for(int i=0;i<num_nd;i++)
    {
      ...
      if(tracks_nd[i]->isSecondary())continue;
      if(tracks_nd[i]->getCharge()==0)continue;
      if(tracks_nd[i]->getNumberOfSecondaryTracks()>0)continue;
  ...
  for(int i=0;i<num_1s;i++)
    {
      if(tracks_1s[i]==NULL)continue;
      if(tracks_1s[i]->isSecondary())continue;
      if(tracks_1s[i]->getCharge()==0)continue;
      vIPl[nPr]=tracks_1s[i]->getPath().Direction()-z*(tracks_1s[i]->getPath().Direction()*z);
      theta[nPr]=acos(tracks_1s[i]->getPath().Direction()*z);
      pris[nPr++]=num_nd+i;
    }
  if(nPr<2)return 1e+5;
  float dphi;
  int best=0;
  float bchi=1e+6;
...
RbaseReaction* Rpp_dpiPlus::checkNew(TTrack **tracks_nd, int num_nd, TTrack **tracks_1s, int num_1s, TTrack **tracks_2s, int num_2s)
{
  if((tracks_nd==NULL&&tracks_1s==NULL&&tracks_2s==NULL)||num_nd+num_1s+num_2s<2)return NULL;
  float chi=justCheck(tracks_nd, num_nd, tracks_1s,num_1s, tracks_2s,num_2s,NULL,NULL);
  if(chi<0||chi>chiLimit)return NULL;
