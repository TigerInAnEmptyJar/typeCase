...
RtwoParticleDecay::RtwoParticleDecay(double mass1, double mass2, double mass3)
    : TBase("RtwoParticleDecay")
{
  fname = "RtwoParticleDecay";
  m_i = mass1;
  m_1 = mass2;
  ...
  p_1_cm = (m_i * m_i - (m_1 + m_2) * (m_1 + m_2)) * (m_i * m_i - (m_1 - m_2) * (m_1 - m_2));
...
void RtwoParticleDecay::check(TTrack* decayingTrack)
{
  ...
  p12a = p12a / part1;
  p12b = p12b / part2;
  //  cout<<p12a<<p12b<<p12i<<(p12a - p12i).R()<<(p12b-p12i).R()<<part1<<"
  //  "<<part2<<endl;
  if (v1 xor v2) {
    if (v1)
      check(diri, dir1, dir2);
...
void RtwoParticleDecay::check(float theta_1, float theta_2)
{
  ...
  if (useFunction) {
    float f = m_i * m_i;
    string formula = "[0]+[1]-x*x*(2*sqrt(pow([3],4)+2*[3]*[3]*[2]*[2]+pow([2],4)))/"
                     "pow([2]+[3],2)+2*sqrt([0]*[1]+x*x*([0]*([3]*[3]+pow([2]*[3],2))+[1]*(["
                     "2]*[2]+"
                     "pow([2]*[3],2)))/"
                     "pow([2]+[3],2)+pow(x,4)*([3]*[3]+2*[3]*[3]*[2]*[2]+[2]*[2])/"
                     "pow([2]+[3],4))-[4]";
    ...
    //   cout<<a<<" "<<b<<" "<<c<<endl;
    //   cout<<b*b<<" "<<a*c*4.<<" "<<b*b-a*c*4.<<" "<<sqrt(b*b-a*c*4.)<<"
    //   "<<a*2.<<"
    //   "<<1/(a*2.)<<endl;
    ...
    //   cout<<asin(sqrt(abs(th1)))*180/M_PI<<"
    //   "<<asin(sqrt(abs(th2)))*180/M_PI<<endl;
    int nSolutions = 0;
    if (th1 >= 0 && th1 <= 1)
      nSolutions++;
    if (th2 >= 0 && th2 <= 1)
    ...
    //  cout<<"theta_cm: "<<theta_cm<<" = "<<theta_cm*180/M_PI<<" ß=
    //  "<<beta_i_lab<<endl;
...
float RtwoParticleDecay::betaInit() const { return beta_i_lab; }

float RtwoParticleDecay::beta1Lab() const { return beta_1_lab; }
float RtwoParticleDecay::beta2Lab() const { return beta_2_lab; }
float RtwoParticleDecay::beta1CM() const { return beta_1_cm; }
float RtwoParticleDecay::beta2CM() const { return beta_2_cm; }
float RtwoParticleDecay::pCM() const { return p_1_cm; }
float RtwoParticleDecay::p1Lab() const { return p_1_lab; }
float RtwoParticleDecay::p2Lab() const { return p_2_lab; }
float RtwoParticleDecay::pInit() const { return p_i_lab; }
momentum4D RtwoParticleDecay::initCM() const { return momentum4D(0, 0, 0, m_i); }
...
bool RtwoParticleDecay::isPossible() const { return possible; }
float RtwoParticleDecay::mass1() const { return m_1; }
float RtwoParticleDecay::mass2() const { return m_2; }
float RtwoParticleDecay::massI() const { return m_i; }
...
bool RtwoParticleDecay::valid() const { return valid_angles; }
float RtwoParticleDecay::getInvariantMass(float mass1, float mass2, float theta1, float theta2,
...
momentum4D RtwoParticleDecay::getPrimary(float mass_p, float mass1, float mass2, vector3D dir_p,
                                         vector3D dir1, vector3D dir2)
{
  ...
  if (s1 < 0) {
    ...
    if (s1 == 0) {
      ...
      cout << "RtwoParticleDecay::getPrimary():2 solutions for incomming "
              "momentum"
           << endl;
      if (s1 < s2) {
        momentum4D ret;
...
momentum4D RtwoParticleDecay::getPrimary(float mass1, float mass2, vector3D dir_p, vector3D dir1,
                                         vector3D dir2, float beta1, int ptt)
{
  switch (ptt) {
  case 0: // beta1=beta1
  {
    double th1 = acos(dir_p * dir1 / dir_p.R() / dir1.R()),
           th2 = acos(dir_p * dir2 / dir_p.R() / dir2.R());
    double p_1 = mass1 * beta1 / sqrt(1 - beta1 * beta1);
    double p_d = p_1 * cos(th1) * (tan(th1) + tan(th2)) / tan(th2);
    double p_2 = p_1 * sqrt(sin(th1) * sin(th1) +
                            cos(th1) * cos(th1) * tan(th1) * tan(th1) / tan(th2) / tan(th2));
...
momentum4D RtwoParticleDecay::getSecondary(float mass1, float mass2, vector3D dir_p, vector3D dir1,
                                           vector3D dir2, float value, int ptt)
{
  switch (ptt) {
  case 0: // value=beta1
  {
  ...
  {
    double th1 = acos((dir1 * dir_p) / dir1.R() / dir_p.R()),
           th2 = acos((dir2 * dir_p) / dir2.R() / dir_p.R());
    double sum = tan(th1) + tan(th2);
  ...
  case 3: // value=beta2
  {
    ...
    double p_1 =
        p_2 * sin(th2) *
        sqrt(1 / (tan(th1) * tan(th1)) +
             1); // sqrt(sin(th2)*sin(th2)+cos(th2)*cos(th2)*tan(th2)*tan(th2)/tan(th1)/tan(th1));
    momentum4D ret;
