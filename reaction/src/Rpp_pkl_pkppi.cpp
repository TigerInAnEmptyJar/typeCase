...
track_out_general pro_general_pkl, kao_general_pkl, lbd_general_pkl, dpr_general_pkl, dpi_general_pkl;
Double_t missMass_general_pkl[40];
Double_t mInv_general_pkl[40];
Double_t oA_general_pkl[40];
void fillGeneralTree(vector3D *dirs,float *speeds,momentum4D *init, momentum4D *mea, momentum4D* fals, momentum4D *fitted, vector3D lmea,bool swpri,bool swdec, float chim)
{
  ...
  momentum4D mInv_pri=RtwoParticleDecay::getPrimary(pro_general_pkl.m[0], dpi_general_pkl.m[0], lmea, dirs[DP], dirs[PI],mmiss_pri.Momentum().R(),1);
  if(speeds[DP]>0)
    mInv_beta=RtwoParticleDecay::getPrimary(pro_general_pkl.m[0],dpi_general_pkl.m[0],lmea,dirs[DP],dirs[PI],(speeds[DP]>1?0.9998:speeds[DP]),0);
  else
    mInv_beta=mInv_pri;
      ...
      for(int ii=0;ii<2;ii++)
	{
	  ...
	  switch(jj)
	    ...
	    case 2:
	      ...
	      tmpvv=(ii==0?
		     RtwoParticleDecay::getSecondary(pro_general_pkl.m[0],dpi_general_pkl.m[0],lmea,dirs[DP],dirs[PI],(speeds[DP]>1?0.9998:speeds[DP]),0):
		     RtwoParticleDecay::getSecondary(pro_general_pkl.m[0],dpi_general_pkl.m[0],mmiss_pri.Momentum()/mmiss_pri.Momentum().R(),dirs[DP],dirs[PI],(speeds[DP]>1?0.9998:speeds[DP]),0));
	      break;
	    case 3:
	      ...
	      tmpvv=(ii==0?
		     RtwoParticleDecay::getSecondary(dpi_general_pkl.m[0],pro_general_pkl.m[0],lmea,dirs[PI],dirs[DP],(speeds[DP]>1?0.9998:speeds[DP]),3):
		     RtwoParticleDecay::getSecondary(dpi_general_pkl.m[0],pro_general_pkl.m[0],mmiss_pri.Momentum()/mmiss_pri.Momentum().R(),dirs[PI],dirs[DP],(speeds[DP]>1?0.9998:speeds[DP]),3));
	      break;
	    case 4:
	      ptr=&lbd_general_pkl;
  ...
  tmpvv=RtwoParticleDecay::getSecondary(pro_general_pkl.m[0],dpi_general_pkl.m[0],lmea,dirs[DP],dirs[PI],mmiss_pri.Momentum().R(),3);
  dpr_general_pkl.p[2]=tmpvv.Momentum().R();
  dpr_general_pkl.e[2]=tmpvv.Energy();
  tmpvv=RtwoParticleDecay::getSecondary(dpi_general_pkl.m[0],pro_general_pkl.m[0],lmea,dirs[PI],dirs[DP],mmiss_pri.Momentum().R(),3);
  dpi_general_pkl.p[2]=tmpvv.Momentum().R();
  dpi_general_pkl.e[2]=tmpvv.Energy();
  tmpvv=RtwoParticleDecay::getSecondary(pro_general_pkl.m[0],dpi_general_pkl.m[0],lmea,dirs[DP],dirs[PI],lbd_general_pkl.m[0],2);
  dpr_general_pkl.p[3]=tmpvv.Momentum().R();
  dpr_general_pkl.e[3]=tmpvv.Energy();
  tmpvv=RtwoParticleDecay::getSecondary(dpi_general_pkl.m[0],pro_general_pkl.m[0],lmea,dirs[PI],dirs[DP],lbd_general_pkl.m[0],2);
  ...
  pi[0].setVM(fitted[1].Velocity(),pro_general_pkl.m[0]);pi[1].setVM(fitted[0].Velocity(),kao_general_pkl.m[0]);
  missMass_general_pkl[1+20]=(pI-pi[0]-pi[1]).Mass();
  missMass_general_pkl[2+20]=(pI-fitted[0]-fitted[1]-(fitted[2]+fitted[3]))*(pI-fitted[0]-fitted[1]-(fitted[2]+fitted[3]));
  missMass_general_pkl[3+20]=(pI-pi[0]-pi[1]-(fitted[2]+fitted[3]))*(pI-pi[0]-pi[1]-(fitted[2]+fitted[3]));
  missMass_general_pkl[4+20]=(pI-fitted[0]-fitted[1]-(fitted[2]+fitted[3]))*(pI-fitted[0]-fitted[1]-(fitted[2]+fitted[3]));
  ...
  mInv_general_pkl[12]=RtwoParticleDecay::getPrimary(dpi_general_pkl.m[0],dpi_general_pkl.m[0],lmea,dirs[DP],dirs[PI],(speeds[DP]>1?0.9998:speeds[DP]),0).Mass();
  ...
  mInv_general_pkl[12+20]=RtwoParticleDecay::getPrimary(dpi_general_pkl.m[0],dpi_general_pkl.m[0],(fitted[2]+fitted[3]).Momentum(),fitted[2].Momentum(),fitted[3].Momentum(),fitted[2].Beta(),0).Mass();

  oA_general_pkl[0]=acos((dirs[0]*dirs[1])/dirs[0].R()/dirs[1].R());
  oA_general_pkl[1]=acos((dirs[DP]*dirs[PI])/dirs[DP].R()/dirs[PI].R());
  oA_general_pkl[2]=acos((mmiss_pri.Momentum()*dirs[PR])/mmiss_pri.Momentum().R()/dirs[PR].R());
  oA_general_pkl[3]=acos((mmiss_pri.Momentum()*dirs[KA])/dirs[KA].R()/mmiss_pri.Momentum().R());
  ...
  oA_general_pkl[9]=acos((mmiss_pri.Momentum()*dirs[DP])/mmiss_pri.Momentum().R()/dirs[DP].R());
  oA_general_pkl[10]=acos((mmiss_pri.Momentum()*dirs[PI])/dirs[PI].R()/mmiss_pri.Momentum().R());
  
  oA_general_pkl[0+20]=acos((fitted[0].Momentum()*fitted[1].Momentum())/fitted[0].Momentum().R()/fitted[1].Momentum().R());
  oA_general_pkl[1+20]=acos((fitted[2].Momentum()*fitted[3].Momentum())/fitted[2].Momentum().R()/fitted[3].Momentum().R());
  oA_general_pkl[2+20]=acos(((pI-fitted[0]-fitted[1]).Momentum()*fitted[0].Momentum())/(pI-fitted[0]-fitted[1]).Momentum().R()/fitted[0].Momentum().R());
  oA_general_pkl[3+20]=acos(((pI-fitted[0]-fitted[1]).Momentum()*fitted[1].Momentum())/fitted[1].Momentum().R()/(pI-fitted[0]-fitted[1]).Momentum().R());
  oA_general_pkl[4+20]=acos(((fitted[2]+fitted[3]).Momentum()*fitted[0].Momentum())/(fitted[2]+fitted[3]).Momentum().R()/fitted[0].Momentum().R());
  oA_general_pkl[5+20]=acos(((fitted[2]+fitted[3]).Momentum()*fitted[1].Momentum())/fitted[1].Momentum().R()/(fitted[2]+fitted[3]).Momentum().R());
  oA_general_pkl[6+20]=acos(((pI-fitted[0]-fitted[1]).Momentum()*(fitted[2]+fitted[3]).Momentum())/(fitted[2]+fitted[3]).Momentum().R()/(pI-fitted[0]-fitted[1]).Momentum().R());
  oA_general_pkl[7+20]=acos(((fitted[2]+fitted[3]).Momentum()*fitted[2].Momentum())/(fitted[2]+fitted[3]).Momentum().R()/fitted[2].Momentum().R());
  oA_general_pkl[8+20]=acos(((fitted[2]+fitted[3]).Momentum()*fitted[3].Momentum())/fitted[3].Momentum().R()/(fitted[2]+fitted[3]).Momentum().R());
  oA_general_pkl[9+20]=acos(((pI-fitted[0]-fitted[1]).Momentum()*fitted[2].Momentum())/(pI-fitted[0]-fitted[1]).Momentum().R()/fitted[2].Momentum().R());
  oA_general_pkl[10+20]=acos(((pI-fitted[0]-fitted[1]).Momentum()*fitted[3].Momentum())/fitted[3].Momentum().R()/(pI-fitted[0]-fitted[1]).Momentum().R());
  oA_general_pkl[39]=chim;
  outTree_general_pkl->Fill();
}
#endif
void divideTracks(int num, TTrack** trackIn, int &numP, TTrack** trP, int &numS1, TTrack** trS1,int &numS2, TTrack** trS2)
{
  numP=0;
  numS1=0;
  numS2=0;
  for(int i=0;i<num;i++)
    {
      if(trackIn[i]==NULL)continue;
      if(trackIn[i]->isSecondary())continue;
      switch(trackIn[i]->getNumberOfSecondaryTracks())
	{
	case 0:	
	  {
	    if(trackIn[i]->getCharge()!=0)
	      {trP[numP]=trackIn[i];numP++;}
	    break;
	  }
	case 1: trS1[numS1]=trackIn[i];numS1++;break;
	case 2: trS2[numS2]=trackIn[i];numS2++;break;
	}
    }
}
...
Rpp_pkl_pkppi::Rpp_pkl_pkppi(AFitAlgorithm *kinFitIn,momentum4D p1, momentum4D p2):RbaseReaction(kinFitIn,5,2)
{
  fname="Rpp_pkl_pkppi";
  numberOfParticles=5;
  particles=new momentum4D[numberOfParticles];
...
Rpp_pkl_pkppi::Rpp_pkl_pkppi(const Rpp_pkl_pkppi &rIn):RbaseReaction(rIn)
{
  ...
  setTOFResolution(rIn.getTOFResolution(0), rIn.getTOFResolution(1), rIn.getTOFResolution(2), rIn.getTOFResolution(3));
  setRepresentation(rIn.getRepresentation(0),rIn.getRepresentation(1),rIn.getRepresentation(2),rIn.getRepresentation(3));
}

Rpp_pkl_pkppi::~Rpp_pkl_pkppi()
{
  delete []cmparticles;
}

int Rpp_pkl_pkppi::getNumberOfPrimaries() const
{
  return 2;
}

int Rpp_pkl_pkppi::getNumberOfSecondaries() const
{
  return 5;
}
...
float Rpp_pkl_pkppi::multiLineFit(TTrack &v1,TTrack &v2, TTrack &v3, TTrack & v4, vector3D& r1,vector3D &r2,vector3D& r3,vector3D &r4,point3D &p1,point3D &p2)
{
  vertexFit *vFit[2];
	...
	for(int j=0;j<ttr[k]->getNumberOfCalibHits(i);j++)
	  {
		...
		vFit[k/2]->addPoint(vector3D(p[7],p[8],p[3]),
				   vector3D(p[4],p[5],0),p[0],0);
	      }
	  }
    }
...
float Rpp_pkl_pkppi::applyAKinfit(const vector3D &v1, const vector3D &v2, const vector3D &v3, const vector3D &v4, float beta1, float beta2, float beta3, float beta4, momentum4D *res,vector3D *errors)
{
  ...
  float masses[4]={Eparticles::getMassG(particleIDs[0]),
		   Eparticles::getMassG(particleIDs[1]),
  ...
  for(int j=0;j<4;j++)
    {
      ...
      g=sqrt(vcs[j].X()/vcs[j].Z()*vcs[j].X()/vcs[j].Z()+vcs[j].Y()/vcs[j].Z()*vcs[j].Y()/vcs[j].Z());
      ...
      g=sqrt(vcs[j].X()/vcs[j].Z()*vcs[j].X()/vcs[j].Z()+vcs[j].Y()/vcs[j].Z()*vcs[j].Y()/vcs[j].Z()+1);
  ...
  if(kfit.OutInfo()<0)c=-c;
  return c;
}
float Rpp_pkl_pkppi::applyKinfit(const vector3D &v1,const vector3D &v2, const vector3D &v3, const vector3D& v4, float beta1, float beta2, float beta3, float beta4, int &combination, momentum4D *res, vector3D *err)
{
  if((!(v1.getState()==_regular_||v1.getState()==_zero_))||
     (!(v2.getState()==_regular_||v2.getState()==_zero_))||
     (!(v3.getState()==_regular_||v3.getState()==_zero_))||
  ...
  if(fabs((ii-m11-m22).Mass()-Eparticles::getMassG(particleIDs[2]))>1.5&&
    ...
    return 1e+30;
#ifdef OUTPUT
    cout<<"pk:"<<(ii-m11-m22).Mass()-Eparticles::getMassG(particleIDs[2])<<" kp:"<<(ii-m12-m21).Mass()-Eparticles::getMassG(particleIDs[2])<<endl;
   if(beta3>0&&beta4>0)
...
void Rpp_pkl_pkppi::setInitParticles(int num, momentum4D *p)
{
  ...
  excessEnergy=cm.Energy()-(Eparticles::getMassG(14)+Eparticles::getMassG(18)+Eparticles::getMassG(11));
}

momentum4D Rpp_pkl_pkppi::getPromptProton()const
{
  return particles[0];
}

momentum4D Rpp_pkl_pkppi::getDecayProton()const
{
  return particles[3];
}

momentum4D Rpp_pkl_pkppi::getKaon()const
{
  return particles[1];
}
momentum4D Rpp_pkl_pkppi::getDecayPion()const
{
  return particles[4];
}

momentum4D Rpp_pkl_pkppi::getLambda()const
{
  return particles[2];
}

int Rpp_pkl_pkppi::getPromptProtonID()const
{
  return 0;
}

int Rpp_pkl_pkppi::getDecayProtonID()const
{
  return 3;
}

int Rpp_pkl_pkppi::getKaonID()const
{
  return 1;
}

int Rpp_pkl_pkppi::getLambdaID()const
{
  return 2;
}

int Rpp_pkl_pkppi::getDecayPionID()const
{
  return 4;
}
...
RbaseReaction *Rpp_pkl_pkppi::getLambdaDecay()const
{
  return Ldecay;
}
...
{
  if(Ldecay)delete Ldecay;
  Ldecay=reaction.getClone();
}
void Rpp_pkl_pkppi::setMissingMassLimit(float v)
{
  missMassLimit=v;
}
void Rpp_pkl_pkppi::setAngleLimit(float v)
{
  angleLimit=v;
}
bool Rpp_pkl_pkppi::calculatePrimary(TTrack *t1, TTrack *t2, momentum4D lambda, float *m, momentum4D *results)
{
...
bool Rpp_pkl_pkppi::calculateDecay(TTrack *t1, TTrack *t2, TTrack *t3, float *m, momentum4D *results)
{
...
void Rpp_pkl_pkppi::setExact(bool e)
{
  exactRequirement=e;
}
void Rpp_pkl_pkppi::setMomentumLimit(float v)
{
  momentumLimit=v;
}
...
bool Rpp_pkl_pkppi::getExactRequirement()const
{
  return exactRequirement;
}
bool Rpp_pkl_pkppi::getMultiLineFit()const
{
  return useMultiLineFit;
}
bool Rpp_pkl_pkppi::getUseKinfit()const
{
  return useKinfit;
}
void Rpp_pkl_pkppi::setMultiLineFit(bool value)
{
  useMultiLineFit=value;
}
void Rpp_pkl_pkppi::setUseKinfit(bool value)
{
  useKinfit=value;
}
void Rpp_pkl_pkppi::setMaximumProtonAngle(float ang)
{
  maximumProtonAngle=ang;
}
float Rpp_pkl_pkppi::getMaximumProtonAngle()const
{
  return maximumProtonAngle;
}
void Rpp_pkl_pkppi::setUseDecay(bool value)
{useDecay=value;}
bool Rpp_pkl_pkppi::UseDecay()const
{return useDecay;}
...
float Rpp_pkl_pkppi::check(TTrack **tracks_nd, int num_nd, TTrack **tracks_1s, int num_1s, TTrack **tracks_2s, int num_2s)
{
  chi2=1e+5;
  if(tracks_nd==NULL||tracks_2s==NULL)return chi2;
  if(num_nd<2||num_2s<1)return chi2;
  int index[5];
  momentum4D result[5];
  float chi=justCheck(tracks_nd,num_nd,tracks_1s,num_1s,tracks_2s,num_2s,index, result);
  chi2=chi;
  if(chi>chiLimit||chi<0){return chi;}
  for(int i=0;i<2;i++) 
    {
      if(index[i]<0||index[i]>=num_nd)return chi2;
      setTrack(i,tracks_nd[index[i]]);
    }
  if(index[2]<0||index[2]>=num_2s) return chi2;
  setTrack(2,tracks_2s[index[2]]);
  for(int i=0;i<2;i++) 
    {
      if(index[3+i]<0||index[i+3]>getTrack(2)->getNumberOfSecondaryTracks())return chi2;
      setTrack(i+3,getTrack(2)->getSecondary(index[i+3]));
    }
  for(int i=0;i<5;i++)particles[i]=result[i];
  return chi2;
}

float Rpp_pkl_pkppi::justCheck(TTrack **tracks_nd, int num_nd, TTrack **tracks_1s, int num_1s, TTrack **tracks_2s, int num_2s, int *indexes, momentum4D *results)
{
  if(num_nd<2||num_2s<1)return 1e+5;
  if(tracks_nd==NULL||tracks_2s==NULL)return 1e+5;
  ...
  if(useKinfit)
      ...
      for(int i=0;i<num_nd;i++)
	{
	  ...
	  if(s[0]>0.995)s[0]=0.995;
	  err[0]=vector3D(tracks_nd[i]->getErrorSpeed(),tracks_nd[i]->getErrorTheta(),tracks_nd[i]->getErrorPhi());
	  p[0]=tracks_nd[i]->getPath().Foot();
	  for(int j=i+1;j<num_nd;j++)
	    {
	      d[1]=tracks_nd[j]->getPath().Direction();
	      s[1]=tracks_nd[j]->getSpeed();
	      err[1]=vector3D(tracks_nd[j]->getErrorSpeed(),tracks_nd[j]->getErrorTheta(),tracks_nd[j]->getErrorPhi());
	      if(s[1]>0.995)s[1]=0.995;
	      for(int k=0;k<num_2s;k++)
		{
		  ...
		  if(s[2]>0.995)s[2]=0.995;
		  s[3]=tracks_2s[k]->getSecondary(1)->getSpeed();
		  if(s[3]>0.995)s[3]=0.995;
		  if(useMultiLineFit)
		    {
		      multiLineFit(*tracks_nd[i],*tracks_nd[j], 
				   *tracks_2s[k]->getSecondary(0),
				   *tracks_2s[k]->getSecondary(1), 
				   d[0],d[1],d[2],d[3],p[0],p[1]);//<<endl;
		    }
...
#ifdef MOMENTA_USED
		  float s_2[4];
		  vector3D xxld=p[1]-p[0];xxld.normalize();
		  vector3D xx=solve(d[0],d[1],xxld,(initParticles[0]+initParticles[1]).Momentum());
		  float tt1=tan(acos((d[2]*xxld)/d[2].R()/xxld.R())),tt2=tan(acos((d[3]*xxld)/d[3].R()/xxld.R()));
		  s_2[0]=xx[0];s_2[1]=xx[1];
		  s_2[2]=xx[2]/(tt1+tt2)*sqrt(tt2*tt2+tt1*tt1*tt2*tt2);
		  s_2[3]=xx[2]/(tt1+tt2)*sqrt(tt1*tt1+tt1*tt1*tt2*tt2);
		  cc=applyKinfit(d[0],d[1],d[2],d[3],s_2[0],s_2[1],s_2[2],s_2[3],combo,resul,err);
#else
		  ...
		  switch(combo)
		    {
		    case 0:best=i+j*num_nd+k*num_nd*num_nd;sub=0;break;
		    case 1:best=i+j*num_nd+k*num_nd*num_nd;sub=1;break;
		    case 2:best=j+i*num_nd+k*num_nd*num_nd;sub=0;break;
		    case 3:best=j+i*num_nd+k*num_nd*num_nd;sub=1;break;
		    }
		}
	    }
	}
...
//      float mass[num_nd*num_nd*num_2s];
      ...
      for(int i=0;i<num_2s;i++)
	{
	  ...
	  else solution[i*2].setValues(0,0,0);
	  decay.check(tracks_2s[i]->getPath().Direction(),
		      tracks_2s[i]->getSecondary(1)->getPath().Direction(),
		      tracks_2s[i]->getSecondary(0)->getPath().Direction());
	  if(decay.valid())
	    solution[i*2+1].setValues(decay.pInit(),decay.p1Lab(),decay.p2Lab());
	  else solution[i*2+1].setValues(0,0,0);
	}
      for(int i=0;i<num_nd;i++)
	for(int j=i+1;j<num_nd;j++)
	  {
	    ...
	    if(dir[0].Z()<0) dir[0]=dir[0]*-1;
	    if(dir[1].Z()<0) dir[1]=dir[1]*-1;
	    sp[0]=tracks_nd[i]->getSpeed();
	    sp[1]=tracks_nd[j]->getSpeed();
	    if(sp[0]>=1)sp[0]=0.99;
	    if(sp[1]>=1)sp[1]=0.99;
	    if(sp[0]<=0)sp[0]=abs(sp[0]);
	    if(sp[1]<=0)sp[1]=abs(sp[1]);
	    for(int l=0;l<2;l++)
	      {
		//use Time of flight
		...
		dir[5]=l1.Momentum();
		dir[5].normalize();
		chi=sqrt(abs(l1*l1));//abs(mass[2*(i+j*npri+k*npri*npri)+l]-Eparticles::getMassG(18));
		ma=sqrt(abs(l1*l1))-mL;
		chi=ma;
		if(abs(ma)>abs(bestMass)&&abs(ma)<abs(secBestMass))
		  secBestMass=ma;
#ifdef OUTPUT		
		cout<<l1.toString(4+_spheric_).ascii()<<pI<<pr.toString(12+_spheric_).ascii()<<ka.toString(12+_spheric_).ascii()<<endl;
#endif
		if(abs(ma)>missMassLimit){continue;}
		if(abs(ma)<abs(bestMass))//if(chi<bchi)
		  {
		    ...
		    for(int k=0;k<num_2s;k++)
		      {
			if(solution[k*2].R()==0&&solution[k*2+1].R()==0)continue;
      ...
      if(bchi<=chiLimit)
	{
	  ...
	  if(results!=NULL)
	      ...
	      else
		{
		  ...
		  results[4].setPM(dir[4]*p3*sqrt(tanth1+tanth1*tanth2)/sum,Eparticles::getMassG(particleIDs[4]));
		}
	    }
	}
      return bchi;
...
RbaseReaction* Rpp_pkl_pkppi::checkNew(TTrack **tracks_nd, int num_nd, TTrack **tracks_1s, int num_1s, TTrack **tracks_2s, int num_2s)
{
  if(tracks_nd==NULL||tracks_2s==NULL)return NULL;
  if(num_nd<2||num_2s<1)return NULL;
  if(exactRequirement&&(num_nd!=2||num_2s!=1||num_1s!=0))
    {
      //cout <<"no exact requrement"<<endl;
      return NULL;
...
//     }
// #endif
//   RbaseReaction *ret=new Rpp_pkl_pkppi(fkinFit,initParticles[0],initParticles[1]);
//   ret->setChiLimit(chiLimit);
...
//   ((Rpp_pkl_pkppi*)ret)->setTOFResolution(reso[0], reso[1], reso[2], reso[3]);
...
//   return ret;
// }
extern vector3D solve(const vector3D &d1, const vector3D &d2, const vector3D &d3, const vector3D &s);
