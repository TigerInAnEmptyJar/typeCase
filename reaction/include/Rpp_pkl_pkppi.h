...
class Rpp_pkl_pkppi : public RbaseReaction
{
  ...
  float reso[4];
  int useRep[4];
  float multiLineFit(TTrack& v1, TTrack& v2, TTrack& v3, TTrack& v4, vector3D& r1, vector3D& r2,
                     vector3D& r3, vector3D& r4, point3D& p1, point3D& p2);
  float applyKinfit(const vector3D& v1, const vector3D& v2, const vector3D& v3, const vector3D& v4,
                    float beta1, float beta2, float beta3, float beta4, int& combination,
                    momentum4D* res, vector3D* errors);
  float applyAKinfit(const vector3D& v1, const vector3D& v2, const vector3D& v3, const vector3D& v4,
                     float beta1, float beta2, float beta3, float beta4, momentum4D* res,
                     vector3D* errors);
