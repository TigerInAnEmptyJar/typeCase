...
private:
    ...
    pthread_mutex_t tcount_mutex;// the actual thread number -first to be locked in thread_run
    pthread_mutex_t input_mutex; // given to the input algorithms to block the io-channel
    pthread_mutex_t histo_mutex; // given to the histogramming algorithms not to write the same histo at once
/*     pthread_mutex_t *term_mutex; // is the thread still running -second to be locked */
/*     bool *running;               // see term_mutex */
/*     int count_mutex; number of Events that have been calculated */
/*     int tcount_mutex;the actual thread number -first to be locked in thread_run */
/*     int input_mutex; given to the input algorithms to block the io-channel */
/*     int histo_mutex; given to the histogramming algorithms not to write the same histo at once */
/*     int *term_mutex; is the thread still running -second to be locked */
    ...
    int getAlgorithm(const algorithm_parameter &param,const run_parameter &firstRun, int position, int positionFit, int &nFits);
public:
