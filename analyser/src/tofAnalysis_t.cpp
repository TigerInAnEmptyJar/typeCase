...
tofAnalysis::tofAnalysis(int max, int maxDets, int NThreads)
    : QObject(), maxAlgorithms(max) /*,Event(0,0,100,1000,1000,100,maxDets)*/
      ,
      nThreads((NThreads > 1 ? NThreads : 1)), Setup(maxDets)
{
  NumberOfHits = NULL;
  Raws = NULL;
  CalibratedHits = NULL;
  ...
  for (int i = 0; i < nThreads; i++)
    Event[i].reDimension(200, 300, 300, 200, maxDets);
  ...
  run = true;
  connect(this, SIGNAL(newRun(run_parameter&, beamTime_parameter&)), this,
          SLOT(showNewRun(run_parameter&)));
}
...
void tofAnalysis::doRequestNextEvent() { emit requestNextEvent(); }
...
void* tofAnalysis::thread_run(void* ptr)
{
  ...
  while (num < numberOfEventsToAnalyse && run) {
    //      if(num%1000==0)anaLog<<"Event "<<num<<endli;//"
    //      "<<Event[j].getTotalNumberOfHits()<<endli;
    //     anaLog<<"reset ";
    //     cout<<"Event: "<<num<<endl;
    if (callEventSearch)
      doRequestNextEvent();
