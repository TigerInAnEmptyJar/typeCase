...
void tofAnalysis::createSetup(vector<detector_parameter> &dets, vector<material_parameter> &mats, reaction_parameter col)
{
...
//     ioLog<<"open file"<<endli;
    anaLog<<"read setup from file \""<<setupFileName<<"\" "<<(parameterManager::readDetectorParameter_ascii(setupFileName, dparams, col)?"successfull":"failed")<<endli;
...
//     ioLog<<decD;
    ...
    anaLog<<"read materials from file \""<<materialFileName<<"\" "<<(parameterManager::readMaterialParameter_ascii(materialFileName, mparams)?"successfull":"failed")<<endli;
//     int nMats;
...
void tofAnalysis::defineMaterials(vector<material_parameter> &mats, vector<detector_parameter>&dets)
{
...
// 	}
      ...
      Materials[i]=new TMaterial(mats[i/*numM[i]*/].getName(), 
				 mats[i/*numM[i]*/].NumberOfElements(),
				 mats[i/*numM[i]*/].Element(0).getMass(),
				 mats[i/*numM[i]*/].Element(0).getCharge(),
...
void tofAnalysis::defineDetectors(const string &fileName)
{
    reaction_parameter col;
    vector<detector_parameter> dparams;
    anaLog<<"read setup from file "<<fileName<<"\":"<<(parameterManager::readDetectorParameter_ascii(fileName, dparams, col)?"successfull":"failed")<<endli<<incD;
//     ifstream input;
//     ioLog<<"open file"<<endli;
...
void tofAnalysis::defineDetectors(vector<detector_parameter>&dets)
{
    ...
    for(unsigned int i=0;i<dets.size();i++)
    {
	if(dets[i].getID()>maxID)maxID=dets[i].getID();
    }
