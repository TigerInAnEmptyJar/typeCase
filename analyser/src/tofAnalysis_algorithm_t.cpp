...
int tofAnalysis::getAlgorithm(const algorithm_parameter& param, const run_parameter& firstRun,
                              int position, int positionfit, int& nFits)
{
  AAlgorithm** tmp;
  int n;
  int u;
  ...
  for (int ii = 0; ii < nThreads; ii++) {
    n = tofAnalysis::getAlgorithm(&(tmp), u, param, firstRun, Event[ii], Setup, NumberOfHits[ii],
                                  &Event[ii].getNumberOfTracks(), NumberOfPixels[ii],
                                  NumberOfClusters[ii], NumberOfHitClusters[ii], Raws[ii],
                                  CalibratedHits[ii], HitClusters[ii], Tracks[ii], Pixels[ii],
                                  Clusters[ii], NULL /* TMaterial **materials*/, Detectors,
                                  readInValid, readInID, this, callEventSearch);
    sum = u;
    nFits = (n - u);
    for (int i = 0; i < sum; i++)
                                 ...
                                 const run_parameter& runParam)
{
  ...
  for (unsigned int i = 0; i < ap.size(); i++) {
    ...
    case 16:
      ...
      break;
    case 0:
      numf = numf + Event[0].getNumberOfHits();
      break; // pedestal
    case 1:
      numf = numf + Event[0].getNumberOfHits() + NumberOfDetectors;
      break; // tdc off1
    case 2:
      numf = numf + Event[0].getNumberOfHits() + NumberOfDetectors;
      break; // tdcoff2
    ...
    case 10:
      numf++;
      break; // fillRawTDCHistograms
    case 11:
      numf++;
      break; // fillRawADCHistograms
    case 12:
      numf++;
      break; // fillDetectorHistograms
    case 13:
      numf++;
      break; // fillTDCHistograms
    case 14:
      numf++;
      break; // fillADCHistograms
    case 15:
      numf++;
      break; // writeEventToScreen
    case 17:
      numf++;
      numFit++;
      break; // prompt Tracking
    case 18:
      numf = numf + NumberOfDetectors;
      break; // get Shape
    case 19:
      numf = numf + NumberOfDetectors;
...
int tofAnalysis::getAlgorithm(AAlgorithm*** out, int& executeUpTo, const algorithm_parameter& param,
                              ...
                              TCalibHit*** calibratedHits, THitCluster*** hitClusters,
                              TTrack** tracks, TPixel*** pixels, TCluster*** clusters,
                              TMaterial** materials, TDetector** detectors, bool& readValid,
                              int& readInID, QObject* stearIt, bool& eventRequesting)
{
  int nAlgos = 0;
  executeUpTo = 1;
  nAlgos = 1;
  ...
  // n=getAlgorithm(&tmpA,ec,param.getParam<algorithm_parameter>(0).getData(),firstRun,
  // event,setup,
  // numberOfHits, numberOfTracks, numberOfPixels, numberOfClusters,
  // numberOfHitClusters, raws,
  // calibratedHits, hitClusters, tracks, pixels, clusters, materials,
  // detectors,
  // readValid,readInID,stearIt,eventRequesting);
  // else
  // n=getAlgorithm(&tmpA,ec,algorithm_parameter("line fit",0,0,0),firstRun,
  // event,setup,
  // numberOfHits, numberOfTracks, numberOfPixels, numberOfClusters,
  // numberOfHitClusters, raws,
  // calibratedHits, hitClusters, tracks, pixels, clusters, materials,
  ...
  //{
  // n[i]=getAlgorithm(&(tmpA[i]),ec,param.getParam<algorithm_parameter>(0).getData(),firstRun,
  // event,setup, numberOfHits, numberOfTracks, numberOfPixels,
  // numberOfClusters,
  // numberOfHitClusters, raws, calibratedHits, hitClusters, tracks, pixels,
  // clusters, materials,
  // detectors, readValid,readInID,stearIt,eventRequesting);
  // nAlgos+=n[i];
  //}
  //(*out)=new AAlgorithm*[nAlgos];
  // int sum=0;
  ...
  // int ec;
  // int n1=getAlgorithm(&tmpA,ec,algorithm_parameter("line
  // fit",0,0,0),firstRun, event,setup,
  // numberOfHits, numberOfTracks, numberOfPixels, numberOfClusters,
  // numberOfHitClusters, raws,
  // calibratedHits, hitClusters, tracks, pixels, clusters, materials,
  // detectors,
  // readValid,readInID,stearIt,eventRequesting);
  // int n2=getAlgorithm(&tmpA,ec,algorithm_parameter("line
  // fit",0,0,0),firstRun, event,setup,
  // numberOfHits, numberOfTracks, numberOfPixels, numberOfClusters,
  // numberOfHitClusters, raws,
  // calibratedHits, hitClusters, tracks, pixels, clusters, materials,
  // detectors,
  // readValid,readInID,stearIt,eventRequesting);
  // int n3=getAlgorithm(&tmpA,ec,algorithm_parameter("vertex
  // fit",0,0,1),firstRun, event,setup,
  // numberOfHits, numberOfTracks, numberOfPixels, numberOfClusters,
  // numberOfHitClusters, raws,
  // calibratedHits, hitClusters, tracks, pixels, clusters, materials,
  // detectors,
  ...
  ////	showFrequency=showFrequency/10;
  //(*out)[0]=new
  // ATuVSearch(setup.getNumberOfDetectors(),event.getMaxNumber<TTrack>(),
  ...
  //((AFitAlgorithm*)(*out)[3]),((AFitAlgorithm*)
  //(*out)[2]),((AFitAlgorithm*)(*out)[1]),
  // param.getParam<float>(0).getData(), param.getParam<float>(1).getData(),
  ...
  // param.getParam<vector<int> >(0).getData(),param.getParam<vector<float>
  // >(0).getData(),
  // param.getParam<vector<int> >(2).getData(),param.getParam<vector<int>
  // >(3).getData(),
  // param.getParam<vector<float> >(1).getData(),param.getParam<vector<int>
  // >(1).getData(),
  // param.getParam<vector<int> >(4).getData(),param.getParam<vector<int>
  // >(5).getData(),
  // param.getParam<vector<int> >(6).getData(),param.getParam<vector<float>
  // >(2).getData(),
    ...
    for (unsigned int ii = 0; ii < param.getParam<vector<int>>(0).getData().size(); ii++) {
      ...
      if (tmp->getName() == "wedge") {
        (*out)[ii] = new APulsHeightCorrectionWedge(tracks, *numberOfTracks, *detectors[detID],
                                                    event.getEventNumber(), event.getRunNumber(),
                                                    calibrationFiles);
      } else if (tmp->getName() == "spiral") {
        (*out)[ii] = new APulsHeightCorrectionSpiral(tracks, *numberOfTracks, *detectors[detID],
                                                     event.getEventNumber(), event.getRunNumber(),
                                                     calibrationFiles);
      } else if (tmp->getName() == "cylinder") {
        (*out)[ii] = new APulsHeightCorrectionTube(tracks, *numberOfTracks, *detectors[detID],
                                                   event.getEventNumber(), event.getRunNumber(),
                                                   calibrationFiles);
      }
    }
  ...
  // 	(*out)=new AAlgorithm*[nAlgos];
  // 	int detID;
  // 	for(unsigned int ii=0;ii<param.getParam<vector<int>
  // >(0).getData().size();ii++)
  // 	  {
  // 	    anaLog<<"#"<<ii;
  // 	    detID=param.getParam<vector<int> >(0).getData().at(ii);
  ...
  case 32: {
    ...
    (*out) = new AAlgorithm*[nAlgos];
    (*out)[0] = NULL; // new ACMTransformation(setup, particles,
  ...
  case 41: {
    ...
    // detectors, readValid,readInID,stearIt,eventRequesting);
    // 	else
    // 	  n=getAlgorithm(&tmpA,ec,algorithm_parameter("vertex
    // fit",0,0,1),firstRun,
    // event,setup,
    // numberOfHits, numberOfTracks, numberOfPixels, numberOfClusters,
    // numberOfHitClusters, raws,
  ...
  case 47: {
    ...
    (*out) = new AAlgorithm*[nAlgos];
    (*out)[0] = new ALineTrackSearch(setup, tracks, clusters, pixels, calibratedHits,
                                     event.getMaxNumber<TTrack>(), *(numberOfTracks),
                                     numberOfClusters, numberOfPixels, numberOfHits, param);
    anaLog << " done" << endli;
    break;
  }
  case 48: {
    ...
    (*out) = new AAlgorithm*[nAlgos];
    (*out)[0] = new AGenerateEventsWithDecay(tracks, *(numberOfTracks), event.getEventNumber(),
                                             setup, readValid, param);
    anaLog << " done" << endli;
    break;
  }
  case 49: {
    ...
    (*out) = new AAlgorithm*[nAlgos];
    (*out)[0] = new ACompareToGIN(tracks, *(numberOfTracks), event.getEventNumber(),
  ...
  case 50: {
    ...
    (*out) = new AAlgorithm*[nAlgos];
    (*out)[0] = new AVLineTrackSearch(setup, tracks, clusters, pixels, calibratedHits,
                                      event.getMaxNumber<TTrack>(), *(numberOfTracks),
                                      (numberOfClusters), (numberOfPixels), (numberOfHits), param);
    anaLog << " done" << endli;
    break;
  ...
  case 51: {
    ...
    (*out) = new AAlgorithm*[nAlgos];
    (*out)[0] =
        new AVirtualMachine(tracks, calibratedHits, setup, *numberOfTracks, numberOfHits, param);
    anaLog << " done" << endli;
    break;
  }
  //     case 52:
  ...
  case 53: {
    ...
    (*out) = new AAlgorithm*[nAlgos];
    (*out)[0] = new AHitClusterSearch(setup, hitClusters, numberOfHitClusters, calibratedHits,
                                      numberOfHits, param);
    anaLog << " done" << endli;
    break;
  }
  case 54: {
    ...
    (*out) = new AAlgorithm*[nAlgos];
    (*out)[0] = new AMultipleTreeOutput(20, event.getEventNumber(), event.getRunNumber(),
                                        event.getTrigger(), *numberOfTracks, tracks, param);
    anaLog << " done" << endli;
    break;
  }
  case 55: {
  ...
  case 56: {
    ...
    (*out) = new AAlgorithm*[nAlgos];
    (*out)[0] = new APixelTreeWriteOut(setup, event.getEventNumber(), event.getRunNumber(), pixels,
                                       numberOfPixels, param);
    anaLog << " done" << endli;
    break;
  }
  case 57: {
  ...
  case 61: {
    ...
    nAlgos = 1;
    (*out) = new AAlgorithm*[nAlgos];
    (*out)[0] = new AHitTreeOutput(calibratedHits, numberOfHits, event.getEventNumber(),
                                   event.getRunNumber(), event.getTrigger(),
                                   setup.getNumberOfDetectors(), param);
    anaLog << " done" << endli;
    break;
  }
  case 62: {
  ...
  case 63: {
    ...
    nAlgos = 1;
    (*out) = new AAlgorithm*[nAlgos];
    (*out)[0] = new ATrackTreeOutput(event, tracks, *numberOfTracks, event.getEventNumber(),
                                     event.getRunNumber(), event.getTrigger(),
                                     setup.getNumberOfDetectors(), param);
    anaLog << " done" << endli;
    break;
  ...
  case 64: {
    ...
    nAlgos = 1;
    (*out) = new AAlgorithm*[nAlgos];
    (*out)[0] = new APixelTracking(setup, tracks, pixels, clusters, calibratedHits, *numberOfTracks,
                                   numberOfPixels, numberOfClusters, numberOfHits,
                                   event.getMaxNumber<TTrack>(), param);
    anaLog << " done" << endli;
    break;
  }
