...
void tofAnalysis::initData()
{
  ...
  //    cout<<maxOfCalibratedHits<<" "<<maxOfPixels<<" "<<maxOfClusters<<"
  //    "<<maxOfTracks<<endl;
...
void tofAnalysis::killData()
{
  ...
  for (int ii = 0; ii < nThreads; ii++) {
    ...
    if (CalibratedHits != NULL) {
      if (CalibratedHits[ii] != NULL) {
        for (int i = 0; i < NumberOfDetectors - 1; i++)
          if (CalibratedHits[ii][i] != NULL)
            delete[] CalibratedHits[ii][i];
    ...
    if (Particles != NULL) {
      if (Particles[ii] != NULL)
        delete[] Particles[ii];
      Particles[ii] = NULL;
    }
    if (NumberOfHits != NULL) {
      if (NumberOfHits[ii] != NULL)
        delete[] NumberOfHits[ii];
      NumberOfHits[ii] = NULL;
    }
    if (NumberOfPixels != NULL) {
      if (NumberOfPixels[ii] != NULL)
        delete[] NumberOfPixels[ii];
      NumberOfPixels[ii] = NULL;
    }
    if (NumberOfClusters != NULL) {
      if (NumberOfClusters[ii] != NULL)
        delete[] NumberOfClusters[ii];
